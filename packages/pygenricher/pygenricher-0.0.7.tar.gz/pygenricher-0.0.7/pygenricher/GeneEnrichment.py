...
def enrichmentAnalysis(gmtfile, genelist, ranked, tab_out):
    if ranked == False:
    ...
    # enr.results.head(10)
        ...
        pre_res = gp.prerank(rnk=gen_list,
                             ...
                             outdir='test/prerank_report', format='png', seed=6)
    ...
    if tab_out == True:
        ...
        try:
            ...
            print("Directory ", dirName, " already exists")
        listnames.to_csv(os.getcwd() + "/" + dirName + "enrTable.csv")
...
def main():
    ...
    #if args.custom == True:
    gmtfile = processGmt(os.getcwd() + '/' + 'Data/' + 'c2.cp.v7.4.symbols.gmt')
    #else:
    #    gmtfiledf = args.db
    ...
    resulttable = enrichmentAnalysis(gmtfile=gmt, genelist=glist, ranked=args.ranked, tab_out=args.tab_out)
    #else:
