...
def get_hypotest_data(htr):
    ...
    data['observed'] = {
        'qmu_tilde': htr.GetTestStatisticData(),
    ...
    data['expected'] = {}
    # calculate expected intervals for qmu_tilde
    alt_data = data['alt']['dist'].GetSamplingDistribution().data()
    data['expected']['qmu_tilde'] = get_array_quantiles(alt_data)
    qmu_exp = data['expected']['qmu_tilde'][0]
