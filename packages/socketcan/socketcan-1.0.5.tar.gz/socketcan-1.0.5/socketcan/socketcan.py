...
try:
    ...
    from socket import CAN_ISOTP_TX_PADDING, CAN_ISOTP_RX_PADDING, SOL_CAN_ISOTP, CAN_ISOTP_OPTS, \
        CAN_ISOTP_WAIT_TX_DONE, CAN_ISOTP_LISTEN_MODE, CAN_ISOTP_RECV_FC
except ImportError:
    LOGGER.error("ImportError on isotp CAN constants")
...
try:
    ...
    LOGGER.error("ImportError on socket timestamp constants")
...
class CanRawSocket:
    ...
    def recvmsg(self) -> Optional[Tuple[float, Union[CanFrame, CanFdFrame]]]:
        ...
        # second value necessary otherwise we don't get ancdata
        ...
        for cmsg_level, cmsg_type, cmsg_data in ancdata:
            print(cmsg_level, cmsg_type, cmsg_data)
            if cmsg_type == SO_TIMESTAMPING:
                timespecparts = struct.unpack("llllll", cmsg_data)
                timestamps = [timeval_to_float(sec=timespecparts[idx], usec=timespecparts[idx + 1]) for idx in
                              range(0, len(timespecparts), 2)]
...
class CanIsoTpSocket:
    ...
    def __init__(self,
                 ...
                 fc_bs: Optional[int] = None,
                 fc_stmin: Optional[int] = None,
                 ):
        """
        :param interface: The interface name.
        ...
        if any([fc_bs, fc_stmin]):
            stmin = 0
            bs = 0
            if fc_bs is not None:
                bs = fc_bs
            if fc_stmin is not None:
                stmin = fc_stmin
            fc_opts = IsoTpFcOpts(bs=bs,
                                  stmin=stmin)
