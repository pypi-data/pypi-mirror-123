...
nucleic_residues = ['DA5','DC5','DT3','DG','DG5','DT','RG5','DA3','THY','RA','RU5','DC3','A','T','RA3','G','RG3',
                    'URA','DT5','RC5','RA5','RU3','DA','DG3','RC','GUA','DC','RC3','RG','RU','C','CYT','U','ADE']
...
def select_atoms(parmed_structure, keyword_selection=None, ligand_resname=None, resid_selection=None):
    ...
    if keyword_selection:
        ...
        for keyword in selection_keywords:
            ...
            if keyword not in ['ligand', 'protein', 'lipids', 'water', 'nucleic', 'noh', 'not']:
                raise ValueError("""Keyword {} could not be parsed. 
                Options are noh; not; protein; ligand; lipids; nucleic; water.""".format(keyword))
            ...
            if keyword == 'nucleic':
                nucleic_list=[i.idx for i in parmed_structure.atoms if i.residue.name in nucleic_residues]
                for i in nucleic_list:
                    final_list.append(i)
...
def calc_coverage(fes):
    segmented_fes = np.zeros_like(fes)
    for i,row in enumerate(fes):
        for k, value in enumerate(row):
            if value<-.01:
                segmented_fes[i,k]=1
    coverage = ( np.count_nonzero(segmented_fes) / float(segmented_fes.size) )
    return coverage

def calc_dG(fes,order,cutoff=.75):
    #use cropped fes to avoid end cliffs
    fes = np.array(fes)
    fes = fes[:,:int(np.shape(fes)[0]*cutoff)]
    minimax = signal.argrelextrema(fes, np.less, order=order)
    minimay = signal.argrelextrema(fes, np.less, order=order,axis=1)
...
def Ki_to_dG(conc):
    """
    accepts Ki with concentration units and returns dG
    :param conc:
    :return:
    """
    RT = unit.MOLAR_GAS_CONSTANT_R * 300 * unit.kelvin
    RT = RT.value_in_unit(unit.kilocalorie_per_mole)
    return np.log(conc/unit.molar) * RT

def dG_to_Ki(dG):
    """
    accepts dG (assumed in kcal/mol) returns concentration in molar
    :param dG:
    :return:
    """
    RT = unit.MOLAR_GAS_CONSTANT_R * 300 * unit.kelvin
    RT = RT.value_in_unit(unit.kilocalorie_per_mole)
    return np.e**(dG/RT) * unit.molar


def get_fes_minima(fes, order, energy_cutoff):
    fes = np.array(fes)
    minimax = signal.argrelextrema(fes, np.less, order=order)
    minimay = signal.argrelextrema(fes, np.less, order=order, axis=1)
    ...
    min_set = x_direction.union(y_direction)
    min_set = list(min_set)
    x_mins = [min_set[i][0] for i in range(len(min_set))]
    y_mins = [min_set[i][1] for i in range(len(min_set))]

    minima = list(zip(y_mins, x_mins))
    final_list = []
    for i in range(len(minima)):
        if fes[x_mins[i], y_mins[i]] < energy_cutoff:
            final_list.append((x_mins[i], y_mins[i]))

    return np.array(final_list)

def get_minimum_path_trajectory(fes,CV_trace,CV_list,traj,order,energy_cutoff,frames_per_minima):
    minima = get_fes_minima(fes,order,energy_cutoff)
    frame_list = []
    final_list = []
    for k,minimum in enumerate(minima):
        CV_minima = (minimum[1] / (np.shape(fes)[1] / (CV_list[1][4] - CV_list[1][3])),
                             minimum[0] / (np.shape(fes)[0] / (CV_list[0][4] - CV_list[0][3])))
        #get the distances from FES minima to each point in the traj
        distances = spatial.distance.cdist([CV_minima], CV_trace)
        indices = np.array([i for i in range(len(distances[0]))])
        cutoff = 10
        while len(indices[distances[0] < cutoff]) > frames_per_minima:
            cutoff -= .001
        chosen_indices = indices[distances[0] < cutoff]
        for i in chosen_indices:
            frame_list.append((i,fes[minimum[0],minimum[1]]))
        print('minima {}: frame {}: fes value {}'.format(k,chosen_indices,fes[minimum[0],minimum[1]]))
        final_list.append(chosen_indices)

    final_list = np.unique(np.concatenate(final_list))
    print(final_list)
    chosen_traj = traj[final_list]
    return chosen_traj, frame_list