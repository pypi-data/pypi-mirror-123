...
class MDSimulation():
    ...
    def __init__(self, parmed_structure, coordinates=None,
        ...
        self.wrap_all_chains = wrap_all_chains
    ...
    def add_virtual_bond(self, atom_selection1=None, atom_selection2=None):
        ...
        virt_bond = mm.CustomBondForce(' 0 * r')

        # print(atom_selection1,atom_selection2)
        virt_bond.addBond(atom_selection1,atom_selection2)
    ...
    def _build_sim(self):
        ...
        if self.wrap_all_chains:
            #if no atom selection is given this defaults to protein ligand
            chains = [i for i in self.parmed_structure.topology.chains()]
            atom_list = []
            for i in chains:
                #only get protein or ligand chains, the rest can image freely
                if (next(i.residues()).name in aa_residues) or (next(i.residues()).name in lig_residues):
                    atom_list.append(next(i.atoms()).index)
            for i in itertools.combinations(atom_list,2):
                self.add_virtual_bond(atom_selection1=i[0],atom_selection2=i[1])
    ...
    def update_parmed(self):
        """ updates the parmed object with coordinates, positions, box vectors,
            and current potential energy
    ...
    def run(self, time=None, total_steps=None):
        ...
        -------
        parmed_structure
            parmed_strcutre with updated coordinates, positions, box_vectors
...
class MetadynamicSimulation(MDSimulation):
    ...
    >>> CV_list = [
    >>>    ['Coordination Number',[100,110],[200,210],.5,1,.025,[1/2,1/2],[.7,.6], [.289, .354]]
    >>>]

    where cv[6] and cv[7] correspond to the weights and the unbound_distance/2 for the atom pairs in cv[1] and cv[2],
    and cv[8] corresponds to the initial distance between the groups of atoms cv[1] and cv[2].
    ...
    reference_coordinates : positions
        openMM format coordinates for RMSD bias
    ...
    def __init__(self, CV_list=[],
        ...
        self.fes_interval = fes_interval
        self.dome = dome
        self.funnel = funnel
    ...
    def _add_FESReporter(self,fes_interval=None):
        ...
        if fes_interval is None:
            fes_steps = int(round(self.fes_interval / self.step_length))
        else:
...
class SimulatedTemperingSimulation(MDSimulation):
    ...
    def __init__(self, numTemperatures=7, minTemperature=300 * unit.kelvin, maxTemperature=450 * unit.kelvin,
                 tempChangeInterval = 50, reportInterval=1000, weights = None, reportFile='weights.dat', **kwargs):
        super().__init__(**kwargs)
    ...
    def _build_custom_forces(self):
        self.ser_temp = mm.app.SimulatedTempering(self.simulation,
                                                  ...
                                                  tempChangeInterval = self.tempChangeInterval,
