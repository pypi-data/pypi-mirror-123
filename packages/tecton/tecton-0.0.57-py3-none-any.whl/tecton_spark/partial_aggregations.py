...
def _convert_window_to_anchor_time(
    output_df, is_continuous, time_key, version, window_start_column_name=None, window_end_column_name=None
):
    # For continuous aggregations this will simply be the time key.
    if is_continuous:
        return output_df.withColumn(
            TEMPORAL_ANCHOR_COLUMN_NAME, convert_timestamp_to_epoch(functions.col(time_key), version)
        )
    ...
    output_df = output_df.withColumn(
        anchor_column_name, convert_timestamp_to_epoch(functions.col(f"{WINDOW_COLUMN_NAME}.start"), version)
    )
    if window_end_column_name:
        output_df = output_df.withColumn(
            window_end_column_name, convert_timestamp_to_epoch(functions.col(f"{WINDOW_COLUMN_NAME}.end"), version)
        )
