    ...
    def _node_to_value(self, pipeline_node: PipelineNode) -> _VALUE_TYPE:
        if pipeline_node.HasField("transformation_node"):
            ...
            if self._mock_inputs is not None and pipeline_node.data_source_node.input_name in self._mock_inputs:
                vds_id = IdHelper.to_string(data_source_node.virtual_data_source_id)
                raw_data_time_limits = self._get_time_window_from_data_source_node(data_source_node)
                if vds_id not in self._id_to_vds or raw_data_time_limits is None:
                ...
                return data_source_helper.apply_partition_and_timestamp_filter(
                    ...
                    data_source=self._id_to_vds[vds_id].batch_data_source,
                    raw_data_time_limits=self._get_time_window_from_data_source_node(data_source_node),
                    fwv3=True,
                )
            else:
...
class _PandasPipelineBuilder(_PipelineBuilder):
    ...
    def _udf_wrapper(self, *args):
        ...
        def _convert_output_array(arr: Union[List, np.array], type: DataType) -> List:
            ...
            if field.dataType == ArrayType(LongType()):
                ...
                if arr.dtype == "long":
                    return arr.tolist()
                elif arr.dtype in ("float64", "object"):
                    ...
                    int64_arr = np.where(nulls, 0, arr).astype("long")
                    # Add NaN/None back in as None.
                    int64_and_nulls_arr = np.where(nulls, None, int64_arr)
                    return int64_and_nulls_arr.tolist()
                else:
                    raise TypeError(f"UDF output array with type: {arr.dtype}, expected an array castable to int64.")

            elif field.dataType in (ArrayType(FloatType()), ArrayType(DoubleType())):
                if isinstance(arr, list):
                    arr = np.array(arr)

                if arr.dtype in ("float", "object"):
                ...
                raise TypeError(f"Unexpected array data type: {type}")
        ...
        for field in self._output_schema:
            ...
            if isinstance(field.dataType, ArrayType):
                output_df[field.name] = output_df[field.name].apply(
                    lambda arr: _convert_output_array(arr, field.dataType)
                )
