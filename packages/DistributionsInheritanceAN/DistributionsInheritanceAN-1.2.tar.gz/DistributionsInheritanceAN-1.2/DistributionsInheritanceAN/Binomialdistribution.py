...
from DistributionsInheritanceAN.Gaussiandistribution import Gaussian
...
class Binomial(Gaussian):
    ...
    def __init__(self, prob=.5, size=20):
        self.p = prob
        self.n = size
        Gaussian.__init__(self, self.calculate_mean(), self.calculate_stdev())
    ...
    def replace_stats_with_data(self):
        ...
        # read_data_file = super().read_data_file()
        # self.n = float((len(super().read_data_file())))
        positive_trials = []

        for elem in self.data:
            if elem == 1:
                positive_trials.append(1)
                number_positive_trials = positive_trials.count(1)

        self.n = float(len(self.data))
        self.p = number_positive_trials / float(len(self.data))
    ...
    def plot_bar(self):
        ...
        # read_data_file = super().read_data_file()
        x_axis = [0, 1]
        y_axis = [(1 - self.p) * self.n, self.mean]
        plt.bar(x_axis, y_axis)
        plt.xlabel('lists of distinct values in read_data_file')
    ...
    def pdf(self, k):
        ...
        density_function_fraction = math.factorial(self.n) / (math.factorial(k) * math.factorial((self.n - k)))
        density_function_pow = math.pow(self.p, k) * (math.pow((1 - self.p), (self.n - k)))
        density_function = density_function_fraction * density_function_pow
    ...
    def plot_bar_pdf(self):
        ...
        for k in (self.n + 1):
            self.pdf(k)
            x_value.append(k)
            y_value.append(self.pdf(k))

        plt.bar(x_value, y_value)
        plt.xlabel('k values')
        plt.ylabel('pdf for each k values')
        plt.title('pdf of the binomial function')
        plt.grid()
        plt.show()
    ...
    def __repr__(self):
        ...
        return "mean {}, standard deviation {}, probability {}, size {}".format(self.mean, self.stdev, self.p, self.n)
