...
class Gaussian:
    ...
    def plot_histogram(self):
        ...
        for elem in self.data:
            if not (elem in my_unique):
                my_unique.append(elem)
                y.append(1)
            else:
                freq = my_unique.index(elem)
                y[my_unique.index(elem)] += 1

        plt(self.data, y)
        plt.xlabel('lists of float extracted from the data file')
        plt.ylabel('Y axes')
        plt.title('data_list histogram')
        plt.grid()
        plt.show()
    ...
    def pdf(self, x):
        ...
        a_pdf = (1.0 / math.sqrt((2.0 * math.pi * (self.stdev ** 2)))) * (
            math.exp(-(((x - self.mean) ** 2) / (2.0 * (self.stdev ** 2)))))
        return a_pdf
    ...
    def __add__(self, other):
        ...
        result.stdev = math.ceil(math.sqrt((self.stdev ** 2) + (self.stdev ** 2)))
