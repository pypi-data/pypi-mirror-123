...
setup_logging()
IAM_POLICIES = {"user", "resource", "instance"}
...
class OCIFileSystem(AbstractFileSystem):
    ...
    iam_principal: str (None)
        ...
        Values can be one of ["user", "resource", "instance"]
    default_block_size: int (None)
        If given, the default block size value used for ``open()``, if no
        specific value is given at all time. The built-in default is 5MB.
    config_kwargs : dict of parameters passed to the OCI Client upon connection
        This will first scan for `resource_principal_token_path_provider`
            (in the case of no config)
    ...
    def connect(self, refresh=True):
        ...
        if self._iam_principal is None:
            config, config_kwargs = self._determine_iam_auth(config, config_kwargs)
        else:
            config, config_kwargs = self._get_iam_auth(config, config_kwargs)

        logger.debug(
            f"Object Storage Client is being set up using {self._iam_principal} principal."
        )
        try:
            self.oci_client = ObjectStorageClient(config, **config_kwargs)
        ...
        logger.debug(f"Setting default namespace to: {self.default_namespace} .")
        return self.oci_client
    ...
    def _lsdir(self, path: str, refresh: bool = False, **kwargs):
        ...
        if path not in self.dircache or refresh:
            try:
                ...
                dirs_seen = []
                for f in files:
                    filename_suffix = f.name[len(prefix) :]
                    # determine if it's a directory or a file
                    if "/" in filename_suffix:
                        # Note: in OCI Object Storage foldername//suffix is different to
                        # foldername/suffix, so we need to handle nested unnamed folders
                        folder_name = filename_suffix.split("/")[
                            0
                        ]  # if not filename_suffix.startswith('/') else '/'
                        if folder_name not in dirs_seen:
                            dirs_seen.append(folder_name)
                            full_folder_name = "/".join(
                                [full_bucket_name, prefix + folder_name]
                            )
    ...
    def ls(self, path: str, detail: bool = False, refresh: bool = False, **kwargs):
        ...
        # If key is None and _ls returns [], then folder does exist as a bucket with files []
        # Don't want to return the cached copy, as it should not be user editable
    ...
    def copy_basic(self, path1, path2, destination_region=None, **kwargs):
        ...
        try:
            copy_src = self._call_oci(
                ...
                destination_region=destination_region or self._get_default_region(),
    ...
    def info(self, path, refresh=False, **kwargs):
        if path in ["", "/", "@"]:
            return CaseInsensitiveDict({"name": path, "size": 0, "type": "directory"})
        path = self._strip_protocol(path)
        bucket, namespace, key = self.split_path(path)
        path = _build_full_path(bucket=bucket, namespace=namespace, key=key, **kwargs)
        files = self.ls(path=self._parent(path), detail=True, refresh=refresh, **kwargs)
        try:
            return next(f for f in files if f.get("name", None) == path)
    ...
    def metadata(self, path, **kwargs):
        ...
        if not namespace:
            ...
            try:
                ...
                raise translate_oci_error(e) from e
            ns_dict = {
                k: getattr(ns_metadata, k) for k in ns_metadata.attribute_map.keys()
            }
            ...
            try:
                ...
                if e.status != 404:
                    raise translate_oci_error(e) from e
                # It's not a file, it's a subdir, so delegate to info. Very slow to call
                # metadata on subdir
    ...
    def _get_default_region(self, **kwargs):
        region = self.default_region or self.kwargs.get("region")
        if self._iam_principal == "resource":
    ...
    def _get_iam_auth(self, config, config_kwargs):
        assert (
            self._iam_principal in IAM_POLICIES
        ), f"Unrecognized IAM Prinicipal type: {self._iam_principal}"
        return getattr(self, f"_set_up_{self._iam_principal}_principal")(
            config, config_kwargs
        )

    def _determine_iam_auth(self, config, config_kwargs):
        if not config:
            try:  # pragma: no cover
                config, config_kwargs = self._set_up_resource_principal(
                    config, config_kwargs
                )
            except OSError as e:  # pragma: no cover
                try:
                    config, config_kwargs = self._set_up_instance_principal(
                        config, config_kwargs
                    )
            ...
            config, config_kwargs = self._set_up_user_principal(config, config_kwargs)
        return config, config_kwargs

    def _set_up_resource_principal(self, config, config_kwargs):
        rp_token = config_kwargs.pop("resource_principal_token_path_provider", None)
        rps = config_kwargs.pop("signer", get_resource_principals_signer(rp_token))
        config_kwargs["signer"] = rps  # pragma: no cover
        self._iam_principal = "resource"
        return config, config_kwargs

    def _set_up_instance_principal(self, config, config_kwargs):
        fed_ep = config_kwargs.pop("federation_endpoint", None)
        fed_ep_cert = config_kwargs.pop("federation_client_cert_bundle_verify", None)
        config_kwargs["signer"] = InstancePrincipalsSecurityTokenSigner(
            federation_endpoint=fed_ep,
            federation_client_cert_bundle_verify=fed_ep_cert,
        )
        self._iam_principal = "instance"
        return config, config_kwargs

    def _set_up_user_principal(self, config, config_kwargs):
        if config is None:
            raise ConfigFileNotFound(
                "No config file found, user principal authentication failed."
            )
        if isinstance(config, str):
            ...
            try:  # pragma: no cover
                config = from_file(file_location=config, profile_name=self.profile)
                ...
                logger.error(
                    ...
                    f"{config} and profile: {self.profile}. Config can be passed in "
        ...
        logger.debug(
            ...
            "policy administrator that your user policies are set up. If you call again "
