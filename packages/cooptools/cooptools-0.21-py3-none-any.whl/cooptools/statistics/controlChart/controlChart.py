...
import numpy as np
from cooptools.statistics.trends import *
import math
import itertools
...
def control(data: List[Union[int, float]],
    ...
    for ii in range(0, len(data)):
        ...
        seven_trend_high[ii] = qualify_data_sequence(data, max(ii - 7, start_period), ii + 1, 7, strictly_increasing) if ii >= set_baseline_periods - 1 else None
        seven_trend_low[ii] = qualify_data_sequence(data, max(ii - 7, start_period), ii + 1, 7, strictly_decreasing) if ii >= set_baseline_periods - 1 else None
        ...
        fourteen_alternate[ii] = qualify_data_sequence(data, max(ii - 7, start_period), ii + 1, 14, alternating) if ii >= set_baseline_periods - 1 else None
