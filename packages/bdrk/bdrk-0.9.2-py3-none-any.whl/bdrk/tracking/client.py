...
from spanlib.auth.types import TokenHeader
...
class BedrockClient(object):
    def __init__(
        ...
        if os.getenv(BEDROCK_RUN_TRIGGER) == RunTriggerEnv.BEDROCK:
            configuration.api_key[TokenHeader.API_TOKEN] = access_token
        else:
            configuration.api_key[TokenHeader.USER_ACCESS_TOKEN] = access_token
        ...
        def add_to_zip(zf, fpath, zpath):
            if fpath.is_file():
                ...
                for nm in sorted(fpath.iterdir()):
                    add_to_zip(zf, fpath / nm.name, zpath / nm.name)
            # else: ignore
