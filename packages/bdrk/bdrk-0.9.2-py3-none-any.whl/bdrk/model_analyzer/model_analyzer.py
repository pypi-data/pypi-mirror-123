...
from . import classifier_fairness
from . import explainability as exp
from . import regression_fairness
from .type import ModelTask, ModelTypes
...
class ModelAnalyzer:
    def __init__(
        ...
        self.test_inf: Optional[np.ndarray] = None
        self.fconfig: Optional[Mapping[str, Any]] = None
        self.test_lbs: Optional[np.ndarray] = None
        self.base_values = None
        self.shap_values = None
        self.global_xai_data = None
        self.fairness_metrics = None
        self.fairness_custom_metrics: MutableMapping[str, float] = {}  # User logged metrics
    ...
    def test_inference(self, y: np.ndarray):
    ...
    def fairness_config(self, config: Mapping[str, Mapping[str, List]]):
        ...
        :type config: Mapping[str,  Mapping[str, List]
        """
        self.fconfig = {}
        for feature in config:
            self.fconfig[feature] = config[feature]
            # Converting to the new config, for backward compatibility
            self.fconfig[feature]["group_a"] = config[feature].get("group_a") or config[
                feature
            ].get("unprivileged_attribute_values")
            self.fconfig[feature].pop("unprivileged_attribute_values", None)
            self.fconfig[feature]["group_b"] = config[feature].get("group_b") or config[
                feature
            ].get("privileged_attribute_values")
            self.fconfig[feature].pop("privileged_attribute_values", None)
        return self

    def test_labels(self, labels: np.ndarray):
    ...
    def analyze(self):
        ...
        if self.fconfig:
            if self.test_feat is None or self.test_lbs is None or self.test_inf is None:
                ...
                fairness = (
                    regression_fairness.RegressionFairness(
                        self.fconfig, self.test_feat, self.test_inf, self.test_lbs
                    )
                    if self.model_task.value == ModelTask.REGRESSION.value
                    else classifier_fairness.ClassifierFairness(
                        self.fconfig, self.test_feat, self.test_inf, self.test_lbs
                    )
                )
                self.fairness_metrics = fairness.analyze_fairness()

        self._log_fai_data(
            self.model_task, self.fairness_metrics, self.fconfig, self.fairness_custom_metrics
        )
    ...
    def _log_model_info(self):
        ...
        if self.api.has_api_settings:
            self._log_data(
                self.api.internal_api.get_model_info_upload_url(),
                output,
            )
        return output
    ...
    def _log_xai_data(
        self,
        indv_xai_data: Tuple[List[np.ndarray], np.ndarray],
        global_xai_data: Mapping[str, Any],
        train_feat: pd.DataFrame,
    ):
        shap_values, expected_value = indv_xai_data
        if self.api.has_api_settings:
            indv_data: MutableMapping[str, Union[List, str]] = {}
            indv_data["shap"] = [
            ...
            indv_data["model_type"] = self.model_task.value

            self._log_data(
                self.api.internal_api.get_individual_explainability_data_upload_url(),
                indv_data,
            )
            self._log_data(
                self.api.internal_api.get_global_explainability_data_upload_url(),
                global_xai_data,
            )
    ...
    def _log_fai_data(
        ...
        fai_data: Optional[Mapping[str, pd.DataFrame]],
        fairness_config: Optional[Mapping[str, Any]],
        fairness_custom_metrics: Mapping[str, float],
    ):
        if self.api.has_api_settings:
            output: MutableMapping[str, Any] = {}
            if fai_data and fairness_config:
                # Convert values to str since output metrics have features in json key as str too
                fconfig = fairness_config.copy()  # type: ignore
                for f in fconfig.keys():
                    fconfig[f]["group_a"] = [str(x) for x in fconfig[f]["group_a"]]
                    fconfig[f]["group_b"] = [str(x) for x in fconfig[f]["group_b"]]
                output["fairness_config"] = fconfig
                output["fairness_metrics"] = fai_data
            if len(fairness_custom_metrics):
                output["fairness_custom_metrics"] = fairness_custom_metrics
            if len(output):
                output["fairness_type"] = model_task.value
                self._log_data(self.api.internal_api.get_fairness_metrics_upload_url(), output)

    def _log_sample_data(
        self,
        sample_data: pd.DataFrame,
        inf_data: Optional[np.ndarray],
        sample_data_label: Optional[np.ndarray],
        sample_idx: List[int],
    ):
        if self.api.has_api_settings:
            output = {}
    ...
    def _log_data(
        self,
        upload_url: Union[XafaiUploadUrlSchema, BlobStorageSignedUrlSchema],
        data: Mapping[str, Any],
    ):
        try:
            output = {"version": bdrk.__schema_version__, "data": data}
            output_json = json.dumps(output, cls=NpEncoder).encode("utf-8")
