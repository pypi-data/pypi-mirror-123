...
def compute_shap_values(
    ...
    :param: int kmeans_size: Number of k-means clusters. Only required for explaining generic
        predict_func
    :return Tuple[list(numpy.ndarray), numpy_array]: shap_values, base_value.
...
def compute_corrcoef(features: pd.DataFrame, shap_values: List[np.ndarray]) -> np.ndarray:
    """
    Compute correlation between each feature and its SHAP values.
    :param pandas.DataFrame features:
    :param numpy.ndarray shap_values:
    :return numpy.ndarray: (shape = (dim of predict output, number of features))
    """
    all_corrs = list()
...
def get_global_explainability(features: pd.DataFrame, shap_values: List[np.ndarray]) -> Dict:
    shap_summary = np.abs(np.array(shap_values)).mean(axis=1)
