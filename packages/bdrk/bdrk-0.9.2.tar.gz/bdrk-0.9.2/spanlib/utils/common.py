...
class reraise(AbstractContextManager):
    def __init__(self, exc_a: Type[BaseException], exc_b: Type[Exception], **kwargs):
        ...
        :param exc_a: The original exception class to catch
        :type exc_a: Type[BaseException]
        :param exc_b: The new exception class to reraise
        :type exc_b: Type[Exception]
        """
        self.exc_a = exc_a
        self.exc_b = exc_b
        self.kwargs = kwargs
    ...
    def __call__(self, func: FuncT) -> FuncT:
        ...
        if inspect.isgeneratorfunction(func):
            ...
            @wraps(func)
            def gen_decorated(*args, **kwargs):
                with self:
                    yield from func(*args, **kwargs)

            return cast(FuncT, gen_decorated)
...
def retry_on_exception(  # noqa: C901
    exc: Union[Exception, Type[Exception]], wait: float, tries: int
):
    """
    ...
    :type wait: float
    :param tries: Total number of executions. Set to -1 for infinite retries.
    :type tries: int
    """
...
def _cast_object_to_dataclass(obj: Any, type_: Type, field_name: str, **kwargs):
    ...
    for field in dataclasses.fields(type_):
        field_value = _get_field_value(field=field, obj=obj, **kwargs)
...
def _get_field_value(field: dataclasses.Field, obj: Any, **kwargs):
    default_value = None if field.default is dataclasses.MISSING else field.default
    # Try to get the value from the object
    if isinstance(obj, dict):
        field_value = obj.get(field.name, default_value)
    else:
        field_value = getattr(obj, field.name, default_value)

    # Replace it with kwargs if it is present
    return kwargs.get(field.name, field_value)
