...
from openapi_client.models import ResponseFileMetadata
...
class Endpoint:
    ...
    sweep_logs = (
        WEB_HOST + "/{}/{}/sweeps/{}/logs"
    )  # Organization name, project name, sweep name
...
def print_table(
    objects: List[Any], keys: List[str], data_func: Callable, **kwargs
) -> None:
...
def print_logs(logs: List[Any]):
    ...
    lines = [
        f" [{datetime.fromtimestamp(x.timestamp, tz=timezone).strftime('%H:%M:%S.%f')}] {x.message}"
        for x in logs
    ]
    print("\n".join(lines))
...
def print_volume_files(
    files: List[ResponseFileMetadata],
    keys: List[str] = None,
...
def generic_prompter(
    text: str, type: click.ParamType = click.STRING, default=None
) -> Callable:
