    ...
    **kwargs,
    ...
    return vessl_api.experiment_create_api(
        ...
        experiment_create_api_payload=ExperimentCreateAPIPayload(
            ...
            env_vars=[OrmEnvVar(key, str(value)) for key, value in map(lambda env_var: env_var.split("="), (env_vars or []))],
...
def update_experiment_plots_files(
    experiment_id: int, workload_id:int, files: List[InfluxdbExperimentPlotFile], type: str
) -> object:
