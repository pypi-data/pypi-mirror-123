...
def config():
    ...
    if os.path.isfile(config_path):
        ...
        if config_key in config:
            sys.exit(f"Error: {config_key} already exists in this file, overwrite not permitted.")
        config[config_key] = {}
    else:
        config = {config_key: {}}
    ...
    # Set redirect url
    if input("Use redirect url default 'http://localhost:8080' [Y/n]: ") not in ["n", "N"]:
...
def get_config_file():
    ...
    for root, dirs, files in os.walk(cwd_path):
        for file_name in files:
            if file_name.endswith(".json"):
                ...
                if input("Is this a config file to use? [Y/n]: ").rstrip() not in ["n", "N"]:
    ...
    config_key = "onedrive"
    if config_key in config:
        if input("Config dictionary key 'onedrive' found. Use this key? [Y/n]: ").rstrip() in ["n", "N"]:
            config_key = input("Config dictionary key to use: ").rstrip()
