...
def login_livevox(driver:webdriver, username:str,password:str,client_code:str)->None:
    ...
    try:
        ...
        # LOGIN
        client_code = driver.find_element(By.NAME,'clientCode')
        time.sleep(1)
...
def download_report_cdr_between_dates(driver:webdriver,report_name:str,start_date:str,end_date:str,call_center:str = ''):
    ...
    while start_date <= end_date:
        ...
        start_date_field = driver.find_element(By.ID,'search-panel__start-date')
        start_date_field.clear()
        start_date_field.send_keys(date_for_field)
        time.sleep(0.5)
        end_date_field = driver.find_element(By.ID,'search-panel__end-date')
        end_date_field.clear()
        end_date_field.send_keys(date_for_field)
        time.sleep(0.5)
        call_center_drop_down = Select(driver.find_element(By.ID,'callcenter_combo')) # Report format dropdown menu
        if call_center != '':
            call_center_drop_down.select_by_visible_text(call_center) # select call center 
        time.sleep(0.5)
        report_format = Select(driver.find_element(By.ID,'report_format_combo')) # Report format dropdown menu
        report_format.select_by_visible_text(report_name) # select report 
        driver.find_element(By.CLASS_NAME,'lv-button__inside').click() # click on 'Generate Report'
        time.sleep(1)
