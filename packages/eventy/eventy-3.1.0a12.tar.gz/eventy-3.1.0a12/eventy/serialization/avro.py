...
def _decode_avro_record_legacy(
    ...
    avro_schema_name = avro_schema_data.get('name')
    avro_schema_namespace = avro_schema_data.get('namespace')
    if avro_schema_namespace is None or avro_schema_name is None:
        raise ValueError(f"Namespace or name missing form avro schema {avro_schema_data}.")
    record_data['namespace'] = 'urn:' + avro_schema_namespace.replace('.', ':')
    record_data['name'] = avro_schema_name
...
def _decode_avro_record_v3(
    ...
    if avro_schema_namespace is None or avro_schema_name is None:
        raise ValueError(f"Namespace or name missing form avro schema {avro_schema_data}.")
    avro_record_data['namespace'] = 'urn:' + avro_schema_namespace.replace('.', ':')
    avro_record_data['name'] = avro_schema_name
...
def _gen_avro_schema_v3(
    ...
    if type not in ['EVENT', 'REQUEST', 'RESPONSE']:
        raise ValueError(f'Cannot parse eventy urn:{namespace}:{name}, type must be EVENT, REQUEST, or RESPONSE.')

    # avro namespace format is dot-separated without urn: prefix
    namespace = namespace.replace('urn:', '').replace(':', '.')
