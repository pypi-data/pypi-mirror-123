...
from .errors import RecordAttributeTypeError, RecordAttributeValueError
...
class Record:
    ...
    def namespace(self, namespace: str) -> None:
        ...
        if '.' in namespace:
            raise RecordAttributeValueError('namespace', namespace, 'URN format should use ":" as separator and not "."')
    ...
    def source(self, source: str) -> None:
        ...
        if '.' in source:
            raise RecordAttributeValueError('source', source, 'URN format should use ":" as separator and not "."')
        self._source = source
    ...
    def __repr__(self) -> str:
        return f"{self.type}:{self.qualified_name} v{self.version} [CID:{self.correlation_id}] #{self.uuid} ${self.partition_key}"
