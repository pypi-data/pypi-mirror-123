...
def run_install():
    ...
    commit = "8b032e919d2744a199da152d77f1ed5ee73ea567"
    cmd = f"mkdir -p addons && cd addons && \
        rm -rf master* && \
        rm -rf models* && \
        wget https://codeload.github.com/tensorflow/models/zip/{commit} &&\
        unzip {commit} && mv models-{commit} models && rm -rf {commit}"
    run_cmd(cmd)
