...
def job_send_stocks(job_config_dict: dict):
    ...
    if p_size > 0:
        ...
        jsn_prods = api_okvendas.send_stocks(src.client_data.get('url_api') + '/catalogo/estoque', stocks, src.client_data.get('token_api'))
        ...
        if jsn_prods is not None:
            ...
            for p in jsn_prods:
                if p['Status'] == 1:
                    ...
                    logger.warning(f' | Erro ao atualizar estoque para o sku {p["Identifiers"][0]}: {p["Message"][0]}')
