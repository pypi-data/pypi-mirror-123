...
def job_send_prices(job_config_dict: dict):
    ...
    if db_config.sql is None:
        ...
        prices_to_protocol = list(str)
        prices = query_prices(db_config)
        if len(prices) > 0:
            price_response = api_okvendas.post_prices(src.client_data.get('url_api') + '/catalogo/precos', prices, src.client_data.get('token_api'))
            for res in price_response:
                if res.status == 1:
                    prices_to_protocol.append(res.identifiers[0])
                else:
                    logger.warning(job_config_dict.get('job_name') + f' | Nao foi possivel atualizar o preco do sku {res.identifiers[0]}. Erro recebido da Api: {res.message}')
...
def query_prices(db_config: DatabaseConfig) -> list[PriceResponse]:
    """
    ...
    try:
        ...
        conn.close()
        if len(results) > 0:
            prices = [PriceResponse(**p) for p in results]
...
def protocol_prices(db_config: DatabaseConfig, prices: list[str]) -> None:
    ...
    for p in prices:
        try:
            ...
            logger.warning(f'Erro ao protocolar preco do sku {p}: {e}', exc_info=True)
