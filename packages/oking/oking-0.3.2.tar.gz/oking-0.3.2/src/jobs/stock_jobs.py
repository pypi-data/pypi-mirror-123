...
def job_send_stocks(job_config_dict: dict):
    ...
    if p_size > 0:
        ...
        #for stock in stocks:
        response, status_code = api_okvendas.send_stocks(src.client_data.get('url_api') + '/catalogo/load/estoques', stocks, src.client_data.get('token_api'))
        ...
        if response is not None:
            for res in response:
                ...
                codigo_erp = res['codigo_erp']
                if res['Status'] == 1 or res['Status'] == 'Success':
                    try:
                        ...
                        atualizados.append(res['codigo_erp'])
                    except Exception as ex:
                        logger.error(str(ex), exc_info=True)
                else:
                    logger.warning(job_config_dict.get('job_name') + f' | Erro ao atualizar estoque para o sku {codigo_erp}: {res["Message"]}')
