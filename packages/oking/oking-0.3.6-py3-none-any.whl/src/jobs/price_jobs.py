    ...
    if db_config.sql is None:
        ...
        if len(prices) > 0:
            ...
            for res in price_response:
                if res.status == 1:
                    prices_to_protocol.append(res.codigo_erp[0])
                else:
                    logger.warning(job_config_dict.get('job_name') + f' | Nao foi possivel atualizar o preco do sku {res.codigo_erp[0]}. Erro recebido da Api: {res.message}')
