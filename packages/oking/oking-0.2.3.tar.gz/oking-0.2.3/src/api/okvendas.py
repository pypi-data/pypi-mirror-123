...
import src
from src.entities.response import CatalogoResponse, PriceResponse
import requests
...
def post_prices(url, prices: list, token) -> list[PriceResponse]:
    logger.debug("POST: {}".format(url))
    try:
        headers = {'Content-type': 'application/json',
                   'Accept': 'text/html',
        ...
        json_prices = jsonpickle.encode(prices, unpicklable=False)
        response = requests.post(url, json=json_prices, headers=headers)

        result = []
        if response.ok:
            obj = jsonpickle.decode(response.content)
            for res in obj:
                result.append(PriceResponse(**res))
            else:
                if type(obj) is list:
                    for res in obj:
                        result.append(PriceResponse(**res))
                else:
                    result.append(PriceResponse(**obj))
...
def get_order_queue(url: str, token: str, status: str, limit: int) -> list[Fila]:
    queue = []
    try:
...
def get_order(url: str, token: str, order_id: int) -> Pedido:
    order = None
    try:
