...
class PurchaseBatchHandler(BatchHandler):
    ...
    * ``rattail.enum.PURCHASE_BATCH_MODE_COSTING``
    ...
    def require_invoice_parser(self, batch):
        ...
        """
        return require_invoice_parser(batch.invoice_parser_key)
    ...
    def populate_from_invoice_with_po(self, batch, progress=None):
        ...
        def overlay(invrow, i):
            costrow = self.find_costing_row_for_invoice_row(batch, invrow)
            if costrow:
                self.update_costing_row_from_invoice_row(costrow, invrow)
                self.refresh_row(costrow)
            else:
                costrow = self.make_row_from_invoice(batch, invrow)
                self.add_row(batch, costrow)
    ...
    def update_costing_row_from_invoice_row(self, costrow, invrow):
        """
        Update an existing row from "costing" batch, with a line item
        row parsed from an invoice.
        """
        costrow.invoice_line_number = invrow.line_number
        ...
        case_quantity = costrow.case_quantity

        # nb. we always replace the cost row's case quantity from the
        # invoice row, although if that's empty, then refresh_row()
        # may apply case quantity from product cost record
        costrow.case_quantity = invrow.case_quantity
        if costrow.case_quantity:
            case_quantity = costrow.case_quantity

        costrow.cases_shipped = invrow.shipped_cases
        costrow.units_shipped = invrow.shipped_units

        costrow.invoice_unit_cost = invrow.unit_cost
        costrow.invoice_case_cost = invrow.case_cost
        ...
        if case_quantity:
            ...
            if costrow.invoice_unit_cost and not costrow.invoice_case_cost:
                costrow.invoice_case_cost = costrow.invoice_unit_cost * case_quantity
            elif costrow.invoice_case_cost and not costrow.invoice_unit_cost:
                costrow.invoice_unit_cost = costrow.invoice_case_cost / case_quantity

        costrow.invoice_total = invrow.total_cost
        costrow.out_of_stock = invrow.out_of_stock

    def find_costing_row_for_invoice_row(self, batch, invrow):
        """
    ...
    def make_row_from_po_item(self, batch, po_item):
        """
