...
import adara_privacy.token as tokens
...
class Identifier():
    ...
    def __init__(self, *args, identifier_type: str = None, identifier_dict: dict = None, common_tokens: list = None, label: str = None, **kwargs):
        super().__init__()
        ...
        if identifier_type:
            if isinstance(identifier_type, str) and identifier_type.strip().lower() in {k for k in IDENTIFIER_KEYWORDS.keys()}:
                self._identifier_type = identifier_type.strip().lower()
            else:
                raise TypeError('Argument "identifier_type" be must a valid string value representing a known identifier type.')
        ...
        if identifier_dict:
            if isinstance(identifier_dict, dict):
                self._identifier_type, self._general_values, self._named_values = self._extract_values(self._identifier_type, **identifier_dict)
            else:
            ...
            if len(args) == 1 and isinstance(args[0], dict) and not kwargs:
                self._identifier_type, self._general_values, self._named_values = self._extract_values(self._identifier_type, **args[0])
            elif (not args or all(is_stringable(arg) for arg in args)) and (not kwargs or all(is_stringable(kwargs[k]) for k in kwargs.keys())):
                self._identifier_type, self._general_values, self._named_values = self._extract_values(self._identifier_type, *args, **kwargs)
                if (sdk_config is None or not sdk_config.privacy.ignore_empty_identifiers) and not self._general_values and not self._named_values:
                    raise ValueError('At least one identifier argument must be passed as a non-empty string or number value.  You can skip this error by setting the configuration "privacy.ignore_empty_identifiers" to True.')
            else:
                raise TypeError('Positional arguments must be either all strings (used as identifiers) or must be a single instance of "dict".')
        else:
            raise TypeError('Missing argument value: at least one identifier argument must be passed as a non-empty string or number value.')
    ...
    def tokenization_value(self) -> str:
        ...
        return ':'.join(sorted(v.strip().lower() for v in v))
    ...
    def values(self):
        ...
        if general_values:
            general_values_dict[self._identifier_type if self._identifier_type else 'value'] = general_values[0] if len(general_values) == 1 else general_values
        return {
            **general_values_dict,
            **named_values,
        }
