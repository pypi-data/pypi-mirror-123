...
function generateId(prefix, { generation, offset }) {
    const s = `${hexCharFor(generation)}${hexCharFor(offset)}`;
    return `${prefix}:${Array(7).join(s)}`;
}
function generateEventId({ generation, offset }) {
    return generateId('e', { generation, offset });
}
function generateSpanId({ generation, offset }) {
    return generateId('s', { generation, offset });
}
function generateTransactionName({ generation, offset }) {
    return `transaction-${generation}-${offset}`;
}
function generateProjectSlug({ generation, offset }) {
...
function computePosition(index) {
    ...
    const offset = index - Math.pow(2, generation);
    return { generation, offset };
}
function generateTransactionLite({ generation, offset, }) {
...
function generateTransaction(opts) {
    ...
    return Object.assign(Object.assign({}, generateTransactionLite({ generation, offset })), { errors: [], children: Array(depth <= 0 || generation >= depth - 1 ? 0 : 2)
            .fill(null)
            .map((_, i) => generateTransaction({
            index: 2 * index + i + 1,
            depth,
        })), 
        /**
...
function generateTrace(depth = 1) {
    if (depth < 1) {
...
function generateEventSelector(position, eventType) {
    return { id: generateEventId(position), type: eventType };
}
describe('Quick Trace Utils', function () {
    describe('flattenRelevantPaths', function () {
        it('flattens trace without the expected event', function () {
            ...
            const current = { id: 'you cant find me' };
            expect(() => (0, utils_1.flattenRelevantPaths)(current, trace)).toThrow('No relevant path exists!');
        });
        it('flattens a single transaction trace', function () {
            const trace = generateTrace(1);
            const current = generateEventSelector({ generation: 0, offset: 0 }, 'transaction');
            const relevantPath = (0, utils_1.flattenRelevantPaths)(current, trace);
    ...
    describe('parseQuickTrace', function () {
        ...
        it('parses empty trace', function () {
            const current = generateEventSelector({ generation: 0, offset: 0 }, 'transaction');
            expect(() => (0, utils_1.parseQuickTrace)({ type: 'empty', trace: [] }, current, organization)).toThrow('Current event not in trace navigator');
        });
        describe('partial trace', function () {
            it('parses correctly without the expected event', function () {
                ...
                expect(() => (0, utils_1.parseQuickTrace)({ type: 'partial', trace: relevantPath }, current, organization)).toThrow('Current event not in trace navigator');
            });
            it('parses only the current event', function () {
                const relevantPath = [generateTransactionLite({ generation: 0, offset: 0 })];
                const current = generateEventSelector({ generation: 0, offset: 0 }, 'transaction');
                const parsedQuickTrace = (0, utils_1.parseQuickTrace)({ type: 'partial', trace: relevantPath }, current, organization);
        ...
        describe('full trace', function () {
            it('parses the full trace', function () {
                ...
                const relevantPath = (0, utils_1.flattenRelevantPaths)(current, trace);
                const parsedQuickTrace = (0, utils_1.parseQuickTrace)({ type: 'full', trace: relevantPath }, current, organization);
