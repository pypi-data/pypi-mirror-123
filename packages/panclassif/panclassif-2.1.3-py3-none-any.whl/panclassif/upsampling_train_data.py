def upsampled(names,homepath):
	...
	for index in range(len(names)):
		...
		if( norm_sample <= round(can_sample/3) ):
			...
			Cancer.T.to_csv(r''+homepath+'/train_data/cancer/'+names[index]+'.txt.bz2',compression="bz2", sep='\t',header=None,index=None,index_label=None)
			Normal.T.to_csv(r''+homepath+'/train_data/normal/'+names[index]+'.txt.bz2',compression="bz2", sep='\t',header=None,index=None,index_label=None)
