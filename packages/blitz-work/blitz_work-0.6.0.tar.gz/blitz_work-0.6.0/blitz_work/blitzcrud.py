    ...
    def post(self, request, *args, **kwargs):
        if request.user.has_perm(f'{self.__app_name}.add_{self.__model_name}') or (settings.DEBUG and self.allow_anonimous_in_debug):
            ...
            if view.endswith("create"):
                ...
                if form.is_valid():
                    form.save()
                    return_url = request.GET.getlist("return", None)
                    try:
                        if return_url is None:
                            return redirect(self.crud_base_name+"/view")
                        return redirect(reverse(return_url[0])+"?return="+"&return=".join(return_url[1:]))
                    except:
                        return redirect(self.crud_base_name+"/view")
                else:
    ...
    def create_view(self, request, *args, **kwargs):
        ...
        if request.user.has_perm(f'{self.__app_name}.add_{self.__model_name}') or (settings.DEBUG and self.allow_anonimous_in_debug):
            ...
            if return_url is not None and len(return_url) > 1:
                ...
                return_url = None
                extra_return = None
            return render(request, self.create_template, context={"form": self.form(), "return": return_url,"extra_return": extra_return, "crud_url": self.get_crud_url(), "crud_button": self.crud_buttons, "extend_template": self.extend_template, "dark_mode_switch_label": self.dark_mode_switch_label, "context": {"title": self.create_title}})
        else:
            return HttpResponseForbidden()
