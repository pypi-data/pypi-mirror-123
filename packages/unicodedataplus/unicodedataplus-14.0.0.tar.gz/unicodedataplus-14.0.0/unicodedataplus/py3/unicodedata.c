...
typedef struct {
    const unsigned char total_strokes;  /* index into _PyUnicode_TotalStrokes */
} _PyUnicode_UnihanSet;
...
static const _PyUnicode_PropertySet*
_getpropset_ex(Py_UCS4 code)
{
    ...
    return &_PyUnicode_Property_Sets[index];
}

#include "unicodeunihan_db.h"

static const _PyUnicode_UnihanSet*
_getunihanset_ex(Py_UCS4 code)
{
    ...
    return &_PyUnicode_Unihan_Sets[index];
}
...
/*[clinic end generated code: output=b66bf2a44b193bbd input=b2d3d8b7c3f44c10]*/
{
    ...
    return PyUnicode_FromString(_PyUnicode_ScriptNames[index]);
}
...
/*[clinic end generated code: output=d1a6fa2a441df78a input=cd752824be1b8313]*/
{
    ...
    return PyUnicode_FromString(_PyUnicode_BlockNames[index]);
}
...
/*[clinic end generated code: output=411ff074f9250d4e input=001236eb619944db]*/
{
    int index;
    Py_UCS4 c = (Py_UCS4)chr;
    index = (int) _getunihanset_ex(c)->total_strokes;
    if (self && UCD_Check(self)) {
        const change_record *old = get_old_record(self, c);
        if (old->category_changed == 0)
            index = 0; /* unassigned */
