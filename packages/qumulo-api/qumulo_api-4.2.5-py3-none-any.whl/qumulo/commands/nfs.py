...
def parse_nfs_export_restrictions_file(path: str) -> NFSExportRestrictions:
    with open(path) as f:
        ...
        try:
            ...
            raise ValueError(
                'Error parsing JSON restrictions file ' + str(e) + 'file content' + contents
            )
...
class NFSListExportsCommand(Subcommand):
    ...
    def main(
        ...
        print(
            util.tabulate(
                [[e['id'], e['export_path'], e['fs_path'], e['description']] for e in exports],
...
def do_add_entry(
    ...
    old_export, etag = nfs.nfs_get_export(conninfo, creds, args.ref)
