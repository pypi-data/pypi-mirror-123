...
def parse_sold(p: Product, selector: parsel.Selector = None):
    sold = selector.xpath("//a[@id='sellerProfileTriggerId']/text()").get()
    sold = sold or selector.xpath(
        "normalize-space(string(//div[@id='merchant-info']))"
    ).re_first(r"Ships from and sold by (.+)\.")
    if sold is None:
        p.sold = None
    else:
        p.sold = sold
