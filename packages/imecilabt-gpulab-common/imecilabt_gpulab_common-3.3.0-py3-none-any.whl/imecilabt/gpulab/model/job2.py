...
class JobRequestScheduling:
    ...
    Attributes:
      ...
      killable_after: GPULab might kill this job after this duration. Setting this allows GPULab to schedule this job earlier than it can otherwise, but there is a chance your job will be stopped after this time. (If it is restartable, it will however restart later.) Format: a number followed by a unit (ex: 5 minutes, 3 hour, 2 days, 1 week, ...)
      ...
      reservation_id: The reservation ID to use it for the job. This allows the job to start when it otherwise would not be able to start due to a reservation.
