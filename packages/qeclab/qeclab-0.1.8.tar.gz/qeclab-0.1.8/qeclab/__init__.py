...
class Statistics:
    ...
    def variance(self):
        return self.success_rate() * self.failure_rate() / self.num_samples 
...
def convert_to_dataframe(experiment, statistics, extras=dict()):
    data = {
        "Code length": [len(experiment.code)],
        "Code dimension": [experiment.code.dimension()],
        "Physical error rate": [experiment.noise.error_probability()],
        "Number of samples": [statistics.num_samples],
