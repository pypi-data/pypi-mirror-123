...
from renxianqi.localdata_loader import get_data_dir, generate_info
from renxianqi.pip_trigger import upgrade
...
def make_shortcut():
    ...
    if os_name == "Windows" or "Win" in os_name:
        ...
        if not binpath.endswith(".exe"):
            binpath = binpath + ".exe"
        title = "RenXianQi万能清点"
        status = create_shortcut(binpath, title, "一个工具万能左右对比清点Diff")
        if status:
...
def show_about():
    message = """
操作说明：
界面分为左右两边：左边是全部人员输入框，按照一行一个人名
界面分为左右两边：右边是实际出席人员输入框，一行一个人名
点击按钮'开始校验' 
