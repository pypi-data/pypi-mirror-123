...
class Attup(Config):
    ...
    def add_dates(self, data_date, data, predict, n_features, for_loc):
        ...
        if self.dataset_frequency=="W":
            ...
            dates = [edate + relativedelta(days=+i) for i in range(1,self.n_steps_out+1)]
    ...
    def training(self, data_m,n_features, n_train, verbose=0):
        ...
        for i in range(self.n_backtests+1):
            ...
            encoder_stack_h, encoder_last_h, encoder_last_c = LSTM(n_hidden, activation='relu', dropout=self.dropout_train, recurrent_dropout=self.dropout_train,return_state=True, return_sequences=True)(input_train)
            ...
            if self.lrS:
                if self.save_last_epoch==False:
                    history= model.fit(X, y, epochs=self.epochs, verbose=verbose, batch_size=self.batch_size,   callbacks=[checkpoint_best,LearningScheduler, earlyStopping])
                else:
                    history= model.fit(X, y, epochs=self.epochs, verbose=verbose, batch_size=self.batch_size, callbacks=[LearningScheduler,earlyStopping])
            else:
                if self.save_last_epoch==False:
                    history= model.fit(X, y, epochs=self.epochs, verbose=verbose, batch_size=self.batch_size,  callbacks=[checkpoint_best, earlyStopping])
                else:
                    history= model.fit(X, y, epochs=self.epochs, verbose=verbose, batch_size=self.batch_size,  callbacks=[earlyStopping])
