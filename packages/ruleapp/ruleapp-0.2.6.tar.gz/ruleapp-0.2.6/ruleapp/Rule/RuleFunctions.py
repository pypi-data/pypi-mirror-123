...
from ..Devices.Timer.TimerAntecedentDTO import TimerAntecedent
from ..Devices.Alert.AlertConsequentDTO import AlertConsequent
from ..Devices.Switch.SwitchConsequentDTO import SwitchConsequent
from ..Devices.WaterLevel.WaterLevelAntecedentDTO import WaterLevelAntecedent
from ..Devices.Button.ButtonAntecedentDTO import ButtonAntecedent
...
class RuleFunction(object):
    ...
    def add_rule_antecedent(self, user_id, rule_id, device_id):
        try:
            ...
            result = "true"
            if "timer" in device_id:
                antecedent_json = TimerAntecedent()
                result = self.timer_antecedent_functions.set_antecedent(user_id, rule_id, antecedent_json)
            elif "WATERLEVEL" in device_id:
                antecedent_json = WaterLevelAntecedent()
                result = self.waterlevel_antecedent_functions.set_antecedent(user_id, rule_id, antecedent_json)
            elif "BUTTON" in device_id:
                antecedent_json = ButtonAntecedent()
                result = self.button_antecedent_functions.set_antecedent(user_id, rule_id, antecedent_json)
            return result
        except Exception as error:
            print(repr(error))
            return "error"
    ...
    def add_rule_consequent(self, user_id, rule_id, device_id):
        try:
            ...
            result = "true"
            if "alert" in device_id:
                consequent_json = AlertConsequent()
                result = self.alert_consequent_functions.set_consequent(user_id, rule_id, consequent_json)
            elif "SWITCH" in device_id:
                consequent_json = SwitchConsequent()
                result = self.switch_consequent_functions.set_consequent(user_id, rule_id, consequent_json)
            return result
        except Exception as error:
            print(repr(error))
            return "error"
