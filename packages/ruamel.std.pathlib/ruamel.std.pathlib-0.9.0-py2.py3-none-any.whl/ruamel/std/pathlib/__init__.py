...
class Path(type(pathlib.Path())):
    def utime_ns(self, atime_ns, mtime_ns):
        os.utime(self, ns=(atime_ns, mtime_ns), follow_symlinks=False)

    def read_bytes_atime_preserve(self, *arg, **kw):
        stat = os.stat(self)
        res = self.read_bytes(*arg, **kw)
        os.utime(self, ns=(stat.st_atime_ns, stat.st_mtime_ns), follow_symlinks=False)
        return res

    def read_text_atime_preserve(self, *arg, **kw):
        stat = os.stat(self)
        res = self.read_text(*arg, **kw)
        os.utime(self, ns=(stat.st_atime_ns, stat.st_mtime_ns), follow_symlinks=False)
        return res
...
pathlib.Path.remove = pathlib.Path.unlink
