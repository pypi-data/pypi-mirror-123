...
def test_summarizer_french():
    ...
    resp: Response = formatter.format(Document(text="""Un nuage de fumée juste après l’explosion, le 1er juin 2019.
    ...
    assert len(summary) >= parameters.min_length
    assert len(summary) < parameters.max_length
...
def test_summarizer_english():
    ...
    formatter = SummarizerFormatter()
    resp: Response = formatter.format(Document(text="""The tower is 324 metres (1,063 ft) tall, about the same height as an 81-storey building, and the tallest structure in Paris.
...
def test_summarizer_kantar():
    ...
    json_file = testdir / "Sample Huawei Kairntech.json"
    with json_file.open("r") as fin:
        docs = json.load(fin)
    docs = [Document(**doc) for doc in docs]
    parameters = SummarizerParameters(model=TrfModel.distilbart_cnn_12_6)
    formatter = SummarizerFormatter()
    rows = []
    for doc in docs:
    ...
    row_file = testdir / "Sample_Huawei_Kairntech.xlsx"
...
def test_summarizer_pubmed():
    ...
    formatter = SummarizerFormatter()
    resp: Response = formatter.format(Document(text="""Gene duplications and gene losses have been frequent events in
    ...
    themed issue 'Evo-devo in the genomics era, and the origins of morphological diversity'."""), parameters)
    assert resp.status_code == 200
    assert resp.media_type == "text/plain"
    summary = resp.body.decode(resp.charset)
    assert len(summary) >= parameters.min_length
