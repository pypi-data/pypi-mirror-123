...
class SmemTileIterator(bases.GemmSmemIterator):
    def __init__(self,
        ...
        thread_shape_ = thread_map.calc_thread_access_shape(
            self.tile_shape_input_iter, num_threads, sub_tile_shape)

        delta_shape_ = thread_map.calc_thread_access_delta(
            self.tile_shape_input_iter, num_threads, sub_tile_shape)
        if transposed_input:
            self.thread_access_shape = thread_shape_[::-1]
            self.iteration_delta = delta_shape_[::-1]
        else:
            self.thread_access_shape = thread_shape_
            self.iteration_delta = delta_shape_
    ...
    def ctor(self):
        ...
        code = pccm.FunctionCode(f"""
        int offset = 0;
        if ({pccm.boolean(self.transposed_input)}) {{
            // transposed logical coord.
            // inp: [128, 8], need to save to [8, 128] smem.
            // data will be saved as interleaved layout, not same as input.
            // so we multiple kSubTileShape[kContig] again in contig axis.
            offset = (thread_id % {self.access_shape_input[contig]}) *
                        {self.sub_tile_shape[contig]} * stride_ +
                    (thread_id / {self.access_shape_input[contig]}) *
                        {self.sub_tile_shape[strided]} * {self.sub_tile_shape[contig]};
        }} else {{
            // just origin logical coord in input iter.
            // data will be saved as interleaved layout, not same as input.
            offset = (thread_id / {self.access_shape_input[contig]}) *
                        {self.sub_tile_shape[strided]} * stride_ +
                    (thread_id % {self.access_shape_input[contig]}) *
                        {self.sub_tile_shape[strided]} * {self.sub_tile_shape[contig]};
        }}
    ...
    def python_ctor(self, stride: int, ptr: ArrayPtr, thread_id: int):
        new_obj = SmemTileIterator(self.dtype, self.tmap, self.tile_shape,
        ...
        if new_obj.transposed_input:
            offset = ((thread_id % new_obj.access_shape_input[contig]) *
                      new_obj.sub_tile_shape[contig] * new_obj.stride_ +
                      (thread_id // new_obj.access_shape_input[contig]) *
                      new_obj.sub_tile_shape[strided] *
                      new_obj.sub_tile_shape[contig])
        else:
            offset = ((thread_id // new_obj.access_shape_input[contig]) *
                      new_obj.sub_tile_shape[strided] * new_obj.stride_ +
                      (thread_id % new_obj.access_shape_input[contig]) *
                      new_obj.sub_tile_shape[strided] *
                      new_obj.sub_tile_shape[contig])
        # print(offset)
