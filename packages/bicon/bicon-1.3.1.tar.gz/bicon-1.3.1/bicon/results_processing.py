...
class results_analysis():
    ...
    def __init__(self, solution, labels, convert=False, origID=None, species='human'):
        ...
        #check for intersection of genes
        isec= set(solution[0][0]).intersection(set(solution[0][1]))
        if len(isec)>0:
            solution[0][0] = list(set(solution[0][0]).difference(isec))
            solution[0][1] = list(set(solution[0][1]).difference(isec))
            self.isec = list(isec)
        else:
            self.isec = []
    ...
    def show_networks(self, GE, G, output=None):
        ...
        if len(self.isec)>0:
            isec_entr = [str(self.labels[x]) for x in self.isec]
            all_genes_entr = all_genes_entr + isec_entr
            all_genes = all_genes + self.isec

        if self.convert:
            genes1_name = [self.mapping[x] for x in self.genes1]
            genes2_name = [self.mapping[x] for x in self.genes2]
            if len(self.isec) > 0:
                isec_name = [self.mapping[x] for x in isec_entr]
                all_genes_names = genes1_name + genes2_name + isec_name
            else:
        ...
        if len(self.isec)>0:
            nc3 = nx.draw_networkx_nodes(G_small, nodelist = isec, pos=pos, node_color="r", node_size=1700, alpha=.7,
                                         vmin=vmin, vmax=vmax, cmap=cmap, node_shape="o")
            print(f'node(s) {isec} intersect between clusters')
    ...
    def show_clustermap(self, GE, G, true_labels=None, output=None, class_names = []):
        ...
        all_genes = self.solution[0][0] + self.solution[0][1] + self.isec
        ...
        if self.convert:
            ...
            if len(self.isec) > 0:
                isec_entr = [str(self.labels[x]) for x in self.isec]
                all_genes_names = genes1_name + genes2_name + isec_entr
            else:
        ...
        if true_labels != None:
            ...
            if len(self.isec) >0:
                values = values + ["shared gene"]
            colors = ['#F3FF33', 'm', '#4FB6D3', '#22863E', "r"]
