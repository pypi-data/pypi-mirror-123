...
class ErrorNumber:
    ...
    def __init__(self, value, error, relative=False):
        ...
        if relative:
            ...
            self.absolute_error = abs(self.relative_error * self.value)
        else:
            self.value = float(value)
            self.absolute_error = float(error)
            self.relative_error = abs (float(error) / float(value) )
    ...
    def __mul__(self, other):
        if isinstance(other, ErrorNumber):
            ...
            raise TypeError("Type must be either ErrorNumber, float or int: " + str(type(other)))
