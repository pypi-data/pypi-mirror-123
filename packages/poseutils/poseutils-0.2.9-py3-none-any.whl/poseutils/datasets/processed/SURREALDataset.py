...
class SURREALDataset(object):
    ...
    def __init__(self, path_train, path_valid, center_2d=False, load_metrics=None, skel_norm=False):
    ...
    def load_data(self, path_train, path_valid, load_metrics=None):
        ...
        """
        if os.path.exists("data/surreal_indices.npz"):
            print("Loading indices...")
            random_idx = np.load("data/surreal_indices.npz", allow_pickle=True)['data']
        else:
            random_idx = np.random.choice(data_train["data_3d"].shape[0], data_train["data_3d"].shape[0]//24)
            np.savez_compressed("data/surreal_indices.npz", data=random_idx)
        """

        max_idx = data_train["data_3d"].shape[0]//6

        self._data_train['2d'] = data_train['data_2d'][max_idx:3*max_idx, indices_to_arrange, :][:, indices_to_select,  :] # Dont forget to append it [random_idx, :, :]
        self._data_train['3d'] = data_train['data_3d'][max_idx:3*max_idx, indices_to_arrange, :][:, indices_to_select,  :] # [random_idx, :, :]
    ...
    def get_joints_group(self):
        return skeleton_SURREAL_joints_group
