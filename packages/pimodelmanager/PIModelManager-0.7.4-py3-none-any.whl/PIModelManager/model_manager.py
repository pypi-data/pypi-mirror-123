...
class ModelManager:
    ...
    def __init__(cls, config_key: str, model_class_mapping: Dict[str, Callable] = {}) -> None:
    ...
    def __new__(cls, config_key: str = None):
        if ModelManager.__instance is None:
            ...
            if config_key:
                cls._config_key = config_key
            logger.info(
                "__new__: setting model manager instance")
        else:
    ...
    def add_model_class_mapping(cls, model_name: str, class_instance: Callable) -> None:
        ...
        if not ray.is_initialized() and isinstance(class_instance, ray.serve.api.Deployment):
    ...
    def _remove_unused_models(cls) -> None:
        ...
        for model_name in cls._models:
            for language in cls._models[model_name]:
                for version_model in cls._models[model_name][language]:
                    ...
                    for tenant_name in cls._tenant_configs:
                        try:
                            version_config = cls._tenant_configs[tenant_name][model_name][language]['version']
                    ...
                    if not used:
                        model_path = cls.get_model_path(
                            language=language, name=model_name, client=client, version=version_model
                        )
        ...
        for model in models_to_delete:
            ...
            if isinstance(cls._model_class_mapping[model_name],ray.serve.api.Deployment):
            ...
            for pandora_model in cls._downloaded_models:
                if (pandora_model.name == model_name) and (pandora_model.language == language) and (pandora_model.version == version):
    ...
    def _initialize_model_from_config(cls, config: Dict[str, str]) -> None:
        language, model_name, client, version = config['language'], config[
            'name'], config['client'], config['version']
        ...
        if not cls._models[model_name][language][version]:
            ...
            if isinstance(cls._model_class_mapping[model_name],ray.serve.api.Deployment):
                ...
                cls._models[model_name][language][version] = serve.get_deployment(name)
            else:
                cls._models[model_name][language][version] = cls._model_class_mapping[model_name](config)
            logger.info(
    ...
    def update_config_from_json(cls, token_info: Any, bg_task: Any = None):
        ...
        if cls._busy:
            msg = JSONResponse(status_code=503, content={'message': 'Updating and downloading models: this will take a couple of minutes.'})
            return msg
    ...
    def _check_validity_config(cls, token_info):
        config = token_info.tenant_config[cls._config_key]['configurations']['MODELS_JSON']
        required_keys = ['model_type', 'download_scenario',
    ...
    @ classmethod
    def add_tenant_config(cls, tenant_name: Union[str, None], tenant_config: dict) -> None:
    ...
    @ classmethod
    async def get_model(cls, model_name: str, language: str, token_info: Any = {}, path: bool = False, **kwargs) -> Any:
        version = ''
        tenant_name = None
        client = ''
        if token_info:
        ...
        try:
            ...
            version = cls._tenant_configs[cls._default_tenant_name][model_name][language]['version']
            client = cls._tenant_configs[cls._default_tenant_name][model_name][language]['client']
            logger.warning(
                f'Using model for default tenant {cls._default_tenant_name} instead.')
        if path:
            try:
                return await cls.get_model_path(language=language, name=model_name, client=client, version=version)
            except:
                pass
        else:
            try:
    ...
    def download_model_from_config(cls, config: Dict[str, str]) -> None:
        ...
        if download_scenario == "pip":
            ...
            raise Exception(
                "Invalid config file - missing download scenario.")
    ...
    def __download_model_dataservices(cls, model: Dict[str, str]) -> None:
        ...
        matched_models = [
            f for f in cls._available_files if query in f]
        if len(matched_models) == 0:
            ...
            if temp_path.exists():
                ...
                if pandora_model not in cls._downloaded_models:
                    cls._downloaded_models.append(pandora_model)
                    logger.info(
                        f"Model already downloaded in {temp_path}")
            else:
