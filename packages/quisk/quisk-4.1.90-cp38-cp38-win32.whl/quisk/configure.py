...
#   4: Global data common to all radios. This is similar to the radio dicts.
#   ]
...
# dictionary conf.name2format has formats and defaults for these additional items.
...
class Configuration:
  ...
  def ParseConfig(self):
    ...
    self.format4name = {}
    for name in conf.name2format:
      self.format4name[name] = conf.name2format[name][0]
...
class ComboCtrl(wxcombo.ComboCtrl):
  def __init__(self, parent, value, choices, no_edit=False):
    ...
    self.dirty = False
    #self.bgColor = wx.Choice.GetClassDefaultAttributes().colBg	# This fails on Raspberry Pi
...
class ControlMixin:
  ...
  def AddTextEditHelp(self, col, text1, text2, help_text, border=2, span1=1, span2=1, no_edit=True):
    ...
    if no_edit:
      ...
      edt = wx.TextCtrl(self, -1, text2, style=wx.TE_PROCESS_ENTER)
  ...
  def OnChange2(self, ctrl, value):
    ...
    if ok:
      ...
      if name in conf.name2format and conf.name2format[name][1] == value:
