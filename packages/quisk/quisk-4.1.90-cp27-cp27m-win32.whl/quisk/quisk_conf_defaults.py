...
# This is a dictionary of data items that are in the radio_dict but are not defined below in the sections.
...
def __getattr__(name):	# provide default values for some names
  if name in name2format:
    return name2format[name][1]
  raise AttributeError("'conf' object has no attribute '%s'" % name)
