...
class ConfigConfig(wx.ScrolledWindow):
  def __init__(self, parent, width):
    ...
    ctrl.SetColorGray()
    ctrl.SetPosition((tab2 - ctrl.GetSize().GetWidth(), self.y - self.offset))
    self.y += self.dy
    # Start WSJT-X
...
class App(wx.App):
  ...
  def StartWsjtx(self, method=None, ctrl=None):
    if sys.platform == 'win32':
      prog = "C:\\WSJT\\wsjtx\\bin\\wsjtx.exe"
    else:
      prog = "/usr/bin/wsjtx"
    if method is None:
      method = self.local_conf.globals.get("start_wsjtx", "Never")
    if method == "Main Rx0 on startup":
      self.wsjtx_process[0] = subprocess.Popen([prog, "--rig-name", "quisk"], shell=False)
