...
from starthinker.util.dv import report_get as dbm_report_get
from starthinker.util.bigquery import rows_to_table, query_to_rows, table_exists
...
def account_from_dbm_report(config, report_id, report_name):
  ...
  if report_id or report_name:
    try:
      report = dbm_report_get(config, 'user', report_id, report_name)
...
class Command(BaseCommand):
  ...
  def handle(self, *args, **kwargs):
    ...
    st_config = Configuration(service=settings.UI_SERVICE, verbose=True)

    if table_exists(st_config, 'service', 'google.com:starthinker', 'dashboard', 'ST_Scripts'):
      id_max = next(
        query_to_rows(
          st_config,
          'service',
    ...
    for recipe in Recipe.objects.filter(
      id__gt=id_max
    ).order_by('id')[:kwargs['recipes']]:

      print(recipe.name)

      recipe_config = Configuration(
        user=recipe.account.get_credentials_path(),
    ...
    if impact:
      if kwargs['test']:
        ...
        rows_to_table(
          ...
          'dashboard',
          'ST_Scripts',
          [(i['day'], i['deployment'], i['user'], i['product'], i['script'], i['account']) for i in impact],
          schema=[
            { 'mode': 'REQUIRED', 'name': 'Day', 'type': 'Date' },
            { 'mode': 'REQUIRED', 'name': 'Deployment', 'type': 'INTEGER' },
            { 'mode': 'REQUIRED', 'name': 'User', 'type': 'STRING' },
            { 'mode': 'NULLABLE', 'name': 'Product', 'type': 'STRING' },
            { 'mode': 'NULLABLE', 'name': 'Recipe', 'type': 'STRING' },
            { 'mode': 'NULLABLE', 'name': 'Account', 'type': 'INTEGER' },
          ],
          skip_rows=0,
