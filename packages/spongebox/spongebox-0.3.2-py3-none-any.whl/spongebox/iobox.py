...
@timebox.timeit
def list_all_files(dir_, exp=None):
    lst = []
    if not exp == None:
        __filter = lambda x: filter_list(x, pattern=exp)
    else:
        __filter = lambda x: x
    for root, sub_dir, file_list in os.walk(dir_):
        lst.extend([root + "\\" + f for f in __filter(file_list)])
    return lst
