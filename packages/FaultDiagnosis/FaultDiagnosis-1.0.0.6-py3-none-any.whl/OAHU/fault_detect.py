...
def fault_detect(previous_day, EquipmentClassCode, EquipmentNo, SiteCode, BuildingNo, ZoneNo, CoolHeatingIndication):
    ###################################################
    ########### 유사도 지수 산출 (날짜별) #############
    ###################################################
    ...
    print("startdate, previous_day:", startdate, previous_day)

    fault_total_data['DATE'] = pd.to_datetime(fault_total_data['SensingTime'].dt.strftime('%Y%m%d'))
    fault_data = fault_total_data.query("DATE ==" + previous_day)
    """ 
    if (previous_day >= startdate):
        fault_data = fault_total_data.query("DATE ==" + previous_day)
    else:
    ...
    # 출력
    print('ASHRAE 데이터 날짜  ; ' + previous_day)
    ...
    for i in range(predict_len):
        ...
        row_num = i + 1
        # if (i%17 == 0):
        print("순서 {:d}  보정 {:.8f}  추정 {:.8f}  날짜 {:s}".format(i, label, prediction, previous_day))
        ...
        sql = "UPDATE T_AHU_SENSING_INFO           \
                 ...
                 WHERE b.rownum = %d                "
        val = (label, prediction, EquipmentClassCode, EquipmentNo, SiteCode, BuildingNo, ZoneNo, previous_day, row_num)
        cursor.execute(sql, val)
        # print("prediction, label, previous_day, row_num")
    ...
    if (err_no <= 1):
        ...
        if (coilvlv_std == 0) & (AirCondition == '1'):
            if coilvlv_mean == 0:
                print("냉방코일 밸브 고착(완전 닫힘)")
            else:
                if coilvlv_mean == 1:
                    print("냉방코일 밸브 고착(완전 열림)")
                else:
                    print("냉방코일 밸브 고착(부분 열림 -", int(coilvlv_mean * 100), "%)")
        #            return

        if (coilvlv_std == 0) & (AirCondition == '2'):

            if coilvlv_mean == 0:
                print("난방코일 밸브 고착(완전 닫힘)")
            else:
                if coilvlv_mean == 1:
                    print("닌방코일 밸브 고착(완전 열림)")
                else:
                    print("난냉방코일 밸브 고착(부분 열림 -", int(coilvlv_mean * 100), "%)")
                #            return
        ...
        if oadamper_std == 0:
            if oadamper_mean == 0:
                ...
                if oadamper_mean == 1:
                    ...
                    print("외기댐퍼 고착(부분 열림 -", int(oadamper_mean * 100), "%)")
                #            return
        ...
        if radamper_std == 0:
            if radamper_mean == 0:
                ...
                if radamper_mean == 1:
                    ...
                    print("환기댐퍼 고착(부분 열림 -", int(radamper_mean * 100), "%)")
                #            return
        ...
        if eadamper_std == 0:
            if eadamper_mean == 0:
                ...
                if eadamper_mean == 1:
                    ...
                    print("배기댐퍼 고착(부분 열림 -", int(eadamper_mean * 100), "%)")
                #            return
