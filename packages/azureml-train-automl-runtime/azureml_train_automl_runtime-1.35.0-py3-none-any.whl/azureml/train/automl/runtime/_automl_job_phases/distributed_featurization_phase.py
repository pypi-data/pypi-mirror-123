...
class DistributedFeaturizationPhase:
    ...
    def run(workspace_getter: Callable[..., Any],
        ...
        featurized_data_dir = '{}_{}_featurized'.format(current_run.experiment.name, parent_run_id)
        grain_key_values = training_dataset.get_partition_key_values()
        with logging_utilities.log_activity(logger=logger, activity_name='DistributedTransformation'):
            with parallel_backend(parallel_backend_name):
                Parallel(n_jobs=-1)(delayed(_transform_one_grain)(
                    ...
                    automl_settings.grain_column_names
                ) for grain_key_value in grain_key_values)
...
def _transform_one_grain(workspace_getter: Callable[..., Any],
    ...
    expr_store_for_worker.transformers.set_by_grain(grain_keys_values, ts_transformer)
...
def _build_transformer(training_dataset: TabularDataset,
    ...
    tsdf = construct_tsdf(
        subsampled_X,
        subsampled_Y,
        data_context_params.data_params.label_column_name,  # type: ignore
        ts_params[TimeSeries.TIME_COLUMN_NAME],
        ts_params.get(TimeSeriesInternal.ORIGIN_TIME_COLUMN_NAME, TimeSeriesInternal.ORIGIN_TIME_COLNAME_DEFAULT),
        ts_params.get(TimeSeries.GRAIN_COLUMN_NAMES, [TimeSeriesInternal.DUMMY_GRAIN_COLUMN]),
        None  # type: ignore
    )

    # Force all TimeIndexFeaturizers to have the same feature list.
    tif = TimeIndexFeaturizer()
    ts_params[TimeSeriesInternal.FORCE_TIME_INDEX_FEATURES_NAME] = tif.preview_time_feature_names(tsdf)
...
def _build_transformer_for_all_grains(
    ...
    grain_key_values: List[Dict[str, Any]]
    ...
    for grain_key_value in grain_key_values:
        tr_identifier = distributed_timeseries_util.convert_grain_dict_to_str(grain_key_value)
        transformer = expr_store.transformers.get_by_grain(grain_key_value)  # type: TimeSeriesTransformer
        for step in transformer.pipeline.steps:  # type: ignore
            if not isinstance(step[1], _GrainBasedStatefulTransformer):
                continue
            else:
                if step[0] in grain_based_trs_to_combine:
                    grain_based_trs_to_combine[step[0]][tr_identifier] = step[1]
                else:
                    grain_based_trs_to_combine[step[0]] = {tr_identifier: step[1]}

            if new_tst is None:
                new_tst = copy.deepcopy(transformer)

    for tr_name, tr_dict in grain_based_trs_to_combine.items():
        agg_tr = AutoMLAggregateTransformer(grain_col_names, tr_dict)
        for idx, step in enumerate(new_tst.pipeline.steps):  # type: ignore
            if step[0] == tr_name:
                break
        new_tst.pipeline.steps[idx] = (tr_name, agg_tr)  # type: ignore
    PhaseUtil.log_with_memory("Ending timeseries transformer aggregation")
    return cast(TimeSeriesTransformer, new_tst)
