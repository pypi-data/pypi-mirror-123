...
SENT_EMAILS=[]
EMAIL_BLACKLIST=CpfVz()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",CpfVg(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=CpfVJ,from_name=CpfVJ,smtp_host=CpfVJ,smtp_user=CpfVJ,smtp_pass=CpfVJ,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if CpfVk(recipients,CpfVd)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=CpfVJ):
 ...
 result["From"]=formataddr((CpfVg(Header(from_name,"utf-8")),from_email))
 return result
