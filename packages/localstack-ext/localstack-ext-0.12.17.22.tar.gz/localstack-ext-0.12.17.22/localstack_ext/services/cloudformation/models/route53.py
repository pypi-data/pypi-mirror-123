...
def canonicalize_name(name:Llspo)->Llspo:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[LlspT])[0]
 def get_physical_resource_id(self,attribute=LlspT,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return Llspt(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @Llspk
