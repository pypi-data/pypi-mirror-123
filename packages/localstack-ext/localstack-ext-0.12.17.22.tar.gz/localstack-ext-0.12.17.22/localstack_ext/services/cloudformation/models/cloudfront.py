...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or DsUPl
 @DsUPt
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return DsUPn(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @DsUPY
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if DsUPi(origins,DsUPj)else origins.get("Items")
  origins_to_create=DsUPN([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if DsUPi(aliases,DsUPj)else aliases.get("Items")
  aliases_to_create=DsUPN(aliases)
  for distr in distrs:
   origins=DsUPN([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=DsUPN(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",DsUPR)
   config["Enabled"]=DsUPw(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if DsUPi(custom_errors,DsUPc)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=DsUPw(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":DsUPR}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=DsUPw(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=DsUPl):
    parent=config if parent is DsUPl else parent
    items=parent.get(key)or[]
    if DsUPi(items,DsUPc)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":DsUPQ(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if DsUPi(cache_behavs,DsUPc)else cache_behavs)
