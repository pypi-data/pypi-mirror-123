...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=xrqQB(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not xrqQM
   return result
  except xrqQf:
   props=self.props
   ...
   if parts[0]in candidates and xrqQd(parts)>1:
