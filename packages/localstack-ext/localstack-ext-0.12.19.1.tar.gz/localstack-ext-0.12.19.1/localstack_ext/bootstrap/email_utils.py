...
SENT_EMAILS=[]
EMAIL_BLACKLIST=PYAnd()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",PYAnL(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=PYAnI,from_name=PYAnI,smtp_host=PYAnI,smtp_user=PYAnI,smtp_pass=PYAnI,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if PYAnR(recipients,PYAnx)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=PYAnI):
 ...
 result["From"]=formataddr((PYAnL(Header(from_name,"utf-8")),from_email))
 return result
