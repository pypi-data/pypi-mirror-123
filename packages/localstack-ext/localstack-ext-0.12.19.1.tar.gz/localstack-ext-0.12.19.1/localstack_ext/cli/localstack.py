...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return AfrYS if e else AfrYc
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->AfrYE:
...
@click.option("--revert",is_flag=AfrYS,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:AfrYF):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(AfrYE)
  status.stop()
...
def print_table(columns:Dict[AfrYT,AfrYT],rows:List[Dict[AfrYT,Any]],formatters:Dict[AfrYT,Callable[[Any],AfrYT]]=AfrYE):
 from rich.table import Table
 if formatters is AfrYE:
  formatters=AfrYq()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  AfrYm(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->AfrYE:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->AfrYE:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->AfrYE:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->AfrYE:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
