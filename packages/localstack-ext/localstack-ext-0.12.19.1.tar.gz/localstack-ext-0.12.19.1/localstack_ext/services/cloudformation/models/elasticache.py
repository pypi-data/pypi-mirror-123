...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=UNuQe(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not UNuQO
   return result
  except UNuQb:
   props=self.props
   ...
   if parts[0]in candidates and UNuQA(parts)>1:
