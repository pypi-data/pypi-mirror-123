...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or EHGfQ
 @EHGfx
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return EHGfd(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @EHGfh
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if EHGfs(origins,EHGfu)else origins.get("Items")
  origins_to_create=EHGfX([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if EHGfs(aliases,EHGfu)else aliases.get("Items")
  aliases_to_create=EHGfX(aliases)
  for distr in distrs:
   origins=EHGfX([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=EHGfX(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",EHGft)
   config["Enabled"]=EHGfW(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if EHGfs(custom_errors,EHGfm)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=EHGfW(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":EHGft}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=EHGfW(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=EHGfQ):
    parent=config if parent is EHGfQ else parent
    items=parent.get(key)or[]
    if EHGfs(items,EHGfm)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":EHGfb(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if EHGfs(cache_behavs,EHGfm)else cache_behavs)
