...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=hHmcy(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not hHmcl
   return result
  except hHmcN:
   props=self.props
   ...
   if parts[0]in candidates and hHmcT(parts)>1:
