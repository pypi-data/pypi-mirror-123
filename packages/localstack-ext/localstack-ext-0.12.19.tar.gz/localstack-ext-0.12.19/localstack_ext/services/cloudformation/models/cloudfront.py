...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or KgRwz
 @KgRwA
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return KgRwB(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @KgRwa
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if KgRwG(origins,KgRwI)else origins.get("Items")
  origins_to_create=KgRwF([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if KgRwG(aliases,KgRwI)else aliases.get("Items")
  aliases_to_create=KgRwF(aliases)
  for distr in distrs:
   origins=KgRwF([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=KgRwF(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",KgRwP)
   config["Enabled"]=KgRwb(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if KgRwG(custom_errors,KgRwm)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=KgRwb(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":KgRwP}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=KgRwb(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=KgRwz):
    parent=config if parent is KgRwz else parent
    items=parent.get(key)or[]
    if KgRwG(items,KgRwm)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":KgRwO(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if KgRwG(cache_behavs,KgRwm)else cache_behavs)
