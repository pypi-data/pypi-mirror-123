...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return lyuDh if e else lyuDE
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->lyuDf:
...
@click.option("--revert",is_flag=lyuDh,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:lyuDq):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(lyuDf)
  status.stop()
...
def print_table(columns:Dict[lyuDg,lyuDg],rows:List[Dict[lyuDg,Any]],formatters:Dict[lyuDg,Callable[[Any],lyuDg]]=lyuDf):
 from rich.table import Table
 if formatters is lyuDf:
  formatters=lyuDR()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  lyuDJ(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->lyuDf:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->lyuDf:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->lyuDf:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->lyuDf:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
