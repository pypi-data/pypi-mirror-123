...
def register_localstack_plugins():
 ...
 if nEMRV([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  if "No module named" not in nEMRy(e):
...
def set_default_providers_to_pro():
 ...
 for service in pro_services:
  try:
   ...
   SERVICE_PLUGINS.get_service_container(service)
  except nEMRC as e:
...
def api_key_configured():
 return nEMRd(os.environ.get("LOCALSTACK_API_KEY"))
