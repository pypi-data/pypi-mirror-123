...
SENT_EMAILS=[]
EMAIL_BLACKLIST=tUIkS()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",tUIkr(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=tUIkb,from_name=tUIkb,smtp_host=tUIkb,smtp_user=tUIkb,smtp_pass=tUIkb,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if tUIkz(recipients,tUIkq)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=tUIkb):
 ...
 result["From"]=formataddr((tUIkr(Header(from_name,"utf-8")),from_email))
 return result
