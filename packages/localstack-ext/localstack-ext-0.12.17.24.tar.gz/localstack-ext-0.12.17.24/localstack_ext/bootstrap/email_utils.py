...
SENT_EMAILS=[]
EMAIL_BLACKLIST=dbQkU()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",dbQkY(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=dbQkK,from_name=dbQkK,smtp_host=dbQkK,smtp_user=dbQkK,smtp_pass=dbQkK,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if dbQkq(recipients,dbQkP)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=dbQkK):
 ...
 result["From"]=formataddr((dbQkY(Header(from_name,"utf-8")),from_email))
 return result
