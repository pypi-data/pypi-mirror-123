...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return IaYDE if e else IaYDH
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->IaYDx:
...
@click.option("--revert",is_flag=IaYDE,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:IaYDT):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(IaYDx)
  status.stop()
...
def print_table(columns:Dict[IaYDC,IaYDC],rows:List[Dict[IaYDC,Any]],formatters:Dict[IaYDC,Callable[[Any],IaYDC]]=IaYDx):
 from rich.table import Table
 if formatters is IaYDx:
  formatters=IaYDz()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  IaYDi(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->IaYDx:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->IaYDx:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->IaYDx:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->IaYDx:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
