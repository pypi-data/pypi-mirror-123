...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=sqULB(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not sqULK
   return result
  except sqULf:
   props=self.props
   ...
   if parts[0]in candidates and sqULy(parts)>1:
