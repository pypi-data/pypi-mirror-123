...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or vLwgQ
 @vLwgm
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return vLwgs(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @vLwgN
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if vLwgj(origins,vLwgK)else origins.get("Items")
  origins_to_create=vLwgx([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if vLwgj(aliases,vLwgK)else aliases.get("Items")
  aliases_to_create=vLwgx(aliases)
  for distr in distrs:
   origins=vLwgx([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=vLwgx(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",vLwgA)
   config["Enabled"]=vLwgf(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if vLwgj(custom_errors,vLwgT)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=vLwgf(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":vLwgA}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=vLwgf(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=vLwgQ):
    parent=config if parent is vLwgQ else parent
    items=parent.get(key)or[]
    if vLwgj(items,vLwgT)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":vLwgu(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if vLwgj(cache_behavs,vLwgT)else cache_behavs)
