...
SENT_EMAILS=[]
EMAIL_BLACKLIST=pNOMi()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",pNOMo(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=pNOMV,from_name=pNOMV,smtp_host=pNOMV,smtp_user=pNOMV,smtp_pass=pNOMV,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if pNOMj(recipients,pNOMe)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=pNOMV):
 ...
 result["From"]=formataddr((pNOMo(Header(from_name,"utf-8")),from_email))
 return result
