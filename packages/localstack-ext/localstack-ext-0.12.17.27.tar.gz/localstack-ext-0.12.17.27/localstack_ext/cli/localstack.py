...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return GVCxd if e else GVCxD
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->GVCxW:
...
@click.option("--revert",is_flag=GVCxd,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:GVCxn):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(GVCxW)
  status.stop()
...
def print_table(columns:Dict[GVCxj,GVCxj],rows:List[Dict[GVCxj,Any]],formatters:Dict[GVCxj,Callable[[Any],GVCxj]]=GVCxW):
 from rich.table import Table
 if formatters is GVCxW:
  formatters=GVCxf()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  GVCxh(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->GVCxW:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->GVCxW:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->GVCxW:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->GVCxW:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
