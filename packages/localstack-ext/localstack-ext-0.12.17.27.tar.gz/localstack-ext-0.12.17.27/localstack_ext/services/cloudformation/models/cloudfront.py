...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or PAUdT
 @PAUdB
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return PAUdO(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @PAUdg
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if PAUdr(origins,PAUdC)else origins.get("Items")
  origins_to_create=PAUdo([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if PAUdr(aliases,PAUdC)else aliases.get("Items")
  aliases_to_create=PAUdo(aliases)
  for distr in distrs:
   origins=PAUdo([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=PAUdo(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",PAUdu)
   config["Enabled"]=PAUdR(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if PAUdr(custom_errors,PAUdF)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=PAUdR(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":PAUdu}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=PAUdR(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=PAUdT):
    parent=config if parent is PAUdT else parent
    items=parent.get(key)or[]
    if PAUdr(items,PAUdF)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":PAUdH(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if PAUdr(cache_behavs,PAUdF)else cache_behavs)
