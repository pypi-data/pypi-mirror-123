...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=Urchi,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if Urchg(perm["IpProtocol"])!=Urchg(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",Urchi)
   vpc_id=Urchi
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or Urchi
    vpc_id=groups and groups[0].get("VpcId")or Urchi
   dst_group_id=result.pop("DestinationSecurityGroupId",Urchi)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=Urchg(result.get("IpProtocol"))
   description=result.pop("Description",Urchi)
   cidr_ipv6=result.pop("CidrIpv6",Urchi)
