...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=yQkJn(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not yQkJi
   return result
  except yQkJI:
   props=self.props
   ...
   if parts[0]in candidates and yQkJx(parts)>1:
