...
SENT_EMAILS=[]
EMAIL_BLACKLIST=nlIJf()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",nlIJG(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=nlIJE,from_name=nlIJE,smtp_host=nlIJE,smtp_user=nlIJE,smtp_pass=nlIJE,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if nlIJk(recipients,nlIJs)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=nlIJE):
 ...
 result["From"]=formataddr((nlIJG(Header(from_name,"utf-8")),from_email))
 return result
