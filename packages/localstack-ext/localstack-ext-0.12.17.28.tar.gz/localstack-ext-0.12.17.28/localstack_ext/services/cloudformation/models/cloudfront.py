...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or vSBDC
 @vSBDk
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return vSBDz(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @vSBDi
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if vSBDw(origins,vSBDK)else origins.get("Items")
  origins_to_create=vSBDf([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if vSBDw(aliases,vSBDK)else aliases.get("Items")
  aliases_to_create=vSBDf(aliases)
  for distr in distrs:
   origins=vSBDf([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=vSBDf(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",vSBDm)
   config["Enabled"]=vSBDs(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if vSBDw(custom_errors,vSBDq)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=vSBDs(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":vSBDm}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=vSBDs(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=vSBDC):
    parent=config if parent is vSBDC else parent
    items=parent.get(key)or[]
    if vSBDw(items,vSBDq)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":vSBDM(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if vSBDw(cache_behavs,vSBDq)else cache_behavs)
