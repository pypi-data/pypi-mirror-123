...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=ihLkx(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not ihLku
   return result
  except ihLkj:
   props=self.props
   ...
   if parts[0]in candidates and ihLkD(parts)>1:
