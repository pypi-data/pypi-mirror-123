...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return kzMty if e else kzMto
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->kzMtI:
...
@click.option("--revert",is_flag=kzMty,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:kzMth):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(kzMtI)
  status.stop()
...
def print_table(columns:Dict[kzMtT,kzMtT],rows:List[Dict[kzMtT,Any]],formatters:Dict[kzMtT,Callable[[Any],kzMtT]]=kzMtI):
 from rich.table import Table
 if formatters is kzMtI:
  formatters=kzMti()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  kzMte(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->kzMtI:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->kzMtI:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->kzMtI:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->kzMtI:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
