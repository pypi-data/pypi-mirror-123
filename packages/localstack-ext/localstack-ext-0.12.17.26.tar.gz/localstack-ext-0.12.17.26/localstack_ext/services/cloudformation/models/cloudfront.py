...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or GYcSN
 @GYcSq
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return GYcSx(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @GYcSO
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if GYcSi(origins,GYcSe)else origins.get("Items")
  origins_to_create=GYcSd([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if GYcSi(aliases,GYcSe)else aliases.get("Items")
  aliases_to_create=GYcSd(aliases)
  for distr in distrs:
   origins=GYcSd([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=GYcSd(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",GYcSb)
   config["Enabled"]=GYcSa(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if GYcSi(custom_errors,GYcSU)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=GYcSa(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":GYcSb}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=GYcSa(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=GYcSN):
    parent=config if parent is GYcSN else parent
    items=parent.get(key)or[]
    if GYcSi(items,GYcSU)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":GYcSh(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if GYcSi(cache_behavs,GYcSU)else cache_behavs)
