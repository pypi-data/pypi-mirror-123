...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=qcgDK(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not qcgDx
   return result
  except qcgDi:
   props=self.props
   ...
   if parts[0]in candidates and qcgDI(parts)>1:
