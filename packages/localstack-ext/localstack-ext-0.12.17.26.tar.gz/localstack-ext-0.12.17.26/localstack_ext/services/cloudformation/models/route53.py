...
def canonicalize_name(name:qMvtf)->qMvtf:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[qMvto])[0]
 def get_physical_resource_id(self,attribute=qMvto,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return qMvth(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @qMvtO
