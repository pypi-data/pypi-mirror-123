...
class PodInfo:
 def __init__(self,name=bXUjM,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(bXUjA):
 BACKEND="_none_"
 def __init__(self,pod_name=bXUjM,config=bXUjM):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self)->PodInfo:
  raise bXUjG("Not implemented")
 def pull(self):
  raise bXUjG("Not implemented")
 ...
 def get(cls,pod_name,pre_config=bXUjM):
  ...
  raise bXUjG('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=bXUjN
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=bXUju)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=bXUju
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise bXUjG("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @bXUjT
 def get_state_zip_from_instance(get_content=bXUjN):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise bXUjG("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:bXUjf=bXUjM):
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=bXUju)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(bXUja(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=bXUja(zip_data_content)
  return result
 def pull(self):
  ...
  rm_rf(zip_path)
 @bXUjT
 def presigned_url(pod_name:bXUjf,mode:bXUjf)->bXUjf:
  ...
  if response.status_code>=400:
   raise bXUjG("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  if res.status_code>=400:
   raise bXUjG("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=bXUju)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise bXUjG('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=bXUjH(self,"clone_dir",bXUjM)
...
class PodConfigManagerMeta(bXUjP):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=bXUjM
   for manager in cls.CHAIN:
    try:
     tmp=bXUjH(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif bXUjs(tmp,bXUjF)and bXUjs(result,bXUjF):
       result.extend(tmp)
    except bXUjG:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not bXUjM:
    return result
   raise bXUjG('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(bXUjA,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @bXUjr
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise bXUjG('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(bXUjA):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=bXUju).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=bXUjc([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=bXUjN):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except bXUjG:
   if safe:
    return{}
   raise bXUjG('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(bXUjA):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise bXUjG("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise bXUjG("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def push_state(pod_name,pre_config=bXUjM,**kwargs):
...
def pull_state(pod_name,**kwargs):
 pre_config=kwargs.get("pre_config",bXUjM)
 if not pod_name:
  raise bXUjG("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull()
 print("Done.")
def reset_local_state():
 ...
 if result.status_code>=400:
  raise bXUjG("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->bXUjf:
 try:
...
def get_persisted_resource_names(data_dir)->List[bXUjf]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and bXUja(os.listdir(entry.path))>0:
    names.append(entry.name)
