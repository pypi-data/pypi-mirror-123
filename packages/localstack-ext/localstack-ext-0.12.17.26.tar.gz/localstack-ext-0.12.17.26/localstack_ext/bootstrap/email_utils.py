...
SENT_EMAILS=[]
EMAIL_BLACKLIST=IVMHw()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",IVMHL(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=IVMHh,from_name=IVMHh,smtp_host=IVMHh,smtp_user=IVMHh,smtp_pass=IVMHh,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if IVMHy(recipients,IVMHn)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=IVMHh):
 ...
 result["From"]=formataddr((IVMHL(Header(from_name,"utf-8")),from_email))
 return result
