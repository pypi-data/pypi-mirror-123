...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return cqKsC if e else cqKsj
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->cqKsb:
...
@click.option("--revert",is_flag=cqKsC,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:cqKsL):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(cqKsb)
  status.stop()
...
def print_table(columns:Dict[cqKsH,cqKsH],rows:List[Dict[cqKsH,Any]],formatters:Dict[cqKsH,Callable[[Any],cqKsH]]=cqKsb):
 from rich.table import Table
 if formatters is cqKsb:
  formatters=cqKse()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  cqKsw(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->cqKsb:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->cqKsb:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->cqKsb:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->cqKsb:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
