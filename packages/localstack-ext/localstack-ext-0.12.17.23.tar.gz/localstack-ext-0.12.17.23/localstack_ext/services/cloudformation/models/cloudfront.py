...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or FNUrR
 @FNUrI
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return FNUrg(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @FNUrQ
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if FNUrp(origins,FNUrb)else origins.get("Items")
  origins_to_create=FNUrT([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if FNUrp(aliases,FNUrb)else aliases.get("Items")
  aliases_to_create=FNUrT(aliases)
  for distr in distrs:
   origins=FNUrT([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=FNUrT(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",FNUrq)
   config["Enabled"]=FNUrH(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if FNUrp(custom_errors,FNUri)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=FNUrH(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":FNUrq}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=FNUrH(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=FNUrR):
    parent=config if parent is FNUrR else parent
    items=parent.get(key)or[]
    if FNUrp(items,FNUri)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":FNUrW(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if FNUrp(cache_behavs,FNUri)else cache_behavs)
