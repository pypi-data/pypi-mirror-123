...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=iADly(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not iADlR
   return result
  except iADln:
   props=self.props
   ...
   if parts[0]in candidates and iADlt(parts)>1:
