...
SENT_EMAILS=[]
EMAIL_BLACKLIST=TMiby()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",TMibg(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=TMiba,from_name=TMiba,smtp_host=TMiba,smtp_user=TMiba,smtp_pass=TMiba,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if TMibO(recipients,TMibL)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=TMiba):
 ...
 result["From"]=formataddr((TMibg(Header(from_name,"utf-8")),from_email))
 return result
