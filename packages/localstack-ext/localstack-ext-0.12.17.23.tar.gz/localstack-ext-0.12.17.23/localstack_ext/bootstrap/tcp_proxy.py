...
class AttrDict(rRSUh):
 def __init__(self,*args,**kwargs):
  rRSUo(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and rRSUp(socket,"IPV6_V6ONLY"):
...
class Half(rRSUQ):
 def __init__(self,opt,sock,addr,rRSUW):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.rRSUW=rRSUW
  self.name="peer" if self.rRSUW else "client"
  self.queue=[]
  self.dest=rRSUg
  self.err=rRSUg
  self.ready=rRSUw
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={"ip6":rRSUw,"bind_port":2223,"addr":"192.168.43.191","port":22}
 opt={"ip6":rRSUw,"bind_port":48429,"addr":"128.0.100.3","port":22}
 opt={"ip6":rRSUw,"port":54237,"bind_addr":"128.0.100.2","bind_port":22}
 opt={"ip6":rRSUw,"port":54237,"bind_addr":"192.168.100.3","bind_port":22}
 server_loop(opt)
