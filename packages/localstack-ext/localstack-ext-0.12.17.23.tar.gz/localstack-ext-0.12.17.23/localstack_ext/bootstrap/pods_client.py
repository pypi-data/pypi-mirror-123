...
class PodInfo:
 def __init__(self,name=OpxYA,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(OpxYd):
 BACKEND="_none_"
 def __init__(self,pod_name=OpxYA,config=OpxYA):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self)->PodInfo:
  raise OpxYn("Not implemented")
 def pull(self):
  raise OpxYn("Not implemented")
 ...
 def get(cls,pod_name,pre_config=OpxYA):
  ...
  raise OpxYn('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=OpxYJ
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=OpxYg)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=OpxYg
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise OpxYn("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @OpxYL
 def get_state_zip_from_instance(get_content=OpxYJ):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise OpxYn("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:OpxYy=OpxYA):
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=OpxYg)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(OpxYG(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=OpxYG(zip_data_content)
  return result
 def pull(self):
  ...
  rm_rf(zip_path)
 @OpxYL
 def presigned_url(pod_name:OpxYy,mode:OpxYy)->OpxYy:
  ...
  if response.status_code>=400:
   raise OpxYn("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  if res.status_code>=400:
   raise OpxYn("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=OpxYg)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise OpxYn('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=OpxYB(self,"clone_dir",OpxYA)
...
class PodConfigManagerMeta(OpxYj):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=OpxYA
   for manager in cls.CHAIN:
    try:
     tmp=OpxYB(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif OpxYm(tmp,OpxYM)and OpxYm(result,OpxYM):
       result.extend(tmp)
    except OpxYn:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not OpxYA:
    return result
   raise OpxYn('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(OpxYd,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @OpxYI
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise OpxYn('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(OpxYd):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=OpxYg).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=OpxYw([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=OpxYJ):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except OpxYn:
   if safe:
    return{}
   raise OpxYn('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(OpxYd):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise OpxYn("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise OpxYn("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def push_state(pod_name,pre_config=OpxYA,**kwargs):
...
def pull_state(pod_name,**kwargs):
 pre_config=kwargs.get("pre_config",OpxYA)
 if not pod_name:
  raise OpxYn("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull()
 print("Done.")
def reset_local_state():
 ...
 if result.status_code>=400:
  raise OpxYn("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->OpxYy:
 try:
...
def get_persisted_resource_names(data_dir)->List[OpxYy]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and OpxYG(os.listdir(entry.path))>0:
    names.append(entry.name)
