...
SENT_EMAILS=[]
EMAIL_BLACKLIST=MSWgU()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",MSWgY(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=MSWgx,from_name=MSWgx,smtp_host=MSWgx,smtp_user=MSWgx,smtp_pass=MSWgx,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if MSWgb(recipients,MSWgf)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=MSWgx):
 ...
 result["From"]=formataddr((MSWgY(Header(from_name,"utf-8")),from_email))
 return result
