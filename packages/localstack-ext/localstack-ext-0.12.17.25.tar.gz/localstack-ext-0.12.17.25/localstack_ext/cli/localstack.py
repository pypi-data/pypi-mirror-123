...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return AvgBm if e else AvgBE
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->AvgBn:
...
@click.option("--revert",is_flag=AvgBm,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:AvgBT):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(AvgBn)
  status.stop()
...
def print_table(columns:Dict[AvgBj,AvgBj],rows:List[Dict[AvgBj,Any]],formatters:Dict[AvgBj,Callable[[Any],AvgBj]]=AvgBn):
 from rich.table import Table
 if formatters is AvgBn:
  formatters=AvgBl()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  AvgBb(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->AvgBn:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->AvgBn:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->AvgBn:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->AvgBn:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
