...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=tmjSy(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not tmjSR
   return result
  except tmjSP:
   props=self.props
   ...
   if parts[0]in candidates and tmjSd(parts)>1:
