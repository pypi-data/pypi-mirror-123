...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or gFyxH
 @gFyxI
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return gFyxL(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @gFyxj
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if gFyxf(origins,gFyxl)else origins.get("Items")
  origins_to_create=gFyxn([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if gFyxf(aliases,gFyxl)else aliases.get("Items")
  aliases_to_create=gFyxn(aliases)
  for distr in distrs:
   origins=gFyxn([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=gFyxn(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",gFyxS)
   config["Enabled"]=gFyxk(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if gFyxf(custom_errors,gFyxu)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=gFyxk(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":gFyxS}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=gFyxk(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=gFyxH):
    parent=config if parent is gFyxH else parent
    items=parent.get(key)or[]
    if gFyxf(items,gFyxu)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":gFyxd(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if gFyxf(cache_behavs,gFyxu)else cache_behavs)
