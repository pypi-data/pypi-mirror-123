...
class Optimization(ABC):
        ...
        for gen in range(self.n_gen):
            ...
            for index, individual in enumerate(self.pop):
                ...
                try:
                    ...
                    score = self.calculate_score(fom_dct)
                    scores.append(score)
    ...
    @abstractmethod
    def calculate_figures_of_merit(self, individual: List[int]) -> dict:
...
class GeneticOptimization(Optimization, ABC):
    ...
    def initialize_design_variables(design_variables_df: pd.DataFrame) -> List[GeneticDesignVariable]:
        ...
        for index, row in design_variables_df.iterrows():
            design_variables.append(GeneticDesignVariable(**row.to_dict()))
...
class RoxieGeneticOptimization(GeneticOptimization):
    ...
    def initialize_population(self) -> List[List[int]]:
        ...
        for _ in range(self.n_pop):
            chromosome = RoxieGeneticOptimization.generate_random_chromosome_programmable_geometry(self.design_variables)
    ...
    def decode_chromosome_programmable_geometry(self, individual: List[int]):
        decoded = {}
    ...
    def update_model_parameters(self, decoded_chromosome: Dict) -> Geometry:
        ...
        for block_variable, value in decoded_chromosome.items():
            if ':-:' in block_variable:
                ...
                if variable == 'current':
                    geometry_model.blocks[block_index - 1].block_def.current = value
            elif ':nan:' not in block_variable:
                ...
                if variable == 'nco':
                    geometry_model.blocks[block_index].block_def.nco = value
                elif variable == 'alpha':
                    geometry_model.blocks[block_index].block_def.alpha = value
                elif variable == 'alpha_r':
                    geometry_model.blocks[block_index].block_def.alpha_r = value
                elif variable == 'phi':
                    geometry_model.blocks[block_index].block_def.phi = value
                elif variable == 'phi_r':
                    geometry_model.blocks[block_index].block_def.phi_r = value
