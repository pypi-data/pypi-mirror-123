...
class TestOptimizationCockpitWidget(TestCase):
    ...
    def test_build_programmable_geometry(self):
        model_input_path = create_resources_file_path('resources/geometry/roxie/16T/16T_rel_slotted.json')
        cadata_abs_path = create_resources_file_path('resources/geometry/roxie/16T/roxieold_2.cadata')
        cadata = CableDatabase.read_cadata(cadata_abs_path)
        geometry = GeometryFactory.init_with_json(model_input_path, cadata)
        ...
        prefix = 'resources/optimization/'
        logger_path = create_resources_file_path('%sGeneticOptimizationProgrammableGeometry.csv' % prefix)
