...
export default class BaseStorage {
  ...
  save() {
    throw new Error(`Implement abstract method "save" to save files in storage '${this.storageType}'.`);
  }

  cancel() {
    throw new Error(`Implement abstract method "cancel" to cancel uploads to storage '${this.storageType}'.`);
  }
  ...
  isPausable() {
    throw new Error(`Implement abstract method "isPausable" to check if an upload to storage '${this.storageType}' can`
                    + ' be paused.');
  }

  isResumable() {
    throw new Error(`Implement abstract method "isResumable" to check if an upload to storage '${this.storageType}' can`
                     + ' be resumed.');
  }
