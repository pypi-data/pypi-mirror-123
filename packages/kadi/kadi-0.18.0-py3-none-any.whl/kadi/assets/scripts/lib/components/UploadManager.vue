...
import LocalStorage from 'scripts/lib/storage/local.js';
...
export default {
  ...
  computed: {
    ...
    pausable() {
      return this.uploads.slice().some((upload) => this.storages[upload.storageType].isPausable(upload));
    },
    resumable() {
  ...
  methods: {
    addFile(file, force = false, origin = null) {
      const upload = this.storages.local.open({
        file,
        name: file.name,
        size: file.size,
        forceReplace: force,
    ...
    inputChange(e) {
      ...
      if (file.name !== this.resumedUpload.name) {
        if (!window.confirm(i18n.t('warning.fileNameDiffers'))) {
          return;
        }
      }
    ...
    async handleError(error, upload) {
      ...
      if (error.request.status === 413) {
        ...
        kadi.alert(i18n.t('error.initiateUpload'), {request: error.request});
      }
    ...
    async uploadNextFile() {
      ...
      await this.storages.local.save(upload);
    ...
    cancelUploads(force, upload = null) {
      ...
      for (const _upload of uploads) {
        ...
        this.storages.local.cancel(_upload)
          .then(() => _removeUpload(_upload))
          .catch((error) => {
            if (error.request.status !== 404) {
              kadi.alert(i18n.t('error.removeUpload'), {request: error.request});
            } else {
    ...
    pauseUploads(force, upload = null) {
      ...
      for (const _upload of uploads) {
        const storage = this.storages[_upload.storageType];
    ...
    isResumable(upload) {
      return this.storages[upload.storageType].isResumable(upload);
    },
  ...
  mounted() {
    const localStorage = new LocalStorage(this.newUploadEndpoint, this.getUploadsEndpoint);

    localStorage.onComplete((upload, data) => this.$emit('upload-completed', data._meta.file, upload.origin));
    localStorage.onPause(this.pauseUploads);
    localStorage.onCancel(this.cancelUploads);
    localStorage.onError(this.handleError);

    this.storages.local = localStorage;

    // Get the active uploads so the user can resume them.
    localStorage.loadUploads()
      .then((fetchedUploads) => {
        this.uploads = this.uploads.concat(fetchedUploads);
      })
      .catch((error) => {
        kadi.alert(i18n.t('error.loadUploads'), {request: error.request});
      });
