...
def kadi_register_blueprints(app):
    ...
    An example Flask blueprint, including a custom static path, could look like the
    following:
...
def kadi_publish_record(record, provider, client, token, task):
    ...
    publishing operation, the plugin has to return a tuple containing the result status
    of the operation and a template further describing the result in a user-readable
    manner, e.g. containing a link to view the published result if the operation was
    successful.
    ...
    :param task: A :class:`.Task` object that may be provided if this hook is executed
        ...
        cancelled and to update the current progress of the operation via the task.
    """
...
@hookspec
def kadi_get_preview_templates():
...
def kadi_post_resource_change(resource, user, created):
    ...
    Currently, this includes all revisioned resources, namely records, files,
    collections and groups.

    Note that the hook is only executed after all changes have been persisted in the
    database and the creation or change triggered the creation of a new revision. For
    reacting to specific changes, the resource type can be checked using ``isinstance``,
    e.g. ``isinstance(resource, Record)``, while the latest revision of a resource can
    be retrieved via ``resource.ordered_revisions.first()``.

    :param resource: The resource that was created or changed.
    :param user: The user that triggered the creation or change. Defaults to the current
        user.
...
def kadi_get_resource_nav_items(resource):
    ...
    Currently, this includes the menus of records, files, collections, groups and
    templates.

    For limiting the returned items to a specific resource, the resource type can be
    checked using ``isinstance``, e.g. ``isinstance(resource, Record)``. For ignored
    resources, ``None`` can be returned instead.

    :param resource: The resource which the navigation menu belongs to.
    """
