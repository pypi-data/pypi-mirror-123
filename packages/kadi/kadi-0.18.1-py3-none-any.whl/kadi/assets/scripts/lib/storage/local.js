...
export default class LocalStorage extends BaseStorage {
  ...
  async save(upload) {
    ...
    if (!upload.chunkCount) {
      try {
        await LocalStorage.initiateUpload(upload);
      } catch (error) {
        ...
        if (error.request.status === 409) {
          try {
            // Restart the upload.
            await LocalStorage.initiateUpload(upload, error.response.data.file._actions.edit_data);
    ...
    try {
      ...
      if (error.request.status === 413) {
        ...
        kadi.alert(i18n.t('error.finishUpload'), {request: error.request});
      }
  ...
  getUploadProgress(upload, additionalSize = 0) {
    ...
    return ((LocalStorage.getTotalChunkSize(upload) + additionalSize) / upload.size) * 100;
  }
  ...
  isResumable(upload) {
    ...
    return upload.state === 'paused'
      && (upload.file !== null || LocalStorage.getTotalChunkSize(upload) === upload.size);
  }
    ...
    return axios.get(this.getUploadsEndpoint)
      .then((response) => {
        ...
        response.data.items.forEach((upload) => {
          upload.storageType = this.storageType;
  ...
  finalizeUpload(upload) {
    ...
    const _updateStatus = () => {
      ...
      axios.get(upload.getStatusEndpoint)
        ...
        .catch((error) => {
          kadi.alert(i18n.t('error.updateUpload'), {request: error.request});
        });
    };
  ...
  async uploadChunks(upload) {
    ...
    while (true) {
      ...
      const chunkIndex = LocalStorage.findNextChunkIndex(upload);
      ...
      while (true) {
        ...
        try {
          ...
          timeout += 5000;
          kadi.alert(i18n.t('error.uploadChunk', {timeout: timeout / 1000}), {request: error.request, timeout});
