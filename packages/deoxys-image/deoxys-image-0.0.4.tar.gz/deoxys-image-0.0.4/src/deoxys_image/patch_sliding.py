...
def check_drop(images, patch_indice, patch_size,
    ...
    for i, indice in patch_indice:
        ...
        res.append(passed > drop_fraction)
        percentage.append(passed)

    return res, np.array(percentage)
...
def get_patches(images, target=None, patch_indice=None, patch_size=None,
    ...
    pixel_sum = np.prod(patch_size)
    # finding number of patches to fit in one image
    # total patches / number of patches to fill in the image
    patch_num = int(np.prod(images.shape[1:-1]) // pixel_sum)
    ...
    if drop_fraction > 0:
        if bounding_box:
            check_drop_list, label_percentage = check_drop(
                target, patch_indice, patch_size,
                drop_fraction, check_drop_channel)
        else:
            check_drop_list = check_drop(images, patch_indice, patch_size,
                                         drop_fraction, check_drop_channel)[0]

        patch_indice = patch_indice[check_drop_list]

        # stratify here
        if target is not None and stratified and bounding_box:
            new_index = np.arange(len(patch_indice))
            if batch_size is None or len(patch_indice) < batch_size:
                np.random.shuffle(new_index)
            else:
                # finding number of step to skip in the list
                # actual_patch_num / (patch_num_per_image * image_num)
                steps = max(len(patch_indice)//(patch_num*len(images)), 1)

                # get the shuffle & stratified indice here
                new_index = get_stratified_index(
                    label_percentage[check_drop_list], batch_size, steps)
            patch_indice = patch_indice[new_index]
    ...
    for i, (im_i, indice) in enumerate(patch_indice):
        if len(indice) == 2:
            ...
            if target is not None:
                ...
                if stratified and not bounding_box:
    ...
    if target is None:
        ...
        if stratified and not bounding_box:
            ...
            if batch_size is None or len(patch_indice) < batch_size:
                ...
                # get the shuffle & stratified indice here
                new_index = get_stratified_index(
                    label_percentage, batch_size, steps)
...
def get_stratified_index(values, sample_size, steps):
    # find the median values and split the list into larger and smaller group
    ...
    # number of postitive number in one batch
    pos_num = len(positive_index) // split_num
    step = 1

    inners = [[] for _ in range(split_num)]
    # cannot evenly distribute in each batch, increase number of step to skip
    if pos_num == 0:
        step = int(np.ceil(split_num / len(positive_index)))

    # add positive indice into all groups
    ...
    # add negative indice into free slots in each group
    for inner in inners:
    ...
    return np.concatenate(inners[::steps])
