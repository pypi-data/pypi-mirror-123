...
from platform_services_lib.lib.context.urls import sam_urls, identity_urls
...
class DliClient(AutoRegMetadata, Datafile, Dataset, Dictionary, Package):
    ...
    def __init__(self, api_root, debug=None, strict=True,
        ...
        __sender = AnalyticsSender(__config, self._session)
...
class Session(requests.Session):
    ...
    def _reload_or_web_flow(self, raises=False):
        ...
        if check_key:
            try:
                reloaded = check_key
                decoded = self.__decode_token(str.encode(reloaded))
    ...
    def _get_decoded_token(self):
        return self.__decode_token(self.auth_key)
