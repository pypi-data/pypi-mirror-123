...
class TestExecuteEquation(unittest.TestCase):
    ...
    def test_zones_as_generator(self):
        # generator of zone objects
        zones_gen = (z for z in [self.z_inner])
        self.z_inner.values('X')[1] = 3.1415
        self.assertFalse(np.allclose(self.z_inner.values('X')[:],
                                     self.z_inner.values('Y')[:]))
        tp.data.operate.execute_equation('X=Y', zones=zones_gen)
        self.assertTrue(np.allclose(self.z_inner.values('X')[:],
                                    self.z_inner.values('Y')[:]))

        # generator of indices
        zones_gen = (z.index for z in [self.z_inner])
        self.z_inner.values('X')[1] = 3.1415
        self.assertFalse(np.allclose(self.z_inner.values('X')[:],
                                     self.z_inner.values('Y')[:]))
        tp.data.operate.execute_equation('X=Y', zones=zones_gen)
