...
def enc_apl(bng, x, met):
    ...
    for i in range(m):
        ...
        xe = nans((N, mi))
        for h in range(1, len(bns) + 1):
            if met == 'dv' and not bns[h]['n'] > 0: np.delete(xe, h, axis=1); continue
        ...
        print('isnan(xe): ', np.any(np.isnan(xe)))
        if np.any(np.isnan(xe)):
            prinr("DA EBA MAMA TI BEEEEEEEEEEEEE!!!!!!!!!!!!!!!!!")
...
def nabub(x, w, sv, xtp, ux, y, ytp, yi):
    ...
    bns = {}
    if len(y):
        bns[nu] = setbin([0], [0], xtp, 'Normal', zeros((1, len(ytp))), ytp, yi)
    else:
...
def passChi2(df, df_den, bm12, pt, ytp, ba):
    ...
    if ytp[0] == 'bin':
        ...
        if apt < 1e-200:
            ...
            chi_pass = inv_distr('Chi2', apt, dummy, df2, left, right, tol)
    else:
        left = 0
        right = 1
...
def reduce_bns1(bns, xtp, g, b, nw, gd, bd):
    ...
    try:
        ...
        print('Error: Binning stats are wrong...')  # todo: make it real error
    keep_bin = 1
    while keep_bin < len(bns) - 1:
        ...
        if bin_type_drop != 'Missing' and cnd_mrg(gi, bi, nwi, gd, bd, p1):
            if keep_bin < drop_bin:
                bns = mrgbns(bns, keep_bin, drop_bin)
            else:
                bns = mrgbns(bns, drop_bin, keep_bin)
        else:
            keep_bin += 1
    return bng
...
###################################################################################
def sbng(bng, md=5, skip_mos=['m', 'o', 'sv'], met_dist='manh', met='co1y', nmin_uy=[50, 50], mar=np.inf, pt=0.05, ba=0,
         maxprm=20000, dGBInd=5, tolmindGBInd=5, mt=np.nan, minB=50, tolminB=20, stats=True, dsp=False):
    ...
    for i in range(m):
        sb[i] = {}
        if m == 1:
            bns = bng['bns'];       cname = bng['cname'];       xtp = bng['xtp'];       ytp = bng['ytp']
        else:
            bns = bng[i]['bns'];    cname = bng[i]['cname'];    xtp = bng[i]['xtp'];    ytp = bng[i]['ytp']
        ...
        sb[i]['st'] = {}
        sb[i]['st']['D'] = D
        if stats: st, sb[i]['bns'] = stbng('sb', sb[i]['bns'], xtp, ytp)

        if dsp: print('Run SB: ', i + 1, '/', m, cname, ',      time: ', toc(0))
...
def spih_sb_co1y(bns, xtp, ytp, md, gd, bd, mar, pt, ba, dGBInd, tolmindGBInd, mt, minB, tolminB, maxprm):
    ...
    for md in mm:
        ...
        m = len(bns1)
        if ba:
            pass  # [bm1, bm2] = bonfadj(mp, k, md, xtp)
        else:
            bm1 = bm2 = 1
        Nw = sum(nw)
        df = md - 1
...
def ubng(x, xtp, w, md=100, prcd=1, nmin=50, met='enr', sv=[], xi_order=[], y=np.empty((0, 0)), ytp='', yi=[],
         cnames=None, stats=False, dsp=False):
    m = x.shape[1]
    ...
    for i in range(m):
        ...
        ub[i]['xtp'] = xtpi
        ub[i]['ytp'] = ytp
        print('Run UB: ', i + 1, '/', m, cnames[i], ',      time: ', toc(0))
