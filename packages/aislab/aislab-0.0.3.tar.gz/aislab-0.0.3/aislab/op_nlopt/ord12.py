...
# def cgrad():
###################################################################################
def newton(x, func, grad, hes, args, met_opt='newt', maxiter=50, fcnv=None, afcnv=None, xcnv=None, gcnv=1e-6, sc=None, dsp_op=[2], smeth='fprev2', su=1, p_incr=1.2, p_decr=0.6, rcondH=1e-6):
...
#---------------------------------------------
    ...
    while iterate:
        ...
        if np.all(np.all(H == 0)):
            ...
            msg = 'H'
            continue
        iterate, msg = stopcrt(x, F, xx, FF, g, H, itr, maxiter, fcnv, afcnv, xcnv, gcnv, sc)
    ...
    if dsp_op: dspl(flg=1, x=x, xx=xx, F=F, FF=FF, g=g, H=H, itr=itr, msg=msg, maxiter=maxiter, fcnv=fcnv, afcnv=afcnv, xcnv=xcnv, gcnv=gcnv)
