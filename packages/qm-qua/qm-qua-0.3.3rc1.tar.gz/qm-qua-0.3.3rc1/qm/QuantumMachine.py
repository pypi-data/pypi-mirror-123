...
from qm._errors import CompilationException
...
class QuantumMachine(object):
    ...
    def execute(
        ...
        **kwargs,
        ...
        if type(simulation) is SimulationConfig:
            logger.info(
                "to simulate a program, use QuantumMachinesManager.simulate(..)"
            )
            kwargs.pop("simulate")
    ...
    def compile(self, program: _Program, **kwargs) -> str:
        """
        EXPERIMENTAL: Compiles a QUA program to be executed later. The returned
        `program_id` can then be directly added to the queue. For a detailed explanation
        see `Precompile Jobs <features.html#precompile-jobs>`__.
    ...
    def set_correction(self, qe, values):
        ...
        **DEPRECATED** This method is deprecated. Using this method will update the correction for ALL elements that use the same mixer config
        ...
        """
        logger.warning(
            "set_correction(..) is deprecated. use set_mixer_correction(..) instead"
        )
    ...
    def set_frequency(self, qe, freq):
        ...
        **DEPRECATED** This method is deprecated. Use set_intermediate_frequency(..) instead
        ...
        logger.warning(
            "set_frequency(..) is deprecated. use set_intermediate_frequency(..) instead"
        )
        if type(qe) is not str:
            raise Exception("qe must be a string")
    ...
    def get_dc_offset_by_qe(self, qe, input):
        ...
        **DEPRECATED** This method is deprecated. Use get_output_dc_offset_by_element(..) instead
        ...
        """
        logger.warning(
            "get_dc_offset_by_qe(..) is deprecated. use get_output_dc_offset_by_element(..) instead"
        )
    ...
    def set_dc_offset_by_qe(self, qe, input, offset):
        ...
        ***DEPRECATED***  "This method is deprecated. Use set_output_dc_offset_by_element(..) instead." *****
        ...
        """
        logger.debug(
            "This method is deprecated. Use set_output_dc_offset_by_qe(..) instead."
        )
    ...
    def get_running_job(self) -> Optional[QmJob]:
        ...
        if response.jobId == "":
            ...
            return QmPendingJob(response.jobId, self, self._manager).wait_for_execution(
                timeout=10
            )
