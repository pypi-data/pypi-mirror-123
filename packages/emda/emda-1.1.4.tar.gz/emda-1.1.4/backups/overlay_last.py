...
class EmFit:
    ...
    def minimizer(self, ncycles, t, rotmat, ifit, smax_lf, fobj=None, q_init=None):
        ...
        # part 1: Applying initial transformation
        assert np.ndim(rotmat) == 2
        # apply rotation
        cx, cy, cz = self.e0.shape
        maps2send = np.stack((self.e1, self.cfo), axis = -1)
        maps = fcodes_fast.tricubic(rm=rotmat,
                                    f=maps2send,
                                    mode=0,
                                    nc=2,
                                    nx=cx, ny=cy, nz=cz)        
        ...
        for i in range(ncycles):
            ...
            if i % 2 == 0:
                ...
                #print('q_init: ', q_init)
                #self.q = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float64)#q_init
                # apply rotation on e1
                q = quaternions.quaternion_multiply(self.q, q0)
                q = q / np.sqrt(np.dot(q, q))
                #maps2send = np.stack((self.ert, self.crt), axis = -1)
                maps = fcodes_fast.tricubic(rm=quaternions.get_RM(q),
                                    f=maps2send,
                                    mode=0,
                                    nc=2,
                                    nx=cx, ny=cy, nz=cz)  
