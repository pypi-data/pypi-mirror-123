...
from emda.ext.utils import filter_fsc
...
def prefilter_order(f1, axis, order, ibin, nbin, bin_idx):
    from emda.ext.overlay import get_avg_fsc
    # prime factorization
    ...
    if len(factors) == 1:
        ...
        for fac in factors:
            ...
            rotmat = quaternions.get_RM(q)
            frt = get_FRS(rotmat, f1, interp="linear")[:, :, :, 0]
            fsc, _, bincounts = core.fsc.anytwomaps_fsc_covariance(f1, frt, bin_idx, nbin)#[0]
            avg_fsc = get_avg_fsc(binfsc=fsc, bincounts=bincounts)
            #avg_fsc = np.average(fsc[:ibin])
