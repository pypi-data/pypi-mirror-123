...
def read_file(url):
    ...
    {'a': 'x', 'b': 'y', 'c': 'z'}
    """
    with _read_file_response(url) as r:
        return r.content


def _read_file_response(url):
    resp = _getSession().get('v2/storageLocations/files/content', params={'url': url, 'no-redirect': True})
    file = resp.json()['links']['content']

    # use a different requests.session than the one we use for Rally
    return requests.get(file, timeout=120, stream=True)
...
def read_files(*urls):
    ...
    >>> for f in read_files('rsl://location/file.txt', 'rsl://location/file2.txt'):
    ...     print(f)
    b'some text'
    b'more text'
    """
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(_read_file_response, url) for url in urls]
