...
from cdft1d.io_utils import print_banner
...
def analyze_rdf_peaks(ifft, nametag, h_r):
    ...
    for i, name in enumerate(nametag):
        ...
        print(f"{name} first  max height/position: {rgrid[imax0]:<.2f} {h_r[i, imax0] + 1:<.2f}  ")
        print(f"{name} second max height/position: {rgrid[imax1]:<.2f} {h_r[i, imax1] + 1:<.2f}  ")
        print(f"{name} first  min height/position: {rgrid[imin0]:<.2f} {h_r[i, imin0] + 1:<.2f}  ")
        print("  ")
