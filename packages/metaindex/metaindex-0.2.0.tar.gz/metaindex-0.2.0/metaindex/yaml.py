...
def get(filename, prefix):
    if isinstance(filename, (pathlib.Path, str)):
        logging.debug(f"Reading YAML metadata from {filename}")
...
def get_for_collection(filename, prefix, basepath=None):
    if isinstance(filename, pathlib.Path):
        logging.debug(f"Reading collection YAML metadata from {filename}")
        with open(filename, "rt", encoding="utf-8") as fh:
    ...
    if not any([isinstance(value, dict) for value in data.values()]):
        ...
        logging.debug(f"Assuming flat yaml file means it's for all files in {basepath}")
        data = {'*': data}
...
def _read_yaml_file(filename):
    ...
    if not isinstance(data, dict):
        logging.error(f"YAML metadata file {filename} does not contain a dictionary")
        return False, {}
