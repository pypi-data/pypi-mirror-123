    ...
    def parse(text, prefix=None):
        ...
        for linenr, line in enumerate(text.split("\n")):
            ...
            if directive == 'match':
                ...
                logging.warning(f"Unknown directive '{directive}' in line {linenr}")
...
class Rule:
    ...
    def parse_find(self, text, linenr):
        if ' ' not in text:
            logging.warning(f"Incomplete 'find' directive in {linenr}")
            return
    ...
    def parse_set(self, text, linenr):
        if ' ' not in text:
            logging.warning(f"Incomplete 'set' directive in {linenr}")
            return
        ...
        if section in self.config:
            for entry in self.config[section]:
                ...
                if fullpath is None or not fullpath.is_file():
                    continue
                ruleset = Rule.parse(fullpath.read_text())
                self.rules += ruleset

    def run(self, path, info):
        if 'ocr.fulltext' not in info:
            return False, {}
    
        extra = multidict.MultiDict()

        for fulltext in info.getall('ocr.fulltext'):
            for rule in self.rules:
