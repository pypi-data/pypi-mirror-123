...
def find(config, args):
    ...
    if args.link is not None:
        ...
        if symlink_folder.exists() and not symlink_folder.is_dir():
            logging.fatal(f"{symlink_folder} is not a directory.")
            return 1
        else:
        ...
        if args.force and files-symlinks > 0:
            logging.fatal(f"Can not create symbolic links in {symlink_folder}: some files are not symlinks.")
            return 2

        if not args.force and files > 0:
            logging.fatal(f"Can not create symbolic links in {symlink_folder}: not empty")
            return 2
