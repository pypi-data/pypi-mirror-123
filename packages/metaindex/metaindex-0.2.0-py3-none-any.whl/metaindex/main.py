...
def run():
    ...
    # load indexer addons
    if configuration.ADDONSPATH.exists():
        prev_sys_path = sys.path.copy()
        sys.path = [str(configuration.ADDONSPATH)]
        for item in configuration.ADDONSPATH.iterdir():
            if item.is_file() and item.suffix == '.py':
                logging.info(f"Loading addon {item.name}")
                importlib.import_module(item.stem)
        sys.path = prev_sys_path

    # drop ignored indexers
    indexer.remove_indexers(config.list('General', 'ignore-indexers', ''))
