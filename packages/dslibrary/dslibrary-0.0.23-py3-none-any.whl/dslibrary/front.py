...
from .utils.format_sniffer import find_url_extension, detect_format, looks_like_text_file
...
class DSLibrary(object):
    ...
    def open_resource(self, resource_name: str, mode: str='rb', **kwargs) -> io.RawIOBase:
        ...
        storage_options = open_args.pop("storage_options", None)
        if storage_options:
            # for dask read methods we pass these as 'storage_options', everywhere else they are open_args
            open_args.update(storage_options)
    ...
    def load_dataframe(self, resource_name: str, **kwargs):
        ...
        use_dask = open_args.pop("dask", None)
        ...
        format_info = detect_format(
            uri, sampler=sampler, sizer=None, supplied_options=format_options, fallback_to_text=fallback_to_text
        )
