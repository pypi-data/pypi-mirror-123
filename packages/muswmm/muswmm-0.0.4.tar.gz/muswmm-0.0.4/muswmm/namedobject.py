# -*- coding: utf-8 -*-
"""
Created on Tue Sep 28 23:54:35 2021
...
class _Namedobj():
    ...
    @name.setter
    def name(self, value):
        if self._objs.GetIndex(value) < 0:
            raise Exception(ERROR_IDNOTEXIST.format(value))
...
class Subcatchment(_Namedobj):
    ...
    def outlet(self, value):
        if not self._prj.NameControl_1.Contain(value):
            raise Exception(ERROR_INVALID_ID.format(value))
    ...
    @gage.setter
    def gage(self,value):
        if not self._prj.RainGages.Contain(value):
            raise Exception(ERROR_INVALID_ID.format(value))
    ...
    def area(self, value):
        if value <= 0.0:
            raise Exception(ERROR_INVALID_NUM.format(value))
