...
class RC2(object):
    ...
    def __init__(self, formula, solver='g3', adapt=False, delay=False,
        ...
        # delayed core assumptions and clauses (if core delay is in use)
        self.sdel, self.cdel, self.adel = [], [], []
    ...
    def compute_(self):
        ...
        while True:
            ...
            while not self.oracle.solve(assumptions=self.sels + self.sums):
                ...
                # process the core and activate it or delay
                self.process_core()
            ...
            # do not stop if there are delayed sum assumptions
            # and clauses; activate all of them and proceed
            if self.sdel or self.cdel or self.adel:
                self.enforce_delayed()
                continue

            return True

    def enforce_delayed(self):
        """
            Activate the delayed sum assumptions and add the corresponding
            clauses and atmost constraints to the solver.
        """

        if self.verbose > 1:
            if self.cdel:
                print('c delayed: {0} assumps, {1} clauses'.format(len(self.sdel), len(self.cdel)))
            else:
                print('c delayed: {0} assumps, {1} atmosts'.format(len(self.sdel), len(self.adel)))

        # clauses
        for cl in self.cdel:
            self.oracle.add_clause(cl)

        # native atmost constraints
        for am in self.adel:
        ...
        if self.exhaust:
            self.delay = False
            for assump in self.sdel:
                self.minw = self.wght[assump]
                del self.wght[assump]
                t = self.tobj[assump]
                b = self.exhaust_core(t)
    ...
    def process_core(self):
        ...
        if len(self.core_sels) != 1 or len(self.core_sums) > 0:
            ...
            if len(self.rels) > 1:
                ...
                b = self.exhaust_core(t) if self.exhaust and not self.delay else 1
        ...
        if not self.oracle.supports_atmost():  # the case of standard totalizer encoding
            ...
            if t.nof_new:
                if not self.delay:
                    ...
                    self.cdel += t.cnf.clauses[-t.nof_new:]
    ...
    def set_bound(self, tobj, rhs):
        ...
        if not self.delay:
            ...
            # delaying this core until it becomes useful
            self.sdel.append(-tobj.rhs[rhs])
...
class RC2Stratified(RC2, object):
    ...
    def __init__(self, formula, solver='g3', adapt=False, blo='div',
            delay=False, exhaust=False, incr=False, minz=False, nohard=False,
            trim=0, verbose=0):
        """
            Constructor.
        """
        ...
        super(RC2Stratified, self).__init__(formula, solver=solver,
                adapt=adapt, delay=delay, exhaust=exhaust, incr=incr,
                minz=minz, trim=trim, verbose=verbose)
...
def parse_options():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'ab:c:de:hil:ms:t:vx',
                ['adapt', 'block=', 'comp=', 'delay', 'enum=', 'exhaust',
                    'help', 'incr', 'blo=', 'minimize', 'solver=', 'trim=',
                    'verbose', 'vnew'])
    ...
    return adapt, blo, block, cmode, delay, to_enum, exhaust, incr, minz, \
...
def usage():
    ...
    print('        -d, --delay              Delay core activation (helps detecting dosjoint cores).')
...
if __name__ == '__main__':
    adapt, blo, block, cmode, delay, to_enum, exhaust, incr, minz, solver, \
    ...
    if files:
        ...
        # starting the solver
        with MXS(formula, solver=solver, adapt=adapt, delay=delay,
                exhaust=exhaust, incr=incr, minz=minz, trim=trim,
