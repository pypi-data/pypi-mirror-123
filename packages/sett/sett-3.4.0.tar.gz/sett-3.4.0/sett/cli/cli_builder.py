...
class SubcommandBase:
    ...
    def __init__(
        self, name, action, arguments, help=None
...
class SubcommandGroup(SubcommandBase):
    ...
    def __init__(self, name, *fcts, help=None):  # pylint: disable=redefined-builtin
        ...
        super().__init__(name=name, action=call, arguments=arguments, help=help)
...
class Argument:
    ...
    @classmethod
    def bool(cls, dest, arg_names, positional: bool, **kwargs):
        if positional:
            return cls.positional_bool(dest, arg_names, **kwargs)
        return cls.keyword_bool(dest, arg_names, **kwargs)
    ...
    @classmethod
    def keyword_bool(
        cls, dest, arg_names, default=False, help=None, **kwargs
    ):  # pylint: disable=redefined-builtin
...
def arguments_by_signature(
    f, overrides=None, omit_keywords=frozenset(), omit_positionals=frozenset()
    ...
    if unknown_parameters:
        raise ValueError(
            f"Function {f.__name__} does not have arguments {', '.join(unknown_parameters)} "
            f"(which have been provided as parameter docs)"
        )
    ...
    for pos, (p_name, p) in enumerate(sig.parameters.items()):
        ...
        if t is bool:
            yield Argument.bool(arg_names=arg_names, positional=positional, **args)
            continue
        if is_sequence(t):
...
def make_arg_names(name, aliases, positional: bool):
    ...
    if positional:
        if aliases:
            raise ValueError("Aliases not allowed for positional arguments")
        arg_prefix = ""
...
bool_literal_mapping = dict(
    [(l, True) for l in bool_true_literals] + [(l, False) for l in bool_false_literals]
)
...
def unwrap_opt(t: type, default) -> type:
    opt_type = optional_type(t)
    if opt_type is None:
        return t
    if default is inspect.Signature.empty:
        raise ValueError(
            "typing.Optional arguments are allowed only when accompanied with a default value"
        )
    return opt_type
