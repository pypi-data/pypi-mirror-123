...
class MCTalDS2:
    def __init__(self, tally_dict):
        ...
        self.ds = self.make_dataset(tally_dict)
    ...
    def make_dataset(self, tally_dict):
       ...
       # tally_comment=tally_dict['tally'].values[0].strip() if len(tally_dict['tally'].values)>0 else ''
        ...
        ds = xr.Dataset({'value': (['cell','direct', 'user', 'segment', 'multiplier', 'cosine', 'energy_ub', 'time_shake_ub'],values[:,:,:nu,:,:nm,:ncc,:ne,:nt]),
...
class MCTalData:
    ...
    def __init__(self, inputfile):
        tally_list = self.tally_list(inputfile)
        self.raw_data = {tally_dict['tally'].numbers[0]: self.datatally_builder(tally_dict) for tally_dict in tally_list}
    ...
    def tally_list(self, inputfile):
        with open(inputfile) as fid:
            info = next(fid)
            self.histories_number = int(info.split()[-2])
            self.nps=self.histories_number
            title = next(fid)
            section_list= []
            for line in fid:
                if re.match(r'^[a-zA-Z]', line):
    ...
    def datatally_builder(self, tally_dict):
        mctal_ds2 = MCTalDS2(tally_dict)
        tally_number = tally_dict['tally'].numbers[0]
        ds = mctal_ds2.ds
        tally_info = mctal_ds2.tally_info
        comment = tally_info['comment']
        particle = tally_info['particle']
        detector = tally_info['detector']
        return DataTally(tally_number=tally_number, dataset=ds, comment=comment, particle=particle, detector=detector)
...
def read_mctal2(inputfile):
    return MCTalData(inputfile)