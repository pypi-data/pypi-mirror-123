...
class DeepRecursiveEmbedding:
    ...
    def _plot(self, epoch, step, fig_size='normal'):
        ...
        if self.plot_results:
            plt.savefig(os.getcwd() + "/DRE_labeled_%s_%s.png" % (time.asctime(time.localtime(time.time())), step))
        if step == 're_umap':
            return Y
        else:
            return 0
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch == self.num_pre_epochs:  # 300
                ...
                # save the model:
                    ...
                    torch.save(state, os.getcwd()+'./checkpoint/DRE_{}.pth'.format(recursive_step))
                if self.plot_results:
    ...
    def save_model(self):
        ...
        if not os.path.isdir(os.getcwd()+'/DRE_model_checkpoint'):
            os.mkdir(os.getcwd()+'/DRE_model_checkpoint')
        # Remember to rename the saved model corresponding to the hyper-parameters:
        torch.save(state, os.getcwd()+'/DRE_model_checkpoint/DRE_final.pth')
