...
def open_socket(dst: str, port: int, secure=False) -> ssl.SSLSocket:
    s = None
    ...
    if secure:
        s = ssl.wrap_socket(
            skt, ssl_version=ssl.PROTOCOL_TLSv1_2, ciphers="ADH-AES128-SHA256"
        )  # ADH-AES128-SHA256
    else:
        s = skt
    s.connect(sockaddr)
...
class ApiRos:
    ...
    def __init__(self, dst: str, port: int, secure: bool = False, hide: bool = False):
        self.sk = open_socket(dst, port, secure)
        self.currenttag = 0
        self.hide = hide
