...
class Node:
    def __init__(self, label="", labelIndex=0):
        self.label = label
...
class SuffixTree:
            ...
            while j < len(s):
                if s[j] in curr_node.children:
                    ...
                    if k - j == len(label):
                        ...
                        new_suffix = s[k]
                        split = Node(label[:k - j], child.labelIndex)
    ...
    def findMatches(self, node, matchedIndicesSet):
        matchedIndicesSet.add(node.labelIndex)

        values = node.children.values()
        for val in values:
            self.findMatches(val, matchedIndicesSet)
...
def run(fasta_file, fastq_file):
    ...
    for fasta_seq in fasta_seqs:
        ...
        for fastq_seq in fastq_seqs:
            ...
            matched_indices = set()
            tree.findMatches(node, matched_indices)
