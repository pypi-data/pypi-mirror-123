...
def set_up_template(fw, project, template_type=None, template_file=None):
    ...
    if template_type:
        ...
        if template_type == "BIDS-v1":
            ...
            logger.info("Project curation base template must be 'BIDS-v1' or 'ReproIn', not %s. Exiting.", template_type)
...
def curate_bids(
    ...
    if count.containers != num_proj_ses_acq:
        logger.warning("Container_counter should be %s but it is %s", num_proj_ses_acq, count.containers)
