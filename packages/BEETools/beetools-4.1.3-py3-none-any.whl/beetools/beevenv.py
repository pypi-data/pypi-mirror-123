...
def install_in(p_venv_root_dir, p_venv_name, p_instructions, p_crash = True, p_verbose=True):
    ...
    ret_code = beescript.exec_batch_in_session(
        script_cmds, p_script_name=script_name, p_verbose=p_verbose, p_switches=switches, p_crash = p_crash
    )
    return ret_code
...
def do_examples(p_cls = True):
    ...
    success = True
    b_tls = Archiver(_PROJ_DESC, _PROJ_VERSION, _PROJ_DESC, _PROJ_PATH)
