...
from azureml.automl.dnn.nlp.common.utils import get_run_by_id, _get_language_code
...
def score(
    ...
    :param ner_dir: path to data file
    ...
    run_source = train_run.parent.parent.properties.get(Run._RUNSOURCE_PROPERTY, None)
    ...
    test_file_path = os.path.join(ner_dir, test_file)
    ...
    with log_utils.log_activity(
        ...
        predictions_file_path = os.path.join(output_dir, OutputLiterals.PREDICTION_FILE_NAME)
        ...
        if is_labeling_run:
            ...
            generate_results_for_labeling_service(
                predictions_file_path, labeling_input_file_paths
            )
