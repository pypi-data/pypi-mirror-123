...
def load_dataset(
    ...
    Contract.assert_true(
        ...
        DataLiterals.VALIDATION_DATASET_ID,
    ...
    if is_labeling_run:
        ...
        train_ds_filename = load_dataset_for_labeling_service(
            workspace,
            dataset_id,
            ner_dir,
            DataLiterals.TRAIN_TEXT_FILENAME,
            DataLiterals.TRAIN_DATA_LABEL
        )
        validation_ds_filename = None
        if validation_dataset_id:
            validation_ds_filename = load_dataset_for_labeling_service(
                workspace,
                validation_dataset_id,
                ner_dir,
                DataLiterals.VALIDATION_TEXT_FILENAME,
                DataLiterals.VALIDATION_DATA_LABEL
            )
    else:
    ...
    # Load Dataset
    with open(os.path.join(ner_dir, train_ds_filename), encoding="utf-8") as f:
        train_data = f.read()
    ...
    if validation_dataset_id:
        with open(os.path.join(ner_dir, validation_ds_filename), encoding="utf-8") as f:
            valid_data = f.read()
