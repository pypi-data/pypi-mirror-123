...
from azureml.automl.dnn.nlp.common.constants import OutputLiterals, ScoringLiterals
...
def main():
    ...
    inferencer = MultilabelInferencer(run=run_object,
                                      device=device)

    label_column_name = json.loads(
        run_object.parent.parent.properties.get("AMLSettingsJsonString"))['label_column_name']
    predicted_df = inferencer.score(args.input_dataset_id, label_column_name)

    if args.enable_datapoint_id_output:
        output_cols = [DatasetLiterals.DATAPOINT_ID, label_column_name, DatasetLiterals.LABEL_CONFIDENCE]
    ...
    _logger.info("Results saved at location: {}".format(predictions_path))
