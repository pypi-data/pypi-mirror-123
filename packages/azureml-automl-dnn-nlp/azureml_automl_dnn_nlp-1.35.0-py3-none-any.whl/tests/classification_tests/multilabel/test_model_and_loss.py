...
def test_bert_model(get_input_tensors):
    ...
    get_dir = 'azureml.automl.runtime.featurizer.transformer.data.automl_textdnn_provider.'\
              'AutoMLPretrainedDNNProvider.get_model_dirname'
    with patch(get_dir, return_value="some-model-from-cdn"):
...
def test_bert_class_uses_cdn(get_input_tensors):
    ...
    with patch(get_dir, return_value="some-model-from-cdn"):
        with patch('transformers.BertModel.from_pretrained', return_value=mock_bert) as mock_downloader:
            ...
            mock_downloader.assert_called_with("some-model-from-cdn", return_dict=False)
...
def test_bert_class_no_cdn_throws_client_exception():
    ...
    get_dir = 'azureml.automl.runtime.featurizer.transformer.data.automl_textdnn_provider.'\
              'AutoMLPretrainedDNNProvider.get_model_dirname'
    with patch(get_dir, return_value=None):
        with pytest.raises(ClientException):
            BERTClass(NUM_MULTI_LABEL_COL)
