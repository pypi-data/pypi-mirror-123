...
class JSONParserPyspark:
    ...
    def process(self, sc, spark, drop_col=[]):
        ...
        if isinstance(
            json.loads(sc.wholeTextFiles(self.file_path,use_unicode=False).collect()[0][1]), dict
        ):
            if (
                len(
                    json.loads(sc.wholeTextFiles(self.file_path,use_unicode=False).collect()[0][1]).keys()
                )
                > 1
            ):
                processed_list.append(
                    json.loads(sc.wholeTextFiles(self.file_path,use_unicode=False).collect()[0][1])
                )
            ...
            json.loads(sc.wholeTextFiles(self.file_path,use_unicode=False).collect()[0][1]), list
        ):
            processed_json["items"] = json.loads(
                sc.wholeTextFiles(self.file_path,use_unicode=False).collect()[0][1]
            )
            a_file = open("/normalization_json_data_model.json", "w")
            json.dump(processed_json, a_file)
            a_file.close()
