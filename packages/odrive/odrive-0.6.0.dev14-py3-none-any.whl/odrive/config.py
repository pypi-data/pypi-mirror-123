...
reboot_vars = [
    '^inc_encoder[0-9]+.config.enabled$',
]
...
class EncoderConfig():
    ...
    def __init__(self, type: str, scale: float = 1.0):
        self.type = type
        self.scale = scale
...
class AxisConfig():
    ...
    def __init__(self, motors, encoders, calib_scan_vel: float = None, calib_scan_distance: float = None, calib_scan_range: float = None, calib_torque: float = None, pos_encoder: int = None, vel_encoder: int = None, commutation_encoder: int = None):
        self.motors = motors
...
class MachineConfig():
    ...
    def get_status(self, odrives):
        ...
        for conn in self._rs485_connections:
            rs485_intf_refs, enc_refs = group_conn_by_type(conn, [ODriveRs485Ref, EncoderRef], issues, "Cannot be connected to a RS485 bus")
            ...
            odrv_output_config = output_configs[rs485_intf_ref.odrv_ref.num]
            ...
            for enc_ref in enc_refs:
                ...
                if 'addr0' in amt21_encoder_group_config['config']:
                    issues.append(rs485_intf_ref.dev_ref, odrv_num, f"Current firmware does not support more than AMT21 encoder.")
                    continue # ignore encoder

                amt21_encoder_group_config['config']['addr0'] = encoder_config.get('addr', 0x54) # use default AMT21 address if unspecified
                encoder_ids[odrv_num][enc_ref] = ENCODER_ID_AMT21_ENCODER0
