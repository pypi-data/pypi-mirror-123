...
def launch_shell(args, logger):
    ...
    interactive_variables = {
        ...
        "apply": lambda x: x.apply(odrives),
        "status": lambda x: x.show_status(odrives),
        "calibrate": lambda x: x.calibrate(odrives),
        "example_config": odrive.config.example_config,
        "MotorConfig": odrive.config.MotorConfig,
        "AxisConfig": odrive.config.AxisConfig,
        "MachineConfig": odrive.config.MachineConfig,
    }
    ...
    async def mount(obj):
        ...
        return ("ODrive " + serial_number_str, serial_number_str, "odrv")
