...
def populate_sectors(sectors, hexfile):
    ...
    """
    hexfile_segments = list(hexfile.segments())
    for sector in sectors:
        addr = sector['addr']
        size = sector['len']
        # check if any segment from the hexfile overlaps with this sector
        touched = False
        for (start, end) in hexfile_segments:
...
def dump_otp(dfudev):
    ...
    """
    otp_sectors = dfudev.memories['OTP Memory']['sectors']
    if not is_odrive3(dfudev):
        print('not an ODrive v3.x')
        return

    dfudev.select_memory('OTP Memory')
    dfudev.clear_status()

    # 512 Byte OTP
    otp_sector = [s for s in dfudev.memories['OTP Memory']['sectors'] if s['addr'] == 0x1fff7800][0]
    data = dfudev.read_sector(otp_sector)
    print(' '.join('{:02X}'.format(x) for x in data))

    # 16 lock bytes
    otp_lock_sector = [s for s in dfudev.memories['OTP Memory']['sectors'] if s['addr'] == 0x1fff7A00][0]
...
class FirmwareFromGithub(Firmware):
    ...
    def __init__(self, release_json, asset_json):
        ...
        self.fw_version = odrive.utils.fw_version_str_to_tuple(release_json['tag_name'])
...
def find_device_in_dfu_mode(serial_number, cancellation_token):
    ...
    while not cancellation_token.is_set():
        ...
        for dev in stm_devices:
            try:
                actual_serial_number = dev.serial_number
            except ValueError:
                print("found device but could not check serial number (retrying in 1s)")
            if (serial_number is None) or (actual_serial_number == serial_number):
                return dev
        time.sleep(1)
    return None

def is_odrive3(dfudev):
    return dfudev.memories['OTP Memory']['sectors'][0]['addr'] == 0x1fff7800
...
def update_device(device, firmware, logger, cancellation_token):
    ...
    if (not hw_version is None) and (hw_version < (3, 5, 0)):
    ...
    if hasattr(device, 'fw_version_major') and hasattr(device, 'fw_version_minor') and hasattr(device, 'fw_version_revision') and hasattr(device, 'fw_version_unreleased'):
        fw_version = (device.fw_version_major, device.fw_version_minor, device.fw_version_revision, bool(device.fw_version_unreleased))
    else:
        fw_version = None
    ...
    if (not fw_version is None) and (not firmware.fw_version is None) and (firmware.fw_version <= fw_version):
        print()
    ...
    if dfudev is None:
        ...
        if do_backup_config:
            odrive.legacy_config.backup_config(device, None, logger)
    ...
    hw_version = get_hw_version_in_dfu_mode(dfudev)
    if hw_version is None and is_odrive3(dfudev):
    ...
    for k, mem in dfudev.memories.items():
    ...
    touched_sectors = list(populate_sectors(dfudev.memories['Internal Flash']['sectors'], hexfile))
    ...
    dfudev.select_memory('Internal Flash')
    dfudev.clear_status()

    # Erase
    try:
        internal_flash_sectors = dfudev.memories['Internal Flash']['sectors']
    ...
    if not found_in_dfu:
        ...
        if do_backup_config:
            temp_config_filename = odrive.legacy_config.get_temp_config_filename(device)
            odrive.legacy_config.restore_config(device, None, logger)
