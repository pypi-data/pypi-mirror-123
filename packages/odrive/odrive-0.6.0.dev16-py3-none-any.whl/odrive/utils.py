...
async def attach_metadata(device):
    if not hasattr(device, '_hw_version_major_property') or not hasattr(device, '_hw_version_minor_property'):
        return
    version = (await device._hw_version_major_property.read(), await device._hw_version_minor_property.read())
    metadata = db.try_get('odrive', 'v{}.{}'.format(*version), None)
    if metadata is None:
        return
...
def dump_errors(odrv, clear=False, printfunc = print):
    """
    Shows the error status of the device.
    """

    axes = [(name, getattr(odrv, name)) for name in dir(odrv) if name.startswith('axis')]
    axes.sort()

    def decode_flags(val, enum_prefix):
        errorcodes = {v: k for k, v in odrive.enums.__dict__ .items() if k.startswith(enum_prefix)}
        if val == 0:
            return _VT100Colors['green'], "no error"
        else:
            return _VT100Colors['red'], [
                errorcodes.get((1 << bit), 'UNKNOWN ERROR: 0x{:08X}'.format(1 << bit))
                for bit in range(64) if val & (1 << bit) != 0]

    def decode_enum(val, enum_prefix):
        errorcodes = {v: k for k, v in odrive.enums.__dict__ .items() if k.startswith(enum_prefix)}
        return (_VT100Colors['green'] if (val == 0) else _VT100Colors['red']), errorcodes.get(val, 'Unknown value: ' + str(val))

    def decode_drv_fault(axis, val):
        if val == 0:
            return _VT100Colors['green'], "none"
        elif not hasattr(axis, '_metadata'):
            return _VT100Colors['yellow'], "metadata not loaded"
        else:
            return _VT100Colors['red'], decode_drv_faults(axis._metadata['drv'], val)
    ...
    def dump_item(indent, name, obj, path, decoder):
        ...
        color, val = decoder(obj)

        if isinstance(val, list):
            printfunc(indent + name + ": " + color + "Error(s):" + _VT100Colors['default'])
            for item in val:
                printfunc(indent + "  " + item)
        else:
            printfunc(indent + name + ": " + color + val + _VT100Colors['default'])


    dump_item("", "system", odrv, 'error', lambda x: decode_flags(x, "ODRIVE_ERROR_"))

    for name, axis in axes:
        printfunc(name)

        dump_item("  ", 'axis', axis, 'error', lambda x: decode_flags(x, "AXIS_ERROR_"))
        dump_item("  ", 'motor', axis, 'motor.error', lambda x: decode_flags(x, "MOTOR_ERROR_"))
        dump_item("  ", 'DRV fault', axis, 'last_drv_fault', lambda x: decode_drv_fault(axis, x))
        dump_item("  ", 'sensorless_estimator', axis, 'sensorless_estimator.error', lambda x: decode_flags(x, "SENSORLESS_ESTIMATOR_ERROR"))
        dump_item("  ", 'encoder', axis, 'encoder.error', lambda x: decode_flags(x, "ENCODER_ERROR_"))
        dump_item("  ", 'controller', axis, 'controller.error', lambda x: decode_flags(x, "CONTROLLER_ERROR_"))
        dump_item("  ", 'procedure result', axis, 'procedure_result', lambda x: decode_enum(x, "PROCEDURE_RESULT_"))
    
    if hasattr(odrv, 'n_issues') and odrv.n_issues == 0:
        printfunc("internal issues: " + _VT100Colors['green'] + "none" + _VT100Colors['default'])
    if hasattr(odrv, 'n_issues') and odrv.n_issues > 0:
        printfunc("internal issues: " + _VT100Colors['red'] + str(odrv.n_issues) + _VT100Colors['default'])
