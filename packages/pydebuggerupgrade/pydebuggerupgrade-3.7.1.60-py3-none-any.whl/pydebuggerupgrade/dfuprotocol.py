...
class DfuUsbDevice(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.error("Unable to find backend USB driver for PyUSB!")
            # Error customizations for various operating system types
            if platform.system().startswith("Windows"):
                self.logger.error("Windows USB drivers are available here: http://studio.download.atmel.com/7.0.2542/driver-atmel-installer-x64-7.0.2542.msi")
            elif platform.system().startswith("Linux"):
                self.logger.error("Install libusb-1.0.0-dev, libudev-dev using your package manager.")
            elif platform.system().startswith("Darwin"):
                self.logger.error("Install a USB backend (eg: install MPLAB or use brew: brew install libusb).")
            sys.exit(-1)
