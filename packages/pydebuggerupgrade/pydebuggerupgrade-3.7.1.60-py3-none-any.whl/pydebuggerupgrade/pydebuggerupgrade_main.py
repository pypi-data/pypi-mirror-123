...
def pydebuggerupgrade(args):
    ...
    if backend.is_any_tool_in_bootmode(tool_name):
        if args.all or args.allboot:
            ...
            print("To upgrade all tools already in boot mode use the -b/--allboot option or "
                  "the -a/--all option.")
            return FAILURE
...
def _find_matching_tools(backend, args, timeout_s):
    ...
    if not matching_tools:
        ...
        if not (args.all or args.allboot):
            print("Tool could already be in upgrade mode, use the -b/--allboot option to upgrade all tools "
