...
def _load_shared_library():
    ...
    if platform.system() == "Windows":
        libpath = "./lietorch/lib/lietorch.dll"
    elif platform.system() == "Linux":
        libpath = "./lietorch/lib/lietorch.so"
    else:
        raise NotImplementedError(f"{platform.system()} is not supported at this time")

    if not os.path.isfile(libpath):
        raise FileNotFoundError(f"Could not find {libpath}")
