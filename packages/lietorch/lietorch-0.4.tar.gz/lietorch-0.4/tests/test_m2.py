...
def test_morphological_convolution_m2_cpu_autograd():
    ...
    input = (
        torch.randn(2, 2, 8, 7, 7, requires_grad=True, dtype=torch.float64),
        torch.randn(2, 3, 3, 3, requires_grad=True, dtype=torch.float64),
    )
    ...
    #     ),
    # )
    assert gradcheck(morphological_convolution_m2, input, atol=1e-4)

    module = MorphologicalConvolutionM2(channels=2, kernel_size=(2, 2, 2))

    assert gradcheck(
        module,
        torch.randn(1, 2, 8, 9, 9, requires_grad=True, dtype=torch.float64),
        atol=1e-4,
    )
...
def test_morphological_convolution_m2_cuda_autograd():
    ...
    input = (
        torch.randn(2, 2, 8, 7, 7, requires_grad=True, dtype=torch.float64).cuda(),
        torch.randn(2, 3, 3, 3, requires_grad=True, dtype=torch.float64).cuda(),
    )
    ...
    assert gradcheck(
        module.cuda(),
        torch.randn(1, 2, 8, 9, 9, requires_grad=True, dtype=torch.float64).cuda(),
        atol=1e-4,
    )
