...
class stanford_clause:
    ...
    def main(self, srs_param):
        id_req = partOf(dataFile).fulldataset(srs_param)['ID']
        req = partOf(dataFile).fulldataset(srs_param)[col_param]
        data_clausa = []
        ...
        clausa_df = pd.DataFrame(data_clausa, columns= ['id', 'data', 'clause', 'label', 'jml_clausa'])
        stanford_clause.__del__(self)
        return clausa_df

if __name__ == "__main__":
  try:
    klausa = stanford_clause(dataFile).main(srs_param)
