...
@jsii.enum(jsii_type="monocdk.aws_elasticloadbalancingv2.AlpnPolicy")
class AlpnPolicy(enum.Enum):
    '''(experimental) Application-Layer Protocol Negotiation Policies for network load balancers.
    ...
    '''(experimental) Negotiate only HTTP/1.*. The ALPN preference list is http/1.1, http/1.0.
    ...
    '''(experimental) Prefer HTTP/1.* over HTTP/2 (which can be useful for HTTP/2 testing). The ALPN preference list is http/1.1, http/1.0, h2.

    :stability: experimental
    '''
    HTTP2_PREFERRED = "HTTP2_PREFERRED"
    '''(experimental) Prefer HTTP/2 over HTTP/1.*. The ALPN preference list is h2, http/1.1, http/1.0.
...
class BaseApplicationListenerProps:
    def __init__(
        ...
        *,
        ...
        :param certificates: (experimental) Certificate list of ACM cert ARNs. You must provide exactly one certificate if the listener protocol is HTTPS or TLS. Default: - No certificates.
...
class BaseNetworkListenerProps:
    ...
    def alpn_policy(self) -> typing.Optional[AlpnPolicy]:
        '''(experimental) Application-Layer Protocol Negotiation (ALPN) is a TLS extension that is sent on the initial TLS handshake hello messages.
...
class CfnTargetGroup(
    ...
    def __init__(
        ...
        *,
        ...
        :param ip_address_type: ``AWS::ElasticLoadBalancingV2::TargetGroup.IpAddressType``.
...
class IApplicationListener(
    ...
    def add_targets(
        ...
        *,
        ...
        :param deregistration_delay: (experimental) The amount of time for Elastic Load Balancing to wait before deregistering a target. The range is 0-3600 seconds. Default: Duration.minutes(5)
        :param health_check: (experimental) Health check configuration. Default: No health check
        :param load_balancing_algorithm_type: (experimental) The load balancing algorithm to select targets for routing requests. Default: round_robin.
        :param port: (experimental) The port on which the listener listens for requests. Default: Determined from protocol if known
        :param protocol: (experimental) The protocol to use. Default: Determined from port if known
...
@jsii.enum(
    jsii_type="monocdk.aws_elasticloadbalancingv2.TargetGroupLoadBalancingAlgorithmType"
)
class TargetGroupLoadBalancingAlgorithmType(enum.Enum):
    '''(experimental) Load balancing algorithmm type for target groups.

    :stability: experimental
    '''

    ROUND_ROBIN = "ROUND_ROBIN"
    '''(experimental) round_robin.

    :stability: experimental
    '''
    LEAST_OUTSTANDING_REQUESTS = "LEAST_OUTSTANDING_REQUESTS"
    '''(experimental) least_outstanding_requests.
...
class AddApplicationTargetsProps(AddRuleProps):
    ...
    def load_balancing_algorithm_type(
        ...
        '''(experimental) The load balancing algorithm to select targets for routing requests.
...
class ApplicationTargetGroupProps(BaseTargetGroupProps):
    def __init__(
        ...
        *,
        ...
        :param target_type: (experimental) The type of targets registered to this TargetGroup, either IP or Instance. All targets registered into the group must be of this type. If you register targets to the TargetGroup in the CDK app, the TargetType is determined automatically. Default: - Determined automatically.
        :param vpc: (experimental) The virtual private cloud (VPC). only if ``TargetType`` is ``Ip`` or ``InstanceId`` Default: - undefined
        :param load_balancing_algorithm_type: (experimental) The load balancing algorithm to select targets for routing requests. Default: TargetGroupLoadBalancingAlgorithmType.ROUND_ROBIN
