...
#需捞取订单的成员（FAE+代工）：
members = "周晓莎 罗仪凡 赵壮洪 邓世琼 裘晨曦 肖代琴 刘瑜敏 章德余 郑永祥 江庆元 徐安奎 张仁斌 雷小军 琚陈李 邹代好 武建如 王鹏 谢澔 李阳秋 卢成帆 陈吉平 张宏海 陈潮雄 林祥纳 陈嘉艺8469 陆天等 黄淑荣 杨炳坤 孔令志 冼健荣 张立帅 陈福枢 萧植涛 黎佩龙 郑泽淼 陈讲鹏 孙培壮 赵溪楠 吕鋆泉 何健 邓思敏 李路 刘家辉 梁凯 银泽 何明珠 刘毅军 薛明德 李琨 王航 蔡明福-实习 陆龙飞 李辉 谢锦财 何勋 李晓东 易武进 陈家兴 何志伟 陈凡 师伟 童正伟 梁兆征 李建峤 吴海明 刘家豪"
...
def send_email(mail_receiver,mail_text):
    ...
    try:
        ...
        smtpObj.sendmail(sender, mail_receiver.split(','), message.as_string())
        smtpObj.quit();
        print("邮件发送成功")
...
if __name__ == '__main__':
    ...
    for ocs in ocs_list:
        r = PyocsDemand(ocs)
        #获取订单上最新软件的状态
        # sw_list = PyocsSoftware().get_enable_software_list(str(ocs))
        # if sw_list:
        #     #如果软件状态不是"客户未确认"，则不作处理
        #     if "客户未确认" not in r.get_software_confirm_status(sw_list[0].name):
        #         continue
        #获取订单评论区信息
