...
def init_schema( ) :
    ...
    sql_execute( "CREATE TABLE sys ( sn text UNIQUE , svs text , svn real )" )
    sql_execute( "CREATE INDEX index_sys ON sys ( sn )" )

    sys_create( "ver" , None , 1 )
    sys_create( "serial" , "ODS042e1919c5bd8f41cd8e1a2b8e455dd8f6fb00df0c3916d1137047a7aafc4c42" , None )
...
def sys_create( sn , svs , svn ) :

    sql = "INSERT INTO sys ( sn , svs , svn ) VALUES ( ? , ? , ? )"
    data = ( sn , svs , svn )
...
def sys_update( sn , svs , svn ) :
    sql = "UPDATE sys SET svs=? , svn=? where sn=?"
    data = ( svs , svn , sn )
    sql_execute_parameters( sql , data )

def sys_delete( sn ) :

    sql = "DELETE FROM sys where sn='" + sn + "' limit 1"
    sql_execute( sql )
    return( True )
...
def env_delete( en ) :
    ...
    sql = "DELETE FROM env where en='" + en + "' limit 1"
    sql_execute( sql )
    return( True )
...
def meta_delete( mn ) :
    ...
    sql = "DELETE FROM meta where mn='" + mn + "' limit 1"
    sql_execute( sql )
    return( True )
...
def args_delete( aid ) :
    ...
    sql = "DELETE FROM args where aid=" + aid + " limit 1"
    sql_execute( sql )
    return( True )
...
def templates_delete( tp ) :
    ...
    sql = "DELETE FROM templates where tp='" + tp + "' limit 1"
    sql_execute( sql )
    return( True )
