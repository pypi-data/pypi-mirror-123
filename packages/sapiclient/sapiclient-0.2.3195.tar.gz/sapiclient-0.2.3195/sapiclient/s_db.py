...
def init_db_filepath( ) :
    ...
    workspacedir = s_config.get_key( "sys/workspacedir" , "/tmp/sapiclient" )

    workspacedir=workspacedir+"/db"

    # FIXME TODO
    os.makedirs( workspacedir , exist_ok = True )

    # Add trailing slash
    db_filepath = os.path.join( workspacedir , "" ) + s_config.get_key( "/config/session")+".db" 
...
def init_schema( ) :
    ...
    # OK
    cur.execute( ''' CREATE TABLE meta ( mn text , mv text ) ''' )
    cur.execute( "CREATE INDEX index_meta ON meta ( mn ) ;" )

    ################################################################

    # OK
    cur.execute( ''' CREATE TABLE templates ( tid integer PRIMARY KEY AUTOINCREMENT , tp text , tk text , tv text ) ''' )
...
def meta_get( mn ) :
    ...
    cur.execute( "SELECT mv FROM meta where mn='" + mn + "' limit 1" )
    rows = cur.fetchall( )
    con.close( )
    #s_log.write( rows )
