...
def check_connectivity( ) :
    ...
    s_log.write(endpoint_url.hostname)
    s_log.write(endpoint_url.port)
    
    endpoint_url_port=endpoint_url.port
    if(endpoint_url_port==None):
        endpoint_url_port=443

    try :
        host = socket.gethostbyname( endpoint_url.hostname ) 
        s = socket.create_connection( ( host , endpoint_url_port ) , 2 )
        s.close( )

        if( get_request_response( ) == False ) :
...
#        context_json = base64.b64decode( res.headers[ "X-Omni-Context" ] ).decode( "utf-8" )
    ...
    if( "X-Omni-Jid" in res.headers ) :
        s_db.meta_create( "_server_getjob_jid" , res.headers[ "X-Omni-Jid" ] ) 
...
def get_request_response( request_headers = { }  , payload = "" , zendpoint = "sapiclient" ) :
    ...
    if( request_headers == { } ) :
        request_headers[ "User-Agent" ] = "sapiclient" 
        endpoint_host = s_config.get_key( "endpoint/host" )
        if endpoint_host != None and endpoint_host != "" :
