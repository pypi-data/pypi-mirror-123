...
from .enums import (RegressionAlgorithms, RegressionAlgorithmsCP4D, ClassificationAlgorithms,
                    ...
                    Metrics, TShirtSize, PredictionType, DataConnectionTypes)
...
def download_wml_pipeline_details_from_file(result_client_and_connection: Tuple['DataConnection', 'resource']) -> dict:
    ...
    try:
        path_parts = result_client_and_connection[0].location._model_location.split('model.pickle')
        if len(path_parts) == 1:
            path = result_client_and_connection[0].location._model_location.split('model.zip')[0]
            path = f"{path}pre_hpo_d_output/Pipeline1/"
...
def validate_optimizer_enum_values(
    ...
    # Note: allow pass fairness scoring values: TODO: Remove when official fairness support will be available
    additional_allowed_scoring_values = ['r2_and_disparate_impact', 'accuracy_and_disparate_impact']
    if scoring is not None:
        if scoring not in additional_allowed_scoring_values:
