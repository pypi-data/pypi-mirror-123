...
class TestAutoAIRemote(unittest.TestCase):
    ...
    def test_05a_fit_error_when_cos_file_not_exists(self):
        ...
        with self.assertRaises(NotExistingCOSResource,
                               msg="Training should raise error when non existing COS bucket given"):
        ...
        with self.assertRaises(NotExistingCOSResource,
                               msg="Training should raise error when non existing COS file given"):
            self.remote_auto_pipelines.fit(
                training_data_reference=[
                    DataConnection(
                        connection=S3Connection(endpoint_url=self.cos_endpoint,
    ...
    def test_10_summary_listing_all_pipelines_from_wml(self):
        ...
        TestAutoAIRemote.winning_pipelines_summary = pipelines_summary[pipelines_summary['Winner']]
        TestAutoAIRemote.discarded_pipelines_summary = pipelines_summary[pipelines_summary['Winner'] == False]
    ...
    def test_22a_deploy__deploy_best_computed_pipeline_from_autoai_on_wml(self):
        with self.assertRaises(DiscardedModel,
                               msg="Discarded pipeline was deployed, but it should be blocked"):
            pipeline_name = self.discarded_pipelines_summary.reset_index()['Pipeline Name'][0]
    ...
    def test_22b_deploy__deploy_best_computed_pipeline_from_autoai_on_wml(self):
        with self.assertRaises(WrongModelName,
                               msg="Incorrect pipeline name was not cached."):
            self.service.create(
                experiment_run_id=self.remote_auto_pipelines._engine._current_run_id,
                model="Pipeline_150",
                deployment_name=self.DEPLOYMENT_NAME)

    def test_22c_deploy__deploy_best_computed_pipeline_from_autoai_on_wml(self):
    ...
    def test_32a_deploy__batch_deploy_best_computed_pipeline_from_autoai_on_wml(self):
        with self.assertRaises(DiscardedModel,
                               msg="Discarded pipeline was deployed, but it should be blocked"):
            pipeline_name = self.discarded_pipelines_summary.reset_index()['Pipeline Name'][0]
    ...
    def test_32b_deploy__batch_deploy_best_computed_pipeline_from_autoai_on_wml(self):
        with self.assertRaises(WrongModelName,
                               msg="Incorrect pipeline name was not cached."):
    ...
    def test_32c_deploy__batch_deploy_best_computed_pipeline_from_autoai_on_wml(self):
        pipeline_name = self.winning_pipelines_summary.reset_index()['Pipeline Name'][-1]
        print('Deploying', pipeline_name)
