...
class Environment:
    ...
    def load_config(self):
        if not self.config_path.exists():
            print(f"Config file '{self.config_path}' is not exist.", PrintType.FAILED)
            print("Please run:\n\n\tocean init\n")
            exit()
    ...
    def add_presets(self, preset):
        if self.preset_exist(preset):
            print(f"Preset `{name}` is already exist.", PrintType.FAILED)
            return
    ...
    def delete_presets(self, key):
        for idx, pre in enumerate(self.config[code.PRESETS]):
            ...
            print(f"Preset `{key}` not found.", PrintType.FAILED)
            exit()

    def set_presets_as_default(self, preset):
        if not self.preset_exist(preset):
            print(f"Preset `{preset}` is not exist.", PrintType.FAILED)
            return
        for p in self.config[code.PRESETS]:
            if p[code.NAME] == preset:
                p[code.DEFAULT] = True
            else:
                p[code.DEFAULT] = False
        self.save_config()
        print(f"Preset `{preset}` is now default.", PrintType.SUCCESS)
...
if __name__ == "__main__":
    try:
        ...
        print("Unknown Error Occured.", PrintType.FAILED)
        capture_exception(e)
