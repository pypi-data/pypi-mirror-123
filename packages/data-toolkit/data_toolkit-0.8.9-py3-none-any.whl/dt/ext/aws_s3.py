...
def get_selected_drives(s3, s3_resource, bucket_name, dongle_id):
    ...
    if dongle_id=='064b1a99381ec054':
        # import ipdb; ipdb.set_trace()
        pass

    if len(df) == 0:
        print(f"No files found for {dongle_id}")
...
def get_dongle_drives(s3, s3_resource, bucket_name, dongle_id: str, show_n: int, latest=False):
    ...
    for did in tqdm(sorted(dongle_ids[:show_n])):
        latest_drive, last_complete_drive = get_selected_drives(s3, s3_resource, bucket_name, did)
        # TODO: this does not feel like the last file
...
def get_drives(target, bucket_name: str = 'creationlabs-raw-data', metadata_file: str = '/home/jakub/data/asco/devices.csv',
                        show_n: int = 25, num_segments: int = 1, filter=0):
    # TODO: profile
    # get all dongle ids
    import boto3 
    import humanize
    ...
    if target=='-1':
        df = get_dongle_drives(s3, s3_resource, bucket_name, target, show_n, latest=True)
    elif target=='0':
    ...
    if target in ['0', '-1']:
        ...
        df = df[['dongle_id','drive','time','seg_num','Registration Number']]
...
def add_metadata(df: pd.DataFrame, metadata_file_path: str):
    ...
    df = df.merge(meta_data, left_on='dongle_id', right_on="Dongle ID")
    return df