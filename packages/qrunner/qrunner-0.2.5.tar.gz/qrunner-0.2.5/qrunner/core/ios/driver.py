...
def _inner_retry(self, func, *args, **kwargs):
    ...
    while count < 4:
        ...
        except (requests.exceptions.ConnectionError, wda.WDAError, binascii.Error) as _:
            logger.info(f'重连wda第{count}次')
...
class Driver(object):
    ...
    def __init__(self, serial_no=None):
        ...
        self.bundle_id = conf.get_name('app', 'pkg_name')
        logger.info(f'启动webdriver for {self.serial_no}')
        ws = WDAServer(self.serial_no)
        ws.launch_wda()
        logger.info(f'连接手机: {self.serial_no}')
    ...
    @relaunch_wda
    def allure_shot(self, filename, timeout=3):
