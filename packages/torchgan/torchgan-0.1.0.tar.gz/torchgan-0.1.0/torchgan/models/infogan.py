...
class InfoGANGenerator(DCGANGenerator):
    ...
    def sampler(self, sample_size, device):
        dis_code = torch.zeros(sample_size, self.dim_cont, device=device)
        dis_code[:, 0] = 1
        cont_code = torch.randn(sample_size, self.dim_cont, device=device)
        return [
...
class InfoGANDiscriminator(DCGANDiscriminator):
    ...
    def forward(self, x, return_latents=False, feature_matching=False):
        ...
        dist_dis = distributions.OneHotCategorical(
            logits=self.dis_categorical(x)
        )
