...
class Tokenizer:
    ...
    def adjunta_separadores(tokens, separadores_agrupados=False, segmentado_por_palabras=False,
        ...
        if separadores_agrupados:
            for orden_token, token in enumerate(tokens):
                if not tokens_con_pausas or tokens_con_pausas[-1][-1][-1] in u'\t\n\r\f\v':
                    ...
                    if tokens_con_pausas and elimina_separadores_blancos:
                        tokens_con_pausas[-1] = tokens_con_pausas[-1][:-1]
                    ...
                    if token:
                        if tokens_con_pausas[-1][-1][-1] in u'¿¡([{“«<‘`' or \
                                (tokens_con_pausas[-1][-1][-1] in u'-‒–−­—―'
                                 and token[0] in u'0123456789¿¡([{<') \
                                or (tokens_con_pausas[-1][-1][-1] == u'"' and n_comillas_dobles % 2):
                            # El token anterior era signo de apertura (o negativo), tenemos que unir ese token
                            # con el actual, pero debemos decidir si pasamos el anterior a este, o al revés.
                            if not segmentado_por_palabras and token[0] in u'¿¡'\
                                    and tokens_con_pausas[-1][-1][-1] in u'([{“«<‘`"':
                                # Si estamos segmentando frases, puede ocurrir que el texto tenga algo como
                                # 'Me insultó ("¿Eres tonto?", dijo literalmente) y se fue.', con lo que la
                                # frase se ha cortado en el '¿' pero el corte debería ser antes de '("'.
                                # En vez de meter este token en la frase anterior, creamos una frase nueva
                                # y meteremos esos tokens de apertura blanda que estaban en la frase
                                # anterior en esta nueva.
                                tokens_con_pausas.append([])
                                while tokens_con_pausas[-2][-1][-1] in u'([{“«<‘`'\
                                        or (tokens_con_pausas[-2][-1][-1] == u'"' and n_comillas_dobles % 2):
                                    # Pasamos ese carácter de corte blando como un token de esta frase.
                                    tokens_con_pausas[-1] =\
                                        [tokens_con_pausas[-2][-1][-1]] + tokens_con_pausas[-1]
                                    # Eliminamos el carácter del token anterior.
                                    tokens_con_pausas[-2][-1] = tokens_con_pausas[-2][-1][:-1].strip()
                                    # Si el token anterior solo contenía este carácter, pues lo eliminamos
                                    if not tokens_con_pausas[-2][-1]:
                                        tokens_con_pausas[-2] =\
                                            tokens_con_pausas[-2][:-1]
                                        # Si al eliminar ese token queda una frase vacía, la borramos.
                                        if not tokens_con_pausas[-2]:
                                            tokens_con_pausas = tokens_con_pausas[:-2] + tokens_con_pausas[-1:]
                                        break
                    ...
                    if token:
                        ...
                        if token_strip[-1] in u'¿¡([{“«<‘`' or \
                            ...
                            if not segmentado_por_palabras and token[0] in u'¿¡'\
                                ...
                                # y meteremos esos tokens de apertura blanda que estaban en la frase
                                # anterior en esta nueva.
                                tokens_con_pausas.append(u'')
                                while token_strip[-1] in u'([{“«<‘`'\
                                        or (token_strip[-1] == u'"' and n_comillas_dobles % 2):
                                    # Pasamos ese carácter de corte blando como un token de esta frase.
                                    tokens_con_pausas[-1] = token_strip[-1] + tokens_con_pausas[-1]
                                    # Eliminamos el carácter del token anterior.
                                    token_strip = token_strip[:-1].strip()
                                    # Si el token anterior solo contenía este carácter, pues lo eliminamos
                                    if not token_strip:
                                        tokens_con_pausas = tokens_con_pausas[:-2] + tokens_con_pausas[-1:]
                                    else:
                                        tokens_con_pausas[-2] = token_strip
                                    break
    ...
    def test():
        ...
        """
        texto_prueba = u'Pero O\'Donnell nos dijo: ' \
                       u'"¿no es verdad…?" ' \
                       u'"¿Dime tú?"' \
                       u'Y nosotros no.\n'
        texto_prueba = u'Pero O\'Donnell nos dijo : ' \
                       ...
                       u'" ¿ Dime tú ? " ' \
                       u'Y nosotros no .\n'
        texto_prueba = u'"Dijo que era guapo (¡¡¿en serio?!!)". Y bueno, que eso.'
        texto_prueba = u'Sol-luna es índico-Ártico, pero mató-asesinó a un caló-árabe (\'haber\').' \
                       ...
                       u'"¿Dime tú?"' \
                       u'Y nosotros no.\n' \
                       u'Me halagó (¿por "compasión"?), se volvió y se fue.\n' \
                       u'"Queda París y el marxismo-leninismo (-raro-) -aunque no lo creas- a -2,3 grados".' \
                       u'" Todo ( … ) duele ( … ) cuando duele . " ' \
                       u'Es la verdad … ' \
                       u'Pero … ' \
                       u'no \' se sabe \'. ' \
                       ...
                       u'Me halagó ( ¿ por " compasión " ? ) , se volvió y se fue . \n'
        print(u'Tokenizando el siguiente texto:')
        print(texto_prueba)
