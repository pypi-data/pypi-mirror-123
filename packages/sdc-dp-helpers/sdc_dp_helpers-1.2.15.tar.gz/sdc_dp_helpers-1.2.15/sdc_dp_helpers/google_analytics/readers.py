...
class BaseGAV3Reader:
    """
    Version 3 Reporting API Reader class.
    Does not inherit GAV4 class in order to preserve all existing methods
    expected behaviours.
    """

    def __init__(self, config_file_path, creds_file_path, **kwargs):
        self.creds_file_path = creds_file_path
        self.config = load_file(config_file_path, 'yaml')
    ...
    @retry_handler((TimeoutError, OSError, ServerNotFoundError, HttpError), total_tries=5)
    def run_request(self, service, start_date, end_date, view_id, start_index):
        """
        Separated query method to handle retry and delay methods.
        """
        print(f'Querying View ID: {view_id} at Dates: {start_date} to {end_date}.')
        print(f'At index: {start_index}')
    ...
    def run_query(self):
        ...
        :yield: dict. flat google analytics report dict
        """
        service = self.get_service()

        # batch dates defaults to true
        if self.config.get('batch_dates', True):
            # generate range of dates for date batch
            range_of_dates = date_range(
            ...
            for date in range_of_dates:
                for view_id in self.config.get('view_ids'):
                    ...
                    while True:
                        ...
                        yield self.normalize_data(response)

        else:
            start_date = date_handler(self.config.get('start_date'))
            end_date = date_handler(self.config.get('end_date'))
