...
def create_1step_amp_fst(symbol_table, separator = 'X'):
    ...
    myfst.add_arcs(0, [(s,t,1,1) for s in cns.keys() for t in cns.keys() if ((cns[t]-cns[s]) == 1) & (s!='0')]) # transitions from 0->1
    myfst.add_arcs(1, [(s,t,0,1) for s in cns.keys() for t in cns.keys() if ((cns[t]-cns[s]) == 1) & (s!='0')]) # extension of an open window
    if '0' in cns.keys():
...
def create_1step_del_fst(symbol_table, separator = 'X', exclude_zero=False):
    ...
    if separator is not None and separator != '':
        myfst.add_arc(0, (separator, separator, 0, 0)) # add separator
    if exclude_zero:
        myfst.add_arcs(0, [(s, t, 1, 1) for s in cns.keys() for t in cns.keys() if (cns[s]-cns[t])==1 and t!='0']) # 0->1
        myfst.add_arcs(1, [(s, t, 0, 1) for s in cns.keys() for t in cns.keys() if (cns[s]-cns[t])==1 and t!='0']) # extend an open window
    else:
        myfst.add_arcs(0, [(s, t, 1, 1) for s in cns.keys() for t in cns.keys() if (cns[s]-cns[t])==1]) # 0->1
        myfst.add_arcs(1, [(s, t, 0, 1) for s in cns.keys() for t in cns.keys() if (cns[s]-cns[t])==1]) # extend an open window
    if '0' in cns.keys():
        myfst.add_arc(1, ('0','0',0,1))
    myfst.add_arcs(1, [(s,s,0,0) for s in cns.keys() if s!='0']) # return
    if separator is not None and separator != '':
...
def create_loh_fst(symbol_table, separator = 'X'):
    ...
    for state in range(1,9):
        cost = state
        myfst.add_arcs(0, [(s, t, cost, state) for s in cns.keys() for t in cns.keys() if (cns[s]-cns[t])==cost and t=='0'])
        myfst.add_arcs(state, [(s, t, 0, state) for s in cns.keys() for t in cns.keys() if (cns[s]-cns[t])<=cost and t=='0'])
        myfst.add_arcs(state, [(s, t, cns[s]-cns[t]-cost, cns[s]-cns[t]) for s in cns.keys() for t in cns.keys() if (cns[s]-cns[t])>cost and t=='0'])
