...
def package_xarray(data, parameter, attrs, encoding, run_date=None, compression=False, compress_level=1):
    ...
    if isinstance(attrs, dict):
        for k, v in attrs.items():
            x = v.copy()
            for w in x:
                if not isinstance(v[w], (int, float, str)):
                    v.pop(w)
            attrs1[k] = v
    ...
    ## Assign encodings
    encoding1 = {'lon': {'dtype': 'int32', '_FillValue': -999999, 'scale_factor': 0.0000001}, 'lat': {'dtype': 'int32', '_FillValue': -999999, 'scale_factor': 0.0000001}, 'altitude': {'dtype': 'int32', '_FillValue': -9999, 'scale_factor': 0.001}, 'time': {'_FillValue': -99999999, 'units': "days since 1970-01-01 00:00:00"}, 'modified_date': {'_FillValue': -99999999, 'units': "days since 1970-01-01 00:00:00"}}
    ...
    # Fix string/object encodings
    # vars1 = list(data1.variables)
