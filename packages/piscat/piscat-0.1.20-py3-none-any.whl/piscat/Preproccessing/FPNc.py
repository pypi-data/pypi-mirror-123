...
class MedianProjectionFPNc(QRunnable):
    ...
    def mFPNc(self, select_correction_axis):
        ...
        if False:
            ...
            result = Parallel(n_jobs=self.cpu.n_jobs, backend='multiprocessing', verbose=self.cpu.verbose)(
                delayed(self.FPNc_kernel)(self.video[f_, :, :], select_correction_axis) for f_ in tqdm(range(0, self.video.shape[0])))
            ...
            for i_ in tqdm(range(0, self.video.shape[0])):
                ...
                if select_correction_axis == 1:
                    FPN2D_median = np.repeat(gainMap1D_median[:, np.newaxis], diff.shape[select_correction_axis],
    ...
    def FPNc_kernel(self, diff, select_correction_axis):
        ...
        if select_correction_axis == 1:
            MAD_2D = np.repeat(MAD_1D_MedianAD[:, np.newaxis], diff.shape[select_correction_axis],
                               axis=select_correction_axis)
        elif select_correction_axis == 0:
            MAD_2D = np.repeat(MAD_1D_MedianAD[np.newaxis, :], diff.shape[select_correction_axis],
                               axis=select_correction_axis)
        ...
        if select_correction_axis == 1:
            FPN2D = np.repeat(gainMap1D_mean[:, np.newaxis], diff.shape[select_correction_axis],
                              axis=select_correction_axis)
        elif select_correction_axis == 0:
            FPN2D = np.repeat(gainMap1D_mean[np.newaxis, :], diff.shape[select_correction_axis],
                              axis=select_correction_axis)
    ...
    def cpFPNc(self, select_correction_axis):
        ...
        if False:
            ...
            for i_ in tqdm(range(0, self.video.shape[0])):
                ...
                AD_absDev = np.abs(diff - FPN2D_median)
                MAD_1D_MedianAD = np.median(AD_absDev, axis=select_correction_axis) # convert to median from here
...
class FrequencyFPNc:
    ...
    def wavelet_denoise(self, f_, direction='Horizontal'):
        ...
        if direction == 'Horizontal':
            cH_3_corr = self.guided_filter(cH_3, cA_3, win_size=(1, np.int(0.5 * cA_3.shape[0])), eps=0.3**2)
            coeffs_3_corr = cA_3, (cH_3_corr, cV_3, cD_3)
            cA_2_new_ = pywt.idwt2(coeffs_3_corr, 'sym8')
            cA_2_new = self.size_handling(cH_2, cA_2_new_)

            cH_2_corr = self.guided_filter(cH_2, cA_2_new, win_size=(1, np.int(0.25 * cA_3.shape[0])), eps=0.2**2)
            ...
            cH_1_corr = self.guided_filter(cH_1, cA_1_new, win_size=(1, np.int(0.1 * cA_3.shape[0])), eps=0.1**2)
            ...
            cV_3_corr = self.guided_filter(cV_3, cA_3, win_size=(np.int(0.5 * cA_3.shape[0]), 1), eps=0.3 ** 2)
            coeffs_3_corr = cA_3, (cH_3, cV_3_corr, cD_3)
            cA_2_new_ = pywt.idwt2(coeffs_3_corr, 'sym8')
            cA_2_new = self.size_handling(cV_2, cA_2_new_)

            cV_2_corr = self.guided_filter(cV_2, cA_2_new, win_size=(np.int(0.25 * cA_3.shape[0]), 1), eps=0.2 ** 2)
            coeffs_2_corr = cA_2_new, (cH_2, cV_2_corr, cD_2)
            cA_1_new_ = pywt.idwt2(coeffs_2_corr, 'sym8')
            cA_1_new = self.size_handling(cV_1, cA_1_new_)

            cV_1_corr = self.guided_filter(cV_1, cA_1_new, win_size=(np.int(0.1 * cA_3.shape[0]), 1), eps=0.1 ** 2)
