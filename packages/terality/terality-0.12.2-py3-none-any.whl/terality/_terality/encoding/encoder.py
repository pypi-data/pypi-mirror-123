...
def _encode_inplace_and_upload_df(
    ...
    cols_encoding = [_check_if_array_needs_encoding(df[col].array) for col in df]
    encoding = DFParquetEncoding(indices_encoding, cols_encoding)

    encode_df_inplace(df, encoding)
...
def _upload_ndarray_and_get_metadata(
    ...
    transfer_id, encoding = _encode_inplace_and_upload_df(aws_credentials, df)
    return NDArrayMetadata(transfer_id=transfer_id, encoding=encoding)
...
def _upload_series_and_get_metadata(
    ...
    df = series.copy(deep=True).to_frame()
    transfer_id, encoding = _encode_inplace_and_upload_df(aws_credentials, df)
...
def _upload_df_and_get_metadata(
    ...
    transfer_id, encoding = _encode_inplace_and_upload_df(aws_credentials, df_for_upload)
