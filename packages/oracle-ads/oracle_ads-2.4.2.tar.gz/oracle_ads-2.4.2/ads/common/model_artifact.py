...
from ads.feature_engineering.schema import Schema, SchemaSizeTooLarge
...
class ModelArtifact(Introspectable):
                ...
                try:
                    ...
                    if not self.inference_python_version:
                        if not training_env_info.TRAINING_PYTHON_VERSION:
                            raise Exception(
                                "Python version is not specified, please pass in python version when preparing model."
                            )
                        else:
                            logger.warning(
    ...
    def _populate_schema(self, data, schema_file_name):
        """
