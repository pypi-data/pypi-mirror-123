...
from ads.jobs.builders.infrastructure.base import Infrastructure
...
class DSCJob(OCIDataScienceMixin, oci.data_science.models.Job):
        ...
        if "NB_SESSION_OCID" in os.environ:
            ...
            if nb_session:
                ...
                if isinstance(infra, dict):
                    ...
                    shape_name = infra.shape_name if infra.shape_name else nb_config.shape
                    block_storage = (
                        infra.block_storage_size_in_gbs
                        if infra.block_storage_size_in_gbs
                        else nb_config.block_storage_size_in_gbs
                    )
                    subnet_id = infra.subnet_id if infra.subnet_id else nb_config.subnet_id
    ...
    def create(self) -> DSCJob:
        ...
        if not self.compartment_id:
            raise ValueError("Please specify compartment ID for data science job.")
        if not self.project_id:
            raise ValueError("Please specify project ID for data science job.")
...
class DataScienceJobRun(OCIDataScienceMixin, oci.data_science.models.JobRun):
...
class DataScienceJob(Infrastructure):
    ...
    def create(self, runtime, **kwargs) -> DataScienceJob:
        ...
        """
        if not runtime:
            raise ValueError("Please set a valid runtime.")
    ...
    def run(
        ...
        if not self.runtime:
            raise RuntimeError(
                "Job is not created. Please call create() to create the job first."
            )
