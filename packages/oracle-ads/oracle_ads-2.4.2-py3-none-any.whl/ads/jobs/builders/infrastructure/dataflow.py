...
class DataFlowRun(OCIModelMixin, oci.data_flow.models.Run):
...
class _DataFlowLog:
    ...
    @property
    def stdout(self):
        return self._get_logs("STDOUT")

    @property
    def stderr(self):
        return self._get_logs("STDERR")

    def _get_logs(self, out_type):
        logs = self.client.list_run_logs(self.run_id).data
        out = ""
        for log in logs:
            if log.type == out_type and self.src_type == log.source:
                out += (
...
class DataFlow(Infrastructure):
    def __init__(self, spec: dict = None):
        ...
        if spec is None:
            ...
            super(DataFlow, self).__init__(spec)
    ...
    def create(self, runtime: DataFlowRuntime, **kwargs) -> "DataFlow":
        ...
        if not payload.get("compartment_id", None):
            raise ValueError(
                "Compartment id is required. Please specify compartment id via 'with_compartment_id()'."
            )
        payload.pop("id", None)
