...
int
FTDCParser::parseFiles(std::string file_paths,  bool metadata, bool metric_names) {
...
int
FTDCParser::parseFiles(std::vector<std::string> const *file_paths,   const bool only_metadata, const bool only_metric_names) {
    bson_reader_t *reader;
...
Dataset::Metrics *
FTDCParser::getMetric(std::string name) {
     return dataSet.getMetric(name);
}
