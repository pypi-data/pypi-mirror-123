...
class Agent(Insight, Channel, Catalog, Dataset):
    ...
    def query(self, dataset_slug="", fields:list=[], filters=None, sorted_by=None,  dataset_id="", limit=50):
        ...
        df = pd.DataFrame(columns=codes, data=response["data"])
        df.columns = columns
        return df
    ...
    def load_dataset(self, dataset_slug, aliases:list=[], filters:list=[], limit=50):
...
class Askdata(SignUp):
    ...
    def query(self, workspace='', dataset='', fields:list=[], filters=None, sorted_by=None, limit=50):
        ...
        return agent.query(dataset_slug, fields, filters, sorted_by, limit=limit)


    def get(self, query, workspace='', dataset='', lang="en", fields:list=[], filters=None, sorted_by=None, get_raw_data=False):
        ...
        try:
            if(dataset_id!=''):
                filtered_result = []
                for r in result:
                    if(r["attachment"]["body"]["cards"][0]["details"]["datasetId"]==dataset_id):
            ...
            url_1 = app_url + "/" + workspace+"/"+filtered_result[0]["attachment"]["body"]["cards"][0]["details"]["datasetSlug"]
            ...
            if(len(filtered_result[0]["attachment"]["body"])>1):
                display(HTML("<p>Filtered by: {}</p>".format(filtered_result[0]["attachment"]["body"]["cards"][1]["details"]["text"])))
            if(len(filtered_result)>1):
                display(HTML("<p>{} Alternatives:</p>".format(len(filtered_result)-1)))
                for r in filtered_result[1:]:
                    url_i = app_url + "/" + workspace + "/" + r["attachment"]["body"]["cards"][0]["details"]["datasetSlug"]
            ...
            if get_raw_data:
                df = pd.DataFrame(data=filtered_result[0]["attachment"]["body"]["cards"][0]["details"]["rawData"], columns=filtered_result[0]["attachment"]["body"][0]["details"]["columns"])
            else:
                df = pd.DataFrame(data=filtered_result[0]["attachment"]["body"]["cards"][0]["details"]["rows"], columns=filtered_result[0]["attachment"]["body"][0]["details"]["columns"])
            return df
        except Exception as e:
