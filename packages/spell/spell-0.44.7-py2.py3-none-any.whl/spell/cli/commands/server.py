    ...
    **kwargs,
    ...
    if name is None:
        if len(model_versions) == 0:
            ...
            raise ExitException(
                "A server name must be provided with --name when using multiple models"
            )
...
def make_modelserver_create_request(
    ...
    **kwargs,
    ...
    return ModelServerCreateRequest(
        models=[
            ModelServerModel(model_name=m[0], version_id=m[1], version_name=m[2])
...
def validate_entrypoint(repo, entrypoint):
    ...
    if not os.path.isfile(entrypoint):
        raise ExitException(
            f"ENTRYPOINT {entrypoint} file not found.", SPELL_INVALID_CONFIG
        )
...
def remove(ctx, server_name, force):
    ...
    if spinner:
        spinner.stop_and_persist(
            text=f"Successfully removed model server {server_name}", symbol="🎉"
        )
    else:
...
def update(ctx, server_name, **kwargs):
    ...
    if kwargs["commit_ref"] == "HEAD" and not any(
        v or v == 0 for k, v in kwargs.items() if k != "commit_ref"
    ):
...
def get_grafana_link(kubernetes_name, serving_cluster_domain):
    ...
    query_string_template = (
        f'orgId=1&left=%5B"now-1h","now","",%7B"expr":{prometheus_query_prefill}'
    )
...
def add_models(ctx, server_name, models):
    ...
    for model in parsed_models:
        ...
        if model_name not in existing_model_lookup:
            ...
            if (model[1] and model[1] != existing_model.id) or (
                model[2] and model[2] != existing_model.version
            ):
                raise ExitException(
    ...
    new_models = [
        ModelServerModel(model_name=m[0], version_id=m[1], version_name=m[2]) for m in new_models
    ]
...
def rm_models(ctx, server_name, models):
    ...
    with api_client_exception_handler():
        if not new_models:
            ...
            new_models = [
                ModelServerModel(model_name=model, version_id=version[0])
                for model, version in new_models.items()
            ]
            req = ModelServerUpdateRequest(models=new_models)
            client.update_model_server(server_name, req)
            utf8 = ctx.obj["utf8"]
...
def get_model_server_env_lines(environment):
    ...
    if environment.requirements_file:
        lines.extend(
            format_multiline_text_to_table("Requirements File", environment.requirements_file)
        )
    if environment.pip:
