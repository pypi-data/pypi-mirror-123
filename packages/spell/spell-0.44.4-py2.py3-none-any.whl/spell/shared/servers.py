...
from spell.shared.dependencies import format_apt_versions, format_pip_versions, merge_dependencies
...
def create_environment(
    python_env_deps, pip_packages, requirements_file, apt_packages, envvars
):
    python_deps = merge_dependencies(
        python_env_deps, None, requirements_file, pip_packages
    ).to_payload()

    return Environment(
        pip_env=format_pip_versions(python_deps["pip_env"]),
        requirements_file=python_deps["requirements_file"],
        pip=format_pip_versions(python_deps["pip"]),
        conda_file=None,
