...
def create_aws(ctx, name, profile, vpc_id):
    ...
    if not profile:
        profile = click.prompt(
            "Enter the name of the AWS profile you would like to use", default="default"
        )
...
def add_eks_auth(spell_client):
    ...
    try:
        ...
        if spell_node_role_arn in conf_map.data["mapRoles"]:
            ...
            conf_map.data["mapRoles"] += eks_cluster_aws_auth_template.format(
                rolearn=spell_node_role_arn
            )
...
def eks_configure_k8s(
    ...
    cluster_utils.create_serving_namespace()
...
def has_bucket_encryption_setting(s3, bucket_name):
    ...
    if not rules:
        return False
    return any(
        [r.get("ApplyServerSideEncryptionByDefault") == {"SSEAlgorithm": "AES256"} for r in rules]
    )
...
def create_vpc(ec2, cluster_name):
    ...
    # Create subnets
    zones = [
        z["ZoneName"] for z in ec2.meta.client.describe_availability_zones()["AvailabilityZones"]
    ]
...
def remove_s3_bucket(spell_client, aws_session, cluster, bucket_name):
    ...
    if bucket_name is None:
        ...
        bucket_name = click.prompt(
            "Please choose a bucket to remove", type=click.Choice(existing_bucket_names)
        )
    if bucket_name not in existing_bucket_names:
        raise ExitException(
            f"Bucket name {bucket_name} has not been added to cluster {cluster_name}"
        )

    all_s3_buckets = [bucket.name for bucket in s3.buckets.all()]
    if bucket_name not in all_s3_buckets:
        # Either the Bucket is public or not accessible, either way we treat it as public
        # and IAM role management logic
        click.echo(
            f"Bucket {bucket_name} is either public or inaccessible, no IAM updates required."
        )
        with api_client_exception_handler():
    ...
    if (
        policy_document["Statement"][0]["Resource"] != "*"
        and bucket_arn in policy_document["Statement"][0]["Resource"]
    ):
        policy_document["Statement"][0]["Resource"] = [
            r
            for r in policy_document["Statement"][0]["Resource"]
            if r not in [bucket_arn, bucket_arn + "/*"]
        ]
        update_iam_policy(policy, policy_document)
    else:
...
def update_aws_cluster(ctx, session, cluster):
    ...
    try:
        ...
        raise ExitException(
            "Unable to verify bucket {} encryption settings: {}".format(bucket_name, e)
        )
    try:
        ...
        raise ExitException(
            "Unable to verify bucket {} has versioning enabled: {}".format(bucket_name, e)
        )
