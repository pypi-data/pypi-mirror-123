...
def kube_cluster(ctx, cluster):
    ...
    if version.parse(current_version) < version.parse(version_info["latest_version"]):
        ...
        if version.parse(SERVING_CLUSTER_VERSION) < version.parse(version_info["latest_version"]):
            click.echo(
                "Run `pip install --upgrade spell`, then `spell kube-cluster update` to update."
            )
        else:
...
def create_kube_cluster(
    ...
    if use_existing:
        if not cluster.get("serving_cluster_name") and not kube_cluster_name:
            raise ExitException(
                "No kube cluster found; try running this command without `--use-existing`"
            )
    ...
    creds = aws_session.profile_name if aws_session else gcp_project_name
    kubecfg_yaml = cluster_utils.generate_api_kubeconfig(cluster, creds, kube_cluster_name)
    ...
    with cluster_utils.KubeClusterContext(cluster, creds, kube_cluster_name=kube_cluster_name):
        if support_runs:
        ...
        if cluster["cloud_provider"] == "AWS":
            ...
            supported_features = cluster_gcp.gke_configure_k8s(
                ctx, service_account, cluster, is_public, support_runs
            )
...
def update_kube_cluster(
    ...
    creds = aws_session.profile_name if aws_session else cluster["networking"]["gcp"]["project"]
    with cluster_utils.KubeClusterContext(cluster, creds):
...
def perform_kube_cluster_upgrade(
    ...
    current_version = cluster.get("serving_cluster_version")
    if version.parse(current_version).major != version.parse(SERVING_CLUSTER_VERSION).major:
        raise ExitException(
            (
...
@cluster_utils.check_cli_version
@cluster_utils.pass_cluster
def update_registry_creds(ctx, cluster):
    cluster_utils.check_kube_context()
    cluster_utils.create_reg_secret()
...
def node_group_add(
    ...
    if cluster["cloud_provider"] == "AWS":
        ...
        cluster_aws.eks_add_nodegroup(name, config, aws_session.profile_name)
    elif cluster["cloud_provider"] == "GCP":
