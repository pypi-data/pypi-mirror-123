...
class ReservationSync:
    ...
    def _transform_guest_profile(self, record):
        countryCode = country_name_to_country_alpha2(
            next((x['name'] for x in COUNTRIES if x['id'] == record.get('countryId')), None))
        nationalityCode = country_name_to_country_alpha2(
            next((x['name'] for x in COUNTRIES if x['id'] == record.get('nationalityId')), None))
        ...
        hotelId = hotelId if hotelId else '0'
        guest = GuestsModelV2(
    ...
    def extract_reservation(self, record):
        ...
        if response.status_code == 200:
            try:
                ...
                response_json['propertyId'] = property_info.get('primaryDetails.propertyId') if property_info else 0
