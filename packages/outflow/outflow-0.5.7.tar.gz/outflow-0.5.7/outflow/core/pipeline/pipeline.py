...
class Pipeline:
    ...
    def _init(
        ...
        *,
        ...
        if self.system_args.map_uuid or self.system_args.run_uuid:
            if (
                self.system_args.map_uuid
                and not self.system_args.run_uuid
                or self.system_args.run_uuid
                and not self.system_args.map_uuid
            ):
                raise Exception(
                    "Arguments --map-uuid and --run-uuid must be used together"
                )

            init_remote(self.system_args.map_uuid, self.system_args.run_uuid)
        else:

            if self.system_args.settings is not None:
                os.environ["OUTFLOW_SETTINGS_MODULE"] = self.system_args.settings
            ...
            if self.system_args.no_color:
                pipeline_config["logging"]["handlers"]["console"][
                    "formatter"
                ] = "simple"
    ...
    def run(self):
        ...
        if context.map_uuid and context.run_uuid:
            ...
            result = map_infos["run_workflow"](
                map_infos["inner_workflow"][task_id],
        ...
        # regular run
            ...
            try:
                ...
                with WorkflowManager() as top_level_workflow:
                    workflow = top_level_workflow.workflow
                    task_returning = command.setup_tasks()
                    IOChecker().compute(workflow)
                    self.result = backend.run(
                        workflow=workflow, task_returning=task_returning
                    )
...
def init_remote(map_uuid, run_uuid):
    PipelineContextManager().__enter__()

    run_dir = Path(f"outflow_{run_uuid}")
