...
def prepare_ft_session(portal_base_url: str, user: str, passwd: str) -> Session:
...
def dir_content(
    ...
    for file in sorted(curdir.iterdir(), key=lambda f: f.name):
        ...
        if file.is_dir():
            ...
            if dirdesc.exists():
                dirmetas, with_content = get_md_metas(
                    dirdesc
            ...
            if metas.get("ft:title", None) is not None:
                ...
                if dev_mode:
                    logger.warning(
                        f"Skipping {file.as_posix()} no ft:title given as meta."
                    )
                else:
                    raise MD2FTError(
                        f"no meta ft:title or # title found in {file.as_posix()}"
                    )
    return content
...
def do_publish(
    ...
    if not dry_run:
        ...
        if rep.status_code in [HTTPStatus.UNAUTHORIZED, HTTPStatus.FORBIDDEN]:
            raise MD2FTError(
                f"Server rejected credentials as Unauthorized ({rep.status_code})."
            )
        elif rep.status_code == HTTPStatus.NOT_FOUND:
            raise MD2FTError(
                f'The instance id or the API in "{list_sources_url}" is not found ({rep.status_code}).'
            )
        elif not rep.ok:
    ...
    if not dry_run:
        ...
        logger.info(
            f"DRY RUN: {zipfile['file'][0]} file NOT published on {portal_base_url}."
        )
...
def do_unpublish(
    ...
    if not dry_run:
        ...
        list_maps = (
            f"{portal_base_url}/api/admin/khub/publications?ft:sourceId={source_id}"
        )
        r = ft_session.get(list_maps)
        if r.status_code == HTTPStatus.UNAUTHORIZED:
            logger.fatal(
                f"Server rejected credentials as Unauthorized ({r.status_code})."
            )
        if r.status_code == HTTPStatus.NOT_FOUND:
            logger.fatal(
                f'The instance id or the API in "{list_maps}" is not found ({r.status_code}).'
            )
        r.raise_for_status()
        ...
        if list_maps_json["count"] == 0:
            logger.info(
                f"No publications in this source_id or the source: {source_id} doesn't exist"
            )
            return
        ...
        if selected_documents:
            ...
            for id_to_delete, metas in selected_documents.items():
                ...
                if do_it:
                    ...
                    if r.status_code == HTTPStatus.OK:
                        answer = r.json()
                        logger.info(
                            f" ID MAP {id_to_delete} deleted on {portal_base_url}."
                        )
                        pprint(answer)
                    r.raise_for_status()
                else:
                    logger.info(
                        f"Would delete document id={id_to_delete} (whose metas={metas}) on {portal_base_url}."
                    )
            if not do_it:
                logger.info(
            ...
            logger.info(
                f"No publications selected with <<{select}>> from source_id : {source_id} on portal {portal_base_url}"
            )
    else:
        logger.info(
            f"DRY RUN: No publications deleted from source_id : {source_id} on portal : {portal_base_url}."
        )
...
publish_opts = [
    ...
    click.option(
        ...
        metavar="<user>",
        help="The FT user",
        prompt="Please, provide a FT user account",
    ),
    click.option(
        "-p",
        "--passwd",
        ...
        prompt="Please, provide the FT user password",
        hide_input=True,
    ),
    click.option(
        "-s",
