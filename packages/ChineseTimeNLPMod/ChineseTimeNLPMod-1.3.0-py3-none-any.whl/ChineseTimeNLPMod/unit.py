...
class TimeUnit:
    ...
    def norm_setday(self):
        ...
        match = pattern.search(self.exp_time)
        if match is not None:
            self.tp.set_unit(self.normalizer.baseTime.shift(days=int(match.group())))

        rule = r"((?<!\d))([0-3][0-9]|[1-9])(?=(日|天)前)"
        pattern: Pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            self.tp.set_unit(self.normalizer.baseTime.shift(days=-int(match.group())))
    ...
    def norm_setCurRelated(self):
        ...
        match = pattern.search(self.exp_time)
        if match is not None:
            self.tp.set_unit(self.normalizer.baseTime.shift(years=-2))

        rule = r"[上去]年"
        pattern: Pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            self.tp.set_unit(self.normalizer.baseTime.shift(years=-1))

        rule = r"今年|年初"
        pattern: Pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            self.tp.set_unit(self.normalizer.baseTime.shift(years=0))
        ...
        if match is not None:
            ...
            self.tp.set_unit(self.normalizer.baseTime.shift(months=-len(match)))

        rule = r"(本|这个)月"
        pattern: Pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            self.tp.set_unit(self.normalizer.baseTime.shift(months=0))
        ...
        if match is not None:
            ...
            pattern: Pattern = re.compile(rule)
            match = pattern.findall(self.exp_time)
            self.tp.set_unit(self.normalizer.baseTime.shift(days=2 + len(match)))

        rule = r"(?<!大)前天"
        pattern: Pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            self.tp.set_unit(self.normalizer.baseTime.shift(days=-2))

        rule = r"昨"
        pattern: Pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            self.tp.set_unit(self.normalizer.baseTime.shift(days=-1))

        rule = r"今(?!年)|现在"
        pattern: Pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            self.tp.set_unit(self.normalizer.baseTime.shift(days=0))
        ...
        match = pattern.search(self.exp_time)
        if match is not None:
            self.tp.set_unit(self.normalizer.baseTime.shift(hours=-int(match.group())))
