...
from ..models import CacheMode, PercentChange, HealthReportStatus, DEFAULT_LIMIT, INF, NEGATIVE_ONE, TimeGranularity
...
def query(
    ...
    if df is not None:
        if df.empty:
            ...
            # NOTE: remove `to_string` if no pandas dependency is < 1.0.0
            if parse(pd.__version__) >= parse("1.0.0"):
                click.echo(df.to_markdown(index=False, floatfmt=".2f"))
            else:
                click.echo(df.to_string(index=False, float_format=lambda x: format(x, ".2f")))
...
def list_queries(
    ...
    """Retrieve queries from mql server"""
    click.echo(config.mql.list_queries(active_only=active_only, limit=limit))
...
def validate_configs(config: CLIContext, config_dir: str) -> None:
    ...
    repo, branch, commit = config.mql.validate_configs(config_dir)
...
def commit_configs(ctx: click.core.Context, config: CLIContext, config_dir: str, pin: bool = None) -> None:
    ...
    spinner.start()
    model = config.mql.commit_configs(config_dir)
    spinner.succeed(
