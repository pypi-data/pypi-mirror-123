...
import Parallel_cyclomatic as cv


class CyclomaticTest(unittest.TestCase):
    # 测试并行圈复杂度计算的正确性
    def test_Cyclomatic(self):
        files = [ os.path.split(os.path.realpath(__file__))[0] + '/test/test.c', os.path.split(os.path.realpath(__file__))[0] + '/test/test.java']
        results = cv.para_cyclomatic(files)


        result = results[os.path.split(os.path.realpath(__file__))[0] + '/test/test.java']
        ...
        result = results[os.path.split(os.path.realpath(__file__))[0] + '/test/test.c']
