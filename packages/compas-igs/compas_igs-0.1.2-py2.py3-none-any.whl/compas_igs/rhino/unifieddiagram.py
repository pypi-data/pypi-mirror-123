...
def mesh_ud(formobject, forceobject, translation, rotation=0.0, scale=0.5):
    ...
    for vkey in formdiagram.vertices():
        ...
        base_xyz[vkey] = _rotate_pt_xy(add_vectors(init_xyz, translation), -math.pi/2 + rotation, add_vectors(form_center, translation))
    ...
    for face in forcediagram.faces():
        ...
        for vertex in forcediagram.face_vertices(face):
            xyz = _rotate_pt_xy(forcediagram_xyz[vertex], rotation, force_center)
            arm = scale_vector(subtract_vectors(xyz, base_xyz[face]), scale)
