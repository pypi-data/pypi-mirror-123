...
class DbBot(object):
    ...
    def run(self):
        try:
            for json_file in self._options.file_paths:
                self._parser.json_to_db(json_file, self._options.build_number, self._options.attempt)
        except DataError as message:
            sys.stderr.write('dbbot: error: Invalid JSON: %s\n\n' % message)
            exit(1)
        finally:
            self._db.close()
