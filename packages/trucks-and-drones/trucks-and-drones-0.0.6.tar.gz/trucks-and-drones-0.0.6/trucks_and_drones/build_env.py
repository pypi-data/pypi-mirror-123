...
class BuildEnvironment:
    ...
    def depots(
            ...
            # items (stock if node is depot and demand if node is customer):
            max_items: (None, int, list, tuple, np.ndarray) = None,
            init_items: (str, None, int, list, tuple, np.ndarray) = -1,
            item_rate: (None, int, list, tuple, np.ndarray) = None,
            item_recharge: (None, int, list, tuple, np.ndarray) = 0,
            init_items_at_step: (None, int, list, tuple, np.ndarray) = 0,
            # visualization:
    ...
    def add_custom_obs(self, custom_obs):
        self.obs_params = custom_obs

    def add_custom_acts(self, custom_acts):
        self.act_params = custom_acts
    ...
    def compile(
        ...
        if isinstance(self.obs_params, dict):
            self.obs_encoder = ObsEncoder(self.obs_params, self.temp_db, self.visualizor)
        else:
            self.obs_encoder = self.obs_params.build(self.temp_db)

        if isinstance(self.act_params, dict):
            self.act_decoder = ActDecoder(self.act_params, self.temp_db, self.simulation)
        else:
            self.act_decoder = self.act_params.build(self.temp_db, self.simulation)
