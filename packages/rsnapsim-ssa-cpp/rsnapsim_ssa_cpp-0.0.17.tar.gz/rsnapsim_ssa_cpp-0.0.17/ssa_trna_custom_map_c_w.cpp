...
void print(std::vector <int> const &a) {
   ...
   for(int mk=0; mk < a.size(); mk++)
   std::cout << a.at(mk) << ' ';
}

void translationSSA_custommap_trna( int* connection_mat, int* k_index, double* k_trna, int total_codons, int total_species, double k_diffusion, double* t_array, int Nt, double kbind, double kcompl, int* SSA_result,int* trna_result, int N, int FRAP, int Inhibitor, double inhibit_time, int seed, double* SSA_ribtimes, int* nribs, int ribtimesize, int fNt, int* frap_result, int cNt, int* col_result, double* col_t, int* col_x, int colNp, int* x0, double kelong)
{
	...
	*/
	...
	std::cout << "-------total_codons =" << total_codons << "-------" << std::endl;
    std::cout << "-------total_codons =" << total_species << "-------" << std::endl;


    // ty to selise for giving an example of a c++ map
    std::map<int, std::vector<int>> connection_map = {
    };  // connection dictionary of codons to species

    for(int i=0; i < total_codons; i++)
     {
        std::vector<int> trna_species = {};

        for (int j=0; j < total_species; j++){
            //std::cout << "-------connection_mat =" << connection_mat[i*total_codons + j ] << "-------" << std::endl;
            if (connection_mat[i*total_species + j ]) {
                trna_species.push_back(j);
            } 
        }
        connection_map.insert(  std::pair<int,std::vector<int>> (i, trna_species));    
    }
...
//	N = 10;
    ...
    std::vector<Eigen::Index> idxs; //vector to iterate and get the map indexes
    ...
    while( t < tf)
    {
        ...
        {	
            ...
            if( X(0,i) > 0 ){
                ...
                int total_wn = 0;  // total tRNA here are 
                for (int kk = 0; kk < connection_map[k_index[X(0,i) - 1]].size(); kk++ ){
                    
                    //std::cout << "trna pool val" << trna_pool(connection_map[k_index[X(0,i) - 1]][kk] )<< std::endl; 
                    //std::cout << "trna pool ind" << connection_map[k_index[X(0,i) - 1]][kk] << std::endl; 
                    total_wn += trna_pool(connection_map[k_index[X(0,i) - 1]][kk] );

                }
                //std::cout << "ribosome at codon "  << std::endl;
                //std::cout << k_index[X(0,i) - 1] << std::endl;
                //std::cout << "viable species "  << std::endl;
                //print(connection_map[k_index[X(0,i) - 1]]);
                //std::cout << "current_pool" << trna_pool << std::endl;

                //std::cout << "total_wn" << total_wn << std::endl;
                rib_wn(i) = kelong*total_wn;
                
               // rib_wn(i) = kelong* trna_pool(k_index[X(0,i) - 1]);  //kelong here
				
            }
            if( i>0){   // cant move forward here
                if( X(0,i-1)-X(0,i)<=R ){
		...
		if (ind <= total_species*2){
			...
			if (rib_S.row(rib_ind).sum() < 0){
				...
				if(X(rib_ind) != 0){          // if elongating forward, eat a tRNA ignore when its binding
                    ...
                    std::vector<int> trna_species = connection_map[k_index[X(rib_ind) - 1]];  // get the current trna species to that match current codon
                    
                    if (trna_species.size() > 1){
                        int trna_species_available = 0;
                        Eigen::VectorXd current_pool;
                        current_pool.setZero(trna_species.size()); // set the vector to n trna species to fill
                        for (int kk =0; kk < trna_species.size(); kk++){
                            current_pool(kk) =  trna_pool(trna_species[kk]) ;  // store the species amounts in a vector
                            trna_species_available += trna_pool(trna_species[kk]);  // sum up the total species to uniformly rng pick one
                        }

                        //std::cout << "trna_species: " << std::endl;
                        //print(trna_species);
                        //std::cout << "full_pool " << trna_pool << std::endl;

                        //std::cout << "current_pool " << current_pool << std::endl;
                        r3 =  unif(rng); //get a rnadom uniform number
                        ind2 = 1;
                        //std::cout << "r3  " << r3*trna_species_available << std::endl;
                        while (current_pool.head(ind2).sum() < r3*trna_species_available)  // get which of the species we select (weighted by amount)
                        {	
                            ind2 +=1;
                        }
                        ind2 -=1;
                        //std::cout << "ind2 " << ind2 << std::endl;
                        ind2 = trna_species[ind2];  // get the tRNA species id to delete
                        //std::cout << "total trna species " << trna_species_available << std::endl;

                    }
                    else{
                        ind2 = trna_species[0];   // if theres only one species of trna for this codon skip the uniform rng
                    }

                    //std::cout << "trna_species to eat " << ind2 << std::endl;
					trna_pool(ind2) = trna_pool(ind2)-1; // finally subtract one of that species
					

				}
				
				X.topLeftCorner(1,NR+1) = X.topLeftCorner(1,NR+1) + rib_S.row(rib_ind); // move ribosome forward
