...
class ClientRunner:
    ...
    def _run_train_valid(self, pipeline_spec,
        ...
        with log_utils.log_activity(logger,
            ...
            pipeline = pipeline_spec.instantiate_pipeline_spec(
                ...
                preprocess_pipeline=expr_store.transformers.get_nimbus_preprocessor_pipeline_step(),
        ...
        with log_utils.log_activity(logger, activity_name=constants.TelemetryConstants.RUN_CV_NAME):
            ...
            for split in expr_store.data.materialized.get_CV_splits():
                ...
                m = pipeline_spec.instantiate_pipeline_spec(
                    problem_info, random_state=random_state, is_sparse=expr_store.metadata.is_sparse)
    ...
    def run(self,
        ...
        if (pipeline_spec.supports_constrained_fit() or
                (hasattr(pipeline_spec, 'pipeline_name') and
                    ('Tabnet' in pipeline_spec.pipeline_name or
                        'AutoGluon' in pipeline_spec.pipeline_name))):
