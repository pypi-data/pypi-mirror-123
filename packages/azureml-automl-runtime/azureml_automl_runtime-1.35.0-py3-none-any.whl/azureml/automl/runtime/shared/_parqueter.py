...
class Parqueter:
        ...
        try:
            ...
            if oe.errno == errno.ENOSPC:
                raise ResourceException._with_error(
                    AzureMLError.create(DiskFull, target="pandas.DataFrame.to_parquet",
                                        operation_name="pandas.DataFrame.to_parquet"),
                    inner_exception=oe
                ) from oe
            else:
                logger.error("Failed to store as a parquet file to disk. OS Error Code: {}".format(str(oe.errno)))
                raise
    ...
    def load_pandas_dataframe(path: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.debug('Deserialized parquet file into a pandas dataframe of shape: {}'.format(df.shape))
            return df
        except MemoryError as me:
            raise ResourceException._with_error(AzureMLError.create(InsufficientMemory), inner_exception=me) from me
        except Exception as e:
            if isinstance(e, OSError):
                logger.error("Failed to load the parquet file from disk. OS Error Code: {}".format(str(e.errno)))
            logging_utilities.log_traceback(e, None)
            raise
    ...
    def load_numpy_array(path: str) -> np.ndarray:
        ...
        logger.debug('Deserialized parquet file into a numpy array of shape: {}'.format(array.shape))
        return array
    ...
    def load_single_dim_numpy_array(path: str) -> np.ndarray:
        ...
        Contract.assert_true(array.ndim == 2 and array.shape[1] == 1,
                             message='Expected an array with a single column but received one '
                                     'with shape: {}'.format(array.shape))
        single_dim_array = array.ravel()
        logger.debug('Deserialized parquet file into a single dimensional array of shape: {}'.format(
