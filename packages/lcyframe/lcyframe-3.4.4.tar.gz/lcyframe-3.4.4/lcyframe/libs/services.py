...
class ServicesFindet(object):
    ...
    def import_nsq(self, config):
        ...
        if f:
            ...
            p = os.path.join(config["ROOT"], "producer/nsq")
    ...
    def import_mqtt(self, config):
        ...
        if f and f.get("run", True):
            ...
            p = os.path.join(config["ROOT"], "producer/mqtt")
            if os.path.exists(p) and not f.get("producer_dir"):
    ...
    def import_celery(self, config):
        ...
        if f and f.get("run", True):
            ...
            if not celeryconfig.endswith(".py"):
                ...
                celeryconfig += ".py"
            if not os.path.exists(os.path.join(config["ROOT"], celeryconfig)):
            ...
            p = os.path.join(config["ROOT"], "works/celery")
    ...
    def import_beanstalk(self, config):
        ...
        p = os.path.join(config["ROOT"], "works/beanstalk")
        if os.path.exists(p) and not f.get("workers_dir"):
    ...
    def import_errors(self, config):
        return
        model_name = config["errors_dir"].lstrip("/").replace("/", ".") + "." + "errors.py".rstrip(".py")
            ...
            for root, dirs, files in os.walk(os.path.join(root_path, work)):
                for file in files:
                    ...
                    if not file.endswith(".py"):
                        continue
                    model_name = root.replace(root_path, "").lstrip("/").replace("/", ".") + "." + file.rstrip(".py")
                    objs = __import__(model_name, fromlist=["*"])
    ...
    @classmethod
    def add_service(cls, service_name, service_obj):
        """
        ...
        setattr(cls.app, service_name, service_obj)
        setattr(cls.model, service_name, service_obj)
