...
__all__ = ('upgrade','checkClass','wait','randint','decimal','garbledCode','pausePrint','captcha','judge',
    ...
    'LibraryError','ConscienceError','ArgeementWarning','openWeb','UnknownError','whatNews')
#print("欢迎使用civilianM，当前版本："+v)
def whatNews():
    whatNew = ['2021-10-16 civilianM-2.5.97 更新日志',
        '1.cMES(civilianM Encry Standard |civilianM加密标准)更新至1.0代',
        '2.openFolder()参数 fn可以忽略',
        '3:增加openWeb()功能',
        '4. find()使用search()以后需要find().show获取结果',
        '5.error()类新增加5种错误，并且忽略cont参数时，只会返回 " "',
        '6. cert.py/do()全部改中文',
        '7.civilianM增加一位维护员：AC96',
        '8. login.py缓存文件future.whl更名dontlook.fkyu',
        '9.删除了PyOpenSSL依赖库',
        '10.author()增加新函数：manage()',
        '11.访问author()的函数时无需加括号。（a=author;a.manage）',
        '12.author() old()不返回LibraryError']
    for n in whatNew:
        if n == whatNew[0]:
            print(n)
        print('\t'+n)
