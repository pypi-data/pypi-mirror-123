...
class OpenShift(object):
  ...
  def get_imagestreams(self, label=None):
    custom_ns="odh-imagestreams" #Arun: updated to fetch imagestreams from a target namespace.
    imagestreams = self.oapi_client.resources.get(kind='ImageStream', api_version='image.openshift.io/v1')
    imagestream_list = imagestreams.get(namespace=custom_ns, label_selector=label)
    print(imagestream_list)
