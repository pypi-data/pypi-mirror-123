...
import wagascianpy.utils.topology
...
_BAD_RUNS = [1, 2, 14, 19, 20, 26, 55, 56, 57, 82, 83, 88, 91] + list(range(44, 48 + 1)) + list(range(68, 77 + 1)) + \
            list(range(107, 117 + 1)) + [124, 128, 130, 131, 140, 141, 142, 150, 168, 174, 175, 176, 197] + \
            list(range(186, 190 + 1))

# Temporarily uncheck the bad runs of WAGASCI and WallMRD
_BAD_RUNS_WAGASCI_UPSTREAM = list(range(22, 54 + 1)) + _BAD_RUNS
_BAD_RUNS_WAGASCI_DOWNSTREAM = list(range(22, 54 + 1)) + _BAD_RUNS
_BAD_RUNS_WALLMRD_SOUTH = _BAD_RUNS
_BAD_RUNS_WALLMRD_NORTH = _BAD_RUNS
...
def _documents2records(documents):
    # type: (List) -> List[WagasciRunRecord]
    records = []
    for document in documents:
        records.append(WagasciRunRecord(record=document))
    return records
...
class WagasciDataBase(wagascianpy.database.my_tinydb.MyTinyDB):
    ...
    def __init__(self, db_location, repo_location=None, is_borg_repo=False, update_db=False, rebuild_db=False,
                 wagasci_libdir=None):
        # type: (str, Optional[str], bool, bool, bool, Optional[str]) -> None
    ...
    def _update_wagasci_db(self):
        ...
        with wagascianpy.utils.utils.Cd(self._tmp_dir):
            for run_name in sorted(run_list):
                ...
                run.wagasci_upstream_good_data_flag = run.run_number not in _BAD_RUNS_WAGASCI_UPSTREAM
                run.wagasci_downstream_good_data_flag = run.run_number not in _BAD_RUNS_WAGASCI_DOWNSTREAM
                run.wallmrd_north_good_data_flag = run.run_number not in _BAD_RUNS_WALLMRD_NORTH
                run.wallmrd_south_good_data_flag = run.run_number not in _BAD_RUNS_WALLMRD_SOUTH
                    ...
                    for file_path in file_list:
                        if '.raw' in os.path.basename(file_path):
                            ...
                            if self._is_borg_repo:
                                ...
                                log_file = os.path.join(self._tmp_dir, file_path)
                            else:
                                log_file = file_path
                            ...
                            if self._is_borg_repo:
                                borg_extract = "%s extract --log-json %s::%s %s" \
                                               % (self._borg, self._repository, run_name, file_path)
                                wagascianpy.utils.utils.run_borg_cmd(borg_extract)
                                xml = os.path.join(self._tmp_dir, file_path)
                            else:
                                xml = file_path
                            try:
                                dif_topology = wagascianpy.utils.topology.get_topology_from_xml(
                                    acq_config_xml=xml, wagasci_libdir=self._wagasci_libdir)
                                run.topology = json.dumps(dif_topology)
                            except (OSError, KeyError) as error:
                                print("Error while getting topology : %s" % error)
                                print("Pyrame XML acquisition file : %s" % xml)
                                print("WAGASCI library directory : %s" % self._wagasci_libdir)
                                run.topology = "undef"
    ...
    def get_time_interval(self,
                          datetime_start,  # type: Union[datetime, str, IntTypes, float]
                          datetime_stop,  # type: Union[datetime, str, IntTypes, float]
                          only_good=True,  # type: bool
                          include_overlapping=True  # type: bool
                          ):
        # type: (...) -> List[WagasciRunRecord]
        ...
        return _documents2records(super(WagasciDataBase, self).get_time_interval(
            timestamp_start=timestamp_start, timestamp_stop=timestamp_stop,
            only_good=only_good, include_overlapping=include_overlapping))

    def get_run_interval(self, run_number_start, run_number_stop=None, only_good=True):
        # type: (int, int, bool) -> List[WagasciRunRecord]
        return _documents2records(super(WagasciDataBase, self).get_run_interval(
            run_number_start=run_number_start, run_number_stop=run_number_stop,
            only_good=only_good))

    def get_all(self, only_good=True):
        # type: (bool) -> List[WagasciRunRecord]
        return _documents2records(super(WagasciDataBase, self).get_all(only_good=only_good))
...
def run_to_interval(start,  # type: Union[IntTypes, datetime]
                    stop=None,  # type: Optional[Union[IntTypes, datetime]]
    ...
    if isinstance(start, datetime) and isinstance(stop, datetime):
        ...
        with wagascianpy.database.wagascidb.WagasciDataBase(db_location=database) as db:
            if not stop:
