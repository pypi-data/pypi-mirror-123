...
try:
    ...
    from tinydb.database import Document as tinydbDocument
...
class MyTinyDB(ABC):
    ...
    def update_database(self, run_record_list):
        # type: (List[wagascianpy.database.db_record.DBRecord]) -> None
    ...
    def insert(self, record):
        # type: (Union[tinydbDocument, Dict]) -> None
    ...
    def get_duration_less_than(self, duration_h, only_good=True):
        # type: (float, bool) -> List[tinydbDocument]
    ...
    def get_record(self, name):
        # type: (str) -> tinydbDocument
        """Print info about run with name
        """
        run = tinydb.Query()
        return self._database.search(run.name == name)

    def get_time_interval(self, timestamp_start, timestamp_stop, only_good=True, include_overlapping=True):
        # type: (float, float, bool, bool) -> List[tinydbDocument]
