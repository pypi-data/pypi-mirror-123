...
class Analyzer(ABC):
    ...
    def multiple_input_loop(self, input_files, chains):
        ...
        for input_file in sorted(input_files):
            ...
            dif_id = int(utils.extract_dif_id(input_file))
...
class Decoder(Analyzer):
    ...
    def spawn(self, chains):
        ...
        if os.path.isfile(self.run_root_dir) and self.run_root_dir.endswith('.raw'):
            ...
            input_files = utils.find_files(self.run_root_dir, 'raw')
...
class TdcCalibration(Analyzer):
    ...
    def spawn(self, chains):
        ...
        if os.path.isfile(self.run_root_dir) and self.run_root_dir.endswith('tree.root'):
            ...
            raise OSError("Input file or directory is invalid : {}".format(self.run_root_dir))
        self.multiple_input_loop(input_files=input_files, chains=chains)
...
class BcidDistribution(Analyzer):
    ...
    def _set_runtime_arguments(self, input_path):
        ...
        :return: None
        """
        self.args["input_file"] = input_path
    ...
    def spawn(self, chains):
        ...
        if not topology_source:
            topology_source = acqconfigxml_file_finder(self.run_root_dir, os.path.basename(self.run_root_dir))
    ...
    def spawn(self, chains):
        ...
        if os.path.isfile(self.run_root_dir) and self.run_root_dir.endswith('.root'):
            ...
            input_files = [filename for filename
                           in utils.find_files(self.run_root_dir, 'root')
...
class DataQuality(Analyzer):
    ...
    _default_args = utils.get_arguments_ordered_dict(analysis.WagasciAnalysis.data_quality)
    _default_args.update({'passes': 3, 'enable_plotting': False, 'fixed_wallmrd_gain': False,
    ...
    def spawn(self, chains):
        ...
        topology_source = self.args["topology_source"]
        if not topology_source:
            topology_source = acqconfigxml_file_finder(self.run_root_dir[0], os.path.basename(self.run_root_dir[0]))
        self.args["topology_source"] = topology_source
        ...
        for dif_id in topology:
            ...
            for run_dir in self.run_root_dir:
                for root_file in utils.find_files(path=run_dir, extension='root', recursive=False):
                    if str(dif_id) == str(utils.extract_dif_id(root_file)):
...
class AnalyzerFactoryProducer:
    ...
    _ReturnType = Union[DecoderFactory, MakeHistFactory, TdcCalibrationFactory, SpillNumberFixerFactory,
                        BeamSummaryDataFactory, ApplyDetectorFlagsFactory, TemperatureFactory, DataQualityFactory,
                        BcidDistributionFactory, AdcDistributionFactory]
    ...
    def get_factory(self, type_of_factory, **kwargs):
        ...
        elif type_of_factory == DataQuality.name:
