...
class Graph(object):
    ...
    @xrange.setter
    def xrange(self, myrange):
        # type: (Range) -> None
        if isinstance(myrange, Range):
            self._xrange = myrange
        else:
            raise TypeError("X axis Range type is unknown {}".format(type(myrange).__name__))
...
class Graph2D(Graph):
    ...
    def raw_ydata(self):
        # type: (...) -> bool
        return self._raw_ydata

    @raw_ydata.setter
    def raw_ydata(self, raw_ydata):
        # type: (bool) -> None
        self._raw_ydata = raw_ydata
    ...
    def make_tgraph(self, logscale=False):
        ...
        for xbin, xdata in enumerate(self.xdata):
            if self._raw_ydata:
                for ydata in self.ydata[xbin]:
                    th2d.Fill(xdata, ydata)
            else:
                for ybin, ydata in enumerate(self.ydata[xbin]):
                    th2d.SetBinContent(xbin + 1, ybin + 1, ydata)
