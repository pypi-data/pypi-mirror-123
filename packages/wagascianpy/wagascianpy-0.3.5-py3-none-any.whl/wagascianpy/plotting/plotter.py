...
class Plotter(ABC):
    ...
    def add_bsd_markers(self):
        ...
        if self._enabled_markers.bsd and hasattr(self, "_start") and hasattr(self, "_stop"):
            if hasattr(self, "_bsd_database"):
                database = self._bsd_database
            else:
                database = None
            self.make_bsd_markers(start=self._start, stop=self._stop, bsd_database=database)
...
class DataQualityPlotter(Plotter, ABC):
    ...
    def __init__(self, data_quality_location=None, topology=None, *args, **kwargs):
        # type: (Optional[str], Optional[pl.topology.Topology], *Any, **Any) -> None
        super(DataQualityPlotter, self).__init__(*args, **kwargs)
        self._data_quality_location = data_quality_location
        self._patron = harvest.Patron()
    ...
    def gather_data(self, gr):
        ...
        if isinstance(gr, graph.Graph2D):
...
class GainHistoryPlotter(DataQualityPlotter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._data_quality_harvester_class = harvest.GainHarvester
        self._temperature_harvester_class = harvest.DataQualityTemperatureHarvester
...
class DarkNoiseHistoryPlotter(DataQualityPlotter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._data_quality_harvester_class = harvest.DarkNoiseHarvester
        self._temperature_harvester_class = harvest.DataQualityTemperatureHarvester
