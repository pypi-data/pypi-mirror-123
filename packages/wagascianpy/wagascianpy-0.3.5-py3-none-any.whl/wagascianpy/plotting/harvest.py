...
class WagasciHarvester(Harvester, ABC):
    ...
    def _plant_trees(self, only_good=False):
        ...
        if not self._trees_have_been_planted:
            ...
            with wagascianpy.database.wagascidb.WagasciDataBase(db_location=self._database) as db:
...
class DataQualityHarvester(Harvester, ABC):
    ...
    def __init__(self, topology=None, *args, **kwargs):
        # type: (wagascianpy.plotting.topology.Topology, Any, Any) -> None
        """
    ...
    def _plant_trees(self):
        ...
        if not self._trees_have_been_planted:
            for root_file, dif_id in [(filename, utils.extract_dif_id(filename))
                                      for filename in utils.find_files(self._repository, 'root')
                                      if utils.extract_dif_id(filename) is not None]:
...
class BcidHarvester(WagasciHarvester):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.active_branches = ["spill_mode", "timestamp", "bcid", "hit"]
    ...
    def harvest_data(self, detector_name=None, only_good=False):
        ...
        for detector in self._detectors:
            for dif in [dif for dif in detector if dif.has_tree() and dif.is_enabled()]:
                ...
                if DifIndex.is_wagasci(dif.dif_id):
                    num_chips = _NCHIPS_WAGASCI
                elif DifIndex.is_wallmrd(dif.dif_id):
