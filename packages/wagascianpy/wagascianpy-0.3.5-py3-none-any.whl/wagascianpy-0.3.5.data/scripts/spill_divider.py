...
def _get_active_detectors(runs, root_dir):
    ...
    for one_run in runs:
        for dif_id, file in one_run.raw_files.items():
            decoded_filepath = _decoded_path(root_dir=root_dir, file=file, runname=one_run.name, dif_id=dif_id)
            print("Day {}: DIF {} is enabled".format(day.strftime("%Y-%m-%d"), dif_id))
            print("Adding DIF {} decoded file : {}".format(dif_id, decoded_filepath))
            detectors.enable_dif(dif_id)
            detectors.get_dif(dif_id).add_tree(root_file=decoded_filepath, tree_name="raw",
                                               tree_friends=["bsd", "fixed_spill_number", "temp_hum", "dq",
                                                             "time_ns", "detector_flags"])
    return detectors
        ...
        with db.WagasciDataBase(db_location=arguments.wagasci_database, repo_location=arguments.wagasci_decoded,
            ...
            for detector in active_detectors:
                for dif in [dif for dif in detector if dif.is_enabled()]:
                    ...
                    tree_friends = {"bsd": None, "fixed_spill_number": None, "temp_hum": None, "dq": None,
                                    "time_ns": None, "detector_flags": None}
                    tree_clones = {"bsd": None, "fixed_spill_number": None, "temp_hum": None, "dq": None,
                                   "time_ns": None, "detector_flags": None}
