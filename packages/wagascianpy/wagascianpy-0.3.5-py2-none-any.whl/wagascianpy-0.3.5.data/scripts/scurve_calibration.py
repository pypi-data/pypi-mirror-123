...
from wagascianpy.utils.utils import list_dir_with_integer, join_threads, limit_threads
...
def scurve_decoder(process, threshold_dir, acq_name, dif, n_chips, overwrite_flag=True):
...
def scurve_make_hist(process, threshold_dir, acq_name, dif, acq_config_xml):
    ...
    return process.make_hist(input_tree_file, acq_config_xml, output_dir, ul_flags, dif)
...
def scurve_ana_hist(process, threshold_dir, acq_name, dif, acq_config_xml):
    ...
    ul_flags = int(flags.to01(), 2)
    return process.ana_hist(input_hist_file, acq_config_xml, output_dir, output_img_dir, ul_flags, dif)
...
def scurve_ana_hist_summary(process, threshold_dir, dif):
    ...
    flags[4] = True  # Dark noise
    ul_flags = int(flags.to01(), 2)
    return process.ana_hist_summary(input_dir, output_xml_dir, output_img_dir, ul_flags)
...
def scurve_scurve(process, run_root_dir):
    ...
    paranoid_mode = False
    return process.scurve(run_root_dir, output_dir, output_img_dir, paranoid_mode)
...
def scurve_analysis(run, decoder=True, makehist=True, anahist=True, overwrite=False):
                ...
                for dif in dif_topology:
                    ...
                    if not overwrite and os.path.exists(filepath):
                        ...
                        if not tfile.IsZombie():
                            ...
                            os.remove(filepath)
                    limit_threads(threads=threads, max_threads=MAX_NB_THREAD_CHAINS)
                    n_chips = len(dif_topology[dif])
                    process = WagasciAnalysis(wagasci_lib)
                    threads.append(scurve_decoder(process=process, threshold_dir=threshold_dir, acq_name=acq_name,
                ...
                for dif in dif_topology:
                    ...
                    if not overwrite and os.path.exists(filepath):
                        ...
                        if not tfile.IsZombie():
                            ...
                            os.remove(filepath)
                    process = WagasciAnalysis(wagasci_lib)
                    result = scurve_make_hist(process=process, threshold_dir=threshold_dir, acq_name=acq_name,
            ...
            for threshold, threshold_dir in threshold_dirs:
                ...
                for dif in dif_topology:
                    ...
                    if not overwrite and os.path.exists(anahist_dirname):
                        ...
                        continue
                    process = WagasciAnalysis(wagasci_lib)
                    result = scurve_ana_hist(process=process, threshold_dir=threshold_dir, acq_name=acq_name,
                                             dif=dif, acq_config_xml=actual_acq_config_xml)
                    if result != 0:
            ...
            for threshold, threshold_dir in threshold_dirs:
                ...
                for dif in dif_topology:
                    ...
                    if not overwrite and os.path.exists(anahist_dirname):
                        ...
                        continue
                    process = WagasciAnalysis(wagasci_lib)
                    result = scurve_ana_hist_summary(process=process, threshold_dir=threshold_dir, dif=dif)
                    if result != 0:
    ...
    process = WagasciAnalysis(wagasci_lib)
    result = scurve_scurve(process=process, run_root_dir=run_root_dir)
    del process
    if result != 0:
