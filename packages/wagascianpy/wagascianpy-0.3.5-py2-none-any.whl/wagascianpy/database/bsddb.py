        ...
        for event in bsd_tree:
            ...
            if ((event.good_spill_flag == GoodSpillFlag.GoodHornMinus250kA or
                ...
                if ct_pot:
                    ...
                    self.number_of_pot += event.ct_np[CTNumber.Fifth * len(BunchNumber) + BunchNumber.TotalCurrent]
...
class BsdDataBase(wagascianpy.database.my_tinydb.MyTinyDB):
    ...
    def __init__(self,
        ...
        if self._repository_location and ':' in self._repository_location:
    ...
    def _update_bsd_db(self):
        ...
        with wagascianpy.utils.utils.Cd(download_location):
            ...
            for root, _, files in os.walk(download_location):
                for afile in files:
                    if '.root' in afile:
                        run_list.append(os.path.join(root, afile))
            ...
            for run_path in sorted(run_list):
                if self.has_record(os.path.basename(run_path)):
                    ...
                    run_record_list.append(BsdRecord(file_path=run_path))
    ...
    def get_time_interval(self,
        ...
        documents = super(BsdDataBase, self).get_time_interval(timestamp_start, timestamp_stop, *args, **kwargs)
        records = []
        for document in documents:
            records.append(BsdRecord(record=document))
        return records

    def pretty_print(self, name):
        # type: (str) -> None
        """ Print info about a run """
        BsdRecord(record=self.get_record(name)[0]).pretty_print()
