...
def parse_args(args):
    ...
    parser.add_argument('--batch-mode', dest='batch_mode', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Do not run the graphical interface. (default: %(default)s)'''))

    parser.add_argument('--wagasci-repository', metavar='<WAGASCI repository location>',
    ...
    parser.add_argument('--simple-wagasci-repository', dest='simple_wagasci_repository', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        If set the WAGASCI run repository is assumed to be a simple repository (no encription) 
                        (default: %(default)s)'''))

    parser.add_argument('--borg-wagasci-repository', dest='borg_wagasci_repository', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        If set the WAGASCI run repository is assumed to be a borg repository (default: %(default)s)'''))

    parser.add_argument('--update-wagasci-database', dest='update_wagasci_database', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        If set the WAGASCI database is updated (new runs are added but existing runs are not 
                        modified) (default: %(default)s)'''))

    parser.add_argument('--rebuild-wagasci-database', dest='rebuild_wagasci_database', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        If set the WAGASCI database is rebuilt (all runs are overwritten) (default: %(default)s)'''))

    parser.add_argument('--wagasci-database', metavar='<WAGASCI database location>', dest='wagasci_database',
    ...
    parser.add_argument('--wagasci-libdir', metavar='<WAGASCI library location>', dest='wagasci_libdir',
                        type=str, nargs=1, required=False, help=textwrap.dedent('''\
    ...
    parser.add_argument('--bsd-database', metavar='<BSD database location>', dest='bsd_database', type=str,
                        nargs=1, required=False, help=textwrap.dedent('''\
                        Path to the Beam Summary Data database file. This file is created by the 
                        Wagasci Database Viewer (wagascidb_viewer.py) program and is usually called bsddb.db.
                        The path can be also a remote path of the form <SSH alias>:/path/to/database where the SSH
    ...
    parser.add_argument('--bsd-download-location', metavar='<BSD database download location>',
    ...
    parser.add_argument('--update-bsd-database', dest='update_bsd_database', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        If set the BSD database is updated (new runs are added but existing runs are not 
                        modified) (default: %(default)s)'''))

    parser.add_argument('--rebuild-bsd-database', dest='rebuild_bsd_database', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        If set the BSD database is rebuilt (all runs are overwritten) (default: %(default)s)'''))

    parser.add_argument('--wagasci-download-location', metavar='<WAGASCI runs download location>',
    ...
    parser.add_argument('--wagasci-decoded-location', metavar='<WAGASCI decoded save location>',
    ...
    parser.add_argument('--temperature-sqlite-database', metavar='<temperature SQLite database>',
    ...
    parser.add_argument('--data-quality-folder', metavar='<data quality location>',
    ...
    parser.add_argument('--only-good-runs', dest='only_good_runs', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        If set select only good runs (default: %(default)s)'''))

    parser.add_argument('--include-overlapping', dest='include_overlapping', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
    ...
    parser.add_argument('--download', dest='download', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Download the selected runs. If no run is selected download all runs.'''))

    parser.add_argument('--decoder', dest='decoder', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Decode the selected runs. If no run is selected decoded all runs.'''))

    parser.add_argument('--make-hist', dest='make_hist', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Prepare histograms for the selected runs. If no run is selected apply it to all runs.'''))

    parser.add_argument('--tdc-calibration', dest='tdc_calibration', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Apply TDC calibration to the selected runs. If no run is selected apply it to all runs.'''))

    parser.add_argument('--bcid-distribution', dest='bcid_distribution', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Plot the BCID histogram for the selected runs. If no run is selected apply it to all runs.'''))

    parser.add_argument('--adc-distribution', dest='adc_distribution', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                            Plot the 2D ADC histogram for the selected runs. 
                            If no run is selected apply it to all runs.'''))

    parser.add_argument('--spill-number-fixer', dest='spill_number_fixer', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Fix the spill number for the selected runs. If no run is selected apply it to all runs.'''))

    parser.add_argument('--beam-summary-data', dest='beam_summary_data', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Integrate Beam Summary Data into the selected runs. 
                        If no run is selected apply it to all runs.'''))

    parser.add_argument('--apply-detector-flags', dest='apply_detector_flags', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Apply detector flags to the selected runs. 
                        If no run is selected apply it to all runs.'''))

    parser.add_argument('--data-quality', dest='data_quality', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Run data quality program for the selected runs. If no run is selected apply it to all runs.
                         Recommended to use for a total period of roughly one week.'''))

    parser.add_argument('--temperature', dest='temperature', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Merge the temperature and humidity for the selected runs. If no run is selected apply it 
                        to all runs. This analyzer must be run after the beam-summary-data analyzer.'''))

    parser.add_argument('--all-analyzers', dest='all_analyzers', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
                        Apply all analyzers to the selected runs. If no run is selected apply them to all runs.'''))

    parser.add_argument('--overwrite-flag', dest='overwrite_flag', required=False,
                        default=False, action='store_true', help=textwrap.dedent('''\
