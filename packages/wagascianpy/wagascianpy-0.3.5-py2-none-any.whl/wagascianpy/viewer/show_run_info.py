...
from wagascianpy.viewer.import_tkinter import import_tkinter, import_pygubu

pygubu = import_pygubu()


# noinspection PyArgumentList, PyUnresolvedReferences
def _show_run_info(builder,  # type: pygubu.Builder
    ...
    if not tkinter:
        print("Run info could not be should because tkinter module was not found")
        return
    ...
    if bsd_database:
        ...
        except (RuntimeWarning, ValueError) as exception:
            print(str(exception))
    ...
    start_time = record.get_start_datetime().strftime('%Y/%m/%d %H:%M:%S %Z')
    ...
    stop_time = record.get_stop_datetime().strftime('%Y/%m/%d %H:%M:%S %Z')
