...
class CustomEvents(ABC):
    ...
    def __init__(self, start, stop=None, wagasci_database=None):
        ...
        if isinstance(start, IntTypes):
            self._start, self._stop = wagascianpy.database.wagascidb.run_to_interval(start=start, stop=stop,
                                                                                     database=self._wagasci_database)
        elif isinstance(start, string_types):
            self._start = wagascianpy.database.wagascidb.WagasciRunRecord.str2datetime(start)
            self._stop = wagascianpy.database.wagascidb.WagasciRunRecord.str2datetime(stop)
