...
def renametree(run_root_dir, run_name, dif_topology):
    ...
    file_dif_list = []
    for file_name in sorted(wagascianpy.utils.utils.find_files(path=run_root_dir, extension='raw')):
    ...
    for tmp_file_name in sorted(wagascianpy.utils.utils.find_files(path=run_root_dir, extension='tmp')):
...
def count_threads(run_chains):
    """Count total number of active threads in a chain"""
    num_threads = 0
    for dif_chains in run_chains.values():
        for chain in dif_chains.values():
...
def limit_chains(run_chains, max_threads):
    """Limit the number of simultaneuously running threads"""
    num_threads = count_threads(run_chains)
    for run_name, dif_chains in sorted(run_chains.items()):
        for dif_id, chain in sorted(dif_chains.items()):
...
def list_dir_with_integer(path):
    ...
    for directory in dir_list:
        if bool(nums_re.search(directory)):
            result.append(dir_with_integer(nums_re.search(directory).group(0), directory))
...
def find_files(path, extension, recursive=True):
    file_list = []
    if recursive:
        for root, _, files in os.walk(path):
            for f in files:
                if f.lower().endswith('.%s' % extension.strip('.')):
                    file_list.append(f if os.path.isabs(f) else os.path.join(root, f))
    else:
        for f in [f for f in os.listdir(path) if os.path.isfile(os.path.join(path, f))]:
            if f.lower().endswith('.%s' % extension.strip('.')):
                file_list.append(f if os.path.isabs(f) else os.path.join(path, f))
    return file_list
