...
_DEFAULT_CONFIGURATION_FILENAME = "anpy_conf.ini"

# WAGASCI database configuration
_WAGASCI_DATABASE = "kekcc:/hsm/nu/wagasci/data/wagascidb.db"
_WAGASCI_REPOSITORY = "kekcc:/hsm/nu/wagasci/data/run10/wagasci/rawdata"
_WAGASCI_REPOSITORY_TYPE = RepositoryType.Simple
_WAGASCI_DOWNLOAD_LOCATION = "/tmp/download"
_WAGASCI_DECODED_LOCATION = "/tmp/decoded"
_WAGASCI_DEFAULT_DATABASE_NAME = "wagascidb.db"

# BSD database configuration
_BSD_DATABASE = "kekcc:/hsm/nu/wagasci/data/bsddb.db"
_BSD_REPOSITORY = "kekcc:/gpfs/fs03/t2k/beam/exp/data/beam_summary/current/t2krun10"
...
_WAGASCI_LIBDIR = "/usr/local/lib/wagasci/calibration"

# Temperature database configuration
_TEMPERATURE_SQLITE_DATABASE = "kekcc:/hsm/nu/wagasci/data/run10/temphum/mh_temperature_sensors_t2krun10.sqlite3"
...
def conf_file_finder(filename, project_path=None):
    ...
    """
    Four types of configuration files are considered in the following priority (higher is preferred). If the
    configuration file is not found in the top priority folder the next one in priority is searched. If no file is
    found the 2) User configuration file is returned even if it does not exists.
     1 - Argument : The file path is passed as argument
     2 - Environment : The environment variable WAGASCI_CONFDIR is assumed to be the path to folder containing the
                     configuration file.
     3 - User        : The configuration folder is $HOME/.wagasci
     4 - System      : The configuration folder is /usr/local/wagasci (Only UNIX)
     5 - Project     : Configuration file used during development and stored in the project folder/folders.
                     "filename" is appended to each "project_path" folder.
    :param filename: name of the file
    :param project_path: project paths containing the configuration files (only used during development)
    ...
    try:
        ...
        wagasci_environ = os.path.join(env["WAGASCI_CONFDIR"], filename)
    except KeyError:
        pass

    # Append the filename to the project paths
    ...
    # 5 : Project
        ...
        for path in wagasci_project:
            if os.path.exists(path):
                conf_file = path
                break
    # No file is found (assuming user configuration file)
    else:
        ...
        if config_file_path:
            ...
            if os.path.exists(config_file_path):
                ...
                with open(config_file_path, 'w') as configfile:
                    print('Writing default configuration to file "{}"'.format(config_file_path))
...
def _create_sections(cls, parser_class=WagasciConfigParser):
    conf_file = conf_file_finder(filename=_DEFAULT_CONFIGURATION_FILENAME)
    print("Using INI configuration file %s" % conf_file)
