...
from shapsplain.node import create_node, MultipredicateNode
...
def compute_expectations(node, depth):
    if node._is_leaf:
        ...
        return 0
    elif isinstance(node, MultipredicateNode):
        depth = compute_expectations(node._left, depth + 1)
        node._expectation = node._left._expectation
...
class ShapForest(object):
    def __init__(self, adict):
        if "model" in adict:
            ...
            raise ValueError(
                "Model format not recognized with keys %s", str(sorted(adict.keys()))
            )
