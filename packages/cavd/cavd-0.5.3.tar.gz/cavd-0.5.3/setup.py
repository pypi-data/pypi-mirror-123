...
libdirs = ["libs/Voro++/lib", "libs/Zeo++/lib"]
#runtimedir = os.path.join(os.path.realpath(__file__),"libs/Zeo++")
runtimedir = os.path.realpath("libs/Zeo++")
if sys.platform == 'linux':
    runtimedirlist = [runtimedir]
else:
    runtimedirlist = None
...
setup(name='cavd',
      ...
      install_requires=[
          "Cython>=0.29", "numpy>=1.14.3", "ase>=3.3", "spglib>=1.9.9.44",
          "scipy>=1.0.1", "pandas", "monty>=0.9.6", "networkx>=2.2",
          "pymatgen==2022.0.10"
      ],
      package_data={
          "cavd":
          ["ionic_radii.json", "bvmparam.dat", "bvse.dat", "BVSEParam.dat"]
      },
      classifiers=[
      ...
      ext_modules=[
          Extension("cavd.voronoicell",
                    ...
                    runtime_library_dirs=runtimedirlist,
                    language='c++'),
