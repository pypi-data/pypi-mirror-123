...
def negative_sampling(positive_edges, graph, samples=1, negative_nodes=None):
    ...
    for u, v in positive_edges:
        ...
        for _ in range(samples):
            ...
            edges.append([u, vneg])
    return np.array(edges), ([1.]+[0.]*samples)*len(positive_edges)


class LinkPrediction(Predictor):
    def __init__(self, edges, labels=None, gnn: Layered=None, similarity="dot", loss="diff", regularize=0.5):
        if callable(edges):
    ...
    def loss(self, features: tf.Tensor):
        self._update_labels()
        regularize = 0 if self.regularize == 0 else self.regularize*tf.reduce_sum(features**2)/features.shape[0]
        if self.similarity == "cos":
            features = tf.math.l2_normalize(features, axis=1)
        ...
        return tf.keras.losses.BinaryCrossentropy(from_logits=True)(self.values, self.predict(features, to_logits=True)) + regularize
...
class MeanLinkPrediction(LinkPrediction):
    def __init__(self, *args, graph, positive_nodes=None, negative_nodes=None, k=5, **kwargs):
        ...
        self.k = k
        self.graph = graph
        self.parsed_edges = None

    def evaluate(self, features):
        if self.parsed_edges is None:
        ...
        negative_nodes = [v for edge in self.parsed_edges for v in edge] if self.negative_nodes is None else self.negative_nodes
        for node in tqdm(positive_nodes):
            if node not in self.parsed_edges:
                continue
            self.edges, self.labels = recommend_all(node,
                                               self.graph,
                                               positive_edges=[[node, neighbor] for neighbor in self.parsed_edges[node]],
            ...
            max_cov = set(list(cov)+[self.edges[i][1] if self.edges[i][0] == node else self.edges[i][0] for i in range(len(self.labels)) if self.labels[i]!=0])
