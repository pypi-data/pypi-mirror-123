...
# This is OKTA stuff
LOGIN_SERVER = 'https://olh.optumi.net:12000'
...
def get_path():
    return "https://" + (domain if domain.endswith(LOGIN_DOMAIN_END) else domain + LOGIN_DOMAIN_END) + ":8443"
...
class VersionHandler(IPythonHandler):
    ...
    async def get(self):
        global domain
        try:
            self.write(json.dumps({'domain': domain, 'version': __version__, 'userHome': userHome, 'jupyterHome': jupyterHome}))
...
class LoginHandler(IPythonHandler):
    ...
    async def get(self):
        try:
            ...
            if response.geturl().endswith('/login'):
                self.write(json.dumps({'domain': domain, 'message': 'Not logged in', 'loginFailed': True}))
                return
            if not getattr(response, 'geturl', False) or response.getcode() != 200:
                self.write(json.dumps({'domain': domain, 'loginFailedMessage': response.read().decode('utf-8'), 'message': 'Version exchange failed', 'loginFailed': True}))
            ...
            if not getattr(response, 'geturl', False) or response.getcode() != 200:
                self.write(json.dumps({'domain': domain, 'message': 'Encountered error while getting new user agreement', 'loginFailed': True}))
                jupyter_log.error(optumi_start(self) + str(response))
                IOLoop.current().run_in_executor(None, logout)
                return
            newAgreement = True
            ...
            self.set_status(200)
            self.write(json.dumps({'domain': domain, 'message': 'Encountered error while getting user information', 'loginFailed': True}))
            # We don't want to raise an error here, since it will override the domain we send back to the extension
            # if DEBUG: raise e
    ...
    async def post(self):
        try:
            ...
            ## This can be called with no login name or password to only set the domain
            if loginName is None or password is None:
                self.write(json.dumps({'message': 'Set domain, login not attempted'}))
                return
            ...
            if login_status == 1:
                ...
                if not getattr(response, 'geturl', False) or response.getcode() != 200:
                    self.set_status(response.getcode())
                    self.write(json.dumps({'domain': domain, 'loginFailedMessage': 'Unable to get user information', 'message': 'Unable to get user information', 'loginFailed': True}))
                    jupyter_log.error(optumi_start(self) + str(response))
                    IOLoop.current().run_in_executor(None, logout)
                    return
                user_information = json.load(response)
                ...
                self.write(json.dumps({'domain': domain, 'loginFailedMessage': message, 'message': 'Login failed with message: ' + message, 'loginFailed': True}))
            elif login_status == -2:
                self.write(json.dumps({'domain': domain, 'message': 'Login failed due to invalid request', 'domainFailed': True}))
        except Exception as e:
            self.set_status(401)
            self.write(json.dumps({'domain': domain, 'message': 'Encountered error while handling login'}))
            IOLoop.current().run_in_executor(None, logout)
            jupyter_log.error(optumi_start(self) + str(e))
            if DEBUG: raise e
...
class OauthLoginHandler(IPythonHandler):
    ...
    async def get(self):
        ...
        try:
            ...
            login_state = { 'state': generate_state(), 'origin': self.request.protocol + "://" + self.request.host }
...
class OauthCallbackHandler(IPythonHandler):
    ...
    async def get(self):
        ...
        try:
            ...
            login_status, message = await IOLoop.current().run_in_executor(None, login_rest_server, '', token)

            if login_status == 1:
                self.write(LOGIN_WRAP_START + 'Your have successfully logged into Optumi and you can close this tab' + LOGIN_WRAP_END)
            elif login_status == -1:
                self.write(LOGIN_WRAP_START + 'Login failed with message: ' + message + LOGIN_WRAP_END)
            elif login_status == -2:
                self.write(LOGIN_WRAP_START + 'Login failed due to invalid request' + LOGIN_WRAP_END)
...
class LaunchNotebookHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            lastModifieds = [datetime.datetime.fromtimestamp(stat.st_mtime).isoformat() + 'Z' if stat != None else str(None) for stat in stats]
            sizes = [str(stat.st_size) if stat else str(None) for stat in stats]
            uuid = data['uuid']
            timestamp = data['timestamp']
            IOLoop.current().run_in_executor(None, launch_notebook, requirementsFile, hashes, paths, lastModifieds, sizes, uuid, timestamp)
            self.write(json.dumps({'message': 'success', 'hashes': hashes, 'files': paths, 'filesmod': lastModifieds, 'filessize': sizes}))
        except Exception as e:
            self.set_status(500)
...
class TeardownNotebookHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            data = json.loads(self.request.body)
            workload = data['workload']
            response = await IOLoop.current().run_in_executor(None, teardown_notebook, workload)
            self.set_status(401 if response.geturl().endswith('/login') else response.getcode())
            self.write(response.read())
            if response.getcode() >= 300: jupyter_log.info(optumi_start(self) + str(response))
        except (ConnectionError, URLError) as e:
...
class DeleteFilesHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            hashes = data['hashes']
            paths = data['paths']
            response = await IOLoop.current().run_in_executor(None, delete_files, hashes, paths)
            self.set_status(401 if response.geturl().endswith('/login') else response.getcode())
            self.write(response.read())
            if response.getcode() >= 300: jupyter_log.info(optumi_start(self) + str(response))
        except Exception as e:
...
class CancelProgressHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            self.set_status(500)
            self.write(json.dumps({'message': 'Encountered error while cancelling uploading files'}))
            jupyter_log.error(optumi_start(self) + str(e))
            if DEBUG: raise e
...
class UploadFilesHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            IOLoop.current().run_in_executor(None, upload_files, key, paths, compress)
            self.write(json.dumps({'message': 'success'}))
        except Exception as e:
            self.set_status(500)
...
class DownloadFilesHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            hashes = data['hashes']
            paths = data['paths']
            savePaths = data['savePaths']
            sizes = data['sizes']
            overwrite = data['overwrite']
            response = await IOLoop.current().run_in_executor(None, download_files, key, hashes, paths, savePaths, sizes, overwrite)
...
class GetNotebookOutputFilesHandler(IPythonHandler):
    ...
    async def post(self):
        try:
            ...
            sizes = data['sizes']
            overwrite = data['overwrite']
            response = await IOLoop.current().run_in_executor(None, get_notebook_output_files, workloadUUID, moduleUUID, key, paths, savePaths, sizes, overwrite)
            # We only expect a response if something went wrong
            if response != None: raise response
        except (ConnectionError, URLError) as e:
            # If we can't connect to the REST interface, we want the extension to treat it as the user being loggeed out
...
class MergeNotebooksHandler(IPythonHandler):
    @authenticated
    async def post(self):
        try:
            data = json.loads(self.request.body)
            base = nbformat.from_dict(data['base'])
            change1 = nbformat.from_dict(data['local'])
            change2 = nbformat.from_dict(data['remote'])

            nbformat.write(base, 'base')
            nbformat.write(change1, 'local')
            nbformat.write(change2, 'remote')

            result = subprocess.run(['nbdime', 'merge', '-s', 'base', 'local', 'remote'], stdout=subprocess.PIPE)
            out = result.stdout
            ...
            self.write(json.loads(out))
        except Exception as e:
            self.set_status(500)
            self.write(json.dumps({'message': 'Encountered error while merging notebooks'}))
            jupyter_log.error(optumi_start(self) + str(e))
            # if DEBUG: raise e
            raise e
...
def setup_handlers(server_app):
    ...
    web_app.add_handlers(host_pattern, [(url_path_join(base_url, '/optumi/merge-notebooks'), MergeNotebooksHandler)])

###############################################################################################
####    Login
####

LOGIN_DOMAIN_END = '.optumi.net'

def install_auth_opener(loginName, password):
    policy = DefaultCookiePolicy(allowed_domains=[domain if domain.endswith(LOGIN_DOMAIN_END) else domain + LOGIN_DOMAIN_END])
    cj = CookieJar(policy)
    ...
    info = urlencode({'login_type': 'oauth', "username": loginName, "password": password}).encode('utf8')
    return info
...
def preview_notebook(nbConfig, includeExisting):
    ...
    try:
        ...
        # jupyter_log.info(optumi_start() + request.urlopen(req, timeout=120).read().decode('utf-8'))
...
def launch_notebook(requirementsFile, hashes, paths, lastModifieds, sizes, uuid, timestamp):
    ...
    try:
        ...
        form.add_file('lastModifieds', 'lastModifieds', fileHandle=io.BytesIO(','.join(lastModifieds).encode('utf-8')))
...
def teardown_notebook(workload):
...
def upload_files(key, paths, compress):
    ...
    try:
        ...
        while len(paths) > 1000:
            ...
            for exists, file in zip(json.load(check_if_files_exist(chunk))['exists'], chunk):
                if not exists:
                    filesToUpload.append((file, os.path.getsize(file)))
            paths = paths[1000:]
        ## Check last chunk
        ...
        if len(filesToUpload) > 0:
            ...
            fileChunks = [[]]
            totalSize = 0
            for file, size in filesToUpload:
                ...
                if size > MAX_SIZE:
                    # TODO:JJ We should let the user know we can't upload a file because it is too large
                    continue
            ...
            for chunk in fileChunks:
                ...
                req.add_header('Content-type', form.get_content_type())
                req.add_header('Content-length', len(data))
                # Since this could be a long request, we will se the timeout (somewhat arbitrarily) to 24 hours
                res = request.urlopen(req, timeout=86400)
                if compress:
...
def add_content(key, content, decoded):
    ...
    try:
        lock.acquire()
        downloadProgress[key]['progress'] = downloadProgress[key]['progress'] + len(base64.decodebytes(encoded))
    except:
        pass
    finally:
        lock.release()

def download_files_helper(URL, key, form, savePaths, overwrite):
    data = bytes(form)
    ...
    while True:
        ...
        if ',' in decoded:
            ...
            for chunk in split[:-1]:
                add_content(key, content, chunk)
                save_file(savePaths.pop(0), content, overwrite)
                content = io.BytesIO()
            # Handle the last chunk like the else case below
            add_content(key, content, split[-1])
        else:
            add_content(key, content, decoded)

    # Save the last file
    save_file(savePaths.pop(0), content, overwrite)

def download_files(key, hashes, paths, savePaths, sizes, overwrite):
    ...
    try:
        ...
        # Sort by file size
        filesToDownload = sorted(filesToDownload, key=lambda x:x[3], reverse=True)
        ...
        fileNames = []
        saveNames = []
        for hash, file, savePath, size in filesToDownload:
            if size > MAX_SIZE:
                # TODO:JJ We should let the user know we can;t upload a file because it is too large
                continue
            elif totalSize + size > MAX_SIZE:
                ...
                download_files_helper(URL, key, form, saveNames, overwrite)
...
def get_notebook_output_files(workloadUUID, moduleUUID, key, paths, savePaths, sizes, overwrite):
    ...
    try:
        ...
        filesToDownload = zip(paths, savePaths, sizes)
        # Sort by file size
        filesToDownload = sorted(filesToDownload, key=lambda x:x[2], reverse=True)
        ...
        fileNames = []
        saveNames = []
        for file, savePath, size in filesToDownload:
            if size > MAX_SIZE:
                # TODO:JJ We should let the user know we can;t upload a file because it is too large
                continue
            elif totalSize + size > MAX_SIZE:
    ...
    with ZipFile(path, 'w', ZIP_DEFLATED) as zip:
        ...
        for file in files:
            ...
            try:
                lock.acquire()
                if compressionProgress[key]['progress'] == -1:
                    raise CancelledError("Compression canceled", path)
