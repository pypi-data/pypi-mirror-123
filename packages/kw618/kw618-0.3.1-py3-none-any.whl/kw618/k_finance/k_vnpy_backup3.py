...
class DataModule():
    ...
    def _live_update_margin_rate(self):
        ...
        # else:
            # iii. 开仓杠杆倍数
            # open_leverage = sum_open_notional / marginBalance  # 除数不是'usdt钱包余额' 而是 '保证金余额'
            # # iv. 实际浮盈率  # (貌似没啥意义了...)
        ...
        #     # "notional_pnl_rate" : notional_pnl_rate,  # 不是实时更新的.. 没啥用..
...
class RiskModlue():
    ...
    def check_match(self):
        ...
        futures_volume_df.pivot_table(index="symbol", aggfunc={"futures_volume":"sum"})
...
class TradeModule():
    ...
    def _cal_volume(self, pair_name, multiplier, insurance_usdt=800):
        ...
        if multiplier > insurance_usdt:
            ...
            logger.log(30, f"!!!!!!!!! 开仓金额大于最高上限, 调整至最高上限:{insurance_usdt}usdt [检查是否手误!!]\n\n")
        count = (1 / last_price) * multiplier
    ...
    def one_side_send_order(
        ...
        if value > insurance_usdt:
            volume = insurance_usdt / last_price
            # 在最高位向后保留3位数 (即: 123456.123保留成 123000; 0.00123456保留成0.00123)
            volume = round(volume, -math.floor(math.log10(volume))+2) # 前面是负号, 并在最高位数基础上保留2位数
            logger.log(30, f"!!!!!!!!! 开仓金额大于最高上限, 调整至最高上限:{insurance_usdt}usdt (即: {volume}个{symbol}) [检查是否手误!!]\n\n")
    ...
    def fast_buy_sell(self, pair_name="BINANCE.LUNAUSDT", multiplier=1, time_sleep=10, sell_first=True):
        """
            # '秒抢费率'
    ...
    def divide(self, spot_ratio=0.49):
        ...
        # 1. 查询当前余额
            # i. 现货USDT可用余额
        usdt_available_balance_spot = self.oms.accounts[f"BINANCE.USDT"].balance - self.oms.accounts[f"BINANCE.USDT"].frozen
            # ii. 合约USDT可用余额 (即: 钱包余额) (即: 最真实的保证金, 不考虑合约的浮盈浮亏!!)
        usdt_available_balance_futures = self.Data.futures_index.get("availableBalance")
            # iii. 双边的usdt总和
...
class SpreadClient():
    ...
    def __init__(self, gateway_setting=LSH_BINANCE_SETTING, engines=[]):
        ...
        """
        # 1. 初始化vnpy中的组件
        main_engine = init_main_engine(gateway_setting=gateway_setting, engines=engines)
...
# gateway_setting_lst = [LSH_BINANCE_SETTING]
gateway_setting_lst = [LSH_BINANCE_SETTING, ZL_BINANCE_SETTING]
# gateway_setting_lst = [ZL_BINANCE_SETTING]

# am = AccountManager(main_account_name="X")
# gateway_setting_lst = [XB_BINANCE_SETTING]
# gateway_setting_lst = [X_BINANCE_SETTING, Y_BINANCE_SETTING]
