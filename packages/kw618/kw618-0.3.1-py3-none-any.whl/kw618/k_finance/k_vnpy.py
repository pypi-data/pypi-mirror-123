"""
    导入本模块:
    from kw618.k_finance.k_vnpy import *
"""
...
class DataModule():
    ...
    def _live_update_margin_rate(self):
        """
            ...
            todo:
                - 每10s请求一次太频繁了... 降低频次
                - 每次请求4个接口的账户数据: 全仓+逐仓+u本位+币本位 (风险率+维持保证金率)
        """
        # 1. 请求account数据
...
class ShowModlue():
    ...
    def cal_avg_price(self, df):
        sum_quoteQty = df["order_quoteQty"].sum()
        sum_qty = df["order_qty"].sum()
        avg_price = sum_quoteQty / sum_qty
        return avg_price
...
class TradeModule():
    ...
    def judge_match(self, pair_name):
        ...
        print(msg)
        if abs(delta_marketValue) < 8000000:
            return True
        else:
            return False
        ...
        def _recur_run():
            ...
            try:
                ...
                while True:
                    ...
                    time.sleep(1) # 最好不要用固定值, 随机值比较好, 避免同时抢单....
...
# gateway_setting_lst = [LSH_BINANCE_SETTING, ZL_BINANCE_SETTING, LB_BINANCE_SETTING]
am.add_accounts(gateway_setting_lst=gateway_setting_lst)



# am = AccountManager(main_account_name="O")
# gateway_setting_lst = [O_BINANCE_SETTING]
# am.add_accounts(gateway_setting_lst=gateway_setting_lst)



# am = AccountManager(main_account_name="MF")
# gateway_setting_lst = [MF_BINANCE_SETTING]
# am.add_accounts(gateway_setting_lst=gateway_setting_lst)


# am = AccountManager(main_account_name="C")
# gateway_setting_lst = [C_BINANCE_SETTING]
# am.add_accounts(gateway_setting_lst=gateway_setting_lst)
