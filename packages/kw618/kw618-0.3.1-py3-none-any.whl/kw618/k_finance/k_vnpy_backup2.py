...
class DataModule():
    def __init__(self, main_engine, client):
        ...
        self.spread_rate_close_dict = {}
            # 子线程控制的开关
        self.stop_cal = True # 初始状态是: '不在计算价差率'

        # 3. 执行一些初始启动函数
        pass

    def _init(self):
        # time.sleep(1) # self.Show.show_futures_index 需要用到futures_index数据, 但是太早执行该数据还未获取到...先等异步执行
        # i. 展示一些'关键爆仓指标'
        self.Show.show_futures_index()
        # ii. 订阅我自己持仓币对的tick数据
        self.Data.add_my_pairs()
        # iii. 同时计算所有币种的价差率 (使用多线程)
        self.Data.main_cal()
    ...
    def _live_update_margin_rate(self):
        ...
        # 2. 计算最新的'维持保证金率'
            # 1. 获取基础数据
        usdt_account = self.oms.accounts.get("BINANCES.USDT", "") # 如果取不出数, 可能是网络问题导致上面的req请求还没得到resp (异步就有这种问题...)
            # i.如果合约账户有usdt保证金:
        if usdt_account:
        ...
        self.futures_index = {
            ...
            "notional_pnl_rate" : notional_pnl_rate,
            "open_leverage" : open_leverage,
            "pnl_rate" : pnl_rate,
            "usdt_balance" : usdt_balance,
            "sum_open_notional" : sum_open_notional,
            "pnl" : pnl,
    ...
    def subscribe_many(self, pair_lst=["BINANCE.LUNAUSDT", "BINANCE.ALPHAUSDT"]):
        ...
        # 3. 等待异步订阅完成
        logger.log(10, f"睡眠6秒, 等待所有币种异步订阅完成")
        time.sleep(6) #
    ...
    def main_cal(self):
        ...
        # 如果发现stop_cal已经为Fasle, 说明子线程已经在运行, 此时先kill掉这些子线程!! (睡1s是确保所有子线程都已经停止了)
        if self.stop_cal == False:
            self.stop_cal = True
            time.sleep(1)
        self.stop_cal = False # False表示: 可以开始循环计算'价差率'了
    ...
    def _check_margin_rate(self):
        ...
        if margin_rate >= self.risk_margin_rate:
            if self.to_send_email == True: # 可以手动将该对象的该属性设置成False, 从而禁止发送邮件...
                subject = f"【提醒: 保证金率超过 {self.risk_margin_rate:.2%}】"
...
class ShowModlue():
    ...
    def show_futures_index(self):
        ...
        newest_notional_marketValue = futures_index.get("sum_open_notional")-futures_index.get("pnl") # 最新名义市值总和
        usdt_wallet_balance = futures_index.get("usdt_balance") # USDT钱包余额
        maintMargin = futures_index.get("maintMargin") # 维持保证金
        if newest_notional_marketValue == 0:
            supported_max_decline = 0
        else:
            supported_max_decline = ((usdt_wallet_balance-maintMargin) / newest_notional_marketValue) - 0.02
        txt =f"""
    ========================
    1.【维持保证金率---爆仓率】: 【{futures_index.get("margin_rate"):.2%}】
    2.【名义浮盈率】: {futures_index.get("notional_pnl_rate"):.1%}
    3.【开仓杠杆倍数】: {futures_index.get("open_leverage"):.2f}
    4.【实际浮盈率】: {futures_index.get("pnl_rate"):.1%}
    5.【最大允许涨幅---爆仓涨幅】: 【{supported_max_decline:.2%} 】
    -----------------------
    未实现盈亏 : {futures_index.get("pnl"):.2f}
    USDT钱包余额 : {usdt_wallet_balance:.2f}
    开仓名义市值总和 : {futures_index.get("sum_open_notional"):.2f}
    最新名义市值总和 : {newest_notional_marketValue:.2f}
    保证金余额 : {futures_index.get("marginBalance"):.2f}
    维持保证金 : {maintMargin:.2f}
    可用划转余额 : {futures_index.get("availableBalance"):.2f}
    ========================
        """
        logger.log(20, txt)
        return txt
    ...
    def _check_pair_pos(self, pair_name):
        ...
        # 核验余额
            # i. 该币种的现货余额
        symbol_account = self.oms.accounts.get(f"{exchange_name}.{asset_name}") # 这里是ETH不是LUNAUSDT, 所以把后面4个字符删掉
        if symbol_account:
        ...
        net_volume = spot_balance + futures_volume # 期现净持仓量
        ...
        d = {
            ...
            "net_volume" : net_volume, # # 期现净持仓量 (是否'期现双边配平', 就看这个值)
    ...
    def req_mytrade(self, pair_name="BINANCE.LUNAUSDT", type="spot", df_type="1", start_time="today", end_time="now", direction=None):
        ...
        time_df = df[["orderId", "time"]].drop_duplicates("orderId") # 只有orderId列和time列的df
    ...
    #             if s_order_qty != f_order_qty:
    #                 print(s_order_qty, f_order_qty)
    #                 f_df.drop(i, inplace=True)
    #             i += 1
    #         return (s_df[:len(s_df)], f_df[:len(s_df)])
    #     elif len(s_df) > len(f_df):
    #         while i < len(f_df):
    #             s_order_qty = s_df["order_qty"].iloc[i]
    #             f_order_qty = f_df["order_qty"].iloc[i]
    ...
    #             i += 1
    #         return (s_df[:len(f_df)], f_df[:len(f_df)])
    def _drop_useless_trades(self, s_df, f_df):
        """
            [方法二]
            functions: 将单腿下单的交易剔除 (这部分不能用于计算价差率...)
...
class TradeModule():
    ...
    def _check_balance(self, pair_name, volume, offset):
        ...
        if offset == "OPEN":
            # 1. 查询当前余额
                # i. 现货USDT可用余额
            usdt_available_balance_spot = self.oms.accounts[f"{exchange_name}.USDT"].balance
            ...
            try:
                # i. 该币种的现货余额
                symbol_spot_balance = self.oms.accounts[f"{exchange_name}.{asset_name}"].balance
            except:
            ...
            # 最大支持的可平的volume
            max_supported_volume = min(symbol_spot_balance, symbol_futures_volume)
            if volume > max_supported_volume:
                logger.log(30, f"现货或合约余额已经小于volume: {volume}, 将实际交易volume改为最大支持volume: {max_supported_volume}")
                volume = max_supported_volume
                # 修改volume后, 再核对下是否满足平台最小交易额 10u
                qty_spot = last_price_spot * volume
                qty_futures = last_price_futures * volume
                if (qty_spot <= 10) or (qty_futures <= 10):
                    msg = f"[不满足最低交易额] 待交易现货qty : {qty_spot}; 待交易合约qty : {qty_futures}"
            ...
            if (symbol_spot_balance < volume) or (volume == 0):
                ...
                raise Exception(msg)
            if (symbol_futures_volume < volume) or (volume == 0):
                msg = f"############## 合约-{asset_name}: {symbol_futures_volume}个币, 不足平仓 {volume}个 {asset_name}空头合约!!!  [无法交易]\n"
                raise Exception(msg)
            msg = f"============== 余额检测通过!!  [允许交易]\n"
            logger.log(30, msg)
            return True

    def _cal_volume(self, pair_name, multiplier):
        ...
        supported_deal_count = round(deal_count / min_volume) * min_volume
        if (deal_count != 0) and (supported_deal_count == 0):
            raise Exception("######## 最小精度问题, 导致计算出来的volume为0!!!")
        return supported_deal_count
    ...
    def get_pair_volume(self, pair_name="BINANCE.LUNAUSDT"):
        ...
        (pair_name, exchange_name, symbol, asset_name) = self.Data._split_pair_name(pair_name=pair_name)
        # 该币种的现货余额
        spot_balance = self.oms.accounts[f"{exchange_name}.{asset_name}"].balance
        # 该币种的合约持仓量
    ...
    def send_order(
            ...
            order_type="MARKET", price=10, volume=1,
        ):
        """
            ...
            usage:
                eg1:
                    # 单边下单
                    sc.send_order(pair_name="BINANCE.LUNAUSDT", type="futures", offset="OPEN", direction="BUY", volume=1)
    ...
    def fast_buy_sell(self, pair_name="BINANCE.LUNAUSDT", multiplier=1, time_sleep=10):
        """
            # '秒抢费率'
    ...
    def divide(self, spot_ratio=0.49):
        ...
        # 1. 查询当前余额
            # i. 现货USDT可用余额
        usdt_available_balance_spot = self.oms.accounts[f"BINANCE.USDT"].balance
            # ii. 合约USDT可用余额 (即: 钱包余额) (即: 最真实的保证金, 不考虑合约的浮盈浮亏!!)
        usdt_available_balance_futures = self.Data.futures_index.get("availableBalance")
            # iii. 双边的usdt总和
...
class SpreadClient():
    ...
    def __init__(self, main_engine):
        ...
        self.Data._init() # 展示'关键爆仓指标' and 多线程计算所有持仓币种的价差率
    ...
    def batch_create_open_robot(
        ...
        """
        # 1. 先检测 pair_lst 的价差率
        self.Data._ensure_pairs(pair_lst=pair_lst)
    ...
    def kill_all_robots(self, offset="BOTH", trading_count=None):
        """
        ...
        if offset == "OPEN" or offset == "BOTH":
            ...
            for pair_name, d in self.open_d.items():
                for target_spread_rate, robot in d.items():
                    ...
                    robot.kill(trading_count=trading_count)
        if offset == "CLOSE" or offset == "BOTH":
            for pair_name, d in self.close_d.items():
                for target_spread_rate, robot in d.items():
                    logger.log(20, f"关闭robot: {robot}")
...
class Robot():
    def __init__(
        ...
        pair_name="BINANCE.LUNAUSDT", offset="OPEN", multiplier=1,
        target_spread_rate=0.005, trading_count=1, volume=None, to_print=False
    ):
        """
            ...
            args:
                ...
                offset:
                    'OPEN': '开仓'套利币对
                    'CLOSE': '平仓'套利币对
                volume: 开仓数量 [特殊情况才会用到]:
                        因为每个币种价格差异过大, 每次都要计算数量很麻烦... (改用'乘数'来表示)
        ...
        self.Data._ensure_pairs(pair_lst=[pair_name])
    ...
    def run(self):
        ...
        def _recur_run():
            ...
            try:
                ...
                while True:
                    ...
                    if offset == "OPEN":
                        ...
                        if (spread_rate_open >= self.target_spread_rate) and (self.trading_count >= 1):
                            ...
                            self.Trade.two_sides_send_order(pair_name=pair_name, offset="OPEN", volume=volume) # 双边下单
                        ...
                        if (spread_rate_close <= self.target_spread_rate) and (self.trading_count >= 1):
                            if self.volume:
                                ...
                                volume = self.Trade._cal_volume(pair_name=pair_name, multiplier=self.multiplier)
                            self.Trade.two_sides_send_order(pair_name=pair_name, offset="CLOSE", volume=volume) # 双边下单
                ...
                self.to_print = False
                raise Exception(e)
        t = threading.Thread(target=_recur_run)
        t.start()

    def kill(self, trading_count=None):
        """
...
def init_main_engine(engines=[CtaEngine], gateway_setting=LSH_BINANCE_SETTING):
    ...
    #
    #
    # # for gateway_setting in gateway_settings_lst:
    #     gateway = gateway_setting.get("gateway")
    #     # 添加网关类后, 会返回这个实例化后的'网关对象' (此时才有 gateway_name这个属性)
    #     gateway_obj = main_engine.add_gateway(gateway)
    #     # 把这个settings参数传递给对应gateway_name的网关实例, 供其建立连接
    #     main_engine.connect(setting=gateway_setting, gateway_name=gateway_obj.gateway_name)
...
def init_spread_client(engines=[], gateway_setting=LSH_BINANCE_SETTING):
    """
    ...
    """
    # 1. 初始化main_engine
    main_engine = init_main_engine(engines=engines, gateway_setting=LSH_BINANCE_SETTING)

    # 2. 实例化sc对象
    sc = SpreadClient(main_engine=main_engine)
    sc.user = gateway_setting.get("user")
...
lsh = init_spread_client(gateway_setting=LSH_BINANCE_SETTING)
sc = lsh


# lzc = init_spread_client(gateway_setting=LSH_BINANCE_SETTING)
# sc = lzc
#
#
# wyh = init_spread_client(gateway_setting=LSH_BINANCE_SETTING)
# sc = wyh


# sc.bra_spot = bra_spot
# sc.bra_futures = bra_futures


main_engine = init_main_engine(
        engines=[CtaEngine], gateway_setting=LSH_BINANCE_SETTING
    )
