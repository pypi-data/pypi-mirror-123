...
class BinanceAccount(myRequest):
    ...
    def send_order(
            self, account_type="spot", binance_symbol="ALPHAUSDT", offset="OPEN", direction="BUY", order_type="LIMIT",
        ...
        """
            ...
            usage:
                # 现货买入luna 100个
                bra_spot.send_order(
                    binance_symbol="LUNAUSDT", offset="OPEN", direction="BUY", order_type="LIMIT",
                    price=16, quantity=100,
                )
                bra_spot.send_order(
                    binance_symbol="LUNAUSDT", offset="OPEN", direction="BUY", order_type="MARKET",
                    price=16, quantity=1,
                )
                bra_futures.send_order(
                    binance_symbol="LUNAUSDT", offset="OPEN", direction="BUY", order_type="MARKET",
                    price=16, quantity=1,
                )
        """
        ...
        # ii. 撤销下单
    def cancel_order(self, account_type="spot", binance_symbol="", orderId="", ClientOrderId=""):
        """
            function: 撤销下单
            args:
    ...
    # 3. 历史成交订单
    def req_myTrades(self, account_type="spot", binance_symbol="", start_time=None, end_time=None, pair="", limit=1000, is_obj=False): # 使用币安官网的命名
        """
            functions:
            ...
            return:
                - 多笔交易的列表: [{}, {}]
            usage:
                s_d = ba.req_myTrades(account_type="spot", binance_symbol="BTCUSDT", start_time="2021-05-24")
                s_d = ba.req_myTrades(account_type="isoMargin", binance_symbol="MTLUSDT", start_time="2021-06-08 00:00:00", is_obj=True)

                s_d = ba.req_myTrades(account_type="isoMargin", binance_symbol="MANAUSDT", start_time="2021-06-08 00:00:00")
                f_d = ba.req_myTrades(account_type="futures", binance_symbol="MANAUSDT", start_time="2021-06-08 00:00:00")

                f_d = ba.req_myTrades(account_type="futures", binance_symbol="ADAUSDT", start_time="2021-06-08 00:00:00")
        """
        # 1. 制造/打包请求数据
        req_method = "get"
        verify = True
    ...
    def req_myTrades_df(self, account_type="spot", binance_symbol="", start_time=None, end_time=None, pair="", limit=1000, is_obj=False):
        """
        usage:
            s_df = ba.req_myTrades_df(account_type="spot", binance_symbol="BTCUSDT", start_time="2021-05-24")
            s_df = ba.req_myTrades_df(account_type="isoMargin", binance_symbol="MTLUSDT", start_time="2021-06-08 00:00:00", is_obj=True)
            s_df = ba.req_myTrades_df(account_type="isoMargin", binance_symbol="MTLUSDT", start_time="2021-06-08 00:00:00")
            f_df = ba.req_myTrades_df(account_type="futures", binance_symbol="MTLUSDT", start_time="2021-06-08 00:00:00")

            f_df = ba.req_myTrades_df(account_type="futures", binance_symbol="ethUSDT", start_time="2021-06-09 20:00:00")

            # 计算价差率
            binance_symbol = "MANAUSDT"
            s_df = ba.req_myTrades_df(account_type="isoMargin", binance_symbol=binance_symbol, start_time="2021-06-08 00:00:00")
            s_df["quoteQty"] = s_df["price"] * s_df["qty"]  # 全仓/逐仓杠杆账户的交易记录不会返回'quoteQty'字段, 需要自行计算...
            f_df = ba.req_myTrades_df(account_type="futures", binance_symbol=binance_symbol, start_time="2021-06-08 00:00:00")
        ...
        """
        d = self.req_myTrades(account_type=account_type, binance_symbol=binance_symbol, start_time=start_time, end_time=end_time, pair=pair, limit=limit, is_obj=is_obj)
        df = pd.DataFrame(d)
    ...
    def subscribe_market_data(self, account_type="spot", channels=["ethusdt@bookTicker"]):
        """
            ...
            usage:
                bra_dfutures.subscribe_market_data()
        """
        if account_type == "spot":
    ...
    # 4. 获取交易规则和交易对信息
    def req_contract(self, account_type="spot"):
        """
            functions:
                - 查询'获取交易规则和交易对信息'
            args:
