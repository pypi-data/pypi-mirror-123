...
from azureml._vendor.azure_cli_core.util import get_token_for_adal_auth
...
class AbstractAuthentication(object):
    ...
    def _get_adal_auth_object(self, is_graph_auth=False):
        ...
        token_expiry = get_token_for_adal_auth(token)
...
class InteractiveLoginAuthentication(AbstractAuthentication):
    ...
    def _get_all_subscription_ids_internal(self, arm_token):
        if isinstance(self._ambient_auth, AbstractAuthentication):
            ...
            profile_object = Profile(async_persist=False, cloud_type=self._cloud_type)
    ...
    def _get_arm_token_using_interactive_auth(self, force_reload=False, resource=None):
        ...
        profile_object = Profile(async_persist=False, cloud_type=cloud_type)
...
class AzureCliAuthentication(AbstractAuthentication):
    ...
    def _get_refresh_token(self, resource=None):
        ...
        try:
            profile = Profile(async_persist=False)
            return profile.get_subscription()['user']['type'], profile.get_refresh_token(resource)
        except Exception as e:
...
class ArmTokenAuthentication(AbstractAuthentication):
    ...
    def _get_all_subscription_ids(self):
        ...
        arm_token = self._get_arm_token()
        from azureml._vendor.azure_cli_core.adal_authentication import AdalAuthentication
        token_expiry = get_token_for_adal_auth(arm_token)
...
class ServicePrincipalAuthentication(AbstractAuthentication):
    ...
    def _get_graph_token(self):
        ...
        """
        resource = self._cloud_type.endpoints.active_directory_graph_resource_id
        header = execute_func(self._get_sp_credential_object(resource).signed_session).headers['Authorization']
        return header.split(" ")[1]
    ...
    def _initialize_sp_auth(tenant_id, cloud=None):
        try:
            ...
            return ServicePrincipalAuthentication(tenant_id, service_principal_id, service_principal_password, cloud=cloud)
        except Exception as e:
            module_logger.debug(e)
...
class MsiAuthentication(AbstractAuthentication):
    ...
    def _get_resource_token(self, resource):
        ...
        msi_auth = MSIAuthentication(resource=resource, cloud_environment=self._cloud_type, **self._identity_config)
        msi_auth.set_token()
    ...
    def _get_arm_token(self):
        ...
        # retrieves the AAD token through MSI
        from msrestazure.azure_active_directory import MSIAuthentication
        msi_auth = MSIAuthentication(cloud_environment=self._cloud_type, **self._identity_config)
        msi_auth.set_token()
        return msi_auth.token['access_token']
    ...
    def _get_graph_token(self):
        ...
        # retrieves the AAD token through MSI
        from msrestazure.azure_active_directory import MSIAuthentication
        resource = self._cloud_type.endpoints.microsoft_graph_resource_id
        msi_auth = MSIAuthentication(resource=resource, cloud_environment=self._cloud_type, **self._identity_config)
        msi_auth.set_token()
        return msi_auth.token['access_token']
