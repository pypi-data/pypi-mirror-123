...
class RandomGridMask(VisionTransform):
    ...
    def __init__(self, output_size=None,d1=None, d2=None, rotate_range=(0.2,1), ratio = 0.2, mode=0, keep_prob=0.5, name='gridmask', **kwargs):
        super().__init__(name)
        self.output_size=output_size
        self.d1 = d1
        self.d2=d2
    ...
    def get_grid(self,image):
        ...
        if self.d2 is None:
            self.d2=maximum(h,w)
        if self.d1 is None:
            divisors=get_divisors(maximum(h,w))

            divisors=list(sorted(set(divisors)))
            divisors=[d for d in divisors if d <=maximum(h,w)/2.0]
            if maximum(h,w) in divisors:
                divisors.remove(maximum(h,w))
            self.d1 =random.choice(divisors)
...
class ImageMosaic(VisionTransform):
    ...
    def _apply_boxes(self, boxes, spec: TensorSpec):
        ...
        box0 = np.concatenate([location0.reshape((box0.shape[0], 4)), class_info0, keypoints0.reshape((box0.shape[0], -1))], axis=-1)
        ...
        box1 = np.concatenate([location1.reshape((box1.shape[0], 4)), class_info1, keypoints1.reshape((box1.shape[0], -1))], axis=-1)

        box2 = self.memory_cache[other_idxes[1]][spec].copy()
        ...
        box2 = np.concatenate([location2.reshape((box2.shape[0], 4)), class_info2, keypoints2.reshape((box2.shape[0], -1))], axis=-1)

        box3 = self.memory_cache[other_idxes[2]][spec].copy()
        ...
        box3 = np.concatenate([location3.reshape((box3.shape[0], 4)), class_info3, keypoints3.reshape((box3.shape[0], -1))], axis=-1)

        trans_boxes = np.concatenate([box0, box1, box2, box3], axis=0)
