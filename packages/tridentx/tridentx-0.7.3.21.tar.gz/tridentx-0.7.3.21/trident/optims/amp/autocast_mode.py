...
class autocast(object):
    ...
    def __init__(self, enabled=True):
        if enabled and not is_gpu_available():
            warnings.warn("amp.autocast only affects CUDA ops, but CUDA is not available.  Disabling.")
            self._enabled = False
        else:
            self._enabled = enabled

    def __enter__(self):
        self.prev = is_autocast_enabled()
        set_autocast_enabled(self._enabled)
    ...
    def __exit__(self, *args):
        ...
        set_autocast_enabled(self.prev)
        return False
...
def _cast(value, dtype):
    if is_tensor(value):
        ...
        if ctx.get_backend()=='pytorch':
            ...
            if value.dtype in [dtype.float32]:
                is_eligible=True
            return cast(value,dtype) if is_eligible else value
...
def custom_fwd(fwd=None, **kwargs):
    ...
    def decorate_fwd(*args, **kwargs):
        if cast_inputs is None:
            args[0]._fwd_used_autocast = is_autocast_enabled()
            return fwd(*args, **kwargs)
        else:
            autocast_context = is_autocast_enabled()
