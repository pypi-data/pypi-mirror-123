...
class RFBnet(Layer):
    ...
    def compute_header(self, i, x):
        ...
        if 'softmax' not in self.__dict__:
...
def RfbNet(include_top=True,
    ...
    if num_classes != 2 or  num_regressors != 4:
        pretrained=False
    rfbnet = SsdDetectionModel(input_shape=input_shape, output=RFBnet(base_filters=base_filters, num_classes=num_classes, num_regressors=num_regressors))
