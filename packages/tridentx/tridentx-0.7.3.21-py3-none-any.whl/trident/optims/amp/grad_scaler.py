...
ring_classes = (str, bytes)
from trident.backend.common import dtype as Dtype
from trident.backend.common import get_backend,is_autocast_enabled,set_autocast_enabled,to_list, addindent, get_time_suffix, format_time, get_terminal_size, get_session,get_backend, \
...
class GradScaler(object):
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def _lazy_init_scale_growth_tracker(self, dev):
        ...
        self._scale = torch.full((1,), self._init_scale, dtype=Dtype.float32, device=dev)
        self._growth_tracker = torch.full((1,), self._init_growth_tracker, dtype=Dtype.int32, device=dev)
        ...
        for group in optimizer.param_groups:
            for param in group["params"]:
                if param.grad is None:
                    continue
                if (not allow_fp16) and param.grad.dtype == Dtype.float16:
                ...
                with torch.no_grad():
                    if param.grad.is_sparse:
                        ...
                        if param.grad.dtype is Dtype.float16:
    ...
    def unscale_(self, optimizer):
        ...
        found_inf = torch.full((1,), 0.0, dtype=Dtype.float32, device=self._scale.device)
    ...
    def update(self, new_scale=None):
        ...
        if new_scale is not None:
            ...
            if isinstance(new_scale, float):
                self._scale = ones((1,),  dtype=Dtype.float32).to(_scale.device)*new_scale
            else:
                reason = "new_scale should be a float or a 1-element torch.cuda.FloatTensor with requires_grad=False."
                assert is_tensor(new_scale) and ('cuda' in new_scale.device or 'gpu' in new_scale.device) , reason  # type: ignore[attr-defined]
    ...
    def set_growth_factor(self, new_factor):
        r"""
        Arguments:
            new_factor (float):  Value to use as the new scale growth factor.
        """
    ...
    def set_backoff_factor(self, new_factor):
        r"""
        Arguments:
            new_factor (float):  Value to use as the new scale backoff factor.
        """
    ...
    def _check_inf_per_device(self, optimizer):
        ...
        dummy_inv_scale = ones((1,), dtype=Dtype.float32).to(_scale.device)
        found_inf = zeros((1,), dtype=Dtype.float32).to(_scale.device)
