...
observer.observe(targetNode, config);
    ...
    def make_gif(self, hue='#ff0000', width=350, height=350, border=5):
        '''
        ...
        if isinstance(hue, str):
            ...
            hue = hue / (16 ** lh - 1.0)
            saturation = np.max(np.abs(hue * 2 - 1))

        y = self
        d = y.oscar(maxexp=16, verbose=False)
        ...
        params = y.centre_and_pixel_size(p, disp, width - 2 * border, height - 2 * border)
        frames_per_gen = max(2, min(240 // p, 8))
        frames = frames_per_gen * p

        anim = y.animate(frames, p, disp, params, width, height)
