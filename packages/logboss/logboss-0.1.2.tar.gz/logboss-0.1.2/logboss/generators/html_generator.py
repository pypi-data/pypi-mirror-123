...
class HtmlLogGenerator:
    ...
    def generate(self, log_file: Union[str, Path], title: str = 'Log Results', include_code: bool = False,
        ...
        log_entries = []
        if len(temp_log_entries.keys()) > 1:
            ...
            log_entries = temp_log_entries[self._main_thread_id]
        ...
        print(f'Writing the HTML file here: {log_file.parent.absolute()}/{log_file.stem}.html')
        with open(f'{resources.absolute()}/{log_file.stem}.html', 'w') as f:
            f.write(html)
