    ...
    def __new__(cls, *init_texts, main_folder=None):
        ...
        if len(init_texts) <= 0 or len(init_texts[0]) <= 0:
            if main_folder:
                ...
                self._is_rel = False
                return self
        elif len(init_texts) == 1 and isinstance(init_texts[0], str) and str(init_texts[0]) == "~":
            string = path.homedir
        elif len(init_texts) == 1 and isinstance(init_texts[0], str) and str(init_texts[0]) == os.curdir:
            string = os.path.abspath(os.curdir)
        elif len(init_texts) == 1 and isinstance(init_texts[0], str):
            string = str(init_texts[0])
        else:
