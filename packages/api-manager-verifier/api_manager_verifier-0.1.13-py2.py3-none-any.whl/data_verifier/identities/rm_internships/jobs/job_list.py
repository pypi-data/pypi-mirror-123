...
class RMJobs(RMLogin):
    ...
    def filter_by_open(self):
        for i in self.jobs:
            ...
            close_date,now=self.get_timzone_convesion(datetime.datetime.fromisoformat(i.get('applicationClosed')[:-1]),True)
        ...
        for i in self.job_filter_list:
            ...
            if index==0:
                ...
                if self.safe_list(self.branches, course, None) is not None:
                    if not branch in i.get(course_name.lower()+'Branches'):
                        self.job_filter_list.pop(self.job_filter_list.index(i))

            elif index==2:
                aggr_cgpa = self.profile.get('aggregateCgpa')
                cutoff=i.get(course_name.lower()+'Cutoff')
                ...
                if tenth<i.get('tenthPercentageCutoff') if i.get('tenthPercentageCutoff') is not None else 0:
                    self.job_filter_list.pop(self.job_filter_list.index(i))
                elif twelth<i.get('twelfthPercentageCutoff') if i.get('twelfthPercentageCutoff') is not None else 0:
                    self.job_filter_list.pop(self.job_filter_list.index(i))

            elif index==4:
                if i.get('genderOpen') !="Both":
                    if i.get('genderOpen','').lower()!= self.profile.get('gender','').lower():
                        self.job_filter_list.pop(self.job_filter_list.index(i))

            elif index==5:
                if i.get('pwdOnly') and not self.profile.get('pwd'):
                    self.job_filter_list.pop(self.job_filter_list.index(i))
                return
    ...
    def query_info(self):
        ...
        for i in range(100):
            response=self.smart_request('GET',self.job_url.format(i+1),headers=self.headers)
            json = self.safe_json(response)
            if len(json.get('data'))==0:
                break
    ...
    def extract_info(self, **kwargs):
        ...
        if self.all_open_jobs:
            ...
            return {'course':self.courses[course],'branch':self.safe_list(self.safe_list(self.branches,course,[]),branch),'job_list':job}
...
def test():
    ...
    return RMJobs.extract_data(applicant, persistor=persistor,all_jobs=True)
