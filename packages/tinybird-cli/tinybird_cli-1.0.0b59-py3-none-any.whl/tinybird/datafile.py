    ...
    try:
        while lineno < len(lines):
            ...
            try:
                ...
                lexer = list(shlex.shlex(shlex.quote(line)))
            if lexer:
...
async def new_pipe(p, tb_client: TinyB, replace=False, check=True, populate=False, wait_populate=False, skip_tokens=False, tag='', skip_table_checks=False, ignore_sql_errors=False, only_response_times=False):  # noqa: C901
    exists = False
    materialized = False
            ...
            if populate:
                ...
                if wait_populate:
                    with click.progressbar(label="Populating ", length=100, show_eta=False, show_percent=True, fill_char=click.style("█", fg="green")) as progress_bar:
                        ...
                        try:
                            result = await tb_client.wait_for_job(job_id, status_callback=progressbar_cb)
...
async def exec_file(r, tb_client, override, check, debug, populate, wait_populate, tag, skip_table_checks=False, ignore_sql_errors=False, skip_confirmation=False, only_response_times=False):
...
async def folder_push(tb_client: TinyB, tag='', filenames=None, dry_run=False, check=False, push_deps=False,  # noqa: C901
                      ...
                      workspace_map=None, workspace_lib_paths=None, no_versions=False):
                    ...
                    if name not in remote_resource_names or resource_versions.get(name.replace(".", "_"), '') != latest_datasource_versions.get(name, '') or force:
                        ...
                        try:
                            await exec_file(to_run[name], tb_client, force, check, debug, populate, wait, tag,
                                            skip_table_checks, ignore_sql_errors, skip_confirmation, only_response_times)
                        except Exception as e:
