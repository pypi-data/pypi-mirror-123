    ...
    def __init__(self, name: str, service: "NWService", members: Dict[str, Any]):
        ...
        for command in self.commands:
            for parameters in command.values():
                paths = list(
                    filter(lambda x: x.endswith("_path") or x.endswith("_dir"), parameters))

                # prepend service path for all paths
                for param_name in paths:
                    # default
                    if len(str(parameters[param_name])) == 0:
                        parameters[param_name] = self.path
                    else:
                        parameters[param_name] = os.path.normpath(os.path.join(self.service.path,
