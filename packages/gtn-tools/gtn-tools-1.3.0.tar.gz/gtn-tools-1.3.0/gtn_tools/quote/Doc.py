...
class Doc:
    ...
    def document_workflow(self, source_lang: str, target_langs: str,
        ...
        if uneditable:
            ...
            self.print_boxes(out_dir=prc_dir,
...
class Uneditable(Doc):
    ...
    def do_grayscale(self, images: list) -> list:
        """
        Grayscale Image
    ...
    def do_remove_noise(self, images: list) -> list:
        """
        Remove noise
        ...
        output = []
        for img in images:
            output.append(cv2.medianBlur(img, 5))
        return output

    def do_threshold(self, images: list) -> list:
        """
        Threshold Image
        ...
        output = []
        for img in images:
            output.append(cv2.threshold(img, 0, 255,
                          cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1])
        return output

    def do_dilate(self, images: list) -> list:
        """
        Dilate Image
        ...
        for img in images:
            ...
            output.append(cv2.dilate(img, kernel, iterations=1))
        return output

    def do_erode(self, images: list) -> list:
        """
        Erode Image
        ...
        for img in images:
            kernel = np.ones((5, 5), np.uint8)
            output.append(cv2.erode(img, kernel, iterations=1))
        return output

    def do_opening(self, images: list) -> list:
        """
        Opening Image
        ...
        for img in images:
            kernel = np.ones((5, 5), np.uint8)
            output.append(cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel))
        return output

    def do_rotate(self, images: list) -> list:
        """
        Rotate Image
        ...
        for image in images:
            ...
            angle = int(re.search(r'(?<=Rotate: )\d+', newdata).group(0))
    ...
    def convert(self, save: bool = False, dire: str = None,
                dpi: int = 500, fmt: str = 'jpeg') -> None:
        """
        ...
        """
        if self.type == 'PDF':
            self.img = convert_from_path(self.path, dpi=dpi, fmt=fmt)
        for img in self.img:
            self.cv2_img.append(cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR))
        self.pages = len(self.img)
        self.preprocessed = self.cv2_img
        if save:
            i = 1
            for page in self.img:
                if dire is None:
                    filename = self.path.replace('.pdf', '') \
                               + '(' + str(i) + ').jpg'
                else:
                    if dire[len(dire)-1] == '/':
                        dire = dire[:len(dire)-1]
                    filename = dire + self.path[
                        self.path.rfind('/'):self.path.rfind('.')
                        ] \
                        + '(' + str(i) + ').jpg'
                self.page_paths.append(filename)
                page.save(filename)
                i += 1
        return None
    ...
    def preprocess(self, method: str = None) -> None:
        ...
        if method is not None:
            ...
            # Step 1: rotate
            self.rotated = self.do_rotate(self.cv2_img)
            self.preprocessed = self.rotated
            output = []
            ...
            self.preprocessed = output
            return None

    def print_boxes(self, out_dir: str, config: str = None) -> None:
        """
        ...
        for page in range(self.pages):
            img = self.preprocessed[page]
            raw_img = self.rotated[page]
    ...
    def recognize(self, output_file: str = None, config: str = None):
        ...
        if self.preprocessed is None:
            raise DocumentException('The document must be converted first.\
            ...
            output = ''
            for page in self.preprocessed:
                text = str(pytesseract.image_to_string(page, config=config))
                text = text.replace('-\n', '')
                text = text.replace('\n', ' ')
                output += str(' ' + text)
                if output_file is not None:
