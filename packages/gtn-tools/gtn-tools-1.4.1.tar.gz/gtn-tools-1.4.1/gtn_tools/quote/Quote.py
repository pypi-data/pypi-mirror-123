...
class QuoteClass:
    ...
    def create_draft_order(
        ...
        # FOR DEBUGGING:
        # shopify_url = 'privatkunden-lingua-world.myshopify.com'
        # api_version = '2021-04'
        # shopify_password = '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'
        ...
        with shopify.Session.temp(self.shopify_shop_url,
            ...
            do.name = (f"AN-{id}: Übersetzung von {num_docs} Dokument(en) "
    ...
    def init_quote(
        ...
        self.create_draft_order(self.order_id, email, source_lang,
                                target_langs, num_docs, postal, certs)
        return price
    ...
    def update_draft_order(
        ...
        with shopify.Session.temp(self.shopify_shop_url,
            ...
            if num_docs:
                do_dict = self.__update_num_docs(num_docs, do_dict)
            if languages:
    ...
    def __update_num_docs(
        self,
        num_docs: int,
        do_dict: dict
    ) -> dict:
        title = do_dict['translation']['title']
        old_num_docs = re.findall(r'\d', title)
        title = title.replace(old_num_docs[-1], str(num_docs))
    ...
    def __update_languages(
        ...
        old_languages = re.findall(r'[A-Z]{2}\s-\s[A-Z]{2}', title)
