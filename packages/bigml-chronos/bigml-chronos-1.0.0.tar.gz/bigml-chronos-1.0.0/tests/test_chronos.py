...
valid_dates = [["basic-date-time","19690714T173639.592Z"],
               ...
               ["elasticsearch-nanos", "1969-07-14T17:36:39.592000+0000"]]
...
def test_valid_dates():
    for item in valid_dates:
        fmt_name, date_str = item
        # Check parsing with the format name
        output = chronos.parse(date_str, format_name=fmt_name)
        check_date(output, fmt_name)
        # Check parsing with the Joda directives
        directives = [k for k, v in formats.joda_strings.items()
                      if v == fmt_name]
        if len(directives) > 0:
            output = chronos.parse(date_str, format_name=directives[0])
            check_date(output, fmt_name)
