...
def download_audio(url,ext='mp3'):
    ...
    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
        ydl.download([url])
    return rs
def create_lyric_outro_songs(deezer_artist_id, lyric_outro_repeat, retries=3):
    res = requests.get(f"http://api-magicframe.automusic.win/lyric/deezer/artist/preview-song/{deezer_artist_id}/{lyric_outro_repeat}").json()
    arr_song=[]
    ...
    for item in res:
        ...
        if not "coolbg_ffmpeg" in tmp:
            tmp_clip_path = os.path.join(get_dir('coolbg_ffmpeg'),
                                         str(uuid.uuid4()) + '-' + os.path.basename(tmp))
            shutil.copyfile(tmp, tmp_clip_path)
            os.remove(tmp)
            tmp = tmp_clip_path
        arr_song.append(tmp)
    ...
    if retries>0 and len(arr_song)!=lyric_outro_repeat:
        ...
        return create_lyric_outro_songs(deezer_artist_id, lyric_outro_repeat, retries=retries)
