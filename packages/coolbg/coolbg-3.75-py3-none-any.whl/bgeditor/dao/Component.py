...
def create_video(list_comp_data, path_video, job_id, mf_server):
    ...
    final_vid.close()
    if is_lyric_outro:
        lyricOutroPath = create_lyric_outro(deezer_artist_id, lyric_outro_repeat, arr_composite_no_intro)
...
class VideoComp(Component):
    def __init__(self, json_data):
        ...
        if self.is_canva:
            self.kind = "canva"
    def get_clip(self):
        if self.isLoop and self.isSmoothLoop and self.ext != "gif" and not self.is_intro and not self.is_outro:
