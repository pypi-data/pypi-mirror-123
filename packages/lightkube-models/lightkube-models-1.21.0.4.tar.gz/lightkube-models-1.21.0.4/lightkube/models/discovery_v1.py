...
class Endpoint(DataclassDictMixIn):
    """Endpoint represents a single logical "backend" implementing a service.
      ...
      * **deprecatedTopology** ``dict`` - *(optional)* deprecatedTopology contains topology information part of the v1beta1 API. This
...
@dataclass
class EndpointHints(DataclassDictMixIn):
    """EndpointHints provides hints describing how an endpoint should be consumed.

      **parameters**

      * **forZones** ``List[ForZone]`` - *(optional)* forZones indicates the zone(s) this endpoint should be consumed by to enable
        topology aware routing.
    """
    forZones: 'List[ForZone]' = None
...
@dataclass
class ForZone(DataclassDictMixIn):
