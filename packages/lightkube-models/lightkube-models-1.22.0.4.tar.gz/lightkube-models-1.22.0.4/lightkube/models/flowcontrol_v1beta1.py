...
class Subject(DataclassDictMixIn):
    """Subject matches the originator of a request, as identified by the request
      ...
      * **group** ``GroupSubject`` - *(optional)* `group` matches based on user group name.
      * **serviceAccount** ``ServiceAccountSubject`` - *(optional)* `serviceAccount` matches ServiceAccounts.
      * **user** ``UserSubject`` - *(optional)* `user` matches based on username.
    """
    kind: 'str'
