...
class ContainerImage(DataclassDictMixIn):
    """Describe a container image
      ...
      * **names** ``List[str]`` - *(optional)* Names by which this image is known. e.g. ["k8s.gcr.io/hyperkube:v1.0.7",
...
class NodeSpec(DataclassDictMixIn):
    """NodeSpec describes the attributes that a node is created with.
      ...
      * **configSource** ``NodeConfigSource`` - *(optional)* Deprecated. If specified, the source of the node's configuration. The
...
class PersistentVolumeClaimSpec(DataclassDictMixIn):
    """PersistentVolumeClaimSpec describes the common attributes of storage devices
      ...
      * **dataSource** ``TypedLocalObjectReference`` - *(optional)* This field can be used to specify either: * An existing VolumeSnapshot object
        ...
        field.
      * **dataSourceRef** ``TypedLocalObjectReference`` - *(optional)* Specifies the object from which to populate the volume with data, if a
