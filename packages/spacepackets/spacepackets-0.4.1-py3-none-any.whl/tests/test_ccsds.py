...
from spacepackets.ccsds.spacepacket import get_space_packet_sequence_control, SpacePacketHeader, \
    PacketTypes, get_space_packet_id_bytes, get_space_packet_id_num, SequenceFlags, \
    get_apid_from_raw_space_packet, get_space_packet_header, parse_space_packets
...
class TestCcsds(TestCase):
    def test_spacepacket(self):
        ...
        self.assertRaises(ValueError, get_apid_from_raw_space_packet, raw_packet=bytearray())
        sp_unpacked = SpacePacketHeader.unpack(space_packet_raw=sp_packed)
        self.assertRaises(
            ValueError, SpacePacketHeader,
            apid=982292, data_length=22, source_sequence_count=52, packet_type=PacketTypes.TC
        )
        self.assertRaises(
            ValueError, SpacePacketHeader,
            apid=0x02, data_length=679393, source_sequence_count=52, packet_type=PacketTypes.TC
        )
        self.assertRaises(
            ValueError, SpacePacketHeader,
            apid=0x02, data_length=22, source_sequence_count=96030, packet_type=PacketTypes.TC
        )
        ...
        self.assertRaises(
            ValueError, get_space_packet_sequence_control, source_sequence_count=0xffff,
            sequence_flags=SequenceFlags.UNSEGMENTED
        )
        self.assertRaises(
            ValueError, get_space_packet_sequence_control, source_sequence_count=0x3fff,
            sequence_flags=5
        )
