    ...
    def __init__(self, vm=None, context_id=None):
        ...
        if snapshot is not None:
            if 'created' in snapshot.keys():
                ...
                self._log.info(f"Snapshot id='{snapshot_id}', name='{snapshot_name}' built successfully")
        else:
            message = f"Snapshot not built. Snapshot not found for VirtualMachine id={vm.get_id()}"
            self._log.error(message)
