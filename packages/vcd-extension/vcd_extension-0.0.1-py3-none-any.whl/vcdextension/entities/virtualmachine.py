...
class VirtualMachine(Entity):
    ...
    def __init__(self, vapp=None, vm_name=None, vm_id=None, create=False, context_id=None):
        ...
        self._log.info(f"VirtualMachine object id '{vm_id}', name '{vm_name}' built successfully")
        ...
        if 'num_cpus' in new_resources:
            ...
            try:
                ...
                task = self._client.get_task_monitor().wait_for_status(
                    ...
                    poll_frequency=2,
                    callback=None)
                self._log.info(f"VirtualMachine id='{self._id}' update CPU status {task.get('status')}")
            except Exception as e:
                self._log.error(f"<<{type(e).__qualname__}>>: "
        ...
        if 'memory' in new_resources:
            try:
                ...
                task = self._client.get_task_monitor().wait_for_status(
                    ...
                    poll_frequency=2,
                    callback=None)
                self._log.info(f"VirtualMachine id='{self._id}' modify memory status {task.get('status')}")
            except Exception as e:
                self._log.error(f"<<{type(e).__qualname__}>>: "
    ...
    def get_current_metrics(self, context_id=None):
        ...
        try:
            ...
            self._log.error(f"<<{type(e).__qualname__}>>: Current metrics not "
                            f"available for VirtualMachine '{self._id}' {e}")
            metrics = []
        return metrics
    ...
    def get_historic_metrics(self, context_id=None):
        ...
        try:
            ...
            message = f"<<{type(e).__qualname__}>>: Historic metrics not "\
                      f"available for VirtualMachine '{self._id}' {e}"
            self._log.error(message)
            metrics = []
        return metrics
    ...
    def power_off(self, context_id=None):
        ...
        try:
            ...
            task = self._client.get_task_monitor().wait_for_status(
                ...
                poll_frequency=2,
                callback=None)
            self._log.info(f"VirtualMachine '{self._id}' power off result {task.get('status')}")
            self._pyvcloud_object.reload()
            self._log.info(f"Current VirtualMachine '{self._id}' state "
                           f"{VCLOUD_STATUS_MAP.get(int(self._pyvcloud_object.get_power_state()))}")
        except Exception as e:
            message = f"<<{type(e).__qualname__}>> VirtualMachine id '{self._id}' power off error {e}"
            self._log.error(message)
    ...
    def power_on(self, context_id=None):
        ...
        try:
            ...
            task = self._client.get_task_monitor().wait_for_status(
                ...
                poll_frequency=2,
                callback=None)
            self._log.info(f"VirtualMachine id '{self._id}' power on result {task.get('status')}")
            self._pyvcloud_object.reload()
            self._log.info(f"Current VirtualMachine '{self._id}' state "
                           f"{VCLOUD_STATUS_MAP.get(int(self._pyvcloud_object.get_power_state()))}")
        except Exception as e:
