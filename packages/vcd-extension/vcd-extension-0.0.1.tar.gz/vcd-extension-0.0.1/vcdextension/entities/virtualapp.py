...
class VirtualApp(Entity):
    ...
    def __init__(self, vdc=None, vapp_name=None, vapp_id=None, create=False, context_id=None):
        ...
        self._log.info(f"VirtualApp object id '{vapp_id}', name '{vapp_name}' built successfully")
    ...
    def power_off(self, context_id=None):
        ...
        try:
            ...
            task = self._client.get_task_monitor().wait_for_status(
                ...
                poll_frequency=2,
                callback=None)
            self._log.info(f"vApp '{self._id}' power off result {task.get('status')}")
            ...
            self._log.error(f"vApp id '{self._id}' power off error <<{type(e).__qualname__}>> {e}")
    ...
    def power_on(self, context_id=None):
        ...
        try:
            ...
            self._log.error(f"vApp id '{self._id}' power on error <<{type(e).__qualname__}>> {e}")
