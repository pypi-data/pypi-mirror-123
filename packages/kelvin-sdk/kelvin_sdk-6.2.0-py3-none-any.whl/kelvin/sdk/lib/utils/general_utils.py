...
from kelvin.sdk.lib.exceptions import DependencyNotInstalled, InvalidBaseImageException
...
def check_if_dependency_is_installed(dependency: str) -> bool:
    ...
    ----------
    dependency: str
        the dependency to be verified.

    Returns
    -------
    bool:
        a boolean indicating whether the specified dependency is installed on the operative system.
...
def check_if_image_is_allowed_on_platform(registry_url: Optional[str], docker_image_name: Optional[str]) -> None:
    ...
    registry_url: str, optional
        The registry the provided image should belong to.
    docker_image_name: str, optional
        The docker image to validate against the url.

    Returns
    -------
    bool:
        A boolean indicating whether or no the provided docker image is allowed on the platform.
    """
...
def get_system_requirements_from_file(file_path: Optional[Path]) -> List[str]:
    ...
    ----------
    file_path: Path
        the path to the system's requirements file.

    Returns
    -------
    List[str]:
        A list containing the required system dependencies.
    """
    comment_key = "#"
...
def check_if_requirement_is_present(file_path: Path, requirement: str) -> bool:
    """From a specific file path, verify whether the provided requirement is available.
    ...
    requirement : str
        the requirement to validate against the provided requirements file

    Returns
    -------
    bool:
        a boolean indicating whether the provided requirement is present in the file.

    """
    return requirement in get_requirements_from_file(file_path=file_path)
