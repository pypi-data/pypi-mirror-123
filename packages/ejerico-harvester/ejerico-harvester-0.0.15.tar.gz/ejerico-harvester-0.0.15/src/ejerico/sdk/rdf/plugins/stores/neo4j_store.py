...
class NEO4JStore(Store):
    ...
    def __init__(self, url=None, username=None,password=None,domain=None):
        ...
        self.driver = GraphDatabase.driver(self._url, auth=(self._username, self._password))
        self.session = None
    ...
    def open(self, configuration, create=False):
        ...
        if self.session is None: self.session = self.driver.session()
        return VALID_STORE 
    ...
    def destroy(self, configuration):
        ...
        if self.session is not None: self.close()
    ...
    def addN(self, quads):
        ...
        nodes,relations = self._do_process_tiplets([(q[0],q[1],q[2]) for q in quads])

        for n in nodes:
            result = self.session.write_transaction(self._do_neo4j_createNode, nodes[n]["__label__"], n)
            for row in result: logging.debug("[NEO4JStore:addN] created node: {}".format(row))

        for n in nodes:
            result = self.session.write_transaction(self._do_neo4j_updateNode, nodes[n])
            for row in result: logging.debug("[NEO4JStore:addN] update node: {}".format(row))

        for relation in relations:
            result = self.session.write_transaction(self._do_neo4j_createRelation, relation)
            for row in result: logging.debug("Merged relation: ({})-[{}]->({})".format(row['n'], row['r'], row['m']))
    ...
    def query(self, query, initNs, initBindings, queryGraph, **kwargs):
        ...
        rst = None
        query = query[query.index("SELECT"):] if "SELECT" in query else query[query.index("DESCRIBE"):]
        ...
        if m:
            ...
            query = self.query._re_alias_query.format(label=my_type, url=my_url)
            rst = self.session.read_transaction(self._do_neo4j_generic_query, query)
            print(rst)
        
        m = re.match(self.query._re_totals_by_rdftype_pattern, query) if m is None else None
        if m:
            my_type = self._do_resolve_entity_class(m.group("type"))
            query = self.query._re_totals_by_rdftype_query.format(label=my_type)
            rst = self.session.read_transaction(self._do_neo4j_generic_query, query)  
            rst = [r['c'] for r in rst]

        m = re.match(self.query._re_uri_by_rdftype_pattern, query) if m is None else None
        if m:
            my_type = self._do_resolve_entity_class(m.group("type"))
            my_offset = m.group("offset")
            my_limit = m.group("limit")
            query = self.query._re_uri_by_rdftype_query.format(label=my_type, offset=my_offset, limit=my_limit)
            rst = self.session.read_transaction(self._do_neo4j_generic_query, query)  
        ...
        if m:
            ...
            my_rst = self.session.read_transaction(self._do_neo4j_generic_query, query)
            for row in my_rst:
                a = scope=row['l'][0] if isinstance(row['l'], list) else row['l']
                s = URIRef(my_url)
                o = self.mapper.map_class(a)
                rst.append((s,RDF.type,o))

                for my_p in row['p']:
                    my_o = row['p'][my_p] if isinstance(row['p'][my_p], list) else [row['p'][my_p]]
                    p = self.mapper.map_property(my_p, scope=a)
                    if p is not None:
                        for o in my_o: o = self._do_create_literal(my_p, o); rst.append((s,p,o))
                    else: 
                        logging.warning("[NEO4JStore:query] error unmapping property: {}".format(my_p))
            
            query = self.query._re_describe_relation_query.format(url=my_url)
            my_rst = self.session.read_transaction(self._do_neo4j_generic_query, query)
            for row in my_rst:
                my_p = scope=row['l'][0] if isinstance(row['l'], list) else row['l']
                p = self.mapper.map_property(my_p, scope=a)
        ...
        if rst is None: logging.warning("[NEO4JStore:query] not handle query"); rst = []

        return rst
    query._re_alias_pattern = re.compile(r"(SELECT DISTINCT \?s WHERE { \?s adms:identifier( ))((<)(?P<url>[\w\W]+)(>))(\. \?s rdf:type (?P<type>[\w\W]+)\. })")
    query._re_alias_query = "MATCH (n:{label})-[r {{url: '{url}' }}]->(n) RETURN n.id"
    query._re_totals_by_rdftype_pattern = re.compile(r"SELECT \(COUNT \?s\) as \?c WHERE \{ \?s rdf:type \<(?P<type>[\W\w]+)\>. \}")
    query._re_totals_by_rdftype_query = "MATCH (n:{label}) RETURN COUNT(n) as c"
    query._re_uri_by_rdftype_pattern = re.compile(r"SELECT \?s WHERE \{ \?s rdf:type \<(?P<type>[\W\w]+)\>. \} LIMIT (?P<limit>[\d]+) OFFSET (?P<offset>[\d]+)")
    query._re_uri_by_rdftype_query = "MATCH(n:{label}) RETURN n.id as n SKIP {offset} LIMIT {limit}"
    query._re_describe_uri_pattern = re.compile(r"DESCRIBE (<)(?P<url>[\w\W]+)(>)")
    ...
    def namespaces(self):
        ...
        for k,v in self.ns.items(): yield k,v
    ...
    def _do_neo4j_createNode (self, tx, label, id):
        ...
        try:
            query = self._do_neo4j_createNode._stm_create_node.format(label=label, id=id)
            result = tx.run(query)
    ...
    def _do_neo4j_updateNode (self, tx, node):
        def _do_neo4j_updateNode_special_cases(p):
            if p.startswith("n.created") or p.startswith("n.modified") or p.startswith("n.updated"):
                tn = [n for n in ("n.created", "n.modified", "n.updated") if p.startswith(n)][0]
        ...
        if node is None or 0 == len(node): return []
        ...
        for k,v in node.items():
            ...
            if isinstance(v, list): 
                properties.append("n.{}={}".format(k, str([self._do_prepare_node_property(x) for x in v])))
            elif isinstance(v, dict):
                pass
            else:
                properties.append("n.{}={}".format(k,self._do_prepare_node_property(v)))
        else:
            properties = [_do_neo4j_updateNode_special_cases(p) for p in properties]
            properties = ',\n'.join(properties)

        try:
            query = self._do_neo4j_updateNode._stm_merge_node.format(id=node["__id__"], label=node["__label__"], properties=properties)
            result = tx.run(query)
            return [{"n": row["n"]} for row in result]# Capture any errors along with the query and data for traceability
        except ServiceUnavailable as exception:
            logging.error("[NEO4JStore:_do_neo4j_updateNode] raised an error: \n {exception}".format(query=query, exception=exception))
        return []
    ...
    def _do_neo4j_createRelation (self, tx, relation):
        ...
        try:
            property = "SET r.url='{}'".format(relation[3]) if relation[3] is not None else ""
            query = self._do_neo4j_createRelation._stm_merge_relation.format(node_from=relation[0], role=relation[1].upper(), property=property, node_to=relation[2])
            result = tx.run(query)
    ...
    def _do_process_tiplets (self, tiplets):
        nodes = {str(t[0]): {"__id__": str(t[0]),"__label__": Entity.getInstanceFromRDFType(t[2], t[0]).__class__.__name__} for t in tiplets if t[1] == RDF.type}
        relations = []
        ...
        for t in tiplets:
            ...
            if p is None:
                if "http://www.w3.org/1999/02/22-rdf-syntax-ns#type" != str(t[1]):
                    logging.warning("[do_prepareTipletSet] not resolved mapping for property '{}'".format(str(t[1])))
                continue
            else:
                p = camelCaseToSnakeCase(p)
                if p in self._do_process_tiplets._predicate_renaming_map: p = self._do_process_tiplets._predicate_renaming_map[p]

            o = t[2]
            if isinstance(o, Literal):
                o = o.toPython()
                o = o.replace("'", "\\'") if isinstance(o, str) and "'" in o else o
                ...
                o = o.toPython()
                if "alias" == p:
                    if s != o: relations.append((s, p.upper(), s, o))
                else:
    ...
    def _do_prepare_node (self, node):
        ...
        if "keywords" in node:
            ...
            value = ",".join(value)
            node["keywords"] = list(dict.fromkeys(value)) if isinstance(value, list) else value

        if "geometry" in node:
            print("STOP: extract POINTS")
        if "latitude" in node and "longitude" in node:
            print("STOP: create geopoint")
        if "min_latitude" in node and "min_longitude" in node and "max_latitude" in node and "max_longitude" in node:
            print("STOP: create bounding_box")
    ...
    def _do_resolve_entity_class(self, predicate):
        ...
        m = re.match(self._do_resolve_entity_class._re_predicate_pattern, str(predicate))
        if m:
            predicate = "{}.{}".format(m.group("vocabulary").upper(), m.group("topic"))
            if predicate not in self._do_resolve_entity_class._cache_predicate_qname_to_uri: 
                self._do_resolve_entity_class._cache_predicate_qname_to_uri[predicate] = str(eval(predicate))
    ...
    _do_resolve_entity_class._re_predicate_pattern = re.compile(r"(?P<vocabulary>[\w]+):(?P<topic>[\w]+)")
