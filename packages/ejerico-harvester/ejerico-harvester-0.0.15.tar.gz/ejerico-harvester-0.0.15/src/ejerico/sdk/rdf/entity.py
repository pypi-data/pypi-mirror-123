...
class EntityMetaclass(type):
    def __new__(cls, name, bases, attributes):
        ...
        domain = config.get("domain", default="http://www.ejerico.org")
...
class Entity(object, metaclass=EntityMetaclass):
    ...
    def metadata(cls, schema=None):
        ...
        rdf_type = rdf_type_redefinitions[0][1] if len(rdf_type_redefinitions) else rdf_type 
        ...
        if not Entity._rdf_types: 
            ...
            for clazz in clazzes:
                if issubclass(clazz[1], Entity): 
                    instance = clazz[1]()
                    Entity._rdf_types[clazz[1]] = {"rdf_type": instance.rdf_type}
    ...
    def getRDFTypeForEntity(entity_class):
        ...
        if issubclass(entity_class, Entity):
            mapper = get_mapper()
            for cls in inspect.getmro(entity_class):
                if cls.__name__ == "Entity": break
                rst = cls.__name__
                rst = mapper.map_class(scope=rst)
        else:
    ...
    def isInternalURI(uri):
        ...
        uri = str(uri)
        return uri.startswith(ConfigManager.instance().get("domain", default="http://www.ejerico.org"))

    @staticmethod
    def patternURI():
        entities = '|'.join(sorted([c for c in Entity.scopes()]))
        domain = ConfigManager.instance().get("domain", default="http://www.ejerico.org")
    ...
    def id(self,value):
        if value is not None and isinstance(value,str):
            ...
            c = countWordOcurrences(str(self._id), "http")
            if  c > 1 or not checkers.is_url(str(self._id)): raise ValidationError("not valid uri ({})".format(str(self._id)))
    ...
    def deserialize(clazz, serialized_content, target_class=None, from_jsonld=False):
        ...
        if target_class is None and "_class" in deserialized:
            if from_jsonld:
    ...
    def _deserialize(clazz, data, from_jsonld=False):
        ...
        my_object = clazz()
        re_datetime = re.compile(r'\d{2,4}-\d{1,2}-\d{1,2}T\d{1,2}:\d{1,2}:\d{1,2}')
    ...
    def load(data, format="turtle", subjects_with_types=None):
        ...
        for uri in entities:
            ...
            for row in rows:
                if str(row[0]) in ('http://www.w3.org/ns/dcat#distribution'):
                    print("STOP")
    ...
    def attributes(self, include_empty=False):
        ...
        for key in self.__dict__:
            ...
            if isinstance(self.__dict__[key], str) and '' == self.__dict__[key]: continue
            if isinstance(self.__dict__[key], list) and 0 == len(self.__dict__[key]): continue
            if re.match(r"^[a-zA-Z]+\W*", key):
                if key not in self._excluded_attributes: my_attributes.append(key)
                ...
                if re.match(r"^[a-zA-Z]+\W*", attr):
                    ...
                    try:
                        ...
                        if isinstance(my_predicate,str) and not isinstance(my_predicate,URIRef):
                            my_predicate = eval("EJERICO.{}".format(my_predicate))
                    except Exception as e:
...
def __constructor__(self, id=None, first_born=False, is_raw_id=False, namespace=None, source=None):
    namespace = namespace if namespace else _get_caller_attribute("namespace")
