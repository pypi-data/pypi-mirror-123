...
if __name__ == "__main__":
    ...
    constants = {
        'link_addition_p': 1,
        'link_removal_p': 0.00025,
        'connection_type': 'all', # all / nb_nb
        'max_links': 15,
    ...
    def network_update(nodes, constants):
        ...
        if np.random.random_sample() <= constants['link_addition_p'] and np.sum(adj[node]) < constants['max_links']:
            if constants['connection_type'] == 'nb_nb':
                potential_links = np.where(nb_adj[node] == 1)[0] # Neighbors of neighbors
            elif constants['connection_type'] == 'all':
                potential_links = np.where(adj[node] == 0)[0]

            if len(potential_links) > 0:
                new_link = np.random.choice(potential_links, 1)
                update['add'] = new_link
                changes[node] = update
            elif len(potential_links) == 0 and constants['connection_type'] == 'nb_nb':
                # If there are no neighbors of neighbors available add a random new link
                new_link = np.random.choice(np.where(adj[node] == 1)[0])
                update['add'] = [new_link]
                changes[node] = update

        # # remove x percent of links
        if model.current_iteration <= 7500:
            removable_links_n = 1
