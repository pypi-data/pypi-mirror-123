...
@click.option('-c', '--column', help='(multiple) Columns labels to load, others will be discarded.', multiple=True,
              type=str)
def load_csv(ctx_context, file, column, alias):
    """
    ...
    :param column: columns from the CSV file to be loaded. Other columns will be ignored.
    ...
    if column:
        ...
        lst_the_inexistant_columns: list = list(set(column) - set(df_the_measures.columns))
        ...
        df_the_measures = df_the_measures[list(column)]
...
@click.command('load-xls')
@click.pass_context
@click.argument('file', type=click.Path(exists=True))
@click.argument('alias', type=str, default='xls')
@click.option('-c', '--column', help='Columns labels to load, others will be discarded.', multiple=True, type=str)
def load_xls(ctx_context, file, column, alias):
    """
    Load listed columns from a xls-x file into a dataframe that is labelled using an alias for later use.

    :param alias: name given to the dataframe being loaded, so it can be used later on by other commands.
    :param column: columns from the XLS-X file to be loaded. Other columns will be ignored.
    :param ctx_context: Click context.
