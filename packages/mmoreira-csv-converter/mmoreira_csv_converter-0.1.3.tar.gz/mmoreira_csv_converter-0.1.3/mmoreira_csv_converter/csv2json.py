...
def csv2json(DIR, arquivo, delimitador, prefix):
    ...
    json = ""
    for line in arq:
        list_line = line.split(delimitador)
        saida = ""
        if cab:  # monta cabeçalho de atributos // testa e remove aspas extras
    ...
    arq.close()
    arq = open(
        os.path.join(DIR, prefix + "_" + arquivo.split(".")[0] + ".json"), "w", encoding="UTF-8"
    )
    arq.write(json)
    arq.close()
...
#
def json2csv(DIR, arquivo, prefix):
    print("----> JSON to CSV\n")
    arq = open(os.path.join(DIR, arquivo), "r", encoding="UTF-8")
    ...
    #
    arq = open(
        os.path.join(DIR, prefix + "_" + arquivo.split(".")[0] + ".csv"), "w", encoding="UTF-8"
    )
    arq.write(csv)
    arq.close()
...
@click.option(
    "--delimiter", "--d", default=";", help="Separator used to split csv files", type=str
)
@click.option(
    "--prefix",
    "-prefix",
...
def converter(input: str = "./", output: str = "./", delimiter: str = ",", prefix: str = None):
    ...
    name = input_aux[len(input_aux) - 1]
    DIR = "".join(input[0 : len(input) - len(name)])
    ...
    if os.path.isfile(os.path.join(DIR, name)):  # trata um arquivo isoladamente
        ...
        if arquivos[0].split(".")[1].upper() == "csv".upper():  # teste se a entrada é CSV
            csv2json(DIR, arquivos[0], delimitador, prefix)  # csv2json()
        elif arquivos[0].split(".")[1].upper() == "json".upper():  # teste se a entrada é Json
            json2csv(DIR, arquivos[0], prefix)
    #
        ...
        for cont in range(0, cont_arq):
            if arquivos[cont].split(".")[1].upper() == "csv".upper():  # teste se a entrada é CSV
                csv2json(DIR, arquivos[cont], delimitador, prefix)  # csv2json()
            elif (
                arquivos[cont].split(".")[1].upper() == "json".upper()
            ):  # teste se a entrada é Json
                json2csv(DIR, arquivos[cont], prefix)
                continue
            else:
