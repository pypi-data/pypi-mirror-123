...
def npGetData(data):
	...
	if data is None:
		...
		return Message(npGetData(data.path), npGetData(data.input), npGetData(data.output))
...
def trGetData(data, device=None):
	_device = device if not device is None else (tr.device("cuda") if tr.cuda.is_available() else tr.device("cpu"))
	if data is None:
		return None
	elif isinstance(data, (np.int32, np.int8, np.int16, np.int64, np.float32, np.float64, int, float)):
		return tr.Tensor([data]).to(_device)
	elif isinstance(data, list):
		return [trGetData(x, device) for x in data]
	elif isinstance(data, tuple):
		return tuple(trGetData(x, device) for x in data)
	elif isinstance(data, set):
		return {trGetData(x, device) for x in data}
	elif isinstance(data, (dict, OrderedDict)):
		return {k : trGetData(data[k], device) for k in data}
	elif isinstance(data, tr.Tensor):
		return data.to(_device)
	elif isinstance(data, np.ndarray):
		return tr.from_numpy(data).to(_device)
	elif callable(data):
		return data
	elif isinstance(data, str):
		return data
