...
from nwdata.utils import RunningMean
from .nwmodule import NWModule
from .utilities import logger, trGetData, npGetData, drange
...
class NWTrainer:
	...
	def epochPrologue(self, epochResults, isTraining:bool):
		...
		if isTraining:
			optimizerStr = _getOptimizerStr(self.model.getOptimizer())
	...
	def runOneEpoch(self, iterator:Iterator, stepsPerEpoch:int, isTraining:bool, \
		...
		for i in Range:
			...
			trItems = trGetData(npItems, device)

			# TODO: we still assume inputs, labels = data. Update this to be generic!
			npData, npLabels = npItems["data"], npItems["labels"]
			trData, trLabels = trItems["data"], trItems["labels"]

			self.callbacksOnIterationStart(isTraining, isOptimizing)
			trResults, trLoss = self.model.networkAlgorithm(trData, trLabels, isTraining, isOptimizing)
			npResults, npLoss = npGetData(trResults), npGetData(trLoss)
			self.callbacksOnIterationEnd(npData, npLabels, npResults, npLoss, i, stepsPerEpoch, \
				res, isTraining, isOptimizing)
	...
	def train(self, reader:Iterable, numEpochs:int, validationReader:Optional[Iterable], \
