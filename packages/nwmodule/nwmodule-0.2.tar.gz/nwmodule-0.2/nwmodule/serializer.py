...
from .utilities import isBaseOf, isPicklable, logger, getNumParams, getTrainableParameters, device
from .utilities.tr_utils import _computeNumParams
...
class NWModuleSerializer:
	...
	def loadModel(self, path, stateKeys):
		...
		logger.debug("[NWModuleSerializer::loadModel] Loading model from %s" % (path))
	...
	def doLoadModel(self, loadedState, stateKeys):
		...
		for key in stateKeys:
			if key == "weights":
				...
				nParams = _computeNumParams(loadedState["weights"])
				logger.debug("[NWModuleSerializer::doLoadModel] Succesfully loaded weights (%d parameters) " % nParams)
			elif key == "optimizer":
	...
	def doLoadWeights(self, loadedParams, allowNamedMismatch=False):
		...
		try:
			...
			print(("[NWModule::Serializer::DoLoadWeights] Name mismatch happened, but loading anyway based on ") + \
		...
		if len(missing) > 0:
			logger.debug("Loaded partial model. Missing %d keys (got %d keys)" % (len(missing), len(newParams)))
		if len(unexpected):
			logger.debug("Unexpected %d keys in the loaded model" % len(unexpected))
	...
	def doLoadOptimizer(self, optimizerDict):
		...
		logger.debug("Succesfully loaded optimizer: %s" % (self.model.getOptimizerStr()))
		...
		if "scheduler_state" in optimizerDict:
			...
			logger.debug("Succesfully loaded optimizer scheduler: %s" % (self.model.optimizerScheduler))
	...
	def doLoadHistoryDict(self, trainHistory):
		...
		logger.debug("Succesfully loaded model history (epoch %d)" % (len(trainHistory)))
	...
	def doLoadCallbacks(self, loadedState):
		...
		for i in range(len(originalPositions)):
			...
			# This includes setCriterion as well.
				...
				for callback in metricCallbacks:
					if callback == key:
						value = metricCallbacks[callback]
						break
		...
		logger.debug("Succesfully loaded %d callbacks (%d metrics)" % (numAll, numMetrics))
