...
from coiled.exceptions import (
    ...
    AzureCredentialsParameterError,
    GCPCredentialsError,
...
class Cloud(Generic[IsAsynchronous]):
    ...
    def _normalize_name(
        ...
        account, parsed_name, _ = parse_identifier(name)
        if (
    ...
    async def _list_job_configurations(self, account=None) -> dict:
        return await self._depaginate(
            self._list_job_configurations_page, account=account
        )

    @track_context
    async def _list_job_configurations_page(
        self, page: int, account=None
    ) -> Tuple[dict, Optional[str]]:
        account = account or self.default_account
        response = await self._do_request(
            ...
            params={"only_listable": "True", "page": page},
        )
        if response.status >= 400:
            await handle_api_exception(response)  #  will always raise
        ...
        response_json = await response.json()
        results = {
            r["name"]: format_job_configuration_output(r)
            for r in response_json["results"]
        }
        return results, response_json["next"]
    ...
    async def _list_jobs(self, account: Optional[str] = None) -> dict:
        return await self._depaginate(self._list_jobs_page, account=account)

    @track_context
    async def _list_jobs_page(
        self, page: int, account: Optional[str] = None
    ) -> Tuple[dict, Optional[str]]:
        account = account or self.default_account
        response = await self._do_request(
            "GET", self.server + f"/api/v1/{account}/jobs/", params={"page": page}
        )
        if response.status >= 400:
            await handle_api_exception(response)  # will always raise
    ...
    # Here's a bunch of overloads because of our typing since we return
    # either a dict or None, we need the Literal[False]/Literal[True] so
    # typing is happy
    @overload
    def list_instance_types(
        self: Cloud[Sync], backend: str, json: bool
    ) -> Optional[Dict]:
        ...

    @overload
    def list_instance_types(
        self: Cloud[Async], backend: str, json: bool
    ) -> Awaitable[Optional[dict]]:
        ...

    @overload
    def list_instance_types(
        self: Cloud[Sync], backend: str, json: Literal[False]
    ) -> None:
        ...

    @overload
    def list_instance_types(
        self: Cloud[Async], backend: str, json: Literal[False]
    ) -> Awaitable[None]:
        ...

    @overload
    def list_instance_types(
        self: Cloud[Sync], backend: str, json: Literal[True]
    ) -> Dict:
        ...

    @overload
    def list_instance_types(
        self: Cloud[Async], backend: str, json: Literal[True]
    ) -> Awaitable[Dict]:
        ...

    def list_instance_types(
        self, backend: Union[str] = None, json: bool = False
    ) -> Union[Awaitable[Optional[Dict]], Optional[Dict]]:
        return self._sync(self._list_instance_types, backend=backend, json=json)
    ...
    async def _list_instance_types(
        ...
        if not backend and json:
            return instance_types

        if backend:
            if backend not in SUPPORTED_BACKENDS:
                raise InstanceTypeError(
                    f"Unknown cloud provider provided - {backend} is not of type {SUPPORTED_BACKENDS}"
                )

            if json:
                return instance_types[backend]
            else:
                instance_type_table(
                    backend=backend, instance_types=instance_types[backend]
                )
        else:
            instance_type_table(backend="aws", instance_types=instance_types["aws"])
            instance_type_table(backend="gcp", instance_types=instance_types["gcp"])
            instance_type_table(backend="azure", instance_types=instance_types["azure"])

    @overload
    def list_gpu_types(self: Cloud[Sync], json: Literal[True]) -> dict:
        ...

    @overload
    def list_gpu_types(self: Cloud[Async], json: Literal[True]) -> Awaitable[dict]:
        ...

    @overload
    def list_gpu_types(self: Cloud[Sync], json: bool) -> Optional[dict]:
        ...

    @overload
    def list_gpu_types(self: Cloud[Async], json: bool) -> Awaitable[Optional[dict]]:
        ...

    def list_gpu_types(self, json: bool = False):
        return self._sync(self._list_gpu_types, json=json)

    @track_context
    async def _list_gpu_types(self, json: bool = False):
        title = "GCP GPU Types"
        ...
        table.add_row("GCP", allowed_gpus)
...
def create_notebook(
    ...
    *,
    ...
    account
        The account to save the notebook in. Note that if "account" is
        provided and "name" uses the format "<account>/name", an error
        will be thrown if the two accounts differ.
    container
    ...
    with Cloud() as cloud:
        parsed_account, name = cloud._normalize_name(
            name,
            context_account=account,
            raise_on_account_conflict=True,
        )

        complete_name = f"{parsed_account}/{name}"
        name_software = f"{complete_name}-notebook"
...
def set_backend_options(
    ...
    account: str = None,
    backend: Literal["aws", "azure", "gcp"] = "aws",
    ...
    **kwargs,
    ...
    backend_type
        Supported backends such as AWS VM (vm_aws), GCP VM (vm_gcp) and Azure VM (vm_azure).
    ...
    gcp_zone
        The Google Cloud Compute zone name in which a subnet will be create (Zone should be <region>-<zone letter>)
    ...
    registry_type
        ...
        and one of gcp_service_creds_dict or gcp_service_creds_file. To use Azure Container Registry, pass
        registry_type='acr', azure_resource_group, azure_client_id, azure_secret, azure_subscription_id,
        azure_tenant, and azure_region.
    ...
    # Can be used for azure backend validation, and error display
    missing_azure_kwargs = ", ".join(
        [kw for kw, val in azure_required_kwargs.items() if not val]
    )
    ...
    if use_coiled_defaults is True:
        ...
        if create_vpc:
            ...
            if aws_secret_access_key and aws_access_key_id:
                warning_message = Text(
                    text=(
                        "WARNING: Credentials provided without using 'create_vpc=True', please use this "
        ...
        if create_vpc:
            ...
            if missing_azure_kwargs:
                raise AzureCredentialsParameterError(
                    f"Missing Azure parameters: {missing_azure_kwargs}"
                )
            ...
            backend_options["options"]["azure_region_name"] = azure_region
            ...
            if any(
                ...
                warning_message = Text(
                    text=(
                        "WARNING: Credentials provided without using 'create_vpc=True', please use this "
                        "parameter if you want to use Coiled on your Azure account. \n"
                    ),
                    style="yellow",
                )
                console.print(warning_message)
            backend_options["backend"] = "vm_azure"
            backend_options["options"] = {"azure_region_name": azure_region}
            output_msg = (
    ...
    if use_coiled_defaults is True:
        ...
        backend_options["registry"] = {
            "type": registry_type,
            "credentials": azure_required_kwargs,
...
def list_instance_types(backend: str = "", json: bool = False) -> Optional[Dict]:
    """List allowed instance types for the cloud provider of choice.

    Parameters
    ----------
    backend
        Relevant cloudprovider to get a list of allowed instance types. If not provided
        the list will show all instances from all the allowed cloud providers.
    json
        If set to ``True``, it will return this list in json format instead of a table.

    """
    with Cloud() as cloud:
        return cloud.list_instance_types(backend=backend, json=json)


@overload
def list_gpu_types(json: Literal[False] = False) -> None:
    ...


@overload
def list_gpu_types(json: Literal[True] = True) -> Dict:
    ...


def list_gpu_types(json: bool = False) -> Optional[Dict]:
    """List allowed GPU Types

    You can only specify gpu types in GCP, so this is command is only relevant if you
    are using GCP as your cloud provider.
    ...
    """
    with Cloud() as cloud:
        return cloud.list_gpu_types(json=json)
