...
from cloud.models import ClusterConfiguration
...
def test_public_api_depagination_with_cluster_configuration(sample_user, software_env):

    # create 101 cluster configurations
    for i in range(101):
        coiled.create_cluster_configuration(name=f"foo_{i}", software=software_env)

    results = coiled.list_cluster_configurations()
    expected_num_results = ClusterConfiguration.objects.filter(
        account__slug=sample_user.account.slug
    ).count()

    assert len(results) == expected_num_results

    # create another 101 cluster configurations
    ...
    logger.info(
        f"checking depaginated results from list_cluster_configurations, expected: "
...
def test_set_backend_options_aws_vm_customer_hosted(cloud, sample_user, mocker):
    ...
    with pytest.raises(AWSCredentialsParameterError) as e_info:
        coiled.set_backend_options(backend="aws", create_vpc=True)
    assert (
...
def test_set_backend_options_azure_vm_missing_params(
    ...
    coiled.set_backend_options(backend="azure")
    account.refresh_from_db()

    assert account.options == {"azure_region_name": settings.AZURE_DEFAULT_USER_REGION}
    assert account.container_registry == {
        "type": ContainerRegistryType.ECR,
        "credentials": {},
        "public_ecr": False,
    ...
    with pytest.raises(AzureCredentialsParameterError) as e_info:
        coiled.set_backend_options(backend="azure", create_vpc=True)
    assert "Missing Azure parameters" in str(e_info)
...
def test_set_backend_options_azure_vm_customer_hosted(cloud, sample_user, mocker):
    ...
    options = {
        ...
        "azure_region_name": settings.AZURE_DEFAULT_USER_REGION,
        "credentials": {
            "aws_access_key_id": "",
...
@pytest.mark.test_group("core-slow-group-12")
def test_set_backend_options_registry_acr(cloud, sample_user, mocker):
    account = sample_user.account
    assert account.options == {}
    mocker.patch("backends.cloudbridge.cloudbridge.ClusterManager._configure_backend")
    credentials = {
    ...
    registry = {
        "type": ContainerRegistryType.ACR,
        "credentials": credentials,
        "location": location,
    }
    assert account.options == options
...
@pytest.mark.test_group("core-slow-group-12")
def test_set_backend_options_registry_acr_validation(cloud, sample_user, mocker):
    account = sample_user.account
    assert account.options == {}
    mocker.patch("backends.cloudbridge.cloudbridge.ClusterManager._configure_backend")
    credentials = {
    ...
    with pytest.raises(AzureCredentialsParameterError) as e:
        coiled.set_backend_options(
            backend="azure",
            create_vpc=True,
            azure_region=location,
    ...
    with pytest.raises(Exception) as e:
        coiled.set_backend_options(
            backend="azure",
            create_vpc=True,
            azure_region="im_an_invalid_region",
            registry_type="acr",
            **credentials,
        )
    error_message = e.value.args[0]
    assert "not a supported Azure region" in error_message
...
def test_list_instance_types(sample_user, capfd):
    instance_types = coiled.list_instance_types(json=True)
    assert "aws" in instance_types.keys()
    assert "t3.large" in instance_types["aws"].keys()

    instance_types = coiled.list_instance_types(backend="gcp", json=True)
    assert "e2-standard-2" in instance_types

    instance_types = coiled.list_instance_types("azure", json=True)
    assert "Standard_F4s_v2" in instance_types

    coiled.list_instance_types(backend="aws")
    capture = capfd.readouterr()
    # If table title is there then the table is there right?
    assert "AWS Instance Types" in capture.out


def test_list_instance_types_raises_exception(sample_user):

    with pytest.raises(InstanceTypeError):
        coiled.list_instance_types(backend="stuff")


def test_list_gpu_types(sample_user, capfd):
    gpu_type = coiled.list_gpu_types(json=True)

    assert "nvidia-tesla-t4" in gpu_type.values()

    coiled.list_gpu_types()
    capture = capfd.readouterr()
    assert "nvidia" in capture.out
