...
async def handle_credentials(
    *,
    ...
    async with aiohttp.ClientSession(
        headers={"Authorization": "Token " + token, "Client-Version": client_version}
    ) as session:
...
def get_instance_types(backend: Union[str] = None) -> dict:
    """Get instance types from the file instance_types.yaml

    For now we are hard coding these instances in a file, in the
    future this function will fetch the instance types from our
    API and return all instance types for the provided backend.
    """
    instances_file = os.path.join(os.path.dirname(__file__), "instance_types.yaml")
    with open(instances_file) as file:
        return yaml.safe_load(file)


def instance_type_table(backend: str, instance_types: dict):
    title = f"{backend.upper()} Instance Types"
    table = Table(title=title, show_lines=True)

    table.add_column("Instance Type", justify="center")
    table.add_column("CPU", justify="center")
    table.add_column("Memory", justify="center")
    table.add_column("Cost", justify="center")
    table.add_column("GPU enabled", justify="center")

    for instance, spec in instance_types.items():
        table.add_row(
            instance, str(spec["cpu"]), spec["memory"], spec["cost"], str(spec["gpu"])
        )
...
def validate_instance_type(instance_type_list: list):
    """Validate instance type provided by the user.

    We need to double check if the instance type provided by the user
    is a valid one from the subset of instances that we are accepting
    in the alpha version.

    In the future we probably want to call our API to check if the
    instance type is on our list.

    """
    if not isinstance(instance_type_list, list):
        error_msg = (
            "Invalid type provided: 'scheduler_vm_types' and 'worker_vm_types' must be "
            f"a list. '{instance_type_list}' is of type: {type(instance_type_list)}."
        )
        raise InstanceTypeError(error_msg)
    ...
    for instance_type in instance_type_list:
        if instance_type not in allowed_instance_types:
            error_msg = (
                f"Instance type '{instance_type}' is not a supported instance type \n"
                f"Allowed instances are: {allowed_instance_types}"
            )
            raise InstanceTypeError(error_msg)
    return True
