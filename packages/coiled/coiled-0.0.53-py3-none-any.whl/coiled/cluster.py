...
class Cluster(distributed.deploy.Cluster, Generic[IsAsynchronous]):
    ...
    worker_gpu_type
        Type of GPU to use for this cluster. You can only select GPU type if you are using GCP as
        your backend.
    ...
    def __init__(
        ...
        *,
        ...
        if not self.backend_options.get("spot") and self.worker_gpu:
            self.backend_options["spot"] = False
        if scheduler_vm_types:
            validate_instance_type(scheduler_vm_types)
        self.scheduler_vm_types = scheduler_vm_types
        if worker_vm_types:
            validate_instance_type(worker_vm_types)
