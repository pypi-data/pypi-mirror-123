...
class FireDecoratorsTest(testutils.BaseTestCase):
  ...
  def testSetParseFnsNamedArgs(self):
    self.assertEqual(core.StrictFire(NoDefaults, command=['double', '2']), 4)
    self.assertEqual(core.StrictFire(NoDefaults, command=['triple', '4']), 12.0)

  def testSetParseFnsPositionalArgs(self):
    self.assertEqual(core.StrictFire(NoDefaults, command=['quadruple', '5']), 20)

  def testSetParseFnsFnWithPositionalArgs(self):
    self.assertEqual(core.StrictFire(double, command=['5']), 10)
  ...
  def testSetParseFnsDefaultsFromFire(self):
    ...
    self.assertEqual(core.StrictFire(WithDefaults, command=['example1']), (10, int))
    self.assertEqual(core.StrictFire(WithDefaults, command=['example1', '10']),
                     (10, float))
    self.assertEqual(core.StrictFire(WithDefaults, command=['example1', '13']),
                     (13, float))
    self.assertEqual(core.StrictFire(WithDefaults, command=['example1', '14.0']),
                     (14, float))
  ...
  def testSetParseFnsNamedDefaultsFromFire(self):
    # Fire should use the decorator to know how to parse string arguments.
    self.assertEqual(core.StrictFire(WithDefaults, command=['example2']), (10, int))
    self.assertEqual(core.StrictFire(WithDefaults, command=['example2', '10']),
                     (10, float))
    self.assertEqual(core.StrictFire(WithDefaults, command=['example2', '13']),
                     (13, float))
    self.assertEqual(core.StrictFire(WithDefaults, command=['example2', '14.0']),
                     (14, float))

  def testSetParseFnsPositionalAndNamed(self):
    self.assertEqual(core.StrictFire(MixedArguments, ['example3', '10', '10']),
                     (10, '10'))

  def testSetParseFnsOnlySomeTypes(self):
    self.assertEqual(
        core.StrictFire(PartialParseFn, command=['example4', '10', '10']), ('10', 10))
    self.assertEqual(
        core.StrictFire(PartialParseFn, command=['example5', '10', '10']), (10, '10'))

  def testSetParseFnsForKeywordArgs(self):
    self.assertEqual(
        core.StrictFire(WithKwargs, command=['example6']), ('default', 0))
    self.assertEqual(
        core.StrictFire(WithKwargs, command=['example6', '--herring', '"red"']),
        ('default', 0))
    self.assertEqual(
        core.StrictFire(WithKwargs, command=['example6', '--mode', 'train']),
        ('train', 0))
    self.assertEqual(core.StrictFire(WithKwargs, command=['example6', '--mode', '3']),
                     ('3', 0))
    self.assertEqual(
        core.StrictFire(WithKwargs,
                  command=['example6', '--mode', '-1', '--count', '10']),
        ('-1', 10))
    self.assertEqual(
        core.StrictFire(WithKwargs, command=['example6', '--count', '-2']),
        ('default', -2))

  def testSetParseFn(self):
    self.assertEqual(
        core.StrictFire(WithVarArgs,
