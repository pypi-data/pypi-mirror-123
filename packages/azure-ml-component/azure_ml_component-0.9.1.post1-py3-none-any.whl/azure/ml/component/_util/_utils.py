...
def _download_snapshot_by_id(component_api_caller, snapshot_id, target):
    """
    ...
    :type target: str
    """
    snapshot_path = component_api_caller.download_snapshot_by_id(snapshot_id, target)
    _extract_zip(zip_file=snapshot_path, target_dir=target)
    # remove the zip file to align with previous behavior
    try:
...
def _relative_path(path, basedir):
    ...
    return Path(os.path.relpath(path, basedir)).as_posix()
...
def _get_param_in_group_from_pipeline_parameters(param_name, pipeline_parameters, param_groups=None):
...
def enabled_subpipeline_registration() -> bool:
    ...
    return os.getenv('ENABLE_SUBPIPELINE_REGISTER', 'true').lower() != 'false'
