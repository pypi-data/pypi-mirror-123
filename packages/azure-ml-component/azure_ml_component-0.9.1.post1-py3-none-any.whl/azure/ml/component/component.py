...
from ._component_func import get_dynamic_input_parameter, to_component_func_from_definition
...
from ._util._utils import _get_parameter_static_value, _sanitize_python_variable_name, _get_short_path_name, \
    trans_to_valid_file_name, _ensure_dataset_saved_in_workspace, is_valid_component_name
...
class Input(TelemetryMixin, LinkedParametersMixedIn):
    ...
    _ALLOWED_TYPES = (
        AbstractDataset, _GlobalDataset, _FeedDataset, DataReference, DatasetConsumptionConfig,  # Data in remote
...
class Output(TelemetryMixin):
    ...
    def __init__(
        ...
        :param mode: Specifies whether to use "upload" or "mount" to access the data.
        :type mode: str
    ...
    def configure(self, datastore=None, mode=None, path_on_compute=None, path_on_datastore=None, **kwargs):
        ...
        :param mode: Specify whether to use "upload" or "mount" to access the data.
...
class Component(WorkspaceTelemetryMixin):
    ...
    * `Define a component using component specs <https://aka.ms/azureml-component-specs>`_
    ...
    def _init_public_interface(self, init_params):
        ...
        for k, v in init_params.items():
            if isinstance(v, Enum):
                # Extract enum value
                init_params[k] = v.value
            elif isinstance(v, _GroupAttrDict) and v == self._definition.parameters[k].default:
    ...
    def _update_component_params_and_runsettings(self, workspace, pipeline_parameters=None):
        ...
        if self._specify_runsettings:
            ...
            runsettings._copy_and_update(runsettings, pipeline_parameters)
        if self._specify_k8srunsettings:
            k8srunsettings.configure(**self.k8srunsettings._get_values(ignore_none=True))
            k8srunsettings._copy_and_update(k8srunsettings, pipeline_parameters)
    ...
    def _resolve_compute(self, enable_subpipeline_registration=False, pipeline_parameters={}, workspace=None):
        """
        ...
        def _get_compute(pipeline):
            ...
            pipeline_compute = pipeline._get_default_compute_target(pipeline_parameters=pipeline_parameters)
    ...
    def _get_environment(self, workspace=None):
        ...
        workspace_id = env_workspace._workspace_id
        if workspace_id not in self._env:
            # When component is initialized by ComponentDefinition, using _definition to get environment.
            env = self._definition.environment._to_aml_sdk_env()
            ...
            is_environment_registered = env.name is not None
            ...
            if not is_environment_registered:
                ...
                env.name = self._identifier if self._identifier else self._id
                env = env.register(env_workspace)
            self._env[workspace_id] = env
        return self._env[workspace_id]
    ...
    def _validate_component(
            ...
            inputs=None, parameters=None, skip_datastore_compute_validation=False
    ) -> List:
        """
        ...
        if not is_local:
            ...
            if not skip_datastore_compute_validation:
                ...
                ComponentValidator._validate_compute_on_component(self, default_compute, self.workspace,
    ...
    @staticmethod
    @track(_get_logger, activity_type=_PUBLIC_API, activity_name="Component_create")
    def create(function: Callable, version: str = None, set_as_default: bool = False, *,
        ...
        print("Created Component:", repr_dict)
