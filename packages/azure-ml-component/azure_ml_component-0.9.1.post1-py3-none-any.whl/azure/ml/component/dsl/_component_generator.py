...
from ._component import InputPath, OutputPath
from .types import Input, Output, String, _Param, Enum, _Group
...
class ModulePackageGenerator:
    def __init__(self, assets, source_directory, func_call_directory,
        ...
        self._auth = None
        # Need auth if is workspace or feed assets
        if any(pattern.startswith(ModuleUtil.WS_PREFIX) or pattern.startswith(ModuleUtil.FEED_PREFIX)
               for pattern in itertools.chain(*assets.values())):
    ...
    def _assets_from_default(source_directory):
        workspace = Workspace.from_config(path=source_directory)
        if workspace is None:
            raise UserErrorException('Get default workspace failed, please specify assets manually and try again.')
    ...
    def _prepare_assets(self):
        ...
        for pattern in all_modules:
            ...
            if feed_name in feed_futures:
                continue
            workspace = Workspace.from_config(path=self._source_directory)
