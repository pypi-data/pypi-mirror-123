...
def convert_assign_to_param_annotation(assign):
    ...
    if isinstance(assign.value, ast.Str):
        param = StringParameter()
        param._update_default(assign.value.s)
        ...
        param._update_default(num)
    elif isinstance(assign.value, ast.NameConstant):
        value = assign.value.value
        if str(value) == 'True' or str(value) == 'False':
            param = BoolParameter()
            param._update_default(value)
        elif value is None:
            if assign.annotation:
                param_cls = _get_annotation_by_type_name(assign.annotation.func.id)
                param = param_cls()
            param._update_default(value)
