...
class _Param(_IOBase):
    ...
    _PARAM_TYPE_STRING_MAPPING = {'string': str, 'integer': int, 'float': float,
...
@track(_get_logger, activity_type=_PUBLIC_API, activity_name="parameter_group")
def _parameter_group(_cls):
    ...
    Usage:
        ...
        .. code-block:: python
            ...
            my_param_group = ParamClass(dataset, int_param=2)

            # option 1. use annotation to declare the parameter is a parameter group.
            @dsl.pipeline()
            def pipeline_func(params: ParamClass):
    ...
    def _create_init_fn(cls, fields):
        ...
        default_keys = set({key for key, val in fields.items()
                            if isinstance(val, _Param) and val._has_default})
...
class _Group(_Param):
    ...
    def __init__(self, values: dict):
        ...
        # Note Input do not have default so just set a None
        self._default = _GroupAttrDict({k: v._default for k, v in self.values.items()})
    ...
    def _customer_class_value_to_attr_dict(value):
        if not is_parameter_group(value):
            return value
        attr_dict = {k: v._default if isinstance(v, (_Param, Input)) else v for k, v in value.__dict__.items()}
        return _GroupAttrDict({k: v.value if isinstance(v, PyEnum) else v for k, v in attr_dict.items()})
...
def _get_param_with_standard_annotation(
    ...
    def _update_fields_with_default(annotation_fields, defaults_dict, is_func):
        ...
        for name in all_filed_keys:
            ...
            if not is_func and isinstance(annotation, Input):
                raise UserErrorException(f'Parameter {name!r} with type Input is not supported in parameter group.')
...
def _is_dsl_type_cls(t: type):
    if type(t) is not type:
        return False
    # param: Integer
    return issubclass(t, _Param) or t in {Input, _Path, Output}
