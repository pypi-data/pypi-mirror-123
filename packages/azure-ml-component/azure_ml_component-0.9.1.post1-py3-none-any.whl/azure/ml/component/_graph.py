...
class _GraphEntityBuilder(object):
    ...
    def build_graph_entity(self, for_registration: bool = False, enable_subpipeline_registration: bool = False,
        ...
        if self._context.default_datastore is not None:
            datastore_name = self._context.default_datastore if isinstance(
                self._context.default_datastore, str) else self._context.default_datastore.name
            graph_entity.default_datastore = DatastoreSetting(data_store_name=datastore_name)
    ...
    def _update_module_node_params(
        ...
        pipeline_datastore_name = None
        pipeline_datastore = module._resolve_default_datastore(self._context.workspace)
        if pipeline_datastore:
            pipeline_datastore_name = pipeline_datastore if isinstance(pipeline_datastore, str) \
                else pipeline_datastore.name
