...
from .dsl.types import _Group, _GroupAttrDict
from .run import Run
...
from ._util._utils import _get_short_path_name, _is_prod_workspace, trans_to_valid_file_name, \
    enabled_subpipeline_registration, resolve_data_path_assignments_from_parameter, ensure_valid_experiment_name, \
...
class Pipeline(Component, Visible, WorkspaceTelemetryMixin):
    ...
    def __init__(self, nodes: List[Union[Component, 'Pipeline']],
        ...
        _init_params = self._refine_and_validate_init_params(_init_params)
    ...
    def _refine_and_validate_init_params(self, init_params):
        init_params = {} if init_params is None else init_params
        from azure.ml.component.dsl.types import _Group
        # Extract enum value, convert customer group value to attrdict.
        for k, v in init_params.items():
            if isinstance(v, Enum):
                v = v.value
            init_params[k] = _Group._customer_class_value_to_attr_dict(v)
        # Validate group parameter attribute
        for k, annotation in self._definition.parameters.items():
            if k not in init_params or not isinstance(annotation, _Group):
                continue
            v = init_params[k]
            if not isinstance(v, _GroupAttrDict):
                logging.warning(f'Parameter {k!r} type mismatched, '
    ...
    def _copy_nodes(self, original_nodes, _copied_id_field, **kwargs):
        ...
        kwargs = _build_pipeline_parameter(is_sub_pipeline=self._is_sub_pipeline, func=None, kwargs=kwargs)
        ...
        def copy_component_init(node):
            ...
            _kwargs = self._definition._convert_component_kwargs(kwargs, _index, _for_build=True)
        ...
        def update_component_extra_settings(_pipeline_parameters, complete_node, unfinished_node):
            ...
            # Append those literal values set via `inputs.param=literal`,
            # which not pass through `set_inputs` and not be added into `extra_input_settings`
            all_inputs = {** complete_node._parameter_params}
            all_inputs.update({k: v._dset for k, v in complete_node._input_ports.items() if v._dset})
    ...
    def _get_default_compute_target(self, default_compute_target=None, ignore_runsettings=False,
        ...
        if default_compute_target is None:
            ...
            if not ignore_runsettings:
                ...
                if isinstance(self.runsettings.target, str):
                    res = _ParameterAssignment.resolve(self.runsettings.target, {}, print_warning=False)
                ...
                default_compute_target = _get_parameter_static_value(self.runsettings.target,
                                                                     pipeline_parameters) or default_compute_target
    ...
    def set_inputs(self, *args, **kwargs) -> 'Component':
        ...
        for k, v in kwargs.items():
            if isinstance(v, Enum):
                v = v.value
            kwargs[k] = _Group._customer_class_value_to_attr_dict(v)
    ...
    def submit(self, experiment_name=None, default_compute_target=None, description=None, pipeline_parameters=None,
        ...
        if pipeline_parameters:
            ...
            pipeline_parameters, data_path_assignments = resolve_data_path_assignments_from_parameter(
                pipeline_parameters,
                self.outputs.keys(),
                list(self._parameter_params.keys()) + list(self._input_ports.keys())
            )
    ...
    @track(_get_logger, activity_type=_PUBLIC_API, activity_name="PipelineComponent_publish")
    ...
    def validate(self, raise_error=False, is_local=False, *, workspace=None):
        ...
        if can_visualize:
            ...
            is_prod = _is_prod_workspace(self.workspace or workspace)
            envinfo = WorkspaceTelemetryMixin._get_telemetry_value_from_workspace(self.workspace or workspace)
    ...
    def _validate_pipeline_component(self, is_local, pipeline_parameters=None, default_compute=None, workspace=None):
        ...
        if pipeline_parameters is None:
            pipeline_parameters = {}
        default_compute = self._get_default_compute_target(default_compute)[0]
        errors = []
    ...
    def _get_graph_builder(self, pipeline_parameters=None, compute_target=None, regenerate_outputs=None,
        ...
        graph_builder_context = _GraphEntityBuilderContext(compute_target=default_compute_target,
                                                           ...
                                                           workspace=self.workspace or workspace,
    ...
    def _run(self, experiment_name=None, display_name=None, working_dir=None, mode=RunMode.Docker.value,
        ...
        :rtype: RunStatus
        """
        pipeline_workspace = self.workspace or workspace
    ...
    def _ensure_registered_in_workspace(self, workspace):
        ...
        if unregister_component_definition:
            ...
            self._init_structured_interface()
            # Correct default datastore
            if type(self._default_datastore) == str:
                self._default_datastore = Datastore(workspace, name=self._default_datastore)

            # Update runsettings and k8srunsettings of the non updated node by registered definition module dto.
            pipeline_parameters = _build_pipeline_parameter(self._is_sub_pipeline, None, self._init_params)
            for node in nodes:
                if node._definition in unregister_component_definition and not node._runsettings._workspace:
                    node._update_component_params_and_runsettings(workspace, pipeline_parameters)
    ...
    def _register_sub_pipelines(self, thread_pool, workspace=None):
        ...
        if workspace_id not in self._definition._register_task:
            # Pure node means the empty component with only default params on function as input
            pure_node = self._definition.create_pure_component_for_registration()
            # Note: Here we use pure node to show the link between pipeline parameter and node.
            register_task = thread_pool.submit(pure_node._anonymous_create, workspace)
