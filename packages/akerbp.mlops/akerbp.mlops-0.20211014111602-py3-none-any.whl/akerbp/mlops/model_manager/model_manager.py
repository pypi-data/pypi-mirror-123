...
dataset_external_id = 'mlops'


def setup(cdf_api_keys=None, dataset_external_id=dataset_external_id):
    """
...
def set_active_dataset(external_id):
    """
    ...
    Input:
        - dataset_external_id: (str) external id for the dataset (use None 
            for no dataset)
    """
    global dataset_external_id
    dataset_external_id = cdf.get_dataset_id(external_id)
    m = f"Active dataset: {dataset_external_id=}, {dataset_external_id=}"
    logging.debug(m)
...
def upload_new_model_version(model_name, env, folder, metadata={}):
    ...
    file_list = cdf.query_file_versions(
        ...
        dataset_id=dataset_external_id
    )
...
def promote_model(model_name, version, confirm=True):
    ...
    if not cdf.file_exists(external_id, '/mlops', dataset_external_id):
        return
    ...
    if not confirm or confirmed:
        cdf.copy_file(
            external_id, target_ext_id, dataset_id=dataset_external_id
        )