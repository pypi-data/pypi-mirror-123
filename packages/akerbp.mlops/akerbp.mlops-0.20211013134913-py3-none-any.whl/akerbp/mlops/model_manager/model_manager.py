...
mlops_dataset_id = None


def setup(cdf_api_keys=None, dataset='mlops'):
    """
    ...
    Input:
        ...
        - dataset: (str) external id for the dataset (use None for no dataset)
    """
    if cdf_api_keys:
        cdf.api_keys = cdf_api_keys
    cdf.set_up_cdf_client()
    if dataset:
        global mlops_dataset_id
        mlops_dataset_id = cdf.get_dataset_id(dataset)
        if mlops_dataset_id:
            logging.debug(f"{dataset=}, {mlops_dataset_id=}")
        else:
            raise ValueError(f"Didn't find {dataset=}")
...
def promote_model(model_name, version, confirm=True):
    ...
    if not cdf.file_exists(external_id, '/mlops', dataset_id=mlops_dataset_id):
        return
    ...
    if not confirm or confirmed:
        cdf.copy_file(external_id, target_ext_id, dataset_id=mlops_dataset_id)