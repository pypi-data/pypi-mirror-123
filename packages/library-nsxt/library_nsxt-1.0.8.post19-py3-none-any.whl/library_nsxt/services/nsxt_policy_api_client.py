...
class NSXTPolicyAPIClient:
    ...
    def put_nsx_member_group(self, master_group_name, member_group_name):
        ...
        if len(master_group["expression"]) == 0:
            master_group["expression"].append({
                                                "paths": [URL_GROUP %member_group_name],
                                                "resource_type": "PathExpression"
                                              })

            self.session.patch(URL_GROUP %master_group_id, master_group, True)

        else:
            master_group["expression"][0]["paths"].append(URL_GROUP %member_group_name)
            self.session.patch(URL_GROUP % master_group_id, master_group, True)
