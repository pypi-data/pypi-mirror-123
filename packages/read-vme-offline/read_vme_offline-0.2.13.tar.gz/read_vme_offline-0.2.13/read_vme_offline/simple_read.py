...
def eva_cut_time(filename,  od=0, do=9999999 , chan=1,  tree = False):
    ...
    print("D... x        present:",df['x'].unique() )
    if 'y' in df:
        print("D... y        present:",df['y'].unique() )
    ...
    #---------------------------------------------------DF1 ---------------------------
    #************* reduce the DF
    df1 = general.select_channels(df1, [chan] )
    ...
    #*************
    his = general.column_to_histo(df1['dtus'], savename = outfile, hname = hname, writeondisk = True)
    ...
    #*************
    his = general.column_to_histo(  df1.loc[ (df1.E==0)  ]["time"]  , binmax=1+int(df1.iloc[-1]["time"]), savename = outfile, hname = hname, writeondisk = True)

    #--------------- histogram nonzeroes in time
    hname = general.generate_hname(filename, chan)+"_nzerotime"
    outfile = bname+"nzerotime_ch"+str(chan)+".txt"
    #*************
    his = general.column_to_histo(  df1["time"].loc[ (df1.E!=0)  ]  , binmax=1+int(df1.iloc[-1]["time"]), savename = outfile, hname = hname, writeondisk = True)
