...
A tool for running locust in a distributed fashion. Args that start with '--'
(eg. -f) can also be set in a config file (~/.locust.conf or locust.conf or
~/.swarm.conf or swarm.conf). Config file syntax allows: key=value, flag=true,
...
optional arguments:
  ...
  --selenium            Start selenium server on load gens for use with
                        ...
                        LOCUST_SELENIUM]
  --loadgens LOADGENS   Number of load gen (worker) servers to use [env var:
                        LOCUST_LOADGENS]
...
will also be forwarded to the workers.
```
