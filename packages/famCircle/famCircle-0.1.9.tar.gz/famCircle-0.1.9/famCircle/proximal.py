...
class proximal():
    ...
    def ksrun(self):
        ...
        #     else:
        #         for j in blast[i]:
        #             if gff[j]['chr'] not in lens.keys():
        #                 blast[i] = blast[i].remove(j)
    ...
    def plot_arc_block(self, start, radius):# block
        ...
        plot(x, y, "b-", linewidth=3, alpha=0.5)
    ...
    def run(self):
        ...
        #                 self.plot_bez_inner((chro1, pos1, self.radius_a*0.95), (chro2, pos2, self.radius_a*0.95), "#00a381", total_size,0.8)#绘制内部共线性
            ...
            if id1 in gff.keys():
                for id2 in blast[id1]:
                    if id2 in gff.keys():
                        pos1,pos2 = gff[id1]['end'],gff[id2]['end']
                        chro1,chro2 = gff[id1]['chr'],gff[id2]['chr']
                        if self.position == 'order' and id1 in family and id2 in family:
                            if abs(int(gff[id1][self.position])-int(gff[id2][self.position])) > 1 and abs(int(gff[id1][self.position])-int(gff[id2][self.position])) < 15:
                                proximal_list.append([id1,id2])
                                ...
                                self.plot_bez_inner((chro1, pos1, self.radius_a*0.95), (chro2, pos2, self.radius_a*0.95), "#00a381", total_size,2)#绘制内部共线性
                        if self.position == 'end' and id1 in family and id2 in family:
                            if abs(int(gff[id1][self.position])-int(gff[id2][self.position])) > gene_average and abs(int(gff[id1][self.position])-int(gff[id2][self.position])) < 15*gene_average:
                                proximal_list.append([id1,id2])
                                # self.plot_bez_inner((chro1, pos1, self.radius_a*0.95), (chro2, pos2, self.radius_a*0.95), "#00a381", total_size,2)#绘制内部共线性
                                if id1 not in out_list:
                                    outfile.write(str(id1) + '\n')
                                ...
                                self.plot_bez_inner((chro1, pos1, self.radius_a*0.95), (chro2, pos2, self.radius_a*0.95), "#00a381", total_size,2)#绘制内部共线性
        outfile.close()
        out_other = open(self.outother, 'w', encoding='utf-8')
        for i in family:
            if i not in out_list:
                out_other.write(str(i) + '\n')
            else:
                posi = gff[i]['end'] + self.start_list[gff[i]['chr']]
                start = self.to_radian(posi, total_size)
                self.plot_arc_block(start, self.radius_b + 1 * self.blockthick)# 基因
        out_other.close()
