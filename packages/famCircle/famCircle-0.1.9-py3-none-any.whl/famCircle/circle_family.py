...
class circle_family():
    ...
    def plot_arc(self, start, stop, radius):
        ...
        plot(x, y, '-', color='#ffea00', alpha=.5)# 染色体圆弧
    ...
    def run(self):
        ...
        list000 = []
        list00 = []
        family_list = open(self.family_list, 'r', encoding='utf-8')
        for row in family_list:
            if row != '\n' and row[0] != '#':
                list00.append(readname(str(row.strip('\n').split()[0]),self.species1+self.species2))
        # ['lja1^04212','lja2^00910','lja2^04322','lja2^04914','lja4^01025','lja4^02196','lja5^00060','lja7^01239'
        #   ,'lja8^00488','lja8^01183','lja9^02201','lja9^02588']# utg857g00011,utg857g00009,utg857g00008,utg124g00067
        for lt in blastlist:
            id1, id2 = str(lt[0]),str(lt[1])
            ...
            alp = 0.05
            if id1 in list00 or id2 in list00:
                list000.append([id1, id2])
            else:
                self.plot_bez_inner((order1, pos1, self.radius_a*0.85), (order2, pos2, self.radius_a*0.85), '#e9dfe5', total_size,alp)
                #e9dfe5
        ltx = []
        for lt in list000:
            id1, id2 = str(lt[0]),str(lt[1])
            ...
            order2 = self.labels.index(chro2)
            alp = 0.05
            self.plot_bez_inner((order1, pos1, self.radius_a*0.85), (order2, pos2, self.radius_a*0.85), "#e60033", total_size,1)
            a, b, c = order1, pos1, self.radius_a*0.92
            ex1x, ex1y,rad1 = self.transform_pt0(a, b, c, total_size)
            a, b, c = order2, pos2, self.radius_a*0.92
            ex2x, ex2y,rad2 = self.transform_pt0(a, b, c, total_size)
            # print(rad1,rad2)
            if id1 not in ltx:
                ltx.append(id1)
                text(ex1x, ex1y, str(id1), horizontalalignment="center", verticalalignment="center", fontsize = 2, color = 'black',rotation=(rad1/(2*pi))*360)
            if id2 not in ltx:
                ltx.append(id2)
                text(ex2x, ex2y, str(id2), horizontalalignment="center", verticalalignment="center", fontsize = 2, color = 'black',rotation=(rad2/(2*pi))*360)
                #e9dfe5
