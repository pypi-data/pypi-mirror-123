...
# thanks to you I didn't create it.
...
class ChatBubbleConfig:
    ...
    def ChatBubbleConfig(self):
        ...
        try: self.slots = self.json["slots"]
        except (KeyError, TypeError): pass
        try: self.color = self.json["slots"]
        except (KeyError, TypeError): pass
...
class ChatBubbleConfigList:
    ...
    def ChatBubbleConfigList(self):
        for x in self.json:
            ...
            try: self.slots.append(x["slots"])
            except (KeyError, TypeError): pass
            try: self.color.append(x["slots"])
            except (KeyError, TypeError): pass
...
class ChatBubble:
    def __init__(self, data):
        ...
        try: self.config: ChatBubbleConfig = ChatBubbleConfig(data["config"]).ChatBubbleConfig
        except (KeyError, TypeError): self.config: ChatBubbleConfig = ChatBubbleConfig([])

        self.coverImage = None
        self.isGloballyAvailable = None
        self.bubbleId = None
        self.name = None
        self.modifiedTime = None
        self.ownershipStatus = None
    ...
    def ChatBubble(self):
        ...
        try: self.isGloballyAvailable = self.json["isGloballyAvailable"]
        except (KeyError, TypeError): pass
        ...
        try: self.json_config = self.json["config"]
        except (KeyError, TypeError): pass
        try: self.isNew = self.json["isNew"]
        except (KeyError, TypeError): pass
        try: self.templateId = self.json["templateId"]
        ...
        try: self.comId = self.json["comId"]
        except (KeyError, TypeError): pass
...
class ChatBubbleList:
    def __init__(self, data):
        ...
        self.config: ChatBubbleConfigList = ChatBubbleConfigList(_config).ChatBubbleConfigList
    ...
    def ChatBubbleList(self):
        for x in self.json:
            ...
            try: self.json_config.append(x["config"])
            except (KeyError, TypeError): pass
            try: self.isNew.append(x["isNew"])
            except (KeyError, TypeError): pass
            try: self.templateId.append(x["templateId"])
...
class ReplyMessage:
    def __init__(self, data):
        ...
        try: self.bubble: ChatBubble = ChatBubble(data["chatBubble"]).ChatBubble
        except (KeyError, TypeError): self.bubble: ChatBubble = ChatBubble([])
    ...
    def ReplyMessage(self):
        ...
        try: self.replyJson = self.json["extensions"]["replyMessage"]
        except (KeyError, TypeError): pass
...
class Message:
    def __init__(self, data):
        ...
        try: self.reply: ReplyMessage = ReplyMessage(data["extensions"]["replyMessage"]).ReplyMessage
        except (KeyError, TypeError): self.bubble: ReplyMessage = ReplyMessage([])
...
class MessageList:
    def __init__(self, data, nextPageToken = None, prevPageToken = None):
        _author, _sticker, _bubble, _reply = [], [], [], []
        ...
        for y in data:
            ...
            try: _bubble.append(y["chatBubble"])
            except (KeyError, TypeError): _sticker.append(None)
            try: _reply.append(y["extensions"]["replyMessage"])
            except (KeyError, TypeError): _sticker.append(None)

        self.author: UserProfileList = UserProfileList(_author).UserProfileList
        self.sticker: StickerList = StickerList(_sticker).StickerList
        self.bubble: ChatBubble = ChatBubble(_bubble).ChatBubble
        self.reply: ReplyMessage = ReplyMessage(_reply).ReplyMessage
        self.content = []
...
class AvatarFrame:
    def __init__(self, data):
        ...
        try: self.author: UserProfile = UserProfile(data["operator"]).UserProfile
        except (KeyError, TypeError): self.config: UserProfile = UserProfile([])
        try: self.targetUser: UserProfile = UserProfile(data["targetUser"]).UserProfile
        except (KeyError, TypeError): self.config: UserProfile = UserProfile([])
        ...
        self.userIconBorderColor = None
        self.avatarFramePath = None
        self.avatarId = None
        self.ownershipStatus = None
        self.frameUrl = None
        self.additionalBenefits = None
        self.firstMonthFreeAminoPlusMembershNone = None
        self.restrictionInfo = None
        self.ownerType = None
        self.restrictType = None
        self.restrictValue = None
    ...
    def AvatarFrameList(self):
        ...
        try: self.frameType = self.json["frameType"]
        except (KeyError, TypeError): pass
        try: self.resourceUrl = self.json["resourceUrl"]
        except (KeyError, TypeError): pass
        try: self.md5 = self.json["md5"]
        ...
        try: self.config = self.json["config"]
        except (KeyError, TypeError): pass
        try: self.moodColor = self.json["config"]["moodColor"]
        except (KeyError, TypeError): pass
        try: self.configName = self.json["config"]["name"]
        except (KeyError, TypeError): pass
        try: self.configVersion = self.json["config"]["version"]
        except (KeyError, TypeError): pass
        try: self.userIconBorderColor = self.json["config"]["userIconBorderColor"]
        except (KeyError, TypeError): pass
        try: self.avatarFramePath = self.json["config"]["avatarFramePath"]
        except (KeyError, TypeError): pass
        try: self.avatarId = self.json["config"]["id"]
        except (KeyError, TypeError): pass
        try: self.ownershipStatus = self.json["ownershipStatus"]
        except (KeyError, TypeError): pass
        try: self.frameUrl = self.json["frameUrl"]
        except (KeyError, TypeError): pass
        try: self.additionalBenefits = self.json["additionalBenefits"]
        except (KeyError, TypeError): pass
        try: self.firstMonthFreeAminoPlusMembership = self.json["additionalBenefits"]["firstMonthFreeAminoPlusMembership"]
        except (KeyError, TypeError): pass
        try: self.restrictionInfo = self.json["restrictionInfo"]
        except (KeyError, TypeError): pass
        try: self.ownerType = self.json["restrictionInfo"]["ownerType"]
        ...
        try: self.frameId = self.json["frameId"]
        except (KeyError, TypeError): pass
        try: self.version = self.json["version"]
        except (KeyError, TypeError): pass
        try: self.isNew = self.json["isNew"]
