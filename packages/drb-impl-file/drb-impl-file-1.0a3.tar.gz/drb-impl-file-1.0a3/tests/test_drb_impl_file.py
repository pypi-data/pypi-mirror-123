...
from drb_impl_file.execptions import DrbFileNodeException
...
class TestDrbFileNode(unittest.TestCase):
    ...
    def test_parent(self):
        ...
        self.assertEqual(os.path.basename(os.path.dirname(self.test_path)),
                         parent.name)

        # check if return the same reference/object
        self.assertEqual(parent, node.parent)

        # check for parent of root system node
    ...
    def test_get_named_child(self):
        ...
        self.assertEqual('ab', node[0].name)
    ...
    def test_insert_child(self):
        node = DrbFileNode('/path/to/drb_impl_file')
        child_node = DrbFileNode('/path/to/drb_impl_file/child')
        node.insert_child(1, child_node)
        with self.assertRaises(FileNotFoundError):
            self.assertEqual(node[0], child_node)
    ...
    def test_replace_child(self):
        node = DrbFileNode('/path/to/drb_impl_file')
        child_1 = DrbFileNode('/path/to/drb_impl_file/child')
        node.append_child(child_1)
        child_2 = DrbFileNode('/path/to/drb_impl_file/child')
        with self.assertRaises(DrbException):
            node.replace_child(1, child_2)
        with self.assertRaises(FileNotFoundError):
            node.replace_child(0, child_2)
            self.assertEqual(node[0], child_2)
    ...
    def test_add_attribute(self):
        ...
        with self.assertRaises(NotImplementedError):
            node.add_attribute('add', value=False)
