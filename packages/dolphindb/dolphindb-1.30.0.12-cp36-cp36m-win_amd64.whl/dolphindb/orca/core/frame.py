...
from .common import (MixedTypeWarning, _raise_must_compute_error,
                     _warn_unimplemented_args, default_session)
...
from .utils import (ORCA_COLUMN_NAME_FORMAT, ORCA_INDEX_NAME_FORMAT,
                    _get_python_object_dtype, _infer_axis, _infer_level,
                    _to_column_index, _to_index_map, _to_numpy_dtype,
                    _try_convert_iterable_to_list, _unsupport_columns_axis,
                    check_key_existence, dolphindb_literal_types,
                    dolphindb_numeric_types, dolphindb_temporal_types,
                    get_orca_obj_from_script, is_dolphindb_identifier,
                    is_dolphindb_scalar, is_dolphindb_vector, sql_select,
                    to_dolphindb_literal, to_dolphindb_type_name, _new_orca_identifier)
...
class DataFrame(DataFrameLike, _Frame, IOOpsMixin):
    ...
    def _update_metadata(self, index=None):
        # This function should be called once the columns of a DataFrame is modified
        # This first if/else is used to keep changed name after rolling
        if self._index is not None and self._index.names is not None:
            idx_names=self._index._names
            self._index = Index._from_internal(self._internal, index)
            self._index._names = idx_names
        else:
            self._index = Index._from_internal(self._internal, index)
    ...
    def insert(self, loc: int, column: str, value: Series, allow_duplicates=False):
        if not isinstance(value, Series):
            raise ValueError("value must be orca.Series")
        columns: list = self.columns
        columns = columns.insert(loc, f"{value._var_name}.ORCA_COLUMN_LEVEL_0_ as {column}")
        value._var_name
        script = f"""
        select {",".join(columns)} from {self._var_name}
        """
        var = _InternalFrame.from_run_script(self._session, script)
    ...
    def _binary_op_on_different_indices(self, other, func, axis, ExpressionType=None):    # TODO: add axis check
        """
        ...
        if other._is_dataframe_like:
            ...
            if self_data_columns == other_data_columns:
                if ExpressionType == BooleanExpression:
                    select_list = (f"({self_var_name}.{c} {func} {other_var_name}.{c}) as {c}"
                ...
                if ExpressionType == BooleanExpression:
                    select_list = (f"00f as {s if o is None else o}" if s is None or o is None
                                else f"({self_var_name}.{s} {func} {other_var_name}.{s}) as {s}"
            ...
            #modify the script when doing binary op on a dataframe and a series
            if axis==1:
                index_list, _= _generate_joiner(
                    self._var_name, other._var_name, self._index_columns, other._index_columns, how='left')
                from_clause = self._var_name
            else:
                index_list, from_clause = _generate_joiner(
                    self._var_name, other._var_name, self._index_columns, other._index_columns)
            if ExpressionType == BooleanExpression:
                select_list = (f"({self_var_name}.{c} {func} {other_var_name}.{other_data_column}) as {c}"
                               for c in self_data_columns)
            else:
                #modify the script when doing binary op on a dataframe and a series
                if axis==1:
                    sel_len=max(len(self_data_columns),len(other._index.values))
                    max_len=len(self_data_columns)
                    #the else part add empty columns into the dataframe
                    select_list = (f"{func}({self_var_name}.(columnNames({self_var_name})[{i+1}]), {other_var_name}.{other_data_column}[{i}]) as {self_data_columns[i]}" if i < max_len
                            else f"{other_var_name}.{other_data_column}[{len(other._index.values)}] as {other._index.values[i]}" for i in range(sel_len))
                    for i in range(sel_len):
                        if i >=len(self_data_columns):
                            self_data_columns.append(other._index.values[i])
                else:
    ...
    def _get_columns(self, key):
        # consider Muiltiindex
        #TODO: ORCA-11not finished
        if isinstance(self.columns,pd.MultiIndex):
            new_key=[]
            for i in self.columns.values:
                if key in i:
                    new_key.append("_".join(i))
            key=new_key
        new_odf = self._internal[key]
        if isinstance(self.index,DatetimeIndex) :
            new_odf._index_freq=self._index._freq
    ...
    def stack(self, level=-1, dropna=True):
        if self._column_index_level != 1:
            raise ValueError("multi-level columns are not supported. ")
    ...
    def xs(self, key, axis=0, level=None, drop_level=True):
        ...
        if level is not None:
            if isinstance(level,int):
                new_index_col=np.array(self._index_columns)[[level]]
            elif isinstance(level,str):
                new_index_col=np.array(level)
            elif not is_dolphindb_vector(level):
                raise TypeError(f"{level} is an invalid level")
            #when level is a dolphindb vector
            else:
                for i in level:
                    new_index_col.append(self._index_columns[i]) if isinstance(i,int) else new_index_col.append(i)
            where_list = (
                f"{var_name}.{i} == {to_dolphindb_literal(k)}"
                for k, i in zip(key, new_index_col)
            )
        else:
            where_list = (
                f"{var_name}.{i} == {to_dolphindb_literal(k)}"
        ...
        if drop_level:
            return res.droplevel(range(len(key))) if level is None else res.droplevel(new_index_col)
        else:
            return res
    ...
    def to_csv(
        ...
        if engine == "python":
            ...
            if (na_rep != "" or
                    float_format is not None or
                    columns is not None or
                    header is not True or
                    index is not True or
                    index_label is not None or
                    mode != "w" or
                    encoding is not None or
                    compression != "infer" or
                    quoting is not None or
                    quotechar != '"' or
                    line_terminator is not None or
                    chunksize is not None or
                    date_format is not None or
                    doublequote is not True or
                    escapechar is not None or
                    decimal != "."):
                _warn_unimplemented_args("to_csv")
    ...
    def panel(self, xlabel: str, ylabel: str, columns=List[str]) -> List["DataFrame"]:
        s = self._session
        var = self._var_name
        var_with_where = _ConstantSP.ORCA_IDENTIFIER + _new_orca_identifier()
        from .utils import _get_where_list
        where_content = "where " + ",".join(_get_where_list(self._where_expr)) if self._where_expr is not None else ""
        s.run(f"{var_with_where} = select { ', '.join([xlabel, ylabel] + [col for col in columns]) } from {var} {where_content}")
        keys = "".join(["`"+col for col in columns])
        plist = ", ".join([ f"(select {col} from {var_with_where}).col(0)" for col in columns])
        new_vars = []
        ret_vars = []
        for col in columns:
            obj_id = _new_orca_identifier()
            new_var_name = _ConstantSP.ORCA_IDENTIFIER + obj_id
            new_vars.append(new_var_name)
            ret_vars.append(f"{new_var_name} = table(m as {xlabel}, d['{col}'])")
        ret_script = "; ".join(ret_vars)
        script = f"""
            d = dict({keys},  panel((select {xlabel} from {var_with_where}).col(0), (select {ylabel} from {var_with_where}).col(0), [{plist}]))
            m = select distinct({var_with_where}.{xlabel}) as {xlabel} from {var_with_where} order by {xlabel}
            {ret_script}
        """
        s.run(script)
        panels = []
        from .generic import read_in_memory_table
        for v in new_vars:
            panels.append(read_in_memory_table(table_name=v, index_column=xlabel))
        return panels
