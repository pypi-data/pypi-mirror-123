...
def validate(args, config):
    ...
    try:
        region = config.get(long_term_name, 'region')
    except NoSectionError:
        log_error_and_exit(logger,
                           "Long term credentials section '[%s]' is missing. "
                           "You must add this section to your credentials file "
        ...
        logger.debug('region option not found for profile')
    ...
    if should_refresh:
        get_credentials(short_term_name, key_id, access_key, region, args, config)
...
def get_credentials(short_term_name, lt_key_id, lt_access_key, lt_region, args, config):
    ...
    # Save region individiually, so it can be optional 
    if (lt_region != ""):
        config.set(
            short_term_name,
            'region',
