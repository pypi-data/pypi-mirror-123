...
from cloudrail.cli.error_messages import generate_convert_terraform_plan_to_json_failure_message, \
    generate_process_plan_json_failure_message, generate_simple_message, generate_unknown_aws_region_failure_message
...
from cloudrail.cli.service.evaluation_run_command.evaluation_run_command_service import BaseRunCommandParameters, EvaluationRunCommandService
...
class TerraformEvalRunCmdService(EvaluationRunCommandService[Union[TerraformRunCommandParameters, GenerateFilteredPlanCommandParameters]]):
    ...
    def _set_aws_default_region(self, plan_json_path: str, aws_default_region: Optional[str]):
        ...
        aws_terraform_utils.default_region = aws_default_region or os.environ.get('AWS_DEFAULT_REGION', os.environ.get('AWS_REGION'))

        for provider_key, provider_config in provider_configs.items():
            region = aws_terraform_utils.get_provider_region(provider_key)
            if not region:
                provider_module = provider_config.get('module_address')
                if provider_key == 'aws':
                    if not result_obj['configuration']['root_module']['resources']:
                        continue
                elif provider_module and \
                    all(module_address != provider_module for x in result_obj['resource_changes'] if (module_address := x.get('module_address'))):
                    continue
                echo_error(generate_unknown_aws_region_failure_message())
                self._exit_on_failure(ExitCode.CONTEXT_ERROR)
