...
class CloudrailApiClient(ExternalApiClient):
    ...
    ASSESSMENT_CREATE: str = '/v1/assessments'
    ...
    ASSESSMENTS_LIST_PATH: str = '/v1/assessments'
    ASSESSMENTS_GET_PATH: str = '/v1/assessments/{0}'
    RULES_TASKS_PATH: str = '/ui/task_center/rules/tasks'
    RULES_TASK_RESULTS_PATH: str = '/ui/task_center/rules/{0}/results'
    ...
    def list_assessment_results(self,
        ...
        path = self.ASSESSMENT_RESULTS_PATH.format(assessment_id)
    ...
    def list_assessments(self,
        ...
        path = self.ASSESSMENTS_LIST_PATH
    ...
    def list_rules_tasks(self, api_key: str, query: Optional[str] = None):
        params = {'query': query or ''}
        path = self.RULES_TASKS_PATH
        api_result = self._send_request(lambda: self.get(path=path, params=params), api_key, 'list rule tasks')
    ...
    def get_rules_task_results(self, api_key: str, rule_id: str):
        ...
        api_result = self._send_request(lambda: self.get(path=path), api_key, 'get rule task results')
    ...
    def list_assessment_filters(self, api_key: str):
        api_result = self._send_request(lambda: self.get(path=self.ASSESSMENT_FILTERS_PATH), api_key, 'get assessment filters')
        return self._parse_api_result(api_result, lambda x: [FilterBlockDTO.from_dict(block) for block in x])

    def list_assessment_results_filters(self, api_key: str):
        api_result = self._send_request(lambda: self.get(path=self.ASSESSMENT_RESULTS_FILTERS_PATH), api_key, 'get rule results filters')
        return self._parse_api_result(api_result, lambda x: [FilterBlockDTO.from_dict(block) for block in x])
