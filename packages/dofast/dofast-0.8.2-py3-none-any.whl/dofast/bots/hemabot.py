...
class Psycho:
    ...
    def text_handler(self, update: Update, context: CallbackContext) -> None:
        ...
        if api.textparser.match(['weather', '天气'], text):
            ...
            chat_id = update.message.chat_id
            today = self.axe.today
            due = today + 'T' + '21:45'
            update.message.reply_text(
                'Msg received, alert at {}'.format(due))
            period = self.axe.diff(self.axe.now, due, seconds_only=True)
            context.job_queue.run_once(self.alarm,
                                       period,
                                       context=chat_id,
                                       name=str(chat_id))
    ...
    def photo_handler(self, update: Update, context: CallbackContext) -> None:
        ''' save phones to cloud
        # https://stackoverflow.com/questions/50388435/how-save-photo-in-telegram-python-bot
        '''
        file_id = update.message.photo[-1].file_id
        cf.info(file_id)
        obj = context.bot.get_file(file_id)
        file_name = str(random.randint(11, 99))
        obj.download(file_name)
        dofast.api.bucket.upload(file_name)
        cf.io.rm(file_name)
        update.message.reply_text(f"Image {file_name} received and uploaded.")
    ...
    def main() -> None:
        ...
        dispatcher.add_handler(CommandHandler("start", psy.start))
        dispatcher.add_handler(CommandHandler("help", psy.start))
        ...
        dispatcher.add_handler(MessageHandler(Filters.photo, psy.photo_handler))
