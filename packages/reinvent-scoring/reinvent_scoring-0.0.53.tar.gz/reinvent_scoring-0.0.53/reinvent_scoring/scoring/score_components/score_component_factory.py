...
from reinvent_scoring.scoring.score_components import TanimotoSimilarity, \
    ...
    LinkerLengthRatio, LinkerGraphLength, LinkerEffectiveLength, DockStream
...
class ScoreComponentFactory:
    ...
    def create_score_components(self) -> [BaseScoreComponent]:
        def error_message(key):
            raise KeyError(f'Component: {key} is not implemented. Consider checking your input.')

        components = [self._current_components.get(p.component_type, error_message(p.component_type))(p)
