...
class ConfigFile:
    ...
    def sleep(seconds: float, result: Any) -> Any:
        time.sleep(seconds)
        return result
...
class Eaves:
    def __init__(self) -> None:
        ...
        self._before_startup: Optional[Callable] = None
    ...
    def handler(self, name: str, func_before_running: Optional[Callable[[list[Event]], Any]] = None) -> Callable:
        def inner(func: Callable[[list[Event]], Any]) -> Callable:
            if func_before_running:
                func = lambda event: func(func_before_running(event)) # type: ignore
    ...
    def render_text(
        ...
        self.screen.blit(source, location)
    ...
    def run(
        ...
        # set screen
        if not screen_size:
            res = self.get_current_screen_size()
        else: 
            res = screen_size
        
        self.screen = display.set_mode(res, window_type)
