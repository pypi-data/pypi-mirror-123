...
class static_trajectory(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr
...
class trajectory(generalized_trajectory):
    ...
    def __init__(self, *args):
        _autopysta.trajectory_swiginit(self, _autopysta.new_trajectory(*args))

    def add(self, nuevo):
        return _autopysta.trajectory_add(self, nuevo)
...
class road_object(object):
    ...
    t = property(_autopysta.road_object_t_get, _autopysta.road_object_t_set)
...
def _autopystagraph(self, lane=-1):
	...
	pt.plot(tt,xx,colors[trys[0][0].LANE()])
	...
	pt.show()
results.plotresults = _autopystagraph
