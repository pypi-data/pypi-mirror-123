...
class LatentClassModelMixed(MixedLogit):
    ...
    def _compute_probabilities(self, betas, X, y, panel_info, draws, drawstrans,
        ...
        # test = np.sum(y*p, axis=2, dtype=np.float64)
        p = y*p
    ...
    def _make_short_df(self):
        """Make an shortened dataframe average over alts, used in latent
        class estimation.  # TODO? better descriptions
        """
        k = len(self.varnames)
        # N = self.
        ...
        if self.fit_intercept:
            short_df = np.zeros((self.N, k+(self.J-1)))
    ...
    # def _set_mixedlogit_vars(self, class_num):
    #     self.Kf = sum(class_fxidxs[class_num])
    #     self.Kr = sum(class_rvidxs[class_num])
    #     self.Kbw = sum(class_rvidxs[class_num])  # TODO! CHANGE WHEN ADD CORR IN
    #     self.fxidx = class_fxidxs[class_num]
    #     self.fxtransidx = class_fxtransidxs[class_num]
    #     self.rvidx = class_rvidxs[class_num]
    #     self.rvtransidx = class_rvtransidxs[class_num]
    ...
    def _expectation_maximisation_algorithm(self, tmp_fn, tmp_betas, args,
        ...
        if class_thetas is None:
            # class_thetas = np.stack([np.repeat(.0, len(self.member_params_spec[0]))  # TODO? -> isvars / more flexible
            #                          for _ in range(1, self.num_classes)], axis=0)  # class member ship probability
        ...
        while not converged and iter_num < max_iter:
            ...
            self._set_bw(class_idxs[0])  # TODO: make clear also sets correlation
            ...
            for class_i in range(1, self.num_classes):
                ...
                # self.Kbw = sum(class_rvidxs[class_i])  # TODO! CHANGE WHEN ADD CORR IN
                # self._set_bw(X_class_idx)
                ...
                self._set_bw(class_idxs[class_i])  # TODO: make clear also sets correlation
            ...
            for s in range(0, self.num_classes):
                ...
                # self.Kbw = sum(class_rvidxs[s])  # TODO! CHANGE WHEN ADD CORR IN
                self._set_bw(class_X_idx)
