...
class GSDBS:
    ...
    def getAccessToken(self):
        if self.accessToken["expires_in_datetime"] < datetime.today():
            self.refreshToken()
    ...
    def refreshToken(self):
        data = {'grant_type': 'refresh_token', 'refresh_token': self.accessToken['refresh_token']}
        self.getAccessTokenFromApi(data)

    def getAccessTokenFromApi(self, data):
        refresh_token_response = requests.post(self.credentials['oauthurl'],
                                               data=data,
                                               verify=True,
                                               allow_redirects=False,
                                               auth=(
                                                   self.credentials['client_id'], self.credentials['client_secret']))
        self.accessToken = json.loads(refresh_token_response.text)
        self.accessToken["expires_in_datetime"] = datetime.today() + timedelta(
            seconds=self.accessToken['expires_in'])
    ...
    def prepareClient(self):
        api_call_headers = {'Authorization': 'Bearer ' + self.getAccessToken()}
        return Client(
            transport=AIOHTTPTransport(url=self.credentials['graphqlapiurl'], headers=api_call_headers),
            execute_timeout=40)
