...
class VswmcClient(object):
    ...
    def login(self, credentials):
        ...
        data = {'X-OpenAM-Username': credentials.username, 'X-OpenAM-Password': credentials.password}
        ...
        res = self.session.post(SSO_URL, headers=data)

        if 'Authentication Failed' in res.text:
            print("res.text: Authentication Failed")
            raise Unauthorized('Authentication Failed')
        if 'Maximum Sessions' in res.text:
            print("res.text: Maximum Sessions")
            raise MaximumSessionsReached('Maximum sessions reached or session quota has exhausted')

        parts = res.text.split('"')
        self.session.cookies.set("iPlanetDirectoryPro", parts[3])
        self.session.cookies.set("esa-ssa-sso-cookie", parts[3])
