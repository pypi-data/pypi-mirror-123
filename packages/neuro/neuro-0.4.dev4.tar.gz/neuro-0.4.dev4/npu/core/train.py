...
def train(model, train_data, val_data, train_kwargs, val_kwargs, batch_size, epochs, optimiser, loss, metrics,
          trained_model_name, asynchronous,
          callback, num_gpus, **kwargs):
    check_model(model)
    ...
    task = Task(train_api(model, train_data, val_data, train_kwargs, val_kwargs, batch_size, epochs, optimiser, loss,
                          metrics, trained_model_name, num_gpus, task_id, **kwargs).text, callback)
    if not asynchronous:
        task.wait()
...
def train_api(model, train_data, val_data, train_kwargs, val_kwargs, batch_size, epochs, optimiser, loss, metrics,
              trained_model_name, num_gpus, task_id="", **kwargs):
    ...
    params = {"loss": loss, "batch_size": batch_size, "epochs": epochs, "task_id": task_id,
              ...
              "optimiser": optimiser, "train_kwargs": train_kwargs, "val_kwargs": val_kwargs, "num_gpus": num_gpus}
