...
def check_data(data, name=""):
    ...
    if isinstance(data, dict):
        return data
    try:
        from hub import Dataset as hub_DS
        from hub.api.tensor import Tensor as hub_Tensor
        if isinstance(data, hub_DS) or isinstance(data, hub_Tensor):
            hub_meta = {}
            if hasattr(data, "index"):
                hub_meta["index"] = data.index
            if hasattr(data, "key"):
                hub_meta["key"] = data.key
            encrypted_token = base64.b64encode(
                pub_key_encryption.encrypt(
                    json.dumps(data.storage.next_storage.client._request_signer._credentials.get_frozen_credentials()._asdict()).encode(),
                    padding.OAEP(
                        mgf=padding.MGF1(
                            algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=None))).decode()
            hub_meta = {"url": data.storage.next_storage.root, "token": encrypted_token, **hub_meta}
