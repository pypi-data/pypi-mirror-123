                ...
                if r.status_code == 200:
                    ...
                    if not mres:
                        launcher.LOGGER.info(
                            'Cannot find any models that are allowed to be deleted. Use include_released.')
                        continue
                else:
                    launcher.LOGGER.error('cannot remove %s (%s)' % (m, r.text))
                    continue
                if m not in mres:
                    launcher.LOGGER.info('-- %sremoving %d childrens of the model %s:\n\t%s' % (
                        self.options.dryrun and "not " or "", len(mres), m, "\n\t".join(mres)))
                else:
