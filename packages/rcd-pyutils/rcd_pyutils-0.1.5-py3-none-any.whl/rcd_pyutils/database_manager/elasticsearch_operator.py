...
def index_json_bulk_parallel(index: str, method: str, **kwargs: Any) -> None:
    """
        ...
        Kwargs:
            ...
            pd_dataframe(pd.Dataframe): Only for method="dataframe", pandas dataframe object to index.
        Examples:
            ...
            >>> database_manager.index_json_bulk_parallel(index="my_index", method="dataframe", pd_dataframe=pd.DataFrame())
    """
    ...
    if method == "json":
        ...
        eo.pd_dataframe = kwargs.get("pd_dataframe")
    else:
    ...
    while True:
        try:
            ...
            while request_timeout <= 1200:
                request_timeout = int(input("Enter your request_timeout setting: (integer > 1200)"))
            print(f"Restarting with time_out {request_timeout}.")


def index_json_bulk(json_path: str, index: str, keyword: str = "", **kwargs: Any) -> None:
    """
...
def index_json(json_path: str, index: str, keyword: str = "", show_debug: bool = True, **kwargs: Any):
    """
        Function index_json.
...
class ElasticsearchOperator:
    ...
    def __init__(self, index: str) -> None:
        self.connection = Elasticsearch(
            ...
            http_auth=(os.environ.get("ELASTICSEARCH_USER"), os.environ.get("ELASTICSEARCH_PASSWORD")),
    ...
    def detect_json(self, keyword: str) -> None:
        self.lst_json = [file for file in os.listdir(self.json_path) if
                         (file.endswith(".json")) and (not file.startswith(".")) and (keyword in file)]
    ...
    def index_json(self, json_file: str, show_debug: bool = True) -> None:
        ...
        with open(os.path.join(self.json_path, json_file)) as f:
            ...
            publish = self.connection.index(index=self.index, doc_type='_doc', id=document_id, body=reader,
    ...
    def df_generator(self) -> Generator:
        gen_tuple_json = zip(self.pd_dataframe.index.tolist(), self.pd_dataframe.itertuples(index=False))
        for json_id, json_content in gen_tuple_json:
            dict_json = json_content._asdict()
            dict_json["@timestamp"] = str(datetime.now())
            yield {
                "_id": json_id,
                "_source": dict_json,
                "_index": self.index
    ...
    def get_generator(self):
        if len(self.lst_json) != 0:
            ...
            raise ValueError(f"Define at least one of the following: lst_json or pd_dataframe")

    @decorator_manager.timeit(program_name="Bulk indexing")
    def bulk_index(self) -> None:
        bulk(client=self.connection, actions=self.get_generator(), request_timeout=self._request_timeout)

    @decorator_manager.timeit(program_name="Parallel Bulk indexing")
    def parallel_bulk_index(self) -> None:
        for success, info in parallel_bulk(client=self.connection, actions=self.get_generator(),
                                           request_timeout=self._request_timeout):
            if not success:
                print(info)