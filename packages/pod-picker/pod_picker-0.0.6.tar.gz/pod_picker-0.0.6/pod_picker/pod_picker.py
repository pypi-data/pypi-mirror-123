...
def pod_model_from_dependencies(pod_name, dependency_list) -> pod_picker.pod_model.PodModel:
    pm = pod_picker.pod_model.PodModel()
    for dep_name in dependency_list:
        prefix = '{pod_name} ('.format(pod_name=pod_name)
        if not dep_name.startswith(prefix):
            continue
        pm.name = pod_name
        pm.version = pod_version_from_dependency(dep_name)
...
def fetch_steady_pod_map_list(pod_list, full_pod_map):
    ...
    for pod_name in pod_list:
        if not isinstance(pod_name, str):
            continue
        pm = pod_model_from_dependencies(pod_name, dependency_list)
        if len(pm.name) == 0:
            continue
