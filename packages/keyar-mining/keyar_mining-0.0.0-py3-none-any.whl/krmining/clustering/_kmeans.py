...
class KMeans:
    ...
    def fit(self, data):
        ...
        for i in range(self.n_iters):
            ...
            clusters = np.where(np.array(distance_centroid).T 
    ...
    def __init_kmeans(self, data):
        ...
        for i in range(self.k - 1):
            ...
            nearest_cluster = np.where(np.array(distances).T 
            ...
            dup_indexes = [np.where(indexes == index)[0] for index in np.unique(indexes)]
            ...
            indexes_distances = np.array([d for distance in indexes_distances for d in distance])
