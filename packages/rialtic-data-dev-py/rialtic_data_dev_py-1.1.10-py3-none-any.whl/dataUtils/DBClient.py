        ...
        with open(os.path.join(os.getcwd(), filename), 'r') as history_file:
            ...
            for hc in history_list:
                ...
                if history_claim.transaction_id == transactionId:
                    ...
                    for clue in claim.item:
                        clue = ClaimItem.parse_obj(clue)
                        period = Period.parse_obj(clue.servicedPeriod)
                        begin = period.start
                        end = period.end
                        if start_date <= begin <= end_date or \
                                start_date <= end <= end_date or \
                                begin <= start_date <= end or \
                                begin <= end_date <= end:
