...
def send_http_post(route, data, is_module=False, ip="localhost", port=config.data_server_port):
    result = None
...
def send_http_get(route, param_dict,ip="localhost", port=config.data_server_port):
    url = "http://{}:{}/{}?".format(ip, port, route)
