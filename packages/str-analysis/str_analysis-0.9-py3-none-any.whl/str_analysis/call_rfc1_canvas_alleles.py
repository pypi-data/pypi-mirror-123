...
def parse_args():
    ...
    p.add_argument("--run-expansion-hunter-denovo", action="store_true", help="Optionally run ExpansionHunterDenovo "
        "and copy information relevant to the RFC1 locus from ExpansionHunterDenovo results to the output json.")
    ...
    p.add_argument("--expansion-hunter-denovo-profile", help="Optionally copy information relevant to the RFC1 locus "
...
def run_expansion_hunter_denovo(
        sample_id,
        expansion_hunter_denovo_path,
        reference_fasta_path,
        bam_or_cram_path,
        output_dir=".",
        verbose=False,
        ...
        print("--"*10)
        print(f"Running ExpansionHunterDenovo on {sample_id}")
        output_prefix = f"{os.path.join(output_dir, sample_id)}.expansion_hunter_denovo"
        expansion_hunter_denovo_command = f"""{expansion_hunter_denovo_path} profile \
--reference {reference_fasta_path} \
--reads {bam_or_cram_path} \
--output-prefix {output_prefix} \
"""
...
def main():
    ...
    if args.run_expansion_hunter_denovo:
        args.expansion_hunter_denovo_profile = run_expansion_hunter_denovo(
            args.sample_id,
            args.expansion_hunter_denovo_path,
            args.reference_fasta,
            args.bam_or_cram_path,
            verbose=args.verbose,
        )
