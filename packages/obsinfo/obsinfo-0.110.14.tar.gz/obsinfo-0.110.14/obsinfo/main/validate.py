...
class ValidateObsinfo():
    ...
    def validate_instrumentation(self, info_file):
        ...
        ret = ObsMetadata().validate(self.datapath.validate_path, info_file,
                                     ...
                                     self.verbose, "instrumentation.schema.json", False)
...
def main():
    ...
    try:               
        ...
        print(f"Illegal format: fields may be missing or with wrong format in input file, or there is a programming error: {str(e)}")
...
def retrieve_arguments():
    ...
    parser_args.add_argument("-s", "--schema", 
                             help="Force validation against given schema") 
    
    parser_args.add_argument("-r", "--remote",  action='store_true', default=False,
