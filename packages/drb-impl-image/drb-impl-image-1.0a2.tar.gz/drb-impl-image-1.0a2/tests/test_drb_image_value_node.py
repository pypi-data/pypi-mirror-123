...
class TestDrbValueNodeImage(unittest.TestCase):
    ...
    def test_value_attributes(self):
        ...
        node_value = root_node[DrbImageNodesValueNames.FORMAT.value][0]
    ...
    def test_value_not_implemented(self):
        node = self.open_node(str(self.image_tif_one))
        ...
        with self.assertRaises(DrbException):
            node_value.replace_child(1, root_node)

        with self.assertRaises(AttributeError):
            node_value.insert_child(1, root_node)

        with self.assertRaises(AttributeError):
            node_value.append_child(root_node)

        with self.assertRaises(DrbException):
            node_value.replace_child(0, root_node)

        with self.assertRaises(DrbException):
            node_value.remove_child(0)
    ...
    def test_variable_get_first_last_children(self):
        ...
        self.assertEqual(node_value.children, None)
