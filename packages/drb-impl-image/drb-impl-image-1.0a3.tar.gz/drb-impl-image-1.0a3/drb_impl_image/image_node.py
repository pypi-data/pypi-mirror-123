...
class DrbImageNode(DrbLogicalNode):
    ...
    def __init__(self, parent: DrbNode):
        path = parent.path / DrbImageNodesValueNames.IMAGE.value
    ...
    def _get_file_impl(self):
        if self._data_set_file_source is None:
            if self.parent.has_impl(io.BufferedIOBase):
                self._data_set_file_source = self.parent\
