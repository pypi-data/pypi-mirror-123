...
class AssetManagerClient(AssetManagerClientCache):
    ...
    def put_file(
        ...
        """
        large_size_bytes = 100 * 1024 * 1024 #100 mb
    ...
    def sum(self, field_name: str, search: Optional[dict] = None) -> float:
        '''
        Args:
            field_name: the field to do sum over it
            search: search criteria for the assets
    ...
    def put_asset(self,
        ...
        if self.asset_type in AssetsConstants.METADATA_ONLY_ASSETS:
            ...
            if url:
                ...
                self.put_file(file_object=file_object, id=id, content_type=content_type)
        ...
        if not resp.ok:
            raise AssetError(
                'Could not get search fields::: {} -- {} -- {}'.format(
                    self.asset_type, resp.reason, resp.content))
        return [
            d for d in resp.json() if d['field'].startswith(
                AUXILIARY_KEY + ".")]
