...
class BitmexOrder:
    ...
    def set_position_closed(self, symbol, dir, currentPrice=False):
        ...
        if self.show_slippage and self.props[symbol]['initialSL']:
            acc_pos_size = float(self.settings.symbols[symbol]['position_size'])/100
            ...
            acc_balance_diff = self.entry_acc_balance[symbol] - current_acc_balance + between_profs
            
            self.between_profits[symbol] += acc_balance_diff # in acc balance difference

            initial_risk_points = abs(self.props[symbol]['initialSL'] - self.props[symbol]['entry'])
            pnl_rr = pnl_points / initial_risk_points
            
            pnl_rel_expected = self.entry_acc_balance[symbol] * (1 + (pnl_rr * acc_pos_size))
            pnl_rel_real = self.entry_acc_balance[symbol] * (1 + (acc_balance_diff / self.entry_acc_balance[symbol]))

            # slippage is real acc balance change relative - exepceted pnl percent
            print("Slippage: {}%".format(round((pnl_rel_real - pnl_rel_expected) * 100,2)),flush=True)
            self.entry_acc_balance[symbol] = current_acc_balance
    ...
    def manage_entries(self, symbols, me_count=0, candlePrice=''):
        ...
        if me_count and me_count % 3 == 0:
            if self.props[symbol]['qty'] > 0:
                ...
                self.manage_entries(self._symbols, candlePrice=cp)
            elif self.props[symbol]['qty'] < 0:
                candlePrice = self.client.get_current_price_candle(symbol)
                self.manage_entries(self._symbols, candlePrice=cp)
    ...
    def calc_pos_size(self, symbol, sl_distance):
        ...
        self.entry_acc_balance = acc_balance
    ...
    def bracket_stop_order(self, symbol, orderQty, entry, sl, tp=0):
        try:
            if self.props[symbol]['qty'] != 0:
                # order is open (shouldn't be) > close old order first
                self.close(symbol)
                time.sleep(1)
