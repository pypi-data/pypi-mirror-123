...
class MobioScheduler:
    ...
    def __init__(self, root_node="factory_schedule_cart", node_id=None, nop=100, delays=1):
        if nop > 1000:
            raise Exception("nop maximum is 1000")
        if delays > 3600:
        ...
        self.zk_client = KazooClient(hosts=os.getenv("ZOOKEEPER_CLUSTER", "localhost:2181"))
        ...
        child_watch = ChildrenWatch(client=self.zk_client, path=self.root_path, func=self.watch_children)
        data_watch_root = DataWatch(client=self.zk_client, path=self.root_path, func=self.watch_data_root)
        data_watch_child = DataWatch(client=self.zk_client, path=self.my_node_path, func=self.watch_data_children)
        try:
    ...
    def watch_children(self, lst_children):
        ...
        for i in range(num_childs):
            ...
            self.zk_client.set("{}/{}".format(self.root_path, lst_children[i]), value=value)
        print("set data done")
