...
from azureml.automl.core.shared._diagnostics.automl_error_definitions \
    import (AllowedModelsSubsetOfBlockedModels, AllAlgorithmsAreBlocked, ConflictingFeaturizationConfigDroppedColumns,
            ...
            FeaturizationConfigForecastingStrategy, InvalidArgumentForTask, InvalidArgumentWithSupportedValues,
            InvalidArgumentType, InvalidInputDatatype, NonDnnTextFeaturizationUnsupported, NotSupported)
...
class AutoMLBaseSettings:
    ...
    def __init__(self,
        ...
        if self.task_type != constants.Tasks.CLASSIFICATION and self.positive_label is not None:
            raise ConfigException._with_error(
                AzureMLError.create(
                    InvalidArgumentForTask, target="positive_label",
                    arguments=self.positive_label, task_type=self.task_type
                )
            )
        ...
        if is_timeseries:
            ...
            if self.n_cross_validations is None and self.cv_step_size is not None:
                logging.warning(
                    "User has defined 'cv_step_size' ({}) while not speficifying 'n_cross_validations'. "
                    "Will proceed with train/valid and ignore 'cv_step_size'.".format(self.cv_step_size)
                )
    ...
    def _verify_settings(self):
        ...
        if self.experiment_timeout_minutes is not None and \
            ...
            raise ConfigException._with_error(
                AzureMLError.create(
                    ArgumentOutOfRange, target="experiment_timeout_hours",
                    argument_name="experiment_timeout_hours", min=AutoMLBaseSettings.MIN_EXPTIMEOUT_MINUTES / 60,
    ...
    def _get_private_model_names(self):
        ...
        if self.task_type == constants.Tasks.CLASSIFICATION:
            private_model_names = list(set([model.customer_model_name for model in
                                            _PrivateModelNames.PrivateClassificationModelList]))
        elif self.task_type == constants.Tasks.REGRESSION:
            private_model_names = list(set([model.customer_model_name for model in
                                            _PrivateModelNames.PrivateRegressionModelList]))
        if self.is_timeseries:
            private_model_names = list(set([model.customer_model_name for model in
                                            _PrivateModelNames.PrivateForecastingModelList]))
    ...
    def _validate_ts_featurization_transform_params(
        ...
        if transformer_params is not None and len(transformer_params) > 0:
            if SupportedTransformers.Imputer not in transformer_params:
                raise ConfigException._with_error(
                    AzureMLError.create(
                        NotSupported, target="featurization_config",
    ...
    def _validate_ts_featurization_strategies_columns(
        ...
        if all_cols:
            self._validate_none_target_agg_func(
                "featurization config non label column name customized imputation")
