...
class ReleaseArtifactUploader(Uploader):
    ...
    def ui_url(self) -> str:
        ...
        is_delta = isinstance(info.version, DeltaReleaseVersion)
        ...
        if info.software_type == "android-build":
            ...
            return delta_releases_url if is_delta else releases_url

        if is_delta:
            from_version = url_quote(info.version.delta_from)
            target_version = url_quote(info.version.delta_target)
            return f"{delta_releases_url}/{from_version}/{target_version}"
