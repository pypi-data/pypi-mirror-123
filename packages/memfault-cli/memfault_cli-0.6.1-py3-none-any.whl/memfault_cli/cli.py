...
def upload_aosp_symbols(ctx: MemfaultCliClickContext, path: str, **kwargs):
    ...
    _do_upload_or_raise(
        ctx, path, AospSymbolDirectoryUploader if os.path.isdir(path) else AospSymbolUploader
    )
...
@click.option(
    "--delta-from",
    required=False,
    help="Pass --delta-from FROM_VERSION make the OTA Payload a Delta Release payload from the version FROM_VERSION. Use alongside --delta-target TARGET_VERSION",
)
@click.option(
    "--delta-target",
    required=False,
    help="Pass --delta-target TARGET_VERSION to make the OTA Payload a Delta Release payload target version TARGET_VERSION. Use alongside --delta-from FROM_VERSION.",
)
...
def upload_ota_payload(ctx: MemfaultCliClickContext, path: str, **kwargs):
    ...
    ctx.check_required_either(
        {"software_version"}, {"delta_from", "delta_target"}, mutually_exclusive=True
    )
...
@click.option("--delta-target", required=False)
...
def deploy_release(ctx: MemfaultCliClickContext, cohort: str, rollout_percent: int, **kwargs):
    ...
    ctx.check_required_either(
        {"release_version"}, {"delta_from", "delta_target"}, mutually_exclusive=True
    )
    release_version = ctx.obj.get("release_version") or (
        ctx.obj.get("delta_from"),
        ctx.obj.get("delta_target"),
    )
