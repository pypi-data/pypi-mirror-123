...
def generate_statistics(df, indicator_dic):
    ...
    statistics = OrderedDict([
        (k, v) for k, v in daily_stats_dic.items() if k in (
            ...
            "total_return", "annual_return_rate", "annualized_return", "daily_return", "return_std",
...
def calc_statistic_by_pnl_s(indicator_by_trade_dic, capital, stats_group=None, df: DataFrame = None) -> dict:
    """根据每日盈亏进行相关收益指标统计"""
    ...
    annual_return_rate = np.round(stats.cagr * 100, 2)  # total_return / total_days * 240  # 此处vnpy，原始函数计算有错误
    ...
    return_most_drawdown_ratio = np.round(-annual_return_rate / most_dd_pct, 2)
    ...
    # 手数相关指标
    total_volume = sum([_["total_volume"] for _ in indicator_by_trade_dic.values()])
    profit_volume = sum([_["profit_volume"] for _ in indicator_by_trade_dic.values()])
    loss_volume = sum([_["loss_volume"] for _ in indicator_by_trade_dic.values()])
    # 盈利比率： 盈利手数 / 总交易手数
    profit_rate = profit_volume / total_volume
    # 平均利润： 净利润 / 交易手数
    avg_pnl = total_net_pnl / total_volume
    # 平均盈利： 总盈利 / 盈利交易手数
    avg_profit = total_profit / profit_volume
    # 平均亏损：总亏损金额 / 亏损交易手数
    avg_loss = total_loss / loss_volume
    # 盈利因子：  总利润 / 总亏损
    profit_factor = total_net_pnl / total_loss
    ...
    statistics_dic = OrderedDict([
        ...
        (f"annual_return_rate{stats_group}", annual_return_rate),
        ...
        (f"profit_rate{stats_group}", profit_rate),
        (f"avg_pnl{stats_group}", avg_pnl),
        (f"avg_profit{stats_group}", avg_profit),
        (f"avg_loss{stats_group}", avg_loss),
        (f"profit_factor{stats_group}", profit_factor),
    ])
...
class BacktestingEngine(BacktestingEngineBase):
    ...
    def calculate_by_trade(self, df: DataFrame = None, trade_list=None):
        """
            ...
            if trade_data.offset.value == Offset.OPEN.value:
                ...
                if pos_status_dict['volume'] != 0 and pos_status_dict['direction'] != trade_data.direction.value:
                    self.logger.error(
                        ...
                        self.strategy, trade_data.symbol,
                ...
                if volume == 0:
                    self.logger.warning(
                        ...
                        self.strategy.strategy_name, trade_data.symbol, trade_data.tradeid, trade_data.direction.value,
    ...
    def generate_daily_stats_dic(self, df, daily_df_name: str, indicator_by_trade_dic: dict):
        if df is None:
        ...
        if df is None:
            ...
            statistics_dic = calc_statistic_by_pnl_s(indicator_by_trade_dic=indicator_by_trade_dic,
                                                     capital=self.capital, df=df)
            ...
            # 毛收益率：去极值（std*2） 的收益
            statistics_gross_dic = calc_statistic_by_pnl_s(indicator_by_trade_dic=indicator_by_trade_dic,
                                                           capital=self.capital, df=df, stats_group='gross')
    ...
    def calculate_statistics_new(self, df: DataFrame = None, output=True):
        """"""
        ...
        for end_name in self.three_parts:
            ...
            indicator_by_trade_dic = self.calculate_by_trade(trade_list=trade_list)

            # 生成按每天的计算出的指标
            daily_stats_dic = self.generate_daily_stats_dic(df, daily_df_name, indicator_by_trade_dic)
