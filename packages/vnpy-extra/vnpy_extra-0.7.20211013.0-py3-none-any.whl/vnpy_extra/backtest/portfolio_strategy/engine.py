...
class BacktestingEngine(BacktestingEngineBase):
    ...
    def __init__(self):
        ...
        self.trade_dict_all = {}
        self.trade_dict_long = {}
        self.trade_dict_short = {}
    ...
    def calculate_result_new(self) -> None:
        """"""
        self.output("开始计算逐日盯市盈亏")
        # 这里深拷贝防止赋值混乱
        ...
        self.trade_dict_all: Dict[str, List[TradeDataModel]] = {key: [] for key in self.vt_symbols}
        self.trade_dict_long: Dict[str, List[TradeDataModel]] = {key: [] for key in self.vt_symbols}
        self.trade_dict_short: Dict[str, List[TradeDataModel]] = {key: [] for key in self.vt_symbols}
        ...
        for trade in self.trades.values():
            ...
            self.trade_dict_all[key].append(trade)
            ...
            if (trade.offset == Offset.OPEN and trade.direction == Direction.LONG) \
                ...
                daily_result.add_trade(trade)
                self.trade_dict_long[key].append(trade)
                ...
                daily_result.add_trade(trade)
                self.trade_dict_short[key].append(trade)
        ...
        for end_name in self.three_parts:
            ...
            for daily_result in daily_results_dic.values():
                fields = [
                    "date", "close_prices", "trade_count", "turnover",
                    "commission", "slippage", "trading_pnl",
                    "holding_pnl", "total_pnl", "net_pnl"
                ]
                for key in fields:
    ...
    def calculate_by_trade_new(self, daily_stats_dic, df: DataFrame = None, trade_dict=None):
        """
        按每笔交易计算指标（建仓后平仓为一次交易）
        :return {'indicator1':'value'}:返回指标dict
        """
        ...
        if trade_dict is None:
            trade_dict = {key: [] for key in self.vt_symbols}
        ...
        for symbol in self.vt_symbols:
            instrument_type = get_instrument_type(symbol).upper()
            multiplier = SYMBOL_SIZE_DIC.setdefault(instrument_type, 10)
            trade_data_list = trade_dict[symbol]
            if len(trade_data_list) == 0:
                result[symbol] = {
                    "offset_acc_profit": 0,
                    "offset_acc_loss": 0,
            ...
            # 盈利比率： 盈利手数 / 总交易手数
            profit_rate_by_volume = profit_volume / total_volume
            # 平均利润： 净利润 / 交易手数
            avg_pnl_by_volume = total_net_pnl / total_volume
            # 平均盈利： 总盈利 / 盈利交易手数
            avg_profit_by_volume = total_profit / profit_volume
            # 平均亏损：总亏损金额 / 亏损交易手数
            avg_loss_by_volume = total_loss / loss_volume
            # 盈利因子：  总利润 / 总亏损
            profit_factor_by_volume = total_net_pnl / total_loss
            # 最大连续盈利手数
    ...
    def calc_statistic_by_pnl_s_new(self, stats_group=None, df: DataFrame = None) -> dict:
        """根据每日盈亏进行相关收益指标统计"""
    ...
    def generate_daily_stats_dic(self, df, daily_df_name: str):
        ...
        if df is None:
            ...
            statistics_dic = self.calc_statistic_by_pnl_s(df=df)
            ...
            # 毛收益率：去极值（std*2） 的收益
            statistics_gross_dic = self.calc_statistic_by_pnl_s(df=df, stats_group='gross')
    ...
    def calculate_statistics_new(self, df: DataFrame = None, output=True):
        ...
        for end_name in self.three_parts:
            ...
            trade_dict_name = f'trade_dict{end_name}'
            ...
            # stat by trade
            trade_dict = getattr(self, trade_dict_name)
            indicator_by_trade_dic = self.calculate_by_trade(trade_dict=trade_dict, daily_stats_dic=daily_stats_dic)
