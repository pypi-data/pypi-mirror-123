...
class BaseDBHelper(object):
    ...
    def get_simple_list(self, table_name, fields, conditions=None, raw_conditions='', order_field='id', order_type='DESC', join_statements=None):
        return self.get_list(table_name, fields, conditions=conditions, raw_conditions=raw_conditions, order_field=order_field, order_type=order_type, pagination=False, oin_statements=join_statements)

    def get_list(self, table_name, fields, conditions=None, raw_conditions='', order_field='id', order_type='DESC', page_index=0, page_size=10, pagination=True, join_statements=None):
        params = []
        condition = ''
        rst = None
        cnt = 0
        ...
        rst, cnt = self.get_list_base(table_name, fields, condition, params, order_field=order_field, order_type=order_type, page_index=page_index, page_size=page_size, pagination=pagination, oin_statements=join_statements)
        return rst, cnt

    def get_list_base(self, table_name, fields, condition='', params=None, order_field='id', order_type='DESC', page_index=0, page_size=10, pagination=True, pagination_with_limit=True, join_statements=None):
        cnt = 0

        base_statement = """SELECT %s FROM """ + table_name + ('' if join_statements is None else join_statements) + """ WHERE 1 = 1 """  + condition 

        if pagination:
            # cnt = self.get_count(table_name, condition, params, join_statements=join_statements)
            cnt_sql = (base_statement % """COUNT(1) AS cnt""") + """;"""
        ...
        if pagination_with_limit:
            ...
            # sql += """ WHERE 1 = 1 """
            # sql += condition
            sql = (base_statement % fields)
            # sql = """SELECT """ + fields + base_statement
            sql += order_by_sql
