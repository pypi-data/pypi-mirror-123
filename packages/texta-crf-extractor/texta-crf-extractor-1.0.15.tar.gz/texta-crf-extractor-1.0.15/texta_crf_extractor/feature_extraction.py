...
KNOWN_EXTACTORS = [EXTRACTOR_ISUPPER, EXTRACTOR_ISTITLE, EXTRACTOR_HASDIGIT]
...
class FeatureExtractor:
    ...
    def _extract_features(self, item: dict, word_index: int, extractors_to_use: list):
        """
        Extract features from token and surrounding tokens based on window size.
        """
        features = []
        # select features from selected layers
        layers_to_use = self.config.feature_layers+self.config.context_feature_layers
        ...
        if self.embedding and FEATURE_LAYER_LEMMA in item:
            most_similar_lemmas = self.embedding.get_similar([item[FEATURE_LAYER_LEMMA]], n=1)
    ...
    def word2features(self, sent: List[tuple], i: int):
        ...
        features.extend(self._extract_features(item, "0", self.config.feature_extractors))
        ...
        if i > 0:
            for window in range(1, self.config.window_size+1):
                try:
                    ...
                    features.extend(self._extract_features(item1, f"-{window}", self.config.context_feature_extractors))
                except IndexError:
                    pass
        else:
