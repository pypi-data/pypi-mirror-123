...
class CRFExtractor:
    MLP_FIELDS = [FEATURE_LAYER_WORD, FEATURE_LAYER_LEMMA, FEATURE_LAYER_POS, FEATURE_LAYER_GRAMMAR]
    ...
    def _validate_and_extract_layers(self, mlp_document: dict, field_name: str):
        """
        ...
        fields_to_check = set(self.config.feature_layers + self.config.context_feature_layers)

        # check if mlp fields are present
        for field in CRFExtractor.MLP_FIELDS:
            if field not in mlp_subdoc and field in fields_to_check:
                    ...
                    if fact["fact"] in self.config.labels and fact["sent_index"] == i:
                        ...
                        for span in spans:
                            ...
                            for j in range(num_tokens_before_match, num_tokens_before_match+num_tokens_match):
                                labels[i][j] = label
                try:
                    yield [(token, layers[FEATURE_LAYER_LEMMA][i][j], layers[FEATURE_LAYER_POS][i][j], layers[FEATURE_LAYER_GRAMMAR][i][j], labels[i][j]) for j, token in enumerate(sentence)]
                except IndexError:
                    raise exceptions.InvalidInputError("Layer lengths do not match!")
        else:
            for i, sentence in enumerate(layers[FEATURE_LAYER_WORD]):
                try:
                    yield i, [(token, layers[FEATURE_LAYER_LEMMA][i][j], layers[FEATURE_LAYER_POS][i][j], layers[FEATURE_LAYER_GRAMMAR][i][j]) for j, token in enumerate(sentence)]
                except IndexError:
                    raise exceptions.InvalidInputError("Layer lengths do not match!")
    ...
    @check_model_loaded
    def tag(self, mlp_document: dict, field_name: str = "text_mlp"):
        """
        ...
        for i, seq_to_predict in seqs_to_predict:
            ...
            for tag in self._process_tag_output(result, sentences, i, field_name):
                facts.append(tag)
    ...
    def _process_tag_output(self, tokens: List[str], sentences: List[List[str]], sent_index: int, field_name: str):
        """
