...
def word2features(sent: list, i: int, config, embedding):
    ...
    if FEATURE_LAYER_GRAMMAR in config.feature_layers:
        item[FEATURE_LAYER_GRAMMAR] = sent[i][3]
    ...
    if i > 0:
        for window in range(1, config.window_size+1):
            try:
                ...
                if FEATURE_LAYER_GRAMMAR in config.context_feature_layers:
                    item1[FEATURE_LAYER_GRAMMAR] = sent[i-window][3]
...
def sent2labels(sent: list):
    return [label for token, lemma, pos_tag, word_feature, label in sent]
