...
def nnunet_lung_run(LOAD_PATH, SAVE_MODEL_PATH, GPU_DEVICE_INDEX, patch_size = (208, 208, 112), batch_size = 2, n_classes = 2, n_modalities = 1):
    ...
    current_iter = 0
    val_iter = 0
    criterion1 = FastACELoss3DV3(classes = n_classes).to(GPU_DEVICE_INDEX)
    criterion2 = nn.CrossEntropyLoss(reduction = 'mean', weight = torch.tensor([1,39500]).float().to(GPU_DEVICE_INDEX))
    step = 0
    v_step = 0
    loss_smooth = 1e+7
    for e in range(1000):
        ...
        while not tr_dataloader.is_end():
            ...
            if isinstance(optimizer, SAMSGD):
                def closure():
                    ...
                    loss2 = criterion2(output, seg)
                    writer.add_scalar('Lung/Train/AC Loss', loss1.item(), step)
                    writer.add_scalar('Lung/Train/CE Loss', loss2.item(), step)
                    loss = loss1 + loss2
                    loss.backward()
                    return loss
        ...
        while not val_dataloader.is_end():
            ...
            with torch.no_grad():
                ...
                output = torch.softmax(output, dim = -1)
                output = output.permute(0,-1,1,2,3)
                loss1 = criterion1(images,F.one_hot(seg,5).permute(0,-1,1,2,3))
                loss2 = criterion2(images, seg)
