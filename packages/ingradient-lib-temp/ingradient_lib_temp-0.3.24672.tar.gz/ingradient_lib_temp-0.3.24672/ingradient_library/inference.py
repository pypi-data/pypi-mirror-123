...
class Inference(object):
    ...
    def save_inference_result(self):
        ...
        if self.mode == 'save':
            ...
            temp_seg = [torch.where(seg == 0)] = -1
            intersection = (self.result_map == temp_seg).sum()
            print(intersection, self.result_map.shape, seg.shape, (self.result_map != 0).sum(), (seg != 0).sum(), torch.unique(self.result_map), torch.unique(seg))
            union = (self.result_map != 0).sum() + (seg != 0).sum()
