...
def nnunet_lung_run(LOAD_PATH, SAVE_MODEL_PATH, GPU_DEVICE_INDEX, patch_size = (192, 192, 80), batch_size = 2, n_classes = 2, n_modalities = 1):
    transform = Transform(*get_transform_params(GPU_DEVICE_INDEX))
    ...
    model = Deep_Supervision_UNet3d(patch_size = patch_size, n_modalities = n_modalities, final_output_channels = n_classes, is_binary = False).to(GPU_DEVICE_INDEX)
    loss2 = Multi_Dice_Loss(n_classes)
    loss1 = nn.CrossEntropyLoss(reduction = 'none', weight = torch.tensor([1, 39500]).float().to(GPU_DEVICE_INDEX))
    optimizer = SAMSGD(model.parameters(), lr = 1e-2, nesterov = True, momentum = 0.99, weight_decay= 0.05)
