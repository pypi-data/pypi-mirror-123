...
class Inference(object):
    ...
    def save_inference_result(self):
        ...
        plt.imshow(self.result_map[0][:,:,60])
        plt.show()
        plt.imshow(self.result_map[1][:,:,60])
        plt.show()
        plt.imshow(self.result_map[2][:,:,60])
        plt.show()
        plt.imshow(self.result_map[4][:,:,60])
        plt.show()
        self.result_map = torch.argmax(self.result_map.permute(0,1,2,3), dim = 0)
        if self.mode == 'save':
