...
class FastACELoss3DV3(nn.Module):
    """
    Active contour based elastic model loss
    based on minpooling & maxpooling and laplace filter
    """

    def __init__(self, miu=1.0, alpha=1e-3, beta=2.0, classes=4, types="other", smooth = 1e+3):
        super(FastACELoss3DV2, self).__init__()
        self.miu = miu
        self.alpha = alpha
        self.beta = beta
    ...
    def forward(self, predication, label):
        ...
        c_in = torch.ones_like(predication)
        c_out = torch.zeros_like(predication)
        region_in = torch.abs(torch.sum(predication * ((label - c_in) ** 2))) / self.smooth
        region_out = torch.abs(
            torch.sum((1 - predication) * ((label - c_out) ** 2))) / self.smooth
        region = self.miu * region_in + region_out

        # elastic
        elastic = torch.sum((self.alpha + self.beta * curvature ** 2) * length) / self.smooth
        return region + elastic