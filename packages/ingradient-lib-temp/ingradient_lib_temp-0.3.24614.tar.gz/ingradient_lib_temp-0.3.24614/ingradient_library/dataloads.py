...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.correct_direction:
            ...
            print("direction", x.shape, y.shape)
...
class Dataset2point5(CustomDataset):
    def __init__(self, root_dir, degree, view = 'axial', device=None, normalizer=None, mode='train', dim='3d', correct_direction=True, resample=None):
...
class DataLoader3D(object):
    ...
    def get_image_seg(self, current_index): #patch size보다 전체 이미지 크기가 작을 경우 padding을 진행
        ...
        print(image_shape, self.patch_size)
        if np.any(image_shape < self.patch_size):
            ...
            print(even_pad.shape, odd_pad.shape)
    ...
    def generate_train_batch(self):
        ...
        for i_b in range(self.batch_size//2):
            ...
            print(oversample_seg.shape, normal_seg.shape)
