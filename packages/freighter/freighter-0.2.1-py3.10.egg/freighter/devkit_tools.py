...
class Project(object):
    def __init__(self, name: str, gameid: str, auto_import = True):
        ...
        if (DOLPHIN_MAPS):
            self.map_outpaths = [DOLPHIN_MAPS ]
        else:
            self.map_outpaths = [""]
    ...
    def __generate_linkerscript(self):
        with open(self.temp_dir + self.project_name + "_linkerscript.ld", "w") as f:
            f.writelines(['OUTPUT_FORMAT("elf32-powerpc", "elf32-powerpc","elf32-powerpc");\n',
            'OUTPUT_ARCH("powerpc");\n'])
            f.write("_SDA_BASE_ = 0x8051c5c0;\n")
