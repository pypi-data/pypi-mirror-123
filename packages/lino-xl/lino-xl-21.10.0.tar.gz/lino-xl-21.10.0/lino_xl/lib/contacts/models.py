...
if dd.plugins.contacts.use_vcard_export:
    ...
    class UploadVCardFile(UploadBase):
        ...
        description = dd.CharField(max_length=10, blank=True, verbose_name=_('Description'))
        ...
        def save_newly_uploaded_file(self, uf):
            ...
            for vo in vobject.readComponents(uf.read().decode()):
                ...
                if hasattr(vo, 'adr'):
                    ...
                    if country != "":
                        iso = country.lower()
                        country, created = Country.objects.get_or_create(isocode=country)
                        if created:
                    ...
                    if city != "":
                        city, created = Place.objects.get_or_create(name=city, **kw)
                        if created:
                            city.full_clean()
                            city.save()
                    p.city = city
                    ...
                    if region != "":
                        region, created = Place.objects.get_or_create(name=region, **kw)
                        if created:
                            region.full_clean()
                            region.save()
...
class Partner(Duplicable, ContactDetailsOwner, mixins.Polymorphic,
              ...
              DateRangeObservable, VatSubjectable):
