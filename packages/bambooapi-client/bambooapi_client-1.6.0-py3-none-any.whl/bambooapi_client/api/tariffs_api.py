"""
    Bamboo Flexibility API

     This API provides access to flexibility assets managed by Bamboo Energy: - Create and list flexibility sites & assets - Obtain activations for specific assets - Post and get measurements for specific assets   # noqa: E501

    The version of the OpenAPI document: 1.6.0
    Contact: development@bambooenergy.tech
    Generated by: https://openapi-generator.tech
"""


import re  # noqa: F401
import sys  # noqa: F401

from bambooapi_client.api_client import ApiClient, Endpoint as _Endpoint
from bambooapi_client.model_utils import (  # noqa: F401
    check_allowed_values,
    check_validations,
    date,
    datetime,
    file_type,
    none_type,
    validate_and_convert_types
)
from bambooapi_client.model.http400_bad_request import HTTP400BadRequest
from bambooapi_client.model.http401_unauthorized import HTTP401Unauthorized
from bambooapi_client.model.http403_forbidden import HTTP403Forbidden
from bambooapi_client.model.http404_not_found import HTTP404NotFound
from bambooapi_client.model.http_validation_error import HTTPValidationError
from bambooapi_client.model.monthly_tariff_create import MonthlyTariffCreate
from bambooapi_client.model.monthly_tariff_update import MonthlyTariffUpdate
from bambooapi_client.model.tariff_list_item import TariffListItem
from bambooapi_client.model.tariff_period_item import TariffPeriodItem


class TariffsApi(object):
    """NOTE: This class is auto generated by OpenAPI Generator
    Ref: https://openapi-generator.tech

    Do not edit the class manually.
    """

    def __init__(self, api_client=None):
        if api_client is None:
            api_client = ApiClient()
        self.api_client = api_client

        def __create_tariff(
            self,
            monthly_tariff_create,
            **kwargs
        ):
            """Create a Tariff  # noqa: E501

            Create a Tariff with all the information:  - **tariff_name**: each tariff must have a unique name - **commercial_name**: For example '6A', '3A'. - **type**: Either 'monthly' or 'seasonal'. - **country_code**: Country code in ISO 3166-1 alpha-2 format. For Spain   use ISO 3166-2:ES. - **periods**: A json object with the periods that will appear in   'timetable'. - **timetable**: A json object with keys being months ('january',   'february', etc.) for 'monthly' tariffs, or keys being 'summer' and   'winter' for 'seasonal' tariffs. In both tariffs, an additional key   'weekend_holiday' must be provided. For each key, an array of 24   period names (strings) is required.  # noqa: E501
            This method makes a synchronous HTTP request by default. To make an
            asynchronous HTTP request, please pass async_req=True

            >>> thread = api.create_tariff(monthly_tariff_create, async_req=True)
            >>> result = thread.get()

            Args:
                monthly_tariff_create (MonthlyTariffCreate):

            Keyword Args:
                _return_http_data_only (bool): response data without head status
                    code and headers. Default is True.
                _preload_content (bool): if False, the urllib3.HTTPResponse object
                    will be returned without reading/decoding response data.
                    Default is True.
                _request_timeout (int/float/tuple): timeout setting for this request. If
                    one number provided, it will be total request timeout. It can also
                    be a pair (tuple) of (connection, read) timeouts.
                    Default is None.
                _check_input_type (bool): specifies if type checking
                    should be done one the data sent to the server.
                    Default is True.
                _check_return_type (bool): specifies if type checking
                    should be done one the data received from the server.
                    Default is True.
                _host_index (int/None): specifies the index of the server
                    that we want to use.
                    Default is read from the configuration.
                async_req (bool): execute request asynchronously

            Returns:
                dict
                    If the method is called asynchronously, returns the request
                    thread.
            """
            kwargs['async_req'] = kwargs.get(
                'async_req', False
            )
            kwargs['_return_http_data_only'] = kwargs.get(
                '_return_http_data_only', True
            )
            kwargs['_preload_content'] = kwargs.get(
                '_preload_content', True
            )
            kwargs['_request_timeout'] = kwargs.get(
                '_request_timeout', None
            )
            kwargs['_check_input_type'] = kwargs.get(
                '_check_input_type', True
            )
            kwargs['_check_return_type'] = kwargs.get(
                '_check_return_type', True
            )
            kwargs['_host_index'] = kwargs.get('_host_index')
            kwargs['monthly_tariff_create'] = \
                monthly_tariff_create
            return self.call_with_http_info(**kwargs)

        self.create_tariff = _Endpoint(
            settings={
                'response_type': (dict,),
                'auth': [
                    'BearerAuth'
                ],
                'endpoint_path': '/tariffs',
                'operation_id': 'create_tariff',
                'http_method': 'POST',
                'servers': None,
            },
            params_map={
                'all': [
                    'monthly_tariff_create',
                ],
                'required': [
                    'monthly_tariff_create',
                ],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                ]
            },
            root_map={
                'validations': {
                },
                'allowed_values': {
                },
                'openapi_types': {
                    'monthly_tariff_create':
                        (MonthlyTariffCreate,),
                },
                'attribute_map': {
                },
                'location_map': {
                    'monthly_tariff_create': 'body',
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'application/json'
                ],
                'content_type': [
                    'application/json'
                ]
            },
            api_client=api_client,
            callable=__create_tariff
        )

        def __delete_tariff(
            self,
            tariff_id,
            **kwargs
        ):
            """Delete an existing Tariff  # noqa: E501

            Delete a Tariff.  **WARNING!: Tariff deletion is PERMANENT!** (no way to recover deleted tariff)  # noqa: E501
            This method makes a synchronous HTTP request by default. To make an
            asynchronous HTTP request, please pass async_req=True

            >>> thread = api.delete_tariff(tariff_id, async_req=True)
            >>> result = thread.get()

            Args:
                tariff_id (int):

            Keyword Args:
                _return_http_data_only (bool): response data without head status
                    code and headers. Default is True.
                _preload_content (bool): if False, the urllib3.HTTPResponse object
                    will be returned without reading/decoding response data.
                    Default is True.
                _request_timeout (int/float/tuple): timeout setting for this request. If
                    one number provided, it will be total request timeout. It can also
                    be a pair (tuple) of (connection, read) timeouts.
                    Default is None.
                _check_input_type (bool): specifies if type checking
                    should be done one the data sent to the server.
                    Default is True.
                _check_return_type (bool): specifies if type checking
                    should be done one the data received from the server.
                    Default is True.
                _host_index (int/None): specifies the index of the server
                    that we want to use.
                    Default is read from the configuration.
                async_req (bool): execute request asynchronously

            Returns:
                dict
                    If the method is called asynchronously, returns the request
                    thread.
            """
            kwargs['async_req'] = kwargs.get(
                'async_req', False
            )
            kwargs['_return_http_data_only'] = kwargs.get(
                '_return_http_data_only', True
            )
            kwargs['_preload_content'] = kwargs.get(
                '_preload_content', True
            )
            kwargs['_request_timeout'] = kwargs.get(
                '_request_timeout', None
            )
            kwargs['_check_input_type'] = kwargs.get(
                '_check_input_type', True
            )
            kwargs['_check_return_type'] = kwargs.get(
                '_check_return_type', True
            )
            kwargs['_host_index'] = kwargs.get('_host_index')
            kwargs['tariff_id'] = \
                tariff_id
            return self.call_with_http_info(**kwargs)

        self.delete_tariff = _Endpoint(
            settings={
                'response_type': (dict,),
                'auth': [
                    'BearerAuth'
                ],
                'endpoint_path': '/tariffs/{tariff_id}',
                'operation_id': 'delete_tariff',
                'http_method': 'DELETE',
                'servers': None,
            },
            params_map={
                'all': [
                    'tariff_id',
                ],
                'required': [
                    'tariff_id',
                ],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                    'tariff_id',
                ]
            },
            root_map={
                'validations': {
                    ('tariff_id',): {

                        'inclusive_maximum': 9999,
                        'inclusive_minimum': 1000,
                    },
                },
                'allowed_values': {
                },
                'openapi_types': {
                    'tariff_id':
                        (int,),
                },
                'attribute_map': {
                    'tariff_id': 'tariff_id',
                },
                'location_map': {
                    'tariff_id': 'path',
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'application/json'
                ],
                'content_type': [],
            },
            api_client=api_client,
            callable=__delete_tariff
        )

        def __get_tariff(
            self,
            tariff_id,
            **kwargs
        ):
            """Info about a specific Tariff  # noqa: E501

            Info about a specific Tariff.  # noqa: E501
            This method makes a synchronous HTTP request by default. To make an
            asynchronous HTTP request, please pass async_req=True

            >>> thread = api.get_tariff(tariff_id, async_req=True)
            >>> result = thread.get()

            Args:
                tariff_id (int):

            Keyword Args:
                _return_http_data_only (bool): response data without head status
                    code and headers. Default is True.
                _preload_content (bool): if False, the urllib3.HTTPResponse object
                    will be returned without reading/decoding response data.
                    Default is True.
                _request_timeout (int/float/tuple): timeout setting for this request. If
                    one number provided, it will be total request timeout. It can also
                    be a pair (tuple) of (connection, read) timeouts.
                    Default is None.
                _check_input_type (bool): specifies if type checking
                    should be done one the data sent to the server.
                    Default is True.
                _check_return_type (bool): specifies if type checking
                    should be done one the data received from the server.
                    Default is True.
                _host_index (int/None): specifies the index of the server
                    that we want to use.
                    Default is read from the configuration.
                async_req (bool): execute request asynchronously

            Returns:
                dict
                    If the method is called asynchronously, returns the request
                    thread.
            """
            kwargs['async_req'] = kwargs.get(
                'async_req', False
            )
            kwargs['_return_http_data_only'] = kwargs.get(
                '_return_http_data_only', True
            )
            kwargs['_preload_content'] = kwargs.get(
                '_preload_content', True
            )
            kwargs['_request_timeout'] = kwargs.get(
                '_request_timeout', None
            )
            kwargs['_check_input_type'] = kwargs.get(
                '_check_input_type', True
            )
            kwargs['_check_return_type'] = kwargs.get(
                '_check_return_type', True
            )
            kwargs['_host_index'] = kwargs.get('_host_index')
            kwargs['tariff_id'] = \
                tariff_id
            return self.call_with_http_info(**kwargs)

        self.get_tariff = _Endpoint(
            settings={
                'response_type': (dict,),
                'auth': [
                    'BearerAuth'
                ],
                'endpoint_path': '/tariffs/{tariff_id}',
                'operation_id': 'get_tariff',
                'http_method': 'GET',
                'servers': None,
            },
            params_map={
                'all': [
                    'tariff_id',
                ],
                'required': [
                    'tariff_id',
                ],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                    'tariff_id',
                ]
            },
            root_map={
                'validations': {
                    ('tariff_id',): {

                        'inclusive_maximum': 9999,
                        'inclusive_minimum': 1000,
                    },
                },
                'allowed_values': {
                },
                'openapi_types': {
                    'tariff_id':
                        (int,),
                },
                'attribute_map': {
                    'tariff_id': 'tariff_id',
                },
                'location_map': {
                    'tariff_id': 'path',
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'application/json'
                ],
                'content_type': [],
            },
            api_client=api_client,
            callable=__get_tariff
        )

        def __get_tariff_schedule(
            self,
            tariff_id,
            **kwargs
        ):
            """Return expected tariff schedule between two dates  # noqa: E501

            Return tariff schedule within the given dates.  - **Timestamps are ordered from oldest to newest** - **Data is returned with the same sign convention as when pushed** (not transformation applied)  # noqa: E501
            This method makes a synchronous HTTP request by default. To make an
            asynchronous HTTP request, please pass async_req=True

            >>> thread = api.get_tariff_schedule(tariff_id, async_req=True)
            >>> result = thread.get()

            Args:
                tariff_id (int):

            Keyword Args:
                period_start (datetime): Either a timestamp in RFC3339 format, in UTC timezone (\"YYYY-MM-DDT00:00:00Z\"). Make sure to specify the timezone either with \"Z\" or \"+00:00\". [optional]
                period_stop (datetime): Either a timestamp in RFC3339 format, in UTC timezone (\"YYYY-MM-DDT00:00:00Z\"). Make sure to specify the timezone either with \"Z\" or \"+00:00\". [optional]
                _return_http_data_only (bool): response data without head status
                    code and headers. Default is True.
                _preload_content (bool): if False, the urllib3.HTTPResponse object
                    will be returned without reading/decoding response data.
                    Default is True.
                _request_timeout (int/float/tuple): timeout setting for this request. If
                    one number provided, it will be total request timeout. It can also
                    be a pair (tuple) of (connection, read) timeouts.
                    Default is None.
                _check_input_type (bool): specifies if type checking
                    should be done one the data sent to the server.
                    Default is True.
                _check_return_type (bool): specifies if type checking
                    should be done one the data received from the server.
                    Default is True.
                _host_index (int/None): specifies the index of the server
                    that we want to use.
                    Default is read from the configuration.
                async_req (bool): execute request asynchronously

            Returns:
                [TariffPeriodItem]
                    If the method is called asynchronously, returns the request
                    thread.
            """
            kwargs['async_req'] = kwargs.get(
                'async_req', False
            )
            kwargs['_return_http_data_only'] = kwargs.get(
                '_return_http_data_only', True
            )
            kwargs['_preload_content'] = kwargs.get(
                '_preload_content', True
            )
            kwargs['_request_timeout'] = kwargs.get(
                '_request_timeout', None
            )
            kwargs['_check_input_type'] = kwargs.get(
                '_check_input_type', True
            )
            kwargs['_check_return_type'] = kwargs.get(
                '_check_return_type', True
            )
            kwargs['_host_index'] = kwargs.get('_host_index')
            kwargs['tariff_id'] = \
                tariff_id
            return self.call_with_http_info(**kwargs)

        self.get_tariff_schedule = _Endpoint(
            settings={
                'response_type': ([TariffPeriodItem],),
                'auth': [
                    'BearerAuth'
                ],
                'endpoint_path': '/tariffs/{tariff_id}/schedule',
                'operation_id': 'get_tariff_schedule',
                'http_method': 'GET',
                'servers': None,
            },
            params_map={
                'all': [
                    'tariff_id',
                    'period_start',
                    'period_stop',
                ],
                'required': [
                    'tariff_id',
                ],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                ]
            },
            root_map={
                'validations': {
                },
                'allowed_values': {
                },
                'openapi_types': {
                    'tariff_id':
                        (int,),
                    'period_start':
                        (datetime,),
                    'period_stop':
                        (datetime,),
                },
                'attribute_map': {
                    'tariff_id': 'tariff_id',
                    'period_start': 'period_start',
                    'period_stop': 'period_stop',
                },
                'location_map': {
                    'tariff_id': 'path',
                    'period_start': 'query',
                    'period_stop': 'query',
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'application/json'
                ],
                'content_type': [],
            },
            api_client=api_client,
            callable=__get_tariff_schedule
        )

        def __list_tariffs(
            self,
            **kwargs
        ):
            """List all tariffs  # noqa: E501

            List all Tariffs.  # noqa: E501
            This method makes a synchronous HTTP request by default. To make an
            asynchronous HTTP request, please pass async_req=True

            >>> thread = api.list_tariffs(async_req=True)
            >>> result = thread.get()


            Keyword Args:
                _return_http_data_only (bool): response data without head status
                    code and headers. Default is True.
                _preload_content (bool): if False, the urllib3.HTTPResponse object
                    will be returned without reading/decoding response data.
                    Default is True.
                _request_timeout (int/float/tuple): timeout setting for this request. If
                    one number provided, it will be total request timeout. It can also
                    be a pair (tuple) of (connection, read) timeouts.
                    Default is None.
                _check_input_type (bool): specifies if type checking
                    should be done one the data sent to the server.
                    Default is True.
                _check_return_type (bool): specifies if type checking
                    should be done one the data received from the server.
                    Default is True.
                _host_index (int/None): specifies the index of the server
                    that we want to use.
                    Default is read from the configuration.
                async_req (bool): execute request asynchronously

            Returns:
                [TariffListItem]
                    If the method is called asynchronously, returns the request
                    thread.
            """
            kwargs['async_req'] = kwargs.get(
                'async_req', False
            )
            kwargs['_return_http_data_only'] = kwargs.get(
                '_return_http_data_only', True
            )
            kwargs['_preload_content'] = kwargs.get(
                '_preload_content', True
            )
            kwargs['_request_timeout'] = kwargs.get(
                '_request_timeout', None
            )
            kwargs['_check_input_type'] = kwargs.get(
                '_check_input_type', True
            )
            kwargs['_check_return_type'] = kwargs.get(
                '_check_return_type', True
            )
            kwargs['_host_index'] = kwargs.get('_host_index')
            return self.call_with_http_info(**kwargs)

        self.list_tariffs = _Endpoint(
            settings={
                'response_type': ([TariffListItem],),
                'auth': [
                    'BearerAuth'
                ],
                'endpoint_path': '/tariffs',
                'operation_id': 'list_tariffs',
                'http_method': 'GET',
                'servers': None,
            },
            params_map={
                'all': [
                ],
                'required': [],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                ]
            },
            root_map={
                'validations': {
                },
                'allowed_values': {
                },
                'openapi_types': {
                },
                'attribute_map': {
                },
                'location_map': {
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'application/json'
                ],
                'content_type': [],
            },
            api_client=api_client,
            callable=__list_tariffs
        )

        def __update_tariff(
            self,
            tariff_id,
            monthly_tariff_update,
            **kwargs
        ):
            """Update an existing Tariff  # noqa: E501

            Update information about a Tariff.  # noqa: E501
            This method makes a synchronous HTTP request by default. To make an
            asynchronous HTTP request, please pass async_req=True

            >>> thread = api.update_tariff(tariff_id, monthly_tariff_update, async_req=True)
            >>> result = thread.get()

            Args:
                tariff_id (int):
                monthly_tariff_update (MonthlyTariffUpdate):

            Keyword Args:
                _return_http_data_only (bool): response data without head status
                    code and headers. Default is True.
                _preload_content (bool): if False, the urllib3.HTTPResponse object
                    will be returned without reading/decoding response data.
                    Default is True.
                _request_timeout (int/float/tuple): timeout setting for this request. If
                    one number provided, it will be total request timeout. It can also
                    be a pair (tuple) of (connection, read) timeouts.
                    Default is None.
                _check_input_type (bool): specifies if type checking
                    should be done one the data sent to the server.
                    Default is True.
                _check_return_type (bool): specifies if type checking
                    should be done one the data received from the server.
                    Default is True.
                _host_index (int/None): specifies the index of the server
                    that we want to use.
                    Default is read from the configuration.
                async_req (bool): execute request asynchronously

            Returns:
                dict
                    If the method is called asynchronously, returns the request
                    thread.
            """
            kwargs['async_req'] = kwargs.get(
                'async_req', False
            )
            kwargs['_return_http_data_only'] = kwargs.get(
                '_return_http_data_only', True
            )
            kwargs['_preload_content'] = kwargs.get(
                '_preload_content', True
            )
            kwargs['_request_timeout'] = kwargs.get(
                '_request_timeout', None
            )
            kwargs['_check_input_type'] = kwargs.get(
                '_check_input_type', True
            )
            kwargs['_check_return_type'] = kwargs.get(
                '_check_return_type', True
            )
            kwargs['_host_index'] = kwargs.get('_host_index')
            kwargs['tariff_id'] = \
                tariff_id
            kwargs['monthly_tariff_update'] = \
                monthly_tariff_update
            return self.call_with_http_info(**kwargs)

        self.update_tariff = _Endpoint(
            settings={
                'response_type': (dict,),
                'auth': [
                    'BearerAuth'
                ],
                'endpoint_path': '/tariffs/{tariff_id}',
                'operation_id': 'update_tariff',
                'http_method': 'PUT',
                'servers': None,
            },
            params_map={
                'all': [
                    'tariff_id',
                    'monthly_tariff_update',
                ],
                'required': [
                    'tariff_id',
                    'monthly_tariff_update',
                ],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                    'tariff_id',
                ]
            },
            root_map={
                'validations': {
                    ('tariff_id',): {

                        'inclusive_maximum': 9999,
                        'inclusive_minimum': 1000,
                    },
                },
                'allowed_values': {
                },
                'openapi_types': {
                    'tariff_id':
                        (int,),
                    'monthly_tariff_update':
                        (MonthlyTariffUpdate,),
                },
                'attribute_map': {
                    'tariff_id': 'tariff_id',
                },
                'location_map': {
                    'tariff_id': 'path',
                    'monthly_tariff_update': 'body',
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'application/json'
                ],
                'content_type': [
                    'application/json'
                ]
            },
            api_client=api_client,
            callable=__update_tariff
        )
