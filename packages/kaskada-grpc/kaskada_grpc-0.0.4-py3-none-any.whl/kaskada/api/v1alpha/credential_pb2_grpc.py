...
class CredentialServiceServicer(object):
    ...
    def Get(self, request, context):
        ...
        Credentials can be used to access the Kaskada API using a client library or directly
        using the REST endpoint.

        The returned credentials may be used by providing them to any of the Kaskada-maintained
        client libraries when asked for client-id and client-secret.

        The client-id and client-secret may be used independent of a client library. To do so
        you must generate a short-lived JWT bearer token by making a POST request as follows:

        curl --request POST \
        --url https://kaskada.us.auth0.com/oauth/token \
        --header 'content-type: application/json' \
        --data '{"client_id":"<client-id>","client_secret":"<client-secret>","audience":"https://api.prod.kaskada.com","grant_type":"client_credentials"}'
        ...
        The token grants access to the Kaskada API to any gRPC or HTTP request that provides the
        token in the "Authorization" header / metadata:
