...
def _get_last_updated_file(src_file):
    ...
    # the subprocess module and with that, the exception is not catched
        ...
        LOG.info(
            '[unilakedocstheme] Could not get modification time of %s: %s',
            src_file, err)
    else:
        if last_updated_t:
            try:
...
def _get_last_updated(app, pagename):
    ...
    for filename in candidates:
        ...
        if last_updated:
            LOG.debug(
                '[unilakedocstheme] Last updated for %s is %s',
                pagename, last_updated)
            return last_updated

    if pagename not in ('genindex', 'search'):
        LOG.info(
            '[unilakedocstheme] could not determine last_updated for %r',
            pagename,
        )
...
def setup(app):
    LOG.info('[unilakedocstheme] connecting html-page-context event handler')
