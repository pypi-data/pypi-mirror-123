...
class FileStoreProvider(ProviderBase):
    def list(self):
        raise NotImplementedError()
    def upload(self):
        raise NotImplementedError()
...
class ProviderService():
    ...
    def ConfigProvider(self, config=Configs):
    ...
    def ParameterProvider(self, config=Configs):
    ...
    def SecretProvider(self, config=Configs):
    ...
    def TemplateProvider(self, config=Configs):
    ...
    def StorageProvider(self, config=Configs):
        if not config.storage_provider:
            raise DSOException(MESSAGES['ProviderNotSet'].format('Storage'))
        return self.get_provider('storage/' + config.storage_provider)

    def PackageProvider(self, config=Configs):
    ...
    def ReleaseProvider(self, config=Configs):
        if not config.package_provider:
