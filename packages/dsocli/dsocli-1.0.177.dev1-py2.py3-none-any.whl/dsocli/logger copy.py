...
LEVEL_STYLES = dict(
    ...
    debug=dict(color='green', faint=True, bold=BOLD_LOGS),
)
...
# class DsoLogger(logging.getLoggerClass()):
...
#         ### set formats, and level
#         logging.basicConfig(format=self.get_log_format(), datefmt=LOG_TIME_FORMAT, level=level, force=True)
...
class LoggerClass():
    ...
    def __init__(self, level):
        ...
        # logging.setLoggerClass(DsoLogger)
        self.logger = logging.getLogger('dso')
        # self.logger.root.setLevel(self.mapped_level)
