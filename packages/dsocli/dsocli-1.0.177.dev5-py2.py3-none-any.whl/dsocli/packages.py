...
class PackageService():
    ...
    def version_major(self):
        try:
            ...
            return int(provider.get(key='version.major', service=self.service_name)['Value'])
        except DSOException:
    ...
    def version_major(self, value):
        Providers.ConfigProvider().set(key='version.major', value=str(value), service=self.service_name)
    ...
    def version_minor(self):
        try:
            provider = Providers.ConfigProvider()
            return int(provider.get(key='version.minor', service=self.service_name)['Value'])
        except DSOException:
    ...
    def version_minor(self, value):
        provider = Providers.ConfigProvider()
        provider.set(key='version.minor', value=str(value), service=self.service_name)
    ...
    def version_patch(self):
        try:
            provider = Providers.ConfigProvider()
            return int(provider.get(key='version.patch', service=self.service_name)['Value'])
        except DSOException:
    ...
    def version_patch(self, value):
        provider = Providers.ConfigProvider()
        provider.set(key='version.patch', value=str(value), service=self.service_name)
    ...
    def version_build(self):
        try:
            provider = Providers.ConfigProvider()
            return int(provider.get(key='version.build', service=self.service_name)['Value'])
        except DSOException:
    ...
    def version_build(self, value):
        provider = Providers.ConfigProvider()
        provider.set(key='version.build', value=str(value), service=self.service_name)
