...
class PackageService():
    ...
    def version_major(self):
        try:
            ...
            return int(provider.get(self.config, key='version.major', service='package')['Value'])
        except DSOException:
    ...
    def version_major(self, value):
        Providers.ConfigProvider(self.config).set(self.config, key='version.major', value=str(value), service='package')
    ...
    def version_minor(self):
        try:
            provider = Providers.ConfigProvider(self.config)
            return int(provider.get(self.config, key='version.minor', service='package')['Value'])
        except DSOException:
    ...
    def version_minor(self, value):
        provider = Providers.ConfigProvider(self.config)
        provider.set(self.config, key='version.minor', value=str(value), service='package')
    ...
    def version_patch(self):
        try:
            provider = Providers.ConfigProvider(self.config)
            return int(provider.get(self.config, key='version.patch', service='package')['Value'])
        except DSOException:
    ...
    def version_patch(self, value):
        provider = Providers.ConfigProvider(self.config)
        provider.set(self.config, key='version.patch', value=str(value), service='package')
    ...
    def version_build(self):
        try:
            provider = Providers.ConfigProvider(self.config)
            return int(provider.get(self.config, key='version.build', service='package')['Value'])
        except DSOException:
    ...
    def version_build(self, value):
        provider = Providers.ConfigProvider(self.config)
        provider.set(self.config, key='version.build', value=str(value), service='package')
    ...
    def build(self, config):
        ...
        Logger.info(f"Adding package '{artifactKey}' to the package store...")
        response = ArtifactStore.add(config=config, filepath=artifact, key=artifactKey ,path_prefix=self.PATH_PREFIX)
        return response
