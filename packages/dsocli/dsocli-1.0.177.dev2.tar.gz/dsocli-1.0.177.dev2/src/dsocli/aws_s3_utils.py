...
# def load_s3_path(result, config, bucket, path, filter=None, used_path_prefix='', object_kind='object'):
#     archives = list_s3_objects(bucket, path=path)
#     for archive in archives:
#         key = archive['Key'][len(path)+1:]
#         if filter and not re.match(filter, key): continue
#         if key in result:
#             Logger.warn(f"Inherited {object_kind} '{key}' has been overridden.")
...
# def s3_context_list_files(config, bucket, path_prefix='', uninherited=False, filter=None, object_kind='object'):
...
#         load_s3_path(config=config, bucket=bucket, result=archives, path=path, filter=filter, used_path_prefix=path_prefix, object_kind=object_kind)
#     return archives
...
def s3_context_list_files(config, bucket, path_prefix='', filter=None):
...
def s3_context_add_file(config, filepath, bucket, key, path_prefix=''):
    s3 = boto3.session.Session().client(service_name='s3')
...
def s3_context_get_file(config, bucket, key, path_prefix=''):
    s3 = boto3.session.Session().client(service_name='s3')
    path = get_s3_path(config.context, path_prefix, key)
...
def s3_context_delete_file(config, bucket, key, path_prefix=''):
    s3 = boto3.session.Session().client(service_name='s3')
    path = get_s3_path(config.context, path_prefix, key)
