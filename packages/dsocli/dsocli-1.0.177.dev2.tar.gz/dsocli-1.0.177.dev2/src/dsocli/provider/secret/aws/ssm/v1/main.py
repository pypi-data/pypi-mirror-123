...
class AwsSsmSecretProvider(SecretProvider):
    ...
    def get_path_prefix(self):
        return self.config.secret_spec('pathPrefix')


    def list(self, config, uninherited=False, decrypt=False, filter=None):
        self.config = config
        Logger.debug(f"Listing SSM secrets: namesape={config.namespace}, project={config.project}, application={config.application}, stage={config.stage}")
