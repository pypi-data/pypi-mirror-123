...
class AwsS3ArtifactStoreProvider(ArtifactStoreProvider):
    ...
    def get_bucket_name(self):
        return self.config.artifactStore_spec('bucket')

    ### adds service name to the artifactStore prefix
    def get_path_prefix(self, service):
        storePathPrefix = self.config.artifactStore_spec('pathPrefix')
    ...
    def list(self, config, service, filter=None):
        ...
        items = s3_context_list_files(config=config, bucket=self.get_bucket_name(), path_prefix=self.get_path_prefix(service), filter=filter)
        result = {'Files': items}
        return result


    def add(self, config, filepath, key, service):
        self.config = config
        Logger.debug(f"Adding artifact '{key}' to S3: bucket={self.get_bucket_name()}")
        response = s3_context_add_file(config, filepath=filepath, bucket=self.get_bucket_name(), key=key, path_prefix=self.get_path_prefix(service))
        result = {
    ...
    def get(self, config, key, service, revision=None):
        ...
        response = s3_context_get_file(config, bucket=self.get_bucket_name(), key=key, path_prefix=self.get_path_prefix(service))
        if not response:
            raise DSOException(f"Artifact '{key}' not found in the given context: namespace:{config.namespace}, project={config.project}, application={config.application}, stage={config.short_stage}")
        result = {
                # 'RevisionId': str(response['Version']),
                'Key': key,
                'Context': {
        ...
        #     if not artifact:
        #         raise DSOException(f"Revision '{revision}' not found for artifact '{key}' in the given context: namespace:{config.namespace}, project={config.project}, application={config.application}, stage={config.short_stage}")
        #     result = {
        #             'RevisionId': str(artifact[0]['Version']),
        #             'Date': artifact[0]['LastModifiedDate'].strftime('%Y/%m/%d-%H:%M:%S'),
        #             'Key': key, 
    ...
    def history(self, config, key, include_contents=False):
        ...
        found = locate_ssm_parameter_in_context_hierachy(config=config, key=key, service=self.get_path_prefix())
        if not found:
            raise DSOException(f"Artifact '{key}' not found in the given context: namespace:{config.namespace}, project={config.project}, application={config.application}, stage={config.short_stage}")
        else:
    ...
    def delete(self, config, key, service):
        ...
        response = s3_context_delete_file(config, bucket=self.get_bucket_name(), key=key, path_prefix=self.get_path_prefix(service))
        if not response:
            raise DSOException(f"Artifact '{key}' not found in the given context: namespace:{config.namespace}, project={config.project}, application={config.application}, stage={config.short_stage}")
        result = {
                # 'RevisionId': str(response['Version']),
