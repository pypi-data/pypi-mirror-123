...
class LetsEncrypt:
    ...
    def acme_production(self, csr: str) -> None:  # pragma: no cover
        ...
        subprocess.call('{directory}/acme.sh/acme.sh --set-default-ca --server letsencrypt --signcsr --csr {csr} --dns dns_aws --challenge-alias {domain_validation} --preferred-chain "ISRG Root X1" --force'.format(
            directory=os.getenv('HOME'), csr=f'{csr}', domain_validation=f'{subdomain}.{self.subdelegate}'), shell=True)
