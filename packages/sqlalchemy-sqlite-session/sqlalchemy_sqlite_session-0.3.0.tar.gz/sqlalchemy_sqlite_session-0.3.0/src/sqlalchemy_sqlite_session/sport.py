...
class CaiYanShi(Base):
    ...
    def get_verify(self):
        return [
            '√' if self.v_save_a.find('正确') > -1 else '×',
            '√' if self.v_save_b.find('正确') > -1 else '×',
            '√' if self.v_save_c.find('正确') > -1 else '×',
...
class HuaWuQue(Base):
    ...
    def get_verify(self):
        return [
            '√' if self.v_save_location.find('正确') > -1 else '×',
