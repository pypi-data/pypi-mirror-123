PROJECT_NAME="Equilibrium object from shotfile"

import sys, logging
import numpy as np

from . import sfobj, SFREAD, parse_kwargs
...
class EQU:
    """
    Reads the whols equilibrium shotfile
    ...
    def __init__(self, nshot, **kwargs):
        ...
        if self.sf.status:
            ...
            log.error('Problems opening %s:%s:%d for shot %d' %(exp, diag, ed, nshot))
            return

        log.info('Reading equ scalars')
        self.read_scalars()
        log.info('Reading equ 1d profiles')
        self.read_profiles()
        log.info('Reading equ PFM')
        self.read_pfm()

        log.info('Detected COCO %d' %self.cocos)
...
# Position of mag axis, separatrixx
        ...
        try:
            ...
            log.exception('Signal IpiPSI not found')
    ...
    def get_profile(self, var_name):
        ...
        if var_name not in profs:
            log.error('SignalGroup %s unknown' %var_name)
            return None
...
# Pairs prof, d(prof)/d(psi)
        ...
        if var_name not in mixed:
            log.error('%s not one of the mixed quanties Vol, Area, Pres, Jpol' %var_name)
            return None, None
