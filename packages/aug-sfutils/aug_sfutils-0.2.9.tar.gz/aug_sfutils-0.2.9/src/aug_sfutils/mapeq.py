...
# Trivial case
    ...
    PFL  = eqm.orientation*eqm.pfl
    PSIX = eqm.orientation*eqm.psix
    PSI0 = eqm.orientation*eqm.psi0
    rho_output = np.zeros_like(rho)

    for i in unique_idx:
  
# Calculate a normalized input and output flux 
#            sort_wh = eqm.ind_sort[i][:eqm.lpfp[i]]    # Up to separatrix   
...
def rz2brzt(eqm, r_in, z_in, t_in=None):
    ...
    if r_in.shape != z_in.shape:
        log.error('r_in and z_in must have the same shape')
        return

    if r_in.ndim == 2 and r_in.shape[0] != nt:
        log.error('Time array missmatching first dim of r_in')
        return
...
# COCOS, Eq. 19, 24
    ...
    jc_in    = cocos_in%10 - 1
    ebp_in   = cocos_in//10
    sign_ip  = np.sign(np.nanmean(eqm.ipipsi))
    sign_b0  = np.sign(np.nanmean(eqm.jpol))
    ...
    log.debug('COCOS %d, Sign Bt %d, Sign Ip %d' %(cocos_in, sign_b0, sign_ip))
...
# COCOS, Eq. 24
    ...
    jc_in    = cocos_in%10 - 1
    ebp_in   = cocos_in//10
    sign_ip  =  eqm.orientation
    sign_b0  = -eqm.orientation
    sigma_ip = sign_ip*cocos.sigma['rphiz'][jc_in]
    sigma_b0 = sign_b0*cocos.sigma['rphiz'][jc_in]
    phi_sign = sigma_b0
...
# matplotlib's contour creation
        try:
            ...
            log.exception(R.shape, Z.shape, eqm.pfm[:nr, :nz, i].T.shape)
            raise e 
...
def cross_surf(eqm, rho=1., r_in=1.65, z_in=0, theta_in=0, t_in=None, coord_in='rho_pol'):
    ...
    for i, ii in enumerate(unique_idx):
        ...
        if len(ind_cross) > 2:
            log.warning('More than 2 intersections, problems')
            zout[jt, :] = 0
            Rout[jt, :] = 0
            continue
...
def cross_sep(eqm, r_in=1.65, z_in=0, theta_in=0, t_in=None):
    ...
    line_r = t*np.cos(theta_in) + r_in
    line_z = t*np.sin(theta_in) + z_in
    psi_line = rz2rho(eqm, line_r, line_z, t_in=eqm.time[unique_idx], \
                           coord_out='Psi', extrapolate=True)

    t2 = time.time()
    psi_line *= eqm.orientation
    psi_sep = eqm.pfl[:, max(eqm.lpfp)]*eqm.orientation
    nt_in = len(tarr)
    Rout = np.zeros((nt_in, 2), dtype=np.float32)
    zout = np.zeros((nt_in, 2), dtype=np.float32)
    for i, ii in enumerate(unique_idx):
