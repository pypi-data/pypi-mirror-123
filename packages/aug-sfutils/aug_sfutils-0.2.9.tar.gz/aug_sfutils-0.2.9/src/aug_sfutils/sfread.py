...
from . import sfdics, sfhread, sfobj, manage_ed, parse_kwargs, str_byt
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                log.error('No URL lib found, exiting')
                return None
    log.info('URL reading with %s' %url_lib)
...
def getlastshot():
    ...
    if bshot is None:
        ...
        if page is None:
            ...
            page = str_byt.to_str(page)
            idx = page.find("Current shot:")
            if idx < 0:
                log.warning('Missing substring "Current shot:" in %s, exiting' %url2)
                return None
        ...
        sshot = str_byt.to_str(bshot)
        try:
            nshot = int(sshot)
        except:
            log.exception('ERROR sfread.getlastshot: cannot convert string %s to integer' %sshot)
            nshot = None
...
class SFREAD:
    ...
    def open(self, *args, **kwargs):
        ...
        if hasattr(self, 'sfile'):
            ...
            n_args = len(args) 
            if n_args == 0:
                log.warning('No argument given, need at least diag_name')
                return
            ...
            if 'nshot' not in locals(): 
                log.warning('No argument is a shot number (int), taking last AUG shot')
                nshot = getlastshot()
        ...
        if os.path.isfile(self.sfile):
            ...
            with open(self.sfile, 'rb') as f:
                ...
                if len(byt_str) < 128:
                    log.error('Error: shotfile %s has < 128 bytes, ignored' %(self.sfile))
                    return
                self.sfh = sfhread.read_sfh(byt_str)
                self.status = True
        else:
            log.error('Shotfile %s not found' %self.sfile)
            return

        if self.status:
            self.objects = [ lbl for lbl in self.sfh['obj_nam'] if self.sfh[lbl].obj_type in [6, 7, 8, 13] ]
            self.parsets = [ lbl for lbl in self.sfh['obj_nam'] if self.sfh[lbl].obj_type in [3, 4] ]
        self.cache = {}
    ...
    def getobject(self, obj, cal=False, nbeg=None, nend=None):
        """
...
#            return cache[obj]
        ...
        sfo = self.sfh[obj]
        if sfo.status != 0:
            log.error('Status of SF object %s is %d' %(obj, sfo.status))
            return None
        otyp = sfo.obj_type
        if otyp in (6, 7):
...
# assuming first entry wins
                                if (pscal[mult].size > 1):
                                    pscal[mult] = pscal[mult][0]
                                    pscal[shif] = pscal[shif][0]
    ...
    def ppg_time(self, tbobj):
        ...
        for rel in self.getrelnames(tbobj):
...
#            meta.phys_unit = sfdics.unit_d[unit]
            ...
            log.debug(pname, unit, dfmt, n_items)
            ...
            if dfmt in sfdics.fmt2len.keys(): # char variable
                ...
                if dfmt == 7:
                    ...
                    data = unpack(fmt, buf[j0: j0+bytlen])
                    data = np.squeeze(data[2:])
                dj0 = 8 * ( (bytlen + 7)//8 )
                j0 += dj0

            log.debug(pname, data)
    ...
    def getlist_by_type(self, obj_type=7):
        ...
        return [lbl for lbl in self.sfh['obj_nam'] if self.sfh[lbl].obj_type == obj_type]
    ...
    def getrelnames(self, lbl):
        ...
        return [self.getobjname(jobj) for jobj in rels_id if jobj < 65535]
