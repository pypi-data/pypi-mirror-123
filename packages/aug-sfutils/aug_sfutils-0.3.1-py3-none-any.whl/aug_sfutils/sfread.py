...
from . import sfdics, sfhread, sfobj, manage_ed, parse_kwargs, str_byt, libddc
...
logger = logging.getLogger('sfread')
...
def parse_url(url, blen=100):
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.error('No URL lib found, exiting')
                return None
    logger.info('URL reading with %s' %url_lib)
    ...
    if url_lib == 'urllib3':
        ...
        try:
            ...
            logger.exception(url_not_found)
            return None
    elif url_lib == 'urllib':
        try:
...
def getlastshot():
    ...
    if bshot is None:
        ...
        if page is None:
            ...
            if idx < 0:
                logger.warning('Missing substring "Current shot:" in %s, exiting' %url2)
                return None
        ...
        try:
            ...
            logger.exception('ERROR sfread.getlastshot: cannot convert string %s to integer' %sshot)
            nshot = None
...
class SFREAD:
    ...
    def open(self, *args, **kwargs):
        ...
        if hasattr(self, 'sfile'):
            ...
            if self.sfile is None:
                logger.error('Shotfile not found')
                return
            else:
                self.shot = nshot
    ...
    def __call__(self, name):
        ...
        if name not in self.cache.keys():
            if name in self.objects:
                ...
                logger.error('Signal %s not found in diag %s' %(name, self.diag))
                return None
    ...
    def getobject(self, obj, cal=False, nbeg=None, nend=None):
        ...
        if obj not in self.sfh.keys():
            logger.error('Signal %s not found in diag %s' %(obj, diag))
            return None
...
#            meta.phys_unit = sfdics.unit_d[unit]
            ...
            logger.debug(pname, unit, dfmt, n_items)
