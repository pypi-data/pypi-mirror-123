...
- writes the new shot file header."""
    ...
    def Modpar(self, ps, pn, dat):
        ...
        parrec = self.Lparrec(ps)
        par_dic = {}

        par_dic[pn] = {}
        fmt = parrec[pn][0]
        typ = sfdics.fmt2typ[fmt]
        pnlen = len(dat)
# Input
...
# Output
        ...
        logger.debug('Modpar test: %s %d %d %d', pn, typ, pnlen, len(dat))
        if fmt in sfdics.fmt2len.keys():
            name_len = sfdics.fmt2len[fmt]
            c_len  = ct.c_uint32(pnlen)
