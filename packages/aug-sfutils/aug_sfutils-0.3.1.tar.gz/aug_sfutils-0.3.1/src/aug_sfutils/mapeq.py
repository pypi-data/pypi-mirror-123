...
logger = logging.getLogger('mapeq')
...
# COCOS, Eq. 19, 24
    ...
    logger.debug('COCOS %d, Sign Bt %d, Sign Ip %d' %(cocos_in, sign_b0, sign_ip))
...
def cross_surf(eqm, rho=1., r_in=1.65, z_in=0, theta_in=0, t_in=None, coord_in='rho_pol'):
    ...
    for i, ii in enumerate(unique_idx):
        ...
        if len(ind_cross) > 2:
            logger.warning('More than 2 intersections, problems')
            zout[jt, :] = 0
            Rout[jt, :] = 0
            continue
