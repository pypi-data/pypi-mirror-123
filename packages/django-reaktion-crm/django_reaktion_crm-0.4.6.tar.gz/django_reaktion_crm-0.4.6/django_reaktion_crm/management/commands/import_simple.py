...
def agg_last_visits(uid, days, domain):
    now = datetime.now(tz=tz)
    start = now - timedelta(days)

    count = my_db_tracking['simple_trackings'].aggregate([
        {
            '$match': {
                'uid': uid,
                'domain': domain,
...
def agg_last_7_days_visits(uid, domain):
    return agg_last_visits(uid, 7, domain=domain)


def agg_last_30_days_visits(uid, domain):
    return agg_last_visits(uid, 30, domain=domain)


def agg_all_visits(uid, domain):
...
def create_error_reports(domain, unregistered, unregistered_unique):
    print("raport nr 1")
    ...
    final_csv_name = f"unregister_emails_{domain}.csv"
...
class Command(BaseCommand):
    help = 'Import latest activity from tracking database (simple tracking)'
    ...
    def handle(self, *args, **options):
        ...
        collection_impressions_agg = my_db_tracking['simple_trackings']
        ...
        cursor_agg_impressions = collection_impressions_agg.find({'domain': domain, 'createdAt': {
            '$gte': start_date,
            '$lt': now
        }})
        ...
        for post in cursor_agg_impressions:
            ...
            try:
                ...
                """
                last_7_days = agg_last_7_days_visits(uid, domain=domain)
                last_30_days = agg_last_30_days_visits(uid, domain=domain)
                all_visits = agg_all_visits(uid, domain=domain)
        ...
        if debug is True:
            create_error_reports(domain, unregistered, unregistered_unique)
