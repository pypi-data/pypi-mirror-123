...
from microt_compliance_matrix.utils.get_time_diff import *
...
def combine_intermediate_file(intermediate_participant_path, date_in_study):
    ...
    converter = lambda x: datetime.strptime(x, "%Y-%m-%d %H:%M:%S.%f") if ("." in x) else datetime.strptime(x, "%Y-%m-%d %H:%M:%S")
...
def get_mims_summary(min_before, prompt_time, closest_times, df_logs_combined):
    mims_summary = 0
    num_readings = 0
    start_time = None
    for matched_idx in closest_times:
        closest_time = df_logs_combined.loc[matched_idx, "TIME_DATETIME"]
        if get_min_diff(prompt_time, closest_time) < (min_before + 1):
...
def match_feature(prompt_local_datetime_series, df_logs_combined):
    ...
    min_before_list = list(range(1, 11))
    ...
    for idx in prompt_local_datetime_series.index:
        ...
        if df_logs_combined.shape[0] == 0:
            matched_mims_summary_list = [np.nan] * len(min_before_list)
            matched_readings_list = [np.nan] * len(min_before_list)
            matched_start_time_list = [np.nan] * len(min_before_list)
        else:
            subset_time_list = list(df_logs_combined["TIME_DATETIME"])
            pos = find_closest_time(prompt_time, subset_time_list)
            for min_before in min_before_list:
                closest_times = []
                for i in range(min_before):
                    closest_times.append(pos - 1 - i)
                closest_times = [x for x in closest_times if x >= 0]
                mims_summary, readings, start_time = get_mims_summary(min_before, prompt_time, closest_times,
                                                                      df_logs_combined)
    ...
    for min_before in min_before_list:
        col = min_before - 1
        df = pd.concat([df, mims_summary_df[col].rename("mims_summary_" + str(min_before) + "min")], axis=1)
        df = pd.concat([df, readings_df[col].rename("num_readings_" + str(min_before) + "min")], axis=1)
        df = pd.concat([df, start_time_df[col].rename("start_time_" + str(min_before) + "min")], axis=1)
