...
columns_list = ["Prompt_Type", "Study_Mode", "Initial_Prompt_Local_Time", "Initial_Prompt_UnixTime",
                ...
                "Answer_Status", 'Question_Set_Completion_Local_Time', 'Question_Set_Completion_UnixTime', "Question_X_Answer_Unixtime"]
...
def extract_reprompt(df_pr_watch):
    ...
    for idx in df_pr_watch.index:
        ...
        new_reprompt_row = []
        new_reprompt_row += list(row[['Prompt_Type', 'Study_Mode']])
    ...
    if df_prompt.shape[0] > 0:
        df_prompt.columns = ['Prompt_Type', 'Study_Mode', 'Initial_Prompt_Local_Time', 'Answer_Status', "Actual_Prompt_Local_Time", 'First_Question_Completion_Unixtime', 'UTC_Offset']
...
def clean_response_files(intermediate_participant_path, date_in_study, participant_id):
    ...
    prompt_local_time_timezone_split_len = len(prompt_local_time_timezone_split)
    if prompt_local_time_timezone_split_len == 3:
        converter = lambda x: datetime.strptime(x, "%Y-%m-%d %H:%M:%S") if (type(x) != float) else "NF"
    ...
    df_reprompt = df_reprompt[df_reprompt['Actual_Prompt_Local_DateTime'] != "NF"]
