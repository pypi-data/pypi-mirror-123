...
class ArthurModel(ArthurBaseJsonDataclass):
        ...
        **Examples:**
        ...
        if complete_batch:
            for batch_id, batch_count in batch_counts.items():
                ...
                self._close_dataset(url, batch_count)
    ...
    def set_reference_data(self, directory_path: Optional[str] = None, data: Optional[Union[DataFrame, Series]] = None):
        ...
        if res[DatasetService.COUNTS][DatasetService.FAILURE] != 0:
            ...
            ref_set_close_res = self._close_dataset(f"{self._client.base_path}/models/{self.id}/reference_data",
                                                    num_rows)
    ...
    def send_bulk_ground_truths(self, directory_path: Optional[str] = None,
        ...
        url = f"{self._client.base_path}/models/{self.id}/inferences/file"
        return self._send_parquet_files(url, 'ground_truths.parquet', directory_path, prepped_data,
    ...
    def send_bulk_inferences(self,
        ...
        res = self._send_parquet_files(url, 'inferences.parquet', directory_path, prepped_data,
        ...
        if res[DatasetService.COUNTS][DatasetService.FAILURE] != 0:
            ...
            url = f"{self._client.base_path}/models/{self.id}/batches/{batch_id}"
            batch_res = self._close_dataset(url, num_rows)
    ...
    def _close_dataset(self, url: str, num_inferences: Optional[int] = None) -> Dict:
    ...
    def close_batch(self, batch_id: str, num_inferences: Optional[int] = None) -> Dict:
        ...
        """
        url = f"{self._client.base_path}/models/{self.id}/batches/{batch_id}"
        return self._close_dataset(url, num_inferences)
    ...
    def query(self, body: Dict[str, Any], query_type='base'):
        ...
        if query_type == "base":
            ...
            url = f"{url}/data_drift_psi_bucket_calculation_table"
        else:
    ...
    def update_enrichment(self, enrichment: Enrichment, enabled: bool, config: Optional[Dict[str, Any]] = None) -> Dict[
        str, Any]:
    ...
    def find_hotspots(self,
        ...
        url = f"{self._client.base_path}/models/{self.id}/enrichments/hotspots/find"
        query_params = f"?metric={metric}&threshold={threshold}"
        if batch_id is not None:
            query_params = query_params + f"&batch_id={batch_id}"
        if date is not None:
            query_params = query_params + f"&date={date}"
        if ref_set_id is not None:
            query_params = query_params + f"&ref_set_id={batch_id}"

        url = url + query_params

        resp = self._client.get(url=url, return_raw_response=True)
        validation.validate_response_status(resp, expected_status_code=HTTPStatus.OK)
