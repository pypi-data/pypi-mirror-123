...
class Roi:
    ...
    def unapply(self, image):
        ...
        if full_shape is None:
            assert roi[1] < 0 and roi[3] < 0, "Max rows and cols for roi must be negative if not full shape is given"
        ...
        if len(image.shape) == 2:
            ...
            full_shape = [image.shape[0],] + list(full_shape)
            full_image = np.ma.zeros(full_shape)
            full_image.mask = True
            full_image[:, roi[0]:roi[1], roi[2]:roi[3]] = image
        elif len(image.shape) == 3 and image.shape[0] == 3:
            full_shape = list(full_shape) + [image.shape[2],]
            full_image = np.ma.zeros(full_shape)
            full_image.mask = True
            full_image[roi[0]:roi[1], roi[2]:roi[3], :] = image
        else:
