...
class survey_utilities:
    ...
    def get_tsis_fundamentals(self, vendors = None, industries = None, metrics = ['ADOPTION', 'REPLACING', 'INCREASE', 'DECREASE', 'FLAT'], limit=None):
        ...
        INNER JOIN
            ...
            ON rw.[First] = dw.[First] AND rw.[Last] = dw.[Last] AND rw.[Survey] = dw.[Survey]
        INNER JOIN
            [Classification].[dbo].[Vendor_Universe] vu
            ON rw.[Vendor] = vu.[Vendor]
        WHERE {vendor_query} {industries_query} rw.[Metric] in ('{metric_list}')
        GROUP BY 
            rw.[Survey], rw.[Sector], rw.[Survey Date], rw.[Vendor], rw.[Metric], dw.[Industry], dw.[Region], dw.[Enterprise_Size], dw.[Public_Private], dw.[Fortune100], dw.[Fortune500],dw.[Fortune1000],dw.[SP500],dw.[Global1000],dw.[Global2000],dw.[Private225],dw.[GiantPrivate],dw.[FederalGovernment],dw.[BIG72],dw.[TopTierTelco]
        """
