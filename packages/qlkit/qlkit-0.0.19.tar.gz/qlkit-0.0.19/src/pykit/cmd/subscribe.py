...
def _worker(exchange_name, queue_name, bz_func, transfer, ch, method, properties, body):
    print("exchange %s queue %s Received %r" % (exchange_name, queue_name, body))
    try:
    ...
    if error:
        print("exchange %s queue %s rejected %s" % (exchange_name, queue_name, error))
    else:
        print("exchange %s queue %s done" % (exchange_name, queue_name))
    ch.basic_ack(delivery_tag=method.delivery_tag)
