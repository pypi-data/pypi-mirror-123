...
class AlertCreator:
    ...
    def _set_event_end_time_and_show(self, existing_open_event: Event, end: int, event_end: int) -> Event:
        ...
        if (
            ...
            and (
                not is_string_truthy(existing_open_event.metadata.get(MTD_SENDNOTIFICATION))
                or is_string_truthy(existing_open_event.metadata.get(AIREventMeta.acknowledged))
            )
            and current_time_in_ms() - self.alert_window <= event_end
        ):
            existing_open_event.metadata[MTD_SENDNOTIFICATION] = "True"
            existing_open_event.metadata[AIREventMeta.acknowledged] = "False"
