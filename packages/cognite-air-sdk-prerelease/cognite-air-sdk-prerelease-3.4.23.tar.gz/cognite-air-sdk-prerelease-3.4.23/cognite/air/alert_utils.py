...
from cognite.air.constants import (
    ERROR_MSG_NOTLIST,
    ERROR_MSG_VALIDATION_DF,
    EVENT_END,
    EVENT_ERROR_MESSAGE,
    MTD_NOTIFICATION_MESSAGE,
    MTD_SENDNOTIFICATION,
    MTD_SHOW,
)
...
class AlertCreator:
    ...
    def _retrieve_previous_open_alerts(self, end_point: int) -> List[Event]:
        ...
        filtered_open_alerts = self._filter_open_alerts(open_alerts, end_point)
        return filtered_open_alerts
    ...
    def create_alerts(
        ...
        **Note:** alerts with different alert messages will not be merged.
        ...
        earliest_timestamp = end_point
        if len(deviation_events[0]):
            earliest_timestamp = deviation_events[0][0]

        open_alerts = self._retrieve_previous_open_alerts(earliest_timestamp)
        # close events
    ...
    def _close_event(
        ...
        if len(new_events):
            earliest_timestamp = new_events[0][0]

        existing_open_events = self._filter_open_alerts(existing_events, earliest_timestamp)
        ...
        events_used_for_prolonging = []
        events_not_used_for_prolonging = []
        for existing_open_event in existing_open_events:
            for i, new_event in enumerate(new_events):
                ...
                if self._can_existing_event_be_prolonged(event_end, new_event):
                    print(f"event can be prolonged {existing_open_event.id}")
                    existing_open_event = self._set_event_end_time_and_show(existing_open_event, end, new_event[1])
                    existing_open_event.metadata[EVENT_END] = str(new_event[1])
                    if new_event[0] < existing_open_event.start_time:
                        existing_open_event.start_time = new_event[0]
                    events_used_for_prolonging.append(i)
                else:
                    events_not_used_for_prolonging.append(i)
            ...
            events_to_be_closed.append(existing_open_event)
        # ignore events used for prolonging events since they are now part of the new event
        remaining_new_events = [
            new_events[i] for i in events_not_used_for_prolonging if i not in events_used_for_prolonging
        ]
    ...
    def _can_existing_event_be_prolonged(self, event_end: int, new_event: List[int]) -> bool:
        ...
        """`Checks if the new event can prolong the existing open event`_
        ...
        Returns:
            bool: Either if the new event prolongs the open event (True) or not (False)
        ...
        Examples:
                ...
                False
        """
        # if new event overlaps with event end, it prolongs the existing event
        if new_event[0] <= event_end < new_event[1]:
            return True
        # if the new event is long enough to be a valid alert ...
        event_long_enough = new_event[1] - new_event[0] >= self.minimum_length_of_alert
        # ... and if the start minus merge period is smaller or equal to the existing event end ...
        event_end_close_enough = new_event[0] - self.merge_period <= event_end
        # ... and the new event end is larger that the existing event end ...
        new_event_ends_later = event_end < new_event[1]
        # ... consider it as a prolongment of the event
    ...
    def _set_event_end_time_and_show(self, existing_open_event: Event, end: int, event_end: int) -> Event:
        ...
        if (
            ...
            # only reopen alerts when backfilling is in progress - otherwise user closed alerts will be re-opened
        print(
            is_string_truthy(existing_open_event.metadata.get(MTD_SHOW)),
            self.air_client.backfilling.in_progress,
            not is_string_truthy(existing_open_event.metadata.get(MTD_SENDNOTIFICATION, "False")),
            current_time_in_ms() - self.alert_window <= event_end,
        )
        if (
            is_string_truthy(existing_open_event.metadata.get(MTD_SHOW))
            and self.air_client.backfilling.in_progress
            and not is_string_truthy(existing_open_event.metadata.get(MTD_SENDNOTIFICATION, "False"))
            and current_time_in_ms() - self.alert_window <= event_end
        ):
            existing_open_event.metadata[MTD_SENDNOTIFICATION] = "True"
    ...
    def _filter_open_alerts(self, alerts: List[Event], end_point: int) -> Union[List[Event], List]:
        ...
        if len(open_alerts):
            return open_alerts
        alerts.sort(key=lambda e: e.start_time)
