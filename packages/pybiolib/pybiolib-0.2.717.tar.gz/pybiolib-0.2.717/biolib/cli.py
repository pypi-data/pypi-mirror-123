...
from biolib.lfs import create_large_file_system
...
def main():
    ...
    if len(sys.argv) > 2 and sys.argv[1] == 'run':
        ...
        app_args = sys.argv[3:]
        result = app(args=app_args, stdin=_get_stdin(), files=None)
        ...
        parser_lfs_create.add_argument('--team', required=True, type=str)
        # TODO: Make '--fs-zip' not required when we can accept folders
        parser_lfs_create.add_argument('--fs-zip', required=True, type=str)

        args = parser.parse_args()

        logger.configure(default_log_level=logging.INFO)
        if args.command == 'push':
            push_application(args.author_and_app_name, args.path)
            ...
            if args.lfs_command == 'create':
                create_large_file_system(
                    zip_file_path=args.fs_zip,
                    team_account_handle=args.team,
                )
