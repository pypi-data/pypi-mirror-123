...
def get_flux_query(bucket=None,
    ...
    if time_from is not None:
        if type(time_from) == str:
            time_from = parse(time_from)
        start = time_from.strftime("%Y-%m-%dT%H:%M:%SZ")

    if time_to is not None:
        if type(time_to) == str:
            time_to = parse(time_to)
        stop = time_to.strftime("%Y-%m-%dT%H:%M:%SZ")
    ...
    if aggregate_window:
        ...
        if aggregate_func is None:
            aggregate_func = "mean"
        query_str += f'''\n\t|> aggregateWindow(every: {aggregate_window}, fn: {aggregate_func}, createEmpty: false)'''
