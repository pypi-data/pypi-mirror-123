...
class BigQueryExecutor:
    ...
    def delete_backfill_partitions(self,
                                   backfill_dataset_id,
                                   table_id,
                                   partition_field,
                                   from_partition,
        ...
        """
        Delete partitions from table in backfill dataset between from_partition and to_partition

        Args:
            backfill_dataset_id (string): dataset where the backfill tables are created
            table_id (string): table in backfill dataset that the partitions will be deleted from
            partition_field (string): the time-unit column that the table is partitioned by
            from_partition (date): date of the first partition to be deleted
            to_partition (date): date of the latest partition to be deleted
        ...
        sql = f"""
               DELETE
                 `{backfill_dataset_id}.{table_id}`
               WHERE
                 CAST({partition_field} AS DATE) BETWEEN
                   CAST('{from_partition}' AS DATE) AND CAST('{to_partition}' AS DATE)
               """
    ...
    def run_full_backfill(self,
        ...
        """
        Run a full backfill for a time-unit column partitioned table

        Args:
            dataset_id (string): dataset of the table to be backfilled
            backfill_dataset_id (string): dataset where the backfill tables are created
            table_id (string): name of the table to be backfilled
            schema_path (string): file path to the schema of the table to be backfilled
            sql (string): query that's used in the pipeline to create this table
                Note: it has to contain the clause:
                WHERE <partition_field> BETWEEN CAST(<from_partition> AS DATE) AND CAST(<to_partition> AS DATE)
            partition_field (string): the time-unit column that the table is partitioned by

        Returns:
            None
        """
        logging.info("Starting full backfill")

        logging.info("Creating backfill dataset if it doesn't exist")
        self.create_dataset(
            dataset_id=backfill_dataset_id
        )

        logging.info("Creating table in backfill dataset with all partitions")
        self.create_table(
            dataset_id=backfill_dataset_id,
            table_id=table_id,
            schema_path=schema_path,
        ...
        logging.info("Replacing prod table with backfill table")
        self.copy_table(
            source_dataset_id=backfill_dataset_id,
        ...
        logging.info("Deleting backfill table")
        self.delete_table(
            dataset_id=backfill_dataset_id,
            table_id=table_id
        )
    ...
    def run_partial_backfill(self,
        ...
        """
        Run a partial backfill for a time-unit column partitioned table between the dates
        from_partition and to_partition
        ...
        Args:
            ...
            sql (string): query that's used in the pipeline to create this table
                ...
                WHERE <partition_field> BETWEEN CAST(<from_partition> AS DATE) AND CAST(<to_partition> AS DATE)
            partition_field (string): the time-unit column that the table is partitioned by
            from_partition (date): date of the first partition to be backfilled
            to_partition (date): date of the latest partition to be backfilled

        Returns:
            None
        """
        logging.info("Starting partial backfill")
        ...
        logging.info("Copying table to backfill dataset")
        self.copy_table(
            source_dataset_id=dataset_id,
            source_table_id=table_id,
            dest_dataset_id=backfill_dataset_id,
            dest_table_id=table_id,
            write_disposition="WRITE_TRUNCATE"
        )

        logging.info("Deleting partitions to be backfilled")
        self.delete_backfill_partitions(
            backfill_dataset_id,
            table_id,
            partition_field,
            from_partition,
        ...
        logging.info("Appending new backfill partitions")
        self.create_table(
            dataset_id=backfill_dataset_id,
            table_id=table_id,
            schema_path=schema_path,
    ...
    def run_backfill(self,
        ...
        """
        Run a full or partial backfill for a time-unit column partitioned table

        Args:
            backfill (string): specifies full or partial backfill (if running a backfill)
            dataset_id (string): dataset of the table to be backfilled
            table_id (string): name of the table to be backfilled
            schema_path (string): file path to the schema of the table to be backfilled
            sql (string): query that's used in the pipeline to create this table
        ...
        backfill_dataset = "backfill"

        if backfill not in ("full", "partial"):
            raise BigQueryExecutorError("BACKFILL env var needs to be 'full' or 'partial'")

        if backfill == "full":
            self.run_full_backfill(
                dataset_id=dataset_id,
                backfill_dataset_id=backfill_dataset,
                table_id=table_id,
                schema_path=schema_path,
                sql=sql,
                partition_field=partition_field
        ...
        elif backfill == "partial":
            self.run_partial_backfill(
                dataset_id=dataset_id,
                backfill_dataset_id=backfill_dataset,
                table_id=table_id,
                schema_path=schema_path,
    ...
    def create_partitions_legacy(self,
                                 sql,
                                 file,
                                 partition_dates,
                                 table_id,
    ...
    def create_partition_table(self,
                               ...
                               bq_end_date=bq_end_date(),
                               sql=None,
                               file=None,
                               use_legacy_sql=False,
                               write_disposition='WRITE_TRUNCATE',
                               partition_dates=None,
                               pipeline_partition_date=None,
                               partition_field='_PARTITIONTIME',
                               clustering=None,
                               priority="INTERACTIVE",
                               schema_path="",
        ...
        """
        New and legacy ways of creating partition tables:
        ...
        New:
        pipeline_partition_date is required for a normal pipeline run and is a single date.
        To run a backfill, the BACKFILL env var needs to be set to 'full' or 'partial' and an optional date range 
        can be set for a partial backfill by setting the FROM_PARTITION and TO_PARTITION env vars.
        """
        if sql is None and file is None:
            raise BigQueryExecutorError("Either SQL or file containing the SQL must be provided")
        ...
        if "BACKFILL" not in os.environ:
            if pipeline_partition_date is None: # legacy approach
                self.create_partitions_legacy(
                    sql=sql,
                    file=file,
                    partition_dates=partition_dates,
                    table_id=table_id,
            ...
            else: # new approach, no backfill
                from_partition = pipeline_partition_date
                to_partition = pipeline_partition_date
                sql = read_sql(
                    file=file,
                    from_partition=from_partition,
                    to_partition=to_partition,
                ...
                # change date format from YYYY-MM-DD to YYYYMMDD for partition_id 
                partition_id = pipeline_partition_date.replace("-", "")
                partition_name = self.set_partition_name(
                    table=table_id,
                    date=partition_id
                )
        ...
        else: # new approach, run backfill
            from_partition = os.environ.get("FROM_PARTITION", bq_start_date)
            to_partition = os.environ.get("TO_PARTITION", bq_end_date)
            sql = read_sql(
                file=file,
                from_partition=from_partition,
                to_partition=to_partition,
