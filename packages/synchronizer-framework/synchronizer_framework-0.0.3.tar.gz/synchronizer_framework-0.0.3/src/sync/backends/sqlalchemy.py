...
class SQADataSource(DataSource):
    def __init__(
        self, url="sqlite+pysqlite:///testdb.sqlite", base: Type = declarative_base(), create_metadata=True
    ):
        self.url = url
        self.base = base
    ...
    def find(self, by_type: Type, by_conditions) -> List[object]:
    ...
    def all(self, of_type, limit=None) -> set:
        return self.session.query(of_type).limit(limit).all()
