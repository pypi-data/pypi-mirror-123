...
class Cloudformation(o7lib.aws.base.Base):
    ...
    def LoadStacks(self, stackName = None):
        """Returns all Stacks this Session"""
    ...
    #
    #*************************************************
    def UpdateParameter(self, stackName, paramKey, newValue):
        """Initialize the drift verification for a stack"""

        logger.info(f'UpdateParameter: {stackName=} {paramKey=} {newValue=}')

        stacks = self.LoadStacks(stackName=stackName)
        if len(stacks) == 0:
            logger.error(f'UpdateParameter: Stack: {stackName} was not found')
            return None
        ...
        # Building Parameter structure
        for parameter in stack.get('Parameters',[]):

            newParameter = {'ParameterKey' : parameter['ParameterKey']}
            if parameter['ParameterKey'] == paramKey:
                newParameter['ParameterValue'] = newValue
                found = True
            else:
                newParameter['UsePreviousValue'] = True

            newParameters.append(newParameter)


        if found is False:
            logger.error(f'UpdateParameter: key: {paramKey} was not found')
            return None
        ...
        pprint.pprint(param)
        resp = self.cfn.update_stack(**param)
        pprint.pprint(resp)

        logger.info('UpdateParameter Response : {resp}')
    ...
    def DisplayStacks(self, stacks):
        ...
        self.ConsoleTitle(left='Cloudformation Stacks')
        print('')
    ...
    def DisplayEvents(self, events, maxEvent = None):
        ...
        self.ConsoleTitle(left=f'Cloudformation Events for Stack: TBD')
        print('')
        ...
        o7lib.util.displays.Table(params, events)
    ...
    def DisplayResources(self, resources):
        ...
        self.ConsoleTitle(left=f'Cloudformation Resources for Stack: TBD')
        print('')
        ...
        params = {
            'columns' : [
                {'title' : 'Logical ID' , 'type': 'str',     'dataName': 'LogicalResourceId'},
                {'title' : 'Physical ID' , 'type': 'str',    'dataName': 'PhysicalResourceId', 'maxWidth' : 60},
                {'title' : 'Updated'     , 'type': 'datetime','dataName': 'LastUpdatedTimestamp'},
                {'title' : 'Status'     , 'type': 'str',     'dataName': 'ResourceStatus', 'format' : 'aws-status'},
                {'title' : 'Reason'     , 'type': 'str',     'dataName': 'ResourceStatusReason'},
                {'title' : 'Status'     , 'type': 'str',     'dataName': 'DriftStatus', 'format' : 'aws-drift'},
            ]
        }

        o7lib.util.displays.Table(params, resources)

    #*************************************************
    #
    #*************************************************
    def DisplayDrifters(self, drifters):
        ...
        params = {
            'title' : f"List of Drifts - {self.TitleLine()}",
            'columns' : [
                {'title' : 'Logical ID' , 'type': 'str',     'dataName': 'LogicalResourceId'},
        ...
        o7lib.util.displays.Table(params, drifts)

    #*************************************************
    #
    #*************************************************
    def DisplayParameters(self, parameters, maxParameters = None):
        """Displays a summary of Parameters in a Table Format"""
        params = {
            'columns' : [
                {'title' : 'id'      , 'type': 'i',       'minWidth' : 4  },
                {'title' : 'Key'            , 'type': 'str',     'width' : 30, 'dataName': 'ParameterKey'},
    ...
    def DisplayOutputs(self, outputs, maxOutput = None):
        ...
        params = {
            'columns' : [
                {'title' : 'Key'            , 'type': 'str',     'width' : 30, 'dataName': 'OutputKey'},
        ...
        o7lib.util.displays.Table(params, outputs)
    ...
    def DisplaySingleStack(self, stack):
        ...
        print('')
        print(f'Parameters')
        self.DisplayParameters(stack.get("Parameters", []))
        print('')
        print(f'Outputs')
    ...
    def MenuSignleStack(self, stackName):
        ...
        while True :
            ...
            keyType, key = o7lib.util.input.InputMulti('Option -> Back(b) Raw(r) Events(e) Resources(s) Init Drift Detection(d) Edit Parameters (p) : ')
            ...
            if keyType == 'str':
                ...
                if key.lower() == 'r':
                    ...
                    o7lib.util.input.WaitInput()
                if key.lower() == 'p':
                    pId = o7lib.util.input.InputInt('Enter Parameter Id to Edit -> ')
                    if len(stack.get("Parameters", [])) < pId <= 0 :
                        print('Invalid Parameter Id')
                        continue
                    pKey = stack.get("Parameters")[pId-1]["ParameterKey"]
                    pValue = o7lib.util.input.InputString(f'Enter new value for {pKey} -> ')
                    if o7lib.util.input.IsItOk(f'Confirm that {pKey} -> {pValue}'):
                        self.UpdateParameter(stackName=stackName, paramKey=pKey, newValue=pValue)
    ...
    def MenuEvents(self, stackName):
        ...
        while True :
            ...
            keyType, key = o7lib.util.input.InputMulti('Option -> Back(b) All(a): ')
