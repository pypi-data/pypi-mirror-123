...
from azureml._common._error_definition.user_error import InvalidDimension
...
from ..datasets.timeseries_datasets import (  # noqa: F401
    AbstractValidationTimeSeriesDataset, TimeSeriesDataset, ValidationTimeSeriesDatasetFromTrainValid
)
...
metrics = REGRESSION_SET | FORECASTING_SET
...
class RunUpdateCallback(Callback):
        ...
        if self._is_validation_data_available():
            ...
            if self.ds_valid.cross_validation:
                ...
                for i in eval_list:
                    ...
                    data_loader = self.model_wrapper.create_data_loader(eval_set.val_dataset, num_workers=0)
                    y_pred = self.model_wrapper._predict(data_loader=data_loader).reshape(-1)
                    if(y_pred.shape[0] != eval_set.y_valid.reshape(-1).shape[0]):
                        argument = "invalid prediction dimenstion, y shape {}, pred shape {}, len of reader {}".format
                        (eval_set.y_valid.reshape(-1).shape[0], y_pred.shape[0], len(eval_set.val_dataset))
                        raise ValidationException._with_error(AzureMLError.create(
                            InvalidDimension, argument_name=argument, target="y_pred"))

                    slice_scores = compute_metrics(eval_set.X_valid,
                                                   eval_set.y_valid,
                                                   eval_set.X_train,
                                                   eval_set.y_train,
                                                   y_pred,
                    ...
                    scores = slice_scores if len(cv_scores) > 1 else aggregate_scores(cv_scores, metrics)
    ...
    def _save_model_for_automl_inference(self, model_id: str,
        ...
        try:
            scoring_file_str_v1, scoring_file_str_v2 = self._get_inference_file_content(logger)
