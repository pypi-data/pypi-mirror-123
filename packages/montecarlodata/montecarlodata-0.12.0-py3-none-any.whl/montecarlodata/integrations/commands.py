...
CONNECTION_OPTIONS = [
    click.option('--connection-id', help='ID for the connection.', required=True, type=click.UUID, callback=convert_uuid_callback)
]
...
@integrations.command(help=f'Toggle S3 metadata events for Hive/Presto. {EVENT_VERBIAGE}.')
@click.pass_obj
...
@integrations.command(help=f'Toggle S3 metadata events for Glue/Athena. {EVENT_VERBIAGE}.')
@click.pass_obj
@add_common_options(EVENT_OPTIONS)
@click_config_file.configuration_option(settings.OPTION_FILE_FLAG)
...
@click.pass_obj
@click.argument('ssh-key', required=True, type=click.Path(dir_okay=False, exists=True))
@click.option('--repo-url', help='Repository URL as ssh://[user@]server/project.git or the shorter '
                                 'form [user@]server:project.git.', required=True)
@add_common_options(ONBOARDING_CONFIGURATION_OPTIONS)
...
@integrations.command(help=f'List all active connections.', name='list')
@click.pass_obj
@click_config_file.configuration_option(settings.OPTION_FILE_FLAG)
...
@integrations.command(help=f'Update credentials for a connection.', name='update')
@click.pass_obj
...
@integrations.command(help=f'Remove an existing connection. Deletes any associated jobs, monitors, etc.', name='remove')
@click.pass_obj
@add_common_options(CONNECTION_OPTIONS)
...
@integrations.command(help=f'Retest an existing connection.', name='test')
@click.pass_obj
@add_common_options(CONNECTION_OPTIONS)
@click_config_file.configuration_option(settings.OPTION_FILE_FLAG)
...
@integrations.command(help=f'Setup S3 event notifications for a lake.')
@click.pass_obj
