...
def strToFloat(value):
    num='0'
    uni=''
    units={'f':1e-15,'p':1e-12,'n':1e-9,'µ':1e-6,'m':1e-3,' ':1.0,'k':1e+3,'K':1e+3,'M':1e+6,'T':1e+9}
    m=1.0;
    value=value+' ';
...
class param (object):
      def __init__(self,Value=0.0, Unit='', Description='',List=[]):
          self.Val=Value
          self.Unit=Unit
...
class signal(object):
     def __init__(self,dir_,pf,Porta='0', Portb='0'):
         ...
         if self.pf=='voltage':
             self.pf='potential'
         elif self.pf=='current':
             self.pf='flow'
         elif  self.pf=='sys':
             self.pf='potential'
             self.sys=True

         if self.pf=='potential':
              self.AbsTol=1.0e-6
              self.Unit='V'
         self.ItAcSource=False
         self.Potential=self.pf=='potential'
     def __name__(self):
         return 'signal'
     def __str__(self):
...
class analysis:
      ...
      def getoutput_(self,out_):
                 ...
                 if type(out_)==str:
                    ...
                    return self.x[j],out_
                 elif  type(out_).__name__=='signal':
                    return self.Val[out_.pos],out_.name
                 elif  type(out_).__name__=='param':
                    return out_.Val,out_.name
                 else:
                    return 0
