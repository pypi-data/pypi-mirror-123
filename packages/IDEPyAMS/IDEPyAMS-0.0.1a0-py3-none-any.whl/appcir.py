...
#-------------------------------------------------------------------------------
def pyamscircuit(path,result):
    netList=result[1]

    data=[]
    data=['from sys import path;'];
    data+=['path+=["'+path+'/lib/analysis"];'];

    elems=[];

    for i in range(len(netList)):
        s='path+=["'+path+'/symbols/'+netList[i]['directory']+'"];'
        if not(s in data):
            data+=[s];
...
def opAnalysis(self,result):
    data=pyamscircuit(self.pathapp,result);
...
def getListSignalsNodeParams(self,result):
    ...
    for i in range(len(v)):
       ...
       for j in range(len(signals)):
          id=id+1;
          data += [{'unique_id': id, 'parent_id': r, 'short_name': signals[j].name, 'type': 'signal', 'description': signals[j].Stype, 'dir': signals[j].dir}]
