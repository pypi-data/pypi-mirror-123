...
from sym.flow.cli.errors import SymAPIError, SymAPIMissingEntityError, UnknownOrgError
from sym.flow.cli.helpers.config import Config
from sym.flow.cli.models import Organization, service
from sym.flow.cli.models.service import HIDDEN_SERVICE_TYPES, Service
from sym.flow.cli.models.service_type import ServiceType
from sym.flow.cli.models.user import User

from .rest import SymRESTClient
...
class SymAPI:
    ...
    def verify_login(self, email: str) -> bool:
        ...
        response = self.rest.get("auth/login", {"email": email}, validate=False)
    ...
    def get_users(self) -> List[User]:
        ...
        response = self.rest.get("users").json()
    ...
    def get_services(self) -> List[Service]:
        """Retrieve all Services registered to the currently
        authenticated user's organization.
        """
        response = self.rest.get("services").json()
        services = parse_obj_as(List[Service], response["services"])

        return [s for s in services if s.service_type not in HIDDEN_SERVICE_TYPES]
    ...
    def delete_service(self, service_type: ServiceType, external_id: str):
