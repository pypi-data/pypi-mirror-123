...
from hestia_earth.models.cycle.dataCompleteness.soilAmendments import run, MODEL_KEY
...
def test_run(mock_measurement):
    ...
    # with soil ph below 6.5 => no run
    measurement['value'] = [6]
    assert not run({})

    # with soil ph above 6.5 => no run
    measurement['value'] = [7]
    assert not run({})

    # with added soil ph => run
