...
def init():
    ...
    for n in connect.nodes:
        ...
        f'with the local master: {local.id[n - 1] // 1000}-{local.id[n - 1] % 1000} Software Version {local.soft_ver}')
        print(f'With following modules:')
        for i in range(local.number_of_bricks[n]):
            print(f'{i + 1}: {slave.id[n, i + 1] // 1000}-{slave.id[n, i + 1] % 1000} with Hardware Revision {slave.hard_rev}')
