...
def init():
    ...
    for n in range(len(connect.node)):
        print(f'\nNode {n+1} connected over the IP: {connect.ip_address[n]} \nwith the local master: {local.id[n] // 1000}-{local.id[n] % 1000}' Software Version: {local.soft_ver})
        print(f'With following modules:')
        for i in range(local.number_of_bricks[n]):
            print(f'{i + 1}: {slave.id[n, i ] // 1000}-{slave.id[n, i] % 1000} with Hardware Revision {slave.hard_rev}')
