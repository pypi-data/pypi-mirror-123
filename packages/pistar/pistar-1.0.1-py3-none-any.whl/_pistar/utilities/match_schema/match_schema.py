...
from _pistar.utilities.exceptions import AssertionFailureException
from _pistar.utilities.exceptions import EnumerationFailureException
from _pistar.utilities.exceptions import MissingPropertyException
from _pistar.utilities.exceptions import TypeMismatchException
...
def get_lambda_expression(lambda_expression):
    ...
    """
    if lambda_expression.__doc__:
        return lambda_expression.__doc__
    return None
...
def match_schema(
    ...
    if not isinstance(value, types):
        return TypeMismatchException(
            name=get_value_name(name, parents), value=value, expect_type=[type.__name__ for type in types]
        )
    ...
    if any([issubclass(type, dict) for type in types]):
        # if there is not properties field, it is fine.
    ...
    elif any([issubclass(type, (list, tuple)) for type in types]):
        # if there is not items field, it is fine.
