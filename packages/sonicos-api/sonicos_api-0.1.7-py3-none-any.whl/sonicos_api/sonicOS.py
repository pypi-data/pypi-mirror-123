...
def fwLogin(fwAddress: str, fwUser: str, fwPassword: str, verifySSLCert: bool):
    ...
    response = requests.request("POST", url, data=payload, headers=headers, verify=verifySSLCert, timeout=0.6)
    if (isSuccessStatus):
...
def fwLogout(fwAddress: str, verifySSLCert: bool):
    ...
    response = requests.request("DELETE", url, data=payload, headers=headers, verify=verifySSLCert, timeout=0.6)
    if (isSuccessStatus):
        response = response.text
        response = json.loads(response)
    return response
...
def commitChanges(fwAddress: str, verifySSLCert: bool):
    ...
    payload = ""
    response = requests.request("POST", url, headers=headers, data=payload, verify=verifySSLCert, timeout=0.6)
    if (isSuccessStatus):
        response = response.text
        response = json.loads(response)
    return response
...
def getCFSProfiles(fwAddress: str, verifySSLCert: bool):
    ...
    response = requests.request("GET", url, headers=headers, verify=verifySSLCert, timeout=0.6)
    if (isSuccessStatus):
        response = response.text
        response = json.loads(response)
    return response
...
def getCFSLists(fwAddress: str, verifySSLCert: bool):
    ...
    payload = ""
    response = requests.request("GET", url, headers=headers, data=payload, verify=verifySSLCert, timeout=0.6)
    if (isSuccessStatus):
        response = response.text
        response = json.loads(response)
    return response
...
def insertIntoCFSList(fwAddress: str, cfsName: str, uri: str, verifySSLCert: bool):
    ...
    response = requests.request("PUT", url, json=payload, headers=headers, verify=verifySSLCert, timeout=0.6)
    if (isSuccessStatus):
        response = response.text
        response = json.loads(response)
    return response
...
def removeFromCFS(fwAddress: str, cfsName: str, uri: str, verifySSLCert: bool):
    ...
    response = requests.request("DELETE", url, json=payload, headers=headers, verify=verifySSLCert, timeout=0.6)
    if (isSuccessStatus):
        response = response.text
        response = json.loads(response)
    return response
