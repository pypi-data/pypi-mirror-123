...
from mystran_validation import get_conf, get_profile, init_config, cleandir
...
def run(ctx, report, open_report, dump, pytest_args):
    ...
    if open_report is None:
        open_report = bool(int(ctx.obj["profile"]["report"]))
    # -------------------------------------------------------------------------
    ...
    click.echo(
        click.style(
            (
                "  * mystran_bin={MYSTRAN_BIN}\n"
                "  * rootdir={MYSTRAN_VALIDATION_ROOTDIR}\n"
                "  * builddir={MYSTRAN_VALIDATION_BUILDDIR}\n"
            ).format(**os.environ),
            fg="green",
        )
    )
    click.echo(
        click.style(
            "  * MYSTRAN {}".format(os.environ["MYSTRAN_VALIDATION_VERSION"]),
            fg="green",
        )
    )
    click.echo("\n")
    ...
    if report:
        ...
        index, html = xml2html(junit_file)
