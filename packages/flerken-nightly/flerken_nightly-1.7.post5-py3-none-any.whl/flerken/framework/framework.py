...
class Trainer(DinamicProperty):
    ...
    def allocate_gt(self, vrs: dict) -> torch.device:
        if isinstance(vrs['pred'], (list, tuple)):
            device = vrs['pred'][0].device
        elif isinstance(vrs['pred'], torch.Tensor):
            device = vrs['pred'].device
        else:
            device = self.alloc.main_device
        return device

    def run_epoch(self, dataloader, state, backprop, metrics=[], checkpoint=lambda: None, allocate_input=True,
                  allocate_gt=True, send={}, debug={'isnan': False, 'verbose': False}, free_mem=False):
        self.state = state
        j = -1
        ...
        with tqdm(dataloader,
            ...
            for x in pbar:
                # This placeholder is important for the memory to be fred in case the graph is not exhausted
                # Aka (backward not called)
                # Otherwise this can lead to OOM issues under certain conditions
                vrs = {}
                results = None
                self._loss = None
                if free_mem or self.free_mem:
