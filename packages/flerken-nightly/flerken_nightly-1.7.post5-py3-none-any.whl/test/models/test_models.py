...
class TestUNet(unittest.TestCase):
    ...
    def test_dynamic_maxpool(self):
        vector_array = ['ststs', 'ttsts', 'ttttt', 'ftfts', 'ffsts', 'ftftt']
        for vector in vector_array:
            model = UNet([16, 32, 64, 128, 256, 512], 1, None,
                         layer_kernels=vector,
                         useBN=True, verbose=True)
            out = model(self.x, self.c)
    ...
    def test_channel_expansion(self):
        model = UNet([16, 32, 64, 124], 1, None, activation=torch.nn.Sigmoid(), useBN=True, verbose=True)
        out = model(self.x, self.c)
        model = UNet([24, 12, 64, 124,32], 1, None, activation=torch.nn.Sigmoid(), useBN=True, verbose=True,
                     architecture='sop')
        out = model(self.x, self.c)
    def test_unet_bnmomentum(self):
        ...
        for i in range(len(model.encoder._modules)):
            assert model.encoder._modules[str(0)].layer._modules[str(1)].momentum == MOMENTUM
            assert model.encoder._modules[str(0)].opt_layer._modules[str(1)].momentum == MOMENTUM
        for i in range(len(model.decoder._modules)):
            assert model.decoder._modules[str(0)].layer._modules[str(1)].momentum == MOMENTUM
            assert model.decoder._modules[str(0)].opt_layer._modules[str(1)].momentum == MOMENTUM
