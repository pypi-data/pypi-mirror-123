...
class TestFramework(unittest.TestCase):
    ...
    def test_train(self):
        ...
        with self.ex.autoconfig(trainer) as trainer:
            for trainer.epoch in range(trainer.EPOCHS):
                trainer.run_epoch(dataloader, 'train', checkpoint=trainer.checkpoint(), backprop=True,
                                  debug={'isnan': True})
    ...
    def test_keyboard_interrupt_checkpoint(self):
        ...
        with self.assertRaises(KeyboardInterrupt), self.ex.autoconfig(trainer) as trainer:
            for trainer.epoch in range(trainer.EPOCHS):
                trainer.run_epoch(dataloader, 'train', checkpoint=trainer.checkpoint(), backprop=True, debug={
                    'isnan': True})
                if trainer.epoch == 5:
    ...
    def test_freq_checkpoint(self):
        ...
        with self.ex.autoconfig(trainer) as trainer:
            for trainer.epoch in range(trainer.EPOCHS):
                trainer.run_epoch(dataloader, 'train', checkpoint=trainer.checkpoint(freq=2), backprop=True,
                                  debug={'isnan': True})
