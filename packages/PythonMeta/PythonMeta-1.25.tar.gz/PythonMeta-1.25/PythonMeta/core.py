...
import os
import math
import statsmodels.api as sm             #for Egger's test
import statsmodels.formula.api as smf    #for Egger's test
import matplotlib
...
class Data:
    ...
    def getdata(self,lines):
        ...
        if len(rult)<1:
            Err="error.(no data loaded, check data syntax or data type.)"
            raise Exception(Err)
    ...
    def readfile(self,fname=''):
        if not fname:
            fname =  os.path.join(os.path.normpath(os.getcwd()),'studies.txt')
        try:
            f = open(fname,"r")
            lines = f.readlines()   
        except :
...
class Meta:
    ...
    def meta (self, studies, nosubgrp=False):
        ...
        stds=[];rult=[]
        results=self.meta0 (studies)  #get total
        w_rults = [w[2] for w in results[1:]]   #for subgrp's weight
        ...
        for i in range(1, len(subgrp)):
            ...
            rult[0][2]= w_grp                #reset subgroup's weight
            pos_grp += len(subgrp[i])
            results.insert(pos_grp, rult[0])          #insert subgroup into total results
            stds=[]

        return results

    #Egger's test for pubilsh bias
    ...
    def Eggers_test (self, rults):
        eggers = Stat_Eggers_test(rults) 
        return eggers
...
def Stat_Eggers_test(rults):
    if rults[0][0] in "OR,RR" :
        def _x_tran0(x):
            return math.log(x)
        def _x_tran1(x):
            return math.exp(x)
    elif rults[0][0] in "RD,MD,SMD" :
        def _x_tran0(x):
            return x
        def _x_tran1(x):
            return x
    else:
        Err="error.(failed to get effect size while Eggers_testing)"
        raise Exception(Err)
        
    yArr = []
    xArr = []        
    for i in range(1,len(rults)):
        yArr.append(_x_tran0(rults[i][1])/rults[i][6])
        xArr.append(1/rults[i][6]) 
        
    xArr = sm.add_constant(xArr)
    model = sm.OLS(yArr,xArr)
    results = model.fit()
    ttest = results.t_test([1,0])
     
    #print(results.summary(),"\n\n")
    #print("Intercept:",results.params[0])
    #print("Intercept:",ttest.effect[0])
    #print("t:",ttest.tvalue[0][0])
    #print("p:",ttest.pvalue)
    #print("sd:",ttest.sd[0][0])
    #print("CI:",ttest.conf_int()[0])
    return (ttest.effect[0],       #Intercept
            ttest.tvalue[0][0],    #t-value
            ttest.pvalue,          #p-value
            ttest.sd[0][0],        #std. dev.
            ttest.conf_int()[0])   #CI
###########################################
'''
...
def CATE_chkzero (study):    
    ...
    if a+b==0 or c+d==0:  #n1=0 or n2=0? should be wrong
        Err="error.(data wrong, too many zeros)"
        raise Exception(Err)
    if (a*b*c*d)==0 :
...
def CONT_Heg_SMD (study):
    ...
    md=m1-m2
    smd=md*(1-3/(4*N-9))/s
    se=math.sqrt(N/(n1*n2)+(smd*smd)/(2*(N-3.94)))
    return [smd,se]
...
def MH_total_OR (studies,models='Fixed'):
    ...
    for idx in range(len(studies)):
        ...
        a,b,c,d=CATE_chkzero (studies[idx][0:4])
        N=a+b+c+d
        r1+=a*d/N
        r2+=b*c/N
        r3+=((a+d)*a*d/(N*N))
...
def Fig_Funnel (size,dpi,effs):
    if effs[0][0] in "OR,RR" :
        ...
        Err="error.(failed to get effect size while drawing funnel plot)"
        raise Exception(Err)
        
    myfig = plt.figure(num="Funnel plot by Pymeta",linewidth=1,figsize=size,dpi=dpi)
    myfig.set_size_inches(size)
    x=[];y=[];ci=[]
    ...
    plt.plot ([x0,x0],[0,ymax],color="black", linestyle="--", lw=1)
    #plt.plot ([effs[0][1],effs[0][1]-1.96*ymax],[0,ymax],color="blue", linestyle="--", lw=1)
    #plt.plot ([effs[0][1],effs[0][1]+1.96*ymax],[0,ymax],color="blue", linestyle="--", lw=1)
    plt.plot ([x0,2*x0-cimax],[0,ymax],color="blue", linestyle="--", lw=1)
    plt.plot ([x0,cimax],[0,ymax],color="blue", linestyle="--", lw=1)
    ...
    if len(effs)>10:
        eggers = Stat_Eggers_test(effs)
        test_txt = "Egger's test:\nIntercept=%.2f\np-value=%.3f"%(eggers[0],eggers[2])    
        #plt.rcParams['font.sans-serif'] = ['Arial Unicode MS']
        plt.text(0.8,0.9, s=test_txt, transform=ax.transAxes)
    
    myfig.tight_layout() #保证边界显示完全
    return myfig
