...
class Site(object):
    ...
    Probably deprecated. The maximal file size must be configured on the web
    server, having it also controlled by Lino is redundant and would not protect
    against disk space issues.  OTOH it might make sense to have this in Lino if
    we want to be able to put a smaller limit per Lino site.
    ...
    *within* the framework.
    ...
    def install_settings(self):
        ...
        if self.build_js_cache_on_startup or self.never_build_site_cache:
            self.site_cache_root = self.cache_dir.child('site_cache')
        else:
            self.site_cache_root = self.django_settings['MEDIA_ROOT']
        ...
        if self.build_js_cache_on_startup or self.never_build_site_cache:
            sfd = list(self.django_settings.get('STATICFILES_DIRS', []))
            sfd.append(self.site_cache_root)
            self.update_settings(STATICFILES_DIRS=sfd)
...
site. :manage:`diag` is a command-line shortcut to this.
    ...
    def site_config(self):
        ...
        if self._site_config is None:
            ...
            #~ from django.db.utils import DatabaseError
            try:
                self._site_config = SiteConfig.objects.get(
                    id=self.config_id)
...
Please convert to Plugin method".format(mod, methname)
    ...
    def build_site_cache_url(self, *args, **kw):
        if str(self.site_cache_root) != str(settings.MEDIA_ROOT):
...
signature as `django.core.mail.EmailMessage`.
        ...
        if '@example.com' in sender:
            ...
            print(PRINT_EMAIL.format(
                ...
                recipients=', '.join(recipients)).encode(
