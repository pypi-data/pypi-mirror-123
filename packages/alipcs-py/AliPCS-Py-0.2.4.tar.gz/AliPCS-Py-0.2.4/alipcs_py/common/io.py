...
from alipcs_py.common.log import LogLevels, get_logger
...
def rapid_upload_params(io: IO) -> Tuple[str, str, int, int]:
    ...
    # content_crc32, content_md5 = calc_crc32_and_md5(io, constant.OneM)
...
def rapid_upload_params2(localPath: Path) -> Tuple[str, str, int, int]:
    ...
    slice_md5 = calc_md5(buf[: 256 * constant.OneK])
    content_crc32, content_md5 = calc_crc32_and_md5(localPath.open("rb"), constant.OneM)
...
class NoneCryptography(Cryptography):
...
class FakePngEncryptIO(EncryptIO):
    PNG_HEAD = b"\x89\x50\x4e\x47\x0d\x0a\x1a\x0a"

    def __init__(self, *args, **kwargs):
        self._total_head = FakePngEncryptIO.PNG_HEAD
        super().__init__(*args, **kwargs)
        self._crypto = NoneCryptography()
...
class FakePngDecryptIO(DecryptIO):
    def __init__(self, io: IO, encrypt_key: bytes, nonce: bytes, total_origin_len: int):
        super().__init__(io, b" " * 32, b" " * 32, total_origin_len)

        self._total_head_len = len(FakePngEncryptIO.PNG_HEAD)
        self._crypto = NoneCryptography()
...
def to_decryptio(io: IO, encrypt_password: bytes):
    ...
    png_head_len = len(FakePngEncryptIO.PNG_HEAD)
    if (
        total_head[:png_head_len] == FakePngEncryptIO.PNG_HEAD
        and total_head[png_head_len : png_head_len + 8]
        != b"\x00\x00\x00\x0d\x49\x48\x44\x52"
    ):
        io.seek(png_head_len, 0)
        return FakePngDecryptIO(io, b"", b"", 0)
...
class EncryptType(Enum):
    ...
    def encrypt_io(self, io: IO, encrypt_password: bytes) -> IO:
        ...
        elif self == EncryptType.FakePng:
            return FakePngEncryptIO(io, encrypt_password, io_len)
        else:
