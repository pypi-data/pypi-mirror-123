...
from autotk.AutoCore import UIForm,runmain,license_name,logger,lolang,getnewdir
from autotk.AutoMES import Mes
from autotk.ConfigManage import config_auto,initconfig
...
class MForm(UIForm):
    ...
    def blist_click(self, item, pane_id):
        ...
        try:
            ...
            if (self.plan[item] in ["HDMI_VIDEO", "DVB", "LED_TEST"]):
                if (isinstance(data, str)):
                    mbox.showinfo(self.pane_sn[pane_id].get(), data)
                else:
                    frame, msg = data
                    show_title = "%s %s" % (self.pane_sn[pane_id].get(), msg)
                    moddal = Modal_View(self, show_title, frame)
            if (self.plan[item] in ["HDMI_AUDIO", "SPDIF_AUDIO"]):
                if (isinstance(data, str)):
                    mbox.showinfo(self.pane_sn[pane_id].get(), data)
                else:
                    moddal = Modal_PSD(self, self.pane_sn[pane_id].get(), data)
            if (self.plan[item] in ["RESOLUTION", "USB_TEST", "SD_TEST", "ETH_TEST", "IR_TEST", "BASIC_TEST", "MES",
                                    "SCANNER", "BURN_KEY", "READ_KEY", "CHECK_KEYS", "WIFI_COUPLE", "ROUTER_COUPLE",
                                    "BUTTON_TEST", "HDMI_VOLT", "DVB_VOLT", "AGING_TEST", "WIFI_5", "WIFI_24",
                                    "ROUTER_TELNET", "ADB_CLOSE","LEDS_DETECT",
                                    "BT_TEST"]):
                if (data):
                    mbox.showinfo(self.pane_sn[pane_id].get(), data)
                else:
    ...
    def todo_BUTTON_TEST(self, name, pane_id, item, tagid, report_dict, *args):
        ...
        if (auto == 1):
            ...
            if (not result):
                boxresult = {}
                box_button(self, name + " 倒计时", func=adb.buttontest, timeout=timeout, para=keycodes, result=boxresult)
    ...
    def todo_SCANNER(self, name, pane_id, item, tagid, report_dict, *args):
        ...
        if (mode == "0"):  # 模式0 读取界面控件当前显示
            ...
            # time.sleep(2)
        elif (mode == "7"):  # 模式7 使用GetKeys切换过站SN  AutoRouterCouple从路由板子读取SN
    ...
    def todo_AGING_TEST(self, name, pane_id, item, tagid, report_dict, *args):
        ...
        else:  # 没有limit设定的时候为进入老化
            logger.info("Aging begin..")  # 使用厂测配置模板的设定值set_time暂未使用
            result, value = adb.agingTest()
        # if (result):
        #     logger.warning("%s %s PASS" % (name, self.lo(tagid)))
        # else:
        #     logger.error("%s %s FAIL" % (name, self.lo(tagid)))
            ...
            try:
                ...
                for i,cap in enumerate(cap_list):
                    for a in range(3):
                        ...
                        ret, frame = cap.read()
                        if (ret):
                            _result, _msg,_frame,_rect = self.detect_ledbox(frame,lenlimit=len_limit,ROIw=ROI_led_h,debug=True)
