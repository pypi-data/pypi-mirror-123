...
import os
import json,base64
logger = logging.getLogger(os.path.splitext(os.path.split(sys.argv[0])[-1])[0])  # 20210908 整合MES,修改模块名称,soap的url兼容

# if(os.path.exists("MesApi.dll")):
clr.FindAssembly("MesApi.dll")  ## 加载c#dll文件
from MesApi import *  # 导入命名空间
instance = WS()  # class1是dll里面的类
...
class Mes:
    ...
    def PrintSN(self,SN:str,MAC:str,IMEI:str):
        ...
        IMEI=IMEI.strip()
        try:
            ret,msg=instance.PrintSN(SN,MAC,IMEI,"")
            if(ret==-1):
    ...
    def GetKeys(self,SN:str,keynum=1):
        ...
        if(keynum==2):
            try:
                ret, key1, key2 = instance.GetKeys(SN, "", "")  #
                if (ret == -1):
                    logger.warning("[MES]GetKeys函数执行返回-1")
            except Exception as e:
                logger.critical("[MES]GetKeys DLL 调用出错")
            ...
            try:
                ret,key1, key2 ,key3=instance.GetKeys(SN,"","","")    #
                if(ret==-1):
                    logger.warning("[MES]GetKeys函数执行返回-1")
            except Exception as e:
                logger.critical("[MES]GetKeys DLL 调用出错")
            ...
            try:
                ret,key1, key2 ,key3,key4=instance.GetKeys(SN,"","","","")    #
                if(ret==-1):
                    logger.warning("[MES]GetKeys函数执行返回-1")
            except Exception as e:
                logger.critical("[MES]GetKeys DLL 调用出错")
            ...
            try:
                ret,key1, key2 ,key3,key4,key5=instance.GetKeys(SN,"","","","","")    #
                if(ret==-1):
                    logger.warning("[MES]GetKeys函数执行返回-1")
            except Exception as e:
                logger.critical("[MES]GetKeys DLL 调用出错")
