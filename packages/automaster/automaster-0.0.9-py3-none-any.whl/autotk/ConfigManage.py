...
class config_auto(configparser.ConfigParser):
    def read(self, filenames=inifile):
        if (not os.path.isfile(filenames)):
            self.initleds(filenames)  # AutoBurnKey 20210903
        ...
        try:
            ...
            return super().read(filenames,"utf-8-sig") # utf-8编码的文件时开头会有一个多余的字符\ufeff
    ...
    def save(self, filenames=inifile):
        with open(filenames, 'w') as f:
            self.write(f)
