...
@LastEditTime: 2021-10-13 15:47:58
...
class QueueUpHelper:
    ...
    踢人规则：第一个在办理中，第二个用户的过期时间=第一个结束时间+8*1，第三个=第一个结束时间+8*2，以此类推；第一个没在办理中，第一个过期时间是8*1，第二个是8*2，第三个是8*3，以此类推；
    """
    ...
    @classmethod
    def queue(self, app_id, queue_name, user_id, user_nick="", queue_length=10, app_limit_user_num=500, info_dict={}):
        """
        :description: 加入排队
        :param app_id：应用标识
        :param queue_name：队列名称
            ...
            try:
                ...
                if len(value_list) > 0:
                    ...
                    for i in range(len(value_list)):
                        ...
                        if hash_value:
                            if i == 0:
                                first_queue_timestamp = hash_value["queue_timestamp"]
                            if first_is_process == True and i == 0:
                                ...
                                if now_timestamp > first_queue_timestamp + ((i + 1) * int(config.get_value("queue_confirm_time", 8))):
                                    is_delete = True
                        if is_delete == True:
