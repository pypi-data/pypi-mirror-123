...
@LastEditTime: 2021-10-13 17:13:18
...
class AssetBaseModel():
    ...
    def get_asset_log_list(self, app_id, act_id, asset_type, page_size=20, page_index=0, user_id=0, asset_object_id="", start_date="", end_date="", user_nick="", open_id="", source_type=0, source_object_id="", field="*",is_cache=True):
        """
        ...
        if len(page_list) > 0:
            for item in page_list:
                item["create_day"] = TimeHelper.format_time_to_datetime(item["create_date"]).strftime('%Y-%m-%d')
