...
@LastEditTime: 2021-09-28 20:23:03
...
class TaskBaseModel(FrameBaseModel):
        ...
        for task_info_dict in task_info_list:
            ...
            #加入会员
                ...
                if not user_info_dict:
                    ...
                    if top_base_model.check_is_member(access_token,app_key,app_secret,is_log) == True:
                        result["status"] = 1
                    else:
                        result["status"] = 0
                result["content"] = ""
    ...
    def process_receive_reward(self,app_id,act_id,module_id,user_id,login_token,task_id,task_sub_type,handler_name,request_code,check_new_user=False,check_user_nick=True,continue_request_expire=5,is_stat=True):
        """
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if reward_value <= 0 or task_count.now_count <=0:
                        ...
                        if task_type == int(TaskType.invite_new_user.value) or task_type == int(TaskType.invite_join_member.value):     
                            ...
                            invite_log_model = InviteLogModel(sub_table=invite_sub_table,db_transaction=db_transaction,context=self.context) if task_type == int(TaskType.invite_new_user.value) else InviteLogModel(sub_table="member_" + str(invite_sub_table),db_transaction=db_transaction,context=self.context)
                            #获取邀请用户数
                        ...
                        if invoke_result_data.success == True:
                            try:
                                ...
                                self.context.logging_link_error("【领取任务奖励】" + str(ex))
                                
                            only_id = self.get_only_id(task_info_dict["complete_type"],task_info_dict["task_type"],task_sub_type)
    ...
    def process_free_gift(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,check_new_user=False,check_user_nick=True,continue_request_expire=5,is_stat=True):
        """
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if is_limit == True:
                        ...
                        if reward_value > 0:
                            ...
                            asset_base_model = AssetBaseModel(context=self.context)
                            asset_invoke_result_data = asset_base_model.update_user_asset(app_id,act_id,module_id,user_id,user_info_dict["open_id"],user_info_dict["user_nick"],asset_type,reward_value,asset_object_id,2,task_type,task_info_dict["task_name"],"",only_id,handler_name,request_code,info_json={})
                            if asset_invoke_result_data.success == False:
                                reward_value = 0
                        ...
                        if reward_value > 0 and is_stat == True:
                            ...
                            key_list_dict["FreeGiftRewardCount"] = 1 #完成奖励值
                            stat_base_model.add_stat_list(app_id, act_id, module_id, user_info_dict["user_id"], user_info_dict["open_id"], key_list_dict)
                else:
                    invoke_result_data.success = False
                    invoke_result_data.error_code = task_invoke_result_data.error_code
    ...
    def process_one_sign(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,check_new_user=False,check_user_nick=True,continue_request_expire=5,is_stat=True):
        """
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_info_dict["complete_type"] == 1 and task_count.modify_day == now_day:
                        ...
                        if reward_value > 0:
                            ...
                            asset_type = self.get_task_asset_type(act_info_dict["task_asset_type_json"],task_type)
                            asset_base_model = AssetBaseModel(context=self.context)
                            asset_invoke_result_data = asset_base_model.update_user_asset(app_id,act_id,module_id,user_id,user_info_dict["open_id"],user_info_dict["user_nick"],asset_type,reward_value,asset_object_id,2,task_type,task_info_dict["task_name"],"签到1天",only_id,handler_name,request_code,info_json={})
                            if asset_invoke_result_data.success == False:
                                reward_value = 0
                        task_count.id_md5 = task_count_id_md5
                        task_count.app_id = app_id
    ...
    def process_weekly_sign(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,check_new_user=False,check_user_nick=True,continue_request_expire=5,is_stat=True):
        """
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_count.modify_day == now_day:
                        ...
                        if reward_value > 0:
                            ...
                            asset_type = self.get_task_asset_type(act_info_dict["task_asset_type_json"],task_type)
                            asset_base_model = AssetBaseModel(context=self.context)
                            asset_invoke_result_data = asset_base_model.update_user_asset(app_id,act_id,module_id,user_id,user_info_dict["open_id"],user_info_dict["user_nick"],asset_type,reward_value,asset_object_id,2,task_type,task_info_dict["task_name"],f"签到{task_count.complete_count}天",only_id,handler_name,request_code,info_json={})
                            if asset_invoke_result_data.success == False:
                                reward_value = 0
                        invoke_result_data.data = reward_value
                        if reward_value > 0 and is_stat == True:
    ...
    def process_cumulative_sign(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,check_new_user=False,check_user_nick=True,continue_request_expire=5,is_stat=True):
        """
        ...
        try:
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_count.modify_day == now_day:
                        ...
                        if is_loop == 0 and task_count.complete_count == max_sign_day:
                            ...
                            reward_value = int(day_list[str(task_count.complete_count)]) if day_list.__contains__(str(task_count.complete_count)) else 0
                            if reward_value > 0:
                                only_id = self.get_only_id(3,task_type)
                                asset_type = self.get_task_asset_type(act_info_dict["task_asset_type_json"],task_type)
                                asset_base_model = AssetBaseModel(context=self.context)
                                asset_invoke_result_data = asset_base_model.update_user_asset(app_id,act_id,module_id,user_id,user_info_dict["open_id"],user_info_dict["user_nick"],asset_type,reward_value,asset_object_id,2,task_type,task_info_dict["task_name"],f"签到{task_count.complete_count}天",only_id,handler_name,request_code,info_json={})
                                if asset_invoke_result_data.success == False:
            ...
            self.context.logging_link_error("【累计签到任务】" + str(ex))
            invoke_result_data.success = False
            invoke_result_data.error_code = "exception"
            invoke_result_data.error_message = "系统繁忙,请稍后再试"    
    ...
    def process_invite_new_user(self,app_id,act_id,module_id,user_id,login_token,invite_user_id,handler_name,check_user_nick=True,continue_request_expire=5):
        """
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                user_base_model = UserBaseModel(context=self.context)
                invite_user_info_dict = user_base_model.get_user_info_dict(app_id,act_id,invite_user_id)
                invite_sub_table = SevenHelper.get_sub_table(act_id,config.get_value("invite_sub_table_count",0))
                ...
                if is_invite > 0:
                ...
                elif not invite_user_info_dict:
                    invoke_result_data.success = False
                    invoke_result_data.error_code = "error"
                    ...
                    if task_invoke_result_data.success == True:
                        ...
                        task_sub_table = SevenHelper.get_sub_table(act_id,config.get_value("task_sub_table_count",0))
                        task_count_model = TaskCountModel(context=self.context,sub_table=task_sub_table)
                        task_count_id_md5 = self._get_task_count_id_md5(act_id,module_id,task_info_dict["task_type"],"",invite_user_id)
                        task_count = task_count_model.get_entity("id_md5=%s",params=[task_count_id_md5])
                        task_count = TaskCount() if not task_count else task_count
                        ...
                        if is_invite_limit == True:
                            ...
                            invite_log.open_id = invite_user_info_dict["open_id"]
                            ...
                            task_count.act_id = act_id
                            task_count.module_id = module_id
                            task_count.user_id = invite_user_id
                            task_count.open_id = invite_user_info_dict["open_id"]
                            task_count.task_type = task_type
                            task_count.task_sub_type = ""
    ...
    def process_invite_join_member(self,app_id,act_id,module_id,user_id,login_token,invite_user_id,handler_name,access_token,app_key, app_secret,check_user_nick=True,continue_request_expire=5):
        """
    ...
    def process_collect_goods(self,app_id,act_id,module_id,user_id,login_token,goods_id,handler_name,check_user_nick=True,continue_request_expire=5):
        ...
        try:
            ...
            self.context.logging_link_error("【收藏商品任务】" + str(ex))
            invoke_result_data.success = False
            invoke_result_data.error_code = "exception"
            invoke_result_data.error_message = "系统繁忙,请稍后再试"    
    ...
    def process_browse_goods(self,app_id,act_id,module_id,user_id,login_token,goods_id,handler_name,daily_repeat_browse=True,check_user_nick=True,continue_request_expire=5):
        ...
        try:
            ...
            self.context.logging_link_error("【浏览商品任务】" + str(ex))
            invoke_result_data.success = False
            invoke_result_data.error_code = "exception"
            invoke_result_data.error_message = "系统繁忙,请稍后再试"    
    ...
    def process_favor_store(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,check_user_nick=True,continue_request_expire=5,is_stat=True):
        """
        ...
        try:
            ...
            self.context.logging_link_error("【关注店铺】" + str(ex))
            invoke_result_data.success = False
            invoke_result_data.error_code = "exception"
            invoke_result_data.error_message = "系统繁忙,请稍后再试"    
    ...
    def process_join_member(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,check_user_nick=True,continue_request_expire=5,is_stat=True):
        """
        ...
        try:
            ...
            self.context.logging_link_error("【加入店铺会员】" + str(ex))
            invoke_result_data.success = False
            invoke_result_data.error_code = "exception"
            invoke_result_data.error_message = "系统繁忙,请稍后再试"    
    ...
    def process_browse_site(self,app_id,act_id,module_id,user_id,login_token,task_type,task_sub_type,handler_name,check_user_nick=True,continue_request_expire=5):
        ...
        try:
            ...
            self.context.logging_link_error("【浏览网址任务】" + str(ex))
            invoke_result_data.success = False
            invoke_result_data.error_code = "exception"
            invoke_result_data.error_message = "系统繁忙,请稍后再试"    
