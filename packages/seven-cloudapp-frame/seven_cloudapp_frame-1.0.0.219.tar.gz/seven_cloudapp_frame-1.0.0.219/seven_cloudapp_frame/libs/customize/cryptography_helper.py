...
@LastEditTime: 2021-10-15 11:34:05
...
class CryptographyHelper:
    ...
    @classmethod
    def aes_encrypt(self, source, password, iv, mode=AES.MODE_CBC, encoding="utf-8"):
        """
        :Description: AES加密,ECB & ZeroPadding
        :param source: 待加密字符串
        :param password: 密钥
        :param iv: 偏移量
        :param mode: AES加密模式
        ...
        if count < length:
            add = (length - count)
            source = source + ('\01' * add).encode(encoding)
        elif count > length:
    ...
    def aes_decrypt(self, source, password, iv, mode=AES.MODE_CBC, encoding="utf-8"):
        """
        :Description: AES解密,ECB & ZeroPadding
        :param source: 待解密字符串
        :param password: 密钥
        :param iv: 偏移量
        :param mode: AES加密模式
        ...
        try:
            ...
            plain_text = str(plain_text, encoding).rstrip('\01').rstrip('\00')
            index = SevenHelper.get_last_index(plain_text,'}') + 1
