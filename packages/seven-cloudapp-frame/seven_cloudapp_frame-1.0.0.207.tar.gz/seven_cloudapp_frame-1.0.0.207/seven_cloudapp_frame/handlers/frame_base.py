...
@LastEditTime: 2021-10-12 14:46:16
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def base64_encrypt(self, source, encoding="utf-8"):
        ...
        :last_editors: HuangJianYi
        """
        return CryptographyHelper.base64_encrypt(source, encoding)
    ...
    def response_custom(self, rep_dic):
        ...
        if response_encrypt == True:
            self.http_response("0" + self.base64_encrypt(self.json_dumps(rep_dic)))
        else:
    ...
    def response_common(self, success=True, data=None, error_code="", error_message=""):
        ...
        response_encrypt = config.get_value("response_encrypt", False)
        if response_encrypt == True:
            self.http_reponse("0" + self.base64_encrypt(self.json_dumps(rep_dic)), log_extra_dict)
        else:
    ...
    def check_continue_request(self, handler_name, app_id, object_id, expire=100):
        ...
        if object_id and handler_name and app_id:
            ...
            key = f"continue_request:{handler_name}_{app_id}_{object_id}_{sign}"
            self.logging_link_info("请求key:"+ str(key) + ":" + str(request_param_dict))
            if SevenHelper.is_continue_request(key, expire) == True:
...
class ClientBaseHandler(FrameBaseHandler):
    ...
    def prepare(self):
        ...
        try:
            ...
            self.logging_link_error("【客户端handler基类】" + str(ex))
