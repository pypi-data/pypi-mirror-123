...
@LastEditTime: 2021-10-08 14:18:36
...
class WeChatHelper:
    ...
    def code2_session(self, code, grant_type="authorization_code",app_id="", app_secret=""):
        ...
        try:
            ...
            if respone_data.__contains__("errcode") and respone_data["errcode"] != 0:
                ...
                return invoke_result_data
            open_id = respone_data['openid']
            session_key = respone_data['session_key']
            redis_init.set(redis_key, SevenHelper.json_dumps(respone_data), ex=60 * 60)
            redis_init.set(f"{app_id}_wechat_sessionkey:{open_id}", session_key, ex=60 * 60)
            return respone_data
        except Exception as ex:
            self.logger_error.error("【code2_session】" + str(ex) + ":" + str(respone.text))
            invoke_result_data.success = False
            invoke_result_data.error_code = "exception"
            invoke_result_data.error_message = str(ex)
            return invoke_result_data
    ...
    def get_access_token(self, grant_type="client_credential",app_id="", app_secret=""):
        ...
        try:
            ...
            self.logger_error.error("【get_access_token】" + str(ex) + ":" + str(respone.text))
            invoke_result_data.success = False
            invoke_result_data.error_code = "exception"
            invoke_result_data.error_message = str(ex)
            return invoke_result_data
    ...
    def decrypt_data_by_code(self, open_id, code, encrypted_Data, iv, app_id="", app_secret=""):
        ...
        try:
            ...
            self.logger_error.error("【decrypt_data_by_code】" + str(ex))
        return data
    ...
    def decrypt_data(self, session_key, encrypted_Data, iv, app_id=""):
        ...
        try:
            ...
            self.logger_error.error("【decrypt_data】" + str(ex))
        return data
...
class WeChatPayRequest(object):
    """
    :description: 微信支付请求类,配置文件内容 "wechat_pay": {"api_key": "","mch_id": "","certificate_url":"","pay_notify_url":""}
    """
    """配置账号信息"""
    # =======【基本信息设置】=====================================
    ...
    def query_order(self, pay_order_no="", transaction_id=""):
        ...
        try:
            ...
            self.logger_error.error("【查询订单】" + str(ex) + ":" + str(request_xml))
            invoke_result_data.success = False
            invoke_result_data.error_code="error"
    ...
    def close_order(self, pay_order_no=""):
        ...
        try:
            ...
            self.logger_error.error("【关闭订单】" + str(ex) + ":" + str(request_xml))
            invoke_result_data.success = False
            invoke_result_data.error_code="error"
    ...
    def query_refund(self, refund_no):
        ...
        try:
            ...
            self.logger_error.error("【查询退款单】" + str(ex) + ":" + str(request_xml))
            invoke_result_data.success = False
            invoke_result_data.error_code="error"
