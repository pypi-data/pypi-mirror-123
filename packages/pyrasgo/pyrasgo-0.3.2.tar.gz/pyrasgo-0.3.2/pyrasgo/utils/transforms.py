...
class TransformUtility:
    """
    Class representing a TransformUtility with kwargs as utility arguments
    """
    ...
    def __init__(self,
                 ...
                 **kwargs: Union[str, int, List, Dict]):
        """
        Init a transform with kwarg as Utility arguments
        Args:
            name: Name of Transform Utility
            resultArgumentName: Argument name to pass to transform for utility result
            **kwargs:
        """
        self.name = name
        self.resultArgumentName = resultArgumentName
        self.arguments = kwargs

    def __repr__(self) -> str:
        return f"TransformUtility(name='{self.name}', resultArgumentName='{self.resultArgumentName}', " \
...
class TransformChain(Connection):
    ...
    def transform(self,
                  *,
                  ...
                  utilities: Optional[List[Union[Dict, TransformUtility]]] = None,
        ...
        Args:
            ...
            utilities: Transform Utilities to apply to this Transform. Dict or Obj Format
        ...
        # Convert any Utility Models to Dict
        utilities = [
            x.__dict__ if isinstance(x, TransformUtility) else x for x in utilities
        ]

        # Make sure all arguments entered for transform. Raise error if not.
        # Won't do validation of args if source transform entered with sourcecode
