...
class Node(BaseModel):
    ...
    def base_set_remove_d(self) -> T.Tuple[GQL_D, GQL_D]:
        set_d: GQL_D = {}
        remove_d: GQL_D = {}
    ...
    async def update(self, given_resolver: ResolverType = None) -> bool:
        """Will usually be overwritten assuming there are relationshops with this node"""
        set_d, remove_d = self.base_set_remove_d()
        return await self._update(
            set_d=set_d, remove_d=remove_d, given_resolver=given_resolver
        )

    @staticmethod
    def relationships_set_remove_d(
        to_set: T.Dict[str, T.Optional[T.Union[NodeModel, T.List[NodeModel]]]],
        to_remove: T.Dict[str, T.Optional[T.Union[NodeModel, T.List[NodeModel]]]],
    ) -> T.Tuple[GQL_D, GQL_D]:
        set_d: GQL_D = {}
        for field_name, node_or_nodes in to_set.items():
            ...
            set_d[field_name] = val
        remove_d: GQL_D = {}
        for field_name, node_or_nodes in to_remove.items():
            if node_or_nodes is None:
                continue
            if type(node_or_nodes) is list:
                val = {"id": n.id for n in node_or_nodes}
